...
public:
  ...
  static void GenerateMesh2D(Mesh &mesh,
                             ...
                             double resolution)
  {
    ...
    // Mark subdomains
    ComputeDomainMarkers(mesh, cityModel);
  }
  ...
  static size_t GenerateVolumeMesh(VolumeMesh &volume_mesh,
                                   const Mesh &mesh,
                                   double domainHeight,
                                   double meshResolution)
  {
    info("MeshGenerator: Generating 3D mesh...");
    Timer timer("GenerateVolumeMesh");
  ...
  static void GenerateSurfaces3D(Mesh &groundSurface,
...
private:
  ...
  static void ComputeDomainMarkers(Mesh &mesh, const CityModel &cityModel)
  {
    ...
    for (size_t i = 0; i < mesh.Faces.size(); i++)
    {
      ...
      const int marker = cityModel.FindBuilding(Vector2D(c2d));
      ...
      if (cityModel.FindBuilding(Vector3D(mesh.Vertices[T.v0])) >= 0)
        isBuildingVertex[T.v0] = true;
      if (cityModel.FindBuilding(Vector3D(mesh.Vertices[T.v1])) >= 0)
        isBuildingVertex[T.v1] = true;
      if (cityModel.FindBuilding(Vector3D(mesh.Vertices[T.v2])) >= 0)
        isBuildingVertex[T.v2] = true;
    }
