...
template <typename T>
std::vector<T> vectorIntersection(const std::vector<T>& vector1, const std::vector<T>& vector2) {
    std::vector<T> result;
...
public:
  ...
  static double edge_ratio(std::array<Vector3D, 3> &vertices){
    ...
    if (max_l < e1.magnitude()) max_l = e1.magnitude(); 
    if (max_l < e2.magnitude()) max_l = e2.magnitude(); 
   
    if (min_l > e1.magnitude()) min_l = e1.magnitude(); 
    if (min_l > e2.magnitude()) min_l = e2.magnitude(); 
  
    if (min_l > 0 ) return max_l /min_l;
  ...
  static double aspect_ratio(std::array<Vector3D, 3> &vertices){
    ...
    if (A > 0) return (max_l*(l0+l1+l2))/(element_quality_tri*A);
    return std::numeric_limits<double>::infinity() ; 
  }
...
private: 
  ...
  struct Metric
  {
    ...
    std::string title; 
    Metric(std::function<std::vector<double>(const Mesh)> f, std::string t ) : function(f), title(t) {}; 
  ...
  static std::vector<Metric> getAllMetricFunctions(){
    ...
    metrics.push_back(Metric(element_quality, "Element quality (0-1, 1 is optimal)"));
    metrics.push_back(Metric(edge_ratio,"Edge ratio (1-inf, 1 is optimal)"));
    metrics.push_back(Metric(aspect_ratio, "Aspect ratio (1-inf, 1 is optimal)"));
    metrics.push_back(Metric(equiangular_skew,  "Equiangular skewness (0-1, 0 is optimal)"));
...
public:
  ...
  static void quick_check_mesh(const Mesh &mesh)
  {
    info(mesh);
    {
      ...
      if (max != ar.end()) info("Max aspect ratio (1-inf, 1 is optimal):\t" + std::to_string(*max));
    }
    {
      auto sk = skewness(mesh);
      auto max = std::max_element(sk.begin(), sk.end());
      if (max != sk.end()) info("Max skewness (0-1, 0 is optimal):\t" + std::to_string(*max));
    }
  }
...
public:
  ...
  { 
    const std::array<Vector3D,6> edges = {vertices[1] - vertices[0],
...
private: 
  ...
  struct Metric
  {
    ...
    std::string title; 
    Metric(std::function<std::vector<double>(const VolumeMesh)> f, std::string t ) : function(f), title(t) {}; 
  ...
  static std::vector<Metric> get_face_metric_functions(){
    ...
    metrics.push_back(Metric(orthogonal_quality,"Orthogonal quality (0-1, 1 is optimal)"));
    metrics.push_back(Metric(face_edge_ratio, "Face edge ratio (1-inf, 1 optimal)"));
    metrics.push_back(Metric(face_aspect_ratio, "Face aspect ratio (1-inf, 1 optimal)"));
    metrics.push_back(Metric(face_skewness, "Face skewness (0-1, 0 is optimal)"));
...
public:
  ...
  { 
    ...
    for (size_t c = 0; c < volume_mesh.cells.size(); c++)
    {
      ...
      Vector3D centroid = (vertices[0] + vertices[1]+ vertices[2] + vertices[3])/4 ; 
      ...
      for (size_t face = 0; face < 4; face++)
      {
        ...
        Vector3D face_centroid = (face_vertices[0]+face_vertices[1]+face_vertices[2]) /3;
      ...
      {
        Vector3D adj_centroid = (volume_mesh.vertices[volume_mesh.cells[neighbors[c].cells[n]].v0]+
                                 ...
                                 volume_mesh.vertices[volume_mesh.cells[neighbors[c].cells[n]].v3])/4;
        
        Vector3D e0 =  volume_mesh.vertices[neighbors[c].adjacent_faces[n][1]] - volume_mesh.vertices[neighbors[c].adjacent_faces[n][0]];
        Vector3D e1 =  volume_mesh.vertices[neighbors[c].adjacent_faces[n][2]] - volume_mesh.vertices[neighbors[c].adjacent_faces[n][0]];
      ...
      auto min_orthogonality =std::min_element(orthogonality.begin(), orthogonality.end());
  ...
  static std::vector<double> face_skewness(const VolumeMesh &volume_mesh)
  {
    ...
    static void full_check_volume_mesh(const VolumeMesh &volume_mesh){     
    
    std::cout << format_title("Cell-based quality metrics for volume mesh");
    { 
     auto v = volume(volume_mesh);
     report(v,"Cell volume (allowed to vary)");
    }
    {
     auto eq = element_quality(volume_mesh);
     report(eq,"Element quality (0-1, 1 is optimal)");
     
    }
    {
     auto er = edge_ratio(volume_mesh);
     report(er,"Edge ratio (1-inf, 1 is optimal)");
    } 
    {
     auto sk = skewness(volume_mesh);
     report(sk,"Skewness (0-1, 0 is optimal)");
    }
    std::cout << format_title("Face-based quality metrics for volume mesh");
    { 
     auto ar = area(volume_mesh);
     report(ar,"Face area (allowed to vary)");
    }
    { 
     auto s = size_change(volume_mesh);
     report(s,"Size change (0-inf, 1 is optimal)");
    }
    { 
     auto oq = orthogonal_quality(volume_mesh);
     report(oq,"Orthogonal quality (0-1, 1 is optimal)");
    }
    {
     auto fer = face_edge_ratio(volume_mesh);
     report(fer,"Face edge ratio (1-inf, 1 optimal)");
    }
    {
     auto far = face_aspect_ratio(volume_mesh);
     report(far,"Face aspect ratio (1-inf, 1 optimal)");
    }
    {
     auto sk = face_skewness(volume_mesh);
     report(sk,"Face skewness (0-1, 0 is optimal)");
    }
  }
  ...
  static void quick_check_volume_mesh(const VolumeMesh &volume_mesh)
  {
    info(volume_mesh);
    {
      ...
      auto max = std::max_element(ar.begin(), ar.end());
      if (max != ar.end()) info("Face aspect ratio MAX (1-inf, 1 is optimal):\t" + std::to_string(*max));
    }
    {
      auto sk = skewness(volume_mesh);
      auto max = std::max_element(sk.begin(), sk.end());
      if (max != sk.end()) info("Cell skewness MAX(0-1, 0 is optimal):\t" + std::to_string(*max));
    }
  }

  static void check_volume_mesh(const VolumeMesh &volume_mesh, const int  metrics_code)
  { 
    if(metrics_code == 0){
