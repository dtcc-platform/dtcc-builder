 ...
 *
 * This file is the principal FBX SDK environment definition. It is used at the
 * top of every header and source file so that every unit is using the same
 * definitions.
 */
...
#define FBX_SAFE_DELETE_ARRAY(a)                                               \
  {                                                                            \
    FbxDeleteArray(a);                                                         \
    (a) = NULL;                                                                \
  }
#define FBX_SAFE_DESTROY(p)                                                    \
  if (p)                                                                       \
  {                                                                            \
    (p)->Destroy();                                                            \
    (p) = NULL;                                                                \
  }
#define FBX_SAFE_FREE(p)                                                       \
  if (p)                                                                       \
  {                                                                            \
    FbxFree(p);                                                                \
    (p) = NULL;                                                                \
