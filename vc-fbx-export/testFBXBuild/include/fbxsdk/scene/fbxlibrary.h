...
/** This library class represents libraries that store sub-libraries and shading
 * objects. Shading objects are objects of class FbxTexture, FbxSurfaceMaterial,
 * and FbxLight. \nosubgrouping
 */
...
public:
   ...
   * \param pSystemLibrary       A flag which indicates whether or not this
   * library is a system library.
   */
   ...
   * \return                     \c True if adding the sub-library is
   * successful, returns \c false if not.
   */
  bool AddSubLibrary(FbxLibrary *pSubLibrary);

  /** Removes a sub-library
   * \param pSubLibrary          The sub-library to be removed.
   * \return                     \c True if the sub-library is removed, \c false
   * if not.
   */
  ...
  FbxObject *CloneAsset(FbxObject *pToClone,
                        FbxObject *pOptionalDestinationContainer = NULL) const;
  ...
  /** Transfers ownership from the source library to this library for any assets
   *passing through the filter. It is assumed that name conflicts and other
   *details have been resolved beforehand.
   *
   * External asset files required by the assets are copied (not moved). It's
   * up to the owner of the source library to clean up the files if the files
   *are not on a read-only transport. If this document hasn't been committed
   *yet, the assets will not be copied.
   *
   ...
   *
   * This may leave the source library in an invalid state. For example, the
   *source library would be in an invalid state if you had decided to transfer
   *texture objects to the library, but materials were kept in the source
   *library.
   *
   * To safeguard against this, the transfer disconnects objects, and thus
   *materials are left without textures.
   *
   * When you transfer an object, all its dependents come with it.  If you move
   * a material, it takes the associated textures as well. Although if you moved
   *a texture, the material would not be transferred with it. \param pSrcLibrary
   *The source library to be imported. \return                     \c True if no
   *filtered assets are skipped.
   **/
   ...
   * a material, it takes the associated textures as well. Although if you moved
   *a texture, the material would not be transferred with it. \param pSrcLibrary
   *The source library to be imported. \param pAssetFilter         The asset
   *filter. \return                     \c True if no filtered assets are
   *skipped.
   **/
   ...
   * \param pRecurse             A flag that indicates whether to check
   * sub-libraries \param pOptContainer        Optional container for the cloned
   * asset \return                     A new instance of the member. Note that
   * the new member is not included with this library.
   */
  template <class T>
   ...
   * \return                     The potentially localized text. May return the
   * parameter passed in.
   */
   ...
   * \return                     The shading object located at the specified
   * index.
   */
   ...
   * \returns                    The number of shading objects corresponding to
   * the filtering parameters
   */
  int GetShadingObjectCount(const FbxImplementationFilter &pCriteria) const;

  /** Returns a handle on the shading object at the specified index that
   * corresponds to the given filtering parameters. \param pIndex Shading object
   * index. \param pCriteria            Filtering criteria that identifies what
   * kind of implementations to consider. \returns                    A handle
   * on the shading object at the specified index that corresponds to the given
   * filtering parameters.
   */
  FbxObject *GetShadingObject(int pIndex,
                              const FbxImplementationFilter &pCriteria) const;
...
template <class T>
T *FbxLibrary::InstantiateMember(const T *pFBX_TYPE,
                                 const FbxObjectFilter &pFilter,
                                 bool pRecurse,
                                 FbxObject *pOptContainer)
{
  ...
  if (pRecurse)
  {
    ...
    for (int i = 0; i < GetMemberCount<FbxLibrary>(); ++i)
    {
      ...
      T *lClonedObject = lLibrary->InstantiateMember(pFBX_TYPE, pFilter,
                                                     pRecurse, pOptContainer);
      if (lClonedObject)
...
class FBXSDK_DLL FbxEventPopulateSystemLibrary
...
class FBXSDK_DLL FbxEventUpdateSystemLibrary
...
class FBXSDK_DLL FbxEventWriteLocalization
...
class FBXSDK_DLL FbxEventMapAssetFileToAssetObject
