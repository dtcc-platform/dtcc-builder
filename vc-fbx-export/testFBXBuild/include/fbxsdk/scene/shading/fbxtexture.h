...
/** This class is the base class for textures, ie classes FbxFileTexture,
 * FbxLayeredTexture and FbxProceduralTexture. It describes image mapping on top
 * of a geometry. \nosubgrouping
 */
...
public:
  ...
  enum EUnifiedMappingType
  {
    ...
    eUMT_XY,          //! Maps to EMappingType::ePlanar and
                      //! EPlanarMappingNormal::ePlanarNormalZ.
    eUMT_YZ,          //! Maps to EMappingType::ePlanar and
                      //! EPlanarMappingNormal::ePlanarNormalX.
    eUMT_XZ,          //! Maps to EMappingType::ePlanar and
                      //! EPlanarMappingNormal::ePlanarNormalY.
   ...
   * For example, the texture might be used as a standard texture, as a shadow
   * map, as a bump map, etc. Use SetTextureUse() and GetTextureUse() to access
   * these values.
   */
  enum ETextureUse6
  {
    eTEXTURE_USE_6_STANDARD,                //! Maps to ETextureUse::eStandard.
    eTEXTURE_USE_6_SPHERICAL_REFLEXION_MAP, //! Maps to
                                            //! ETextureUse::eSphericalReflectionMap.
    eTEXTURE_USE_6_SPHERE_REFLEXION_MAP, //! Maps to
                                         //! ETextureUse::eSphereReflectionMap.
   ...
   * Use SetWrapMode(), GetWrapModeU() and GetWrapModeV() to access these
   * values.
   */
  enum EWrapMode
  {
    eRepeat, //! Apply the texture over and over on the model's surface until
             //! the model is covered. This is the default setting.
    eClamp   //! Apply the texture to a model only once, using the color at the
             //! ends of the texture as the "filter".
  };
  ...
  enum EBlendMode
  {
    eTranslucent, //! The texture is transparent, depending on the Alpha
                  //! settings.
    eAdditive,    //! The color of the texture is added to the previous texture.
    eModulate,    //! The color value of the texture is multiplied by the color
                  //! values of all previous layers of texture.
    eModulate2, //! The color value of the texture is multiplied by two and then
                //! multiplied by the color values of all previous layers of
                //! texture.
   ...
   * If PremultiplyAlpha flag is true, the R, G, and B components you store have
   * already been multiplied in with the alpha. Default value is true.
   */
  ...
  /** This property only used by Vector Displacement Texture so it is not added
   * to FbxTexture. It is a dynamic enum property which has values : "World",
   * "Object" and "Tangent" Default value is "Object".
   */
  ...
  /** This property only used by Vector Displacement Texture so it is not added
   * to FbxTexture. It is a dynamic enum property which has values :
   * "Floating-point Absolute" and "Signed Encoding" Default value is
   ...
   * \remarks            If the swap UV flag is enabled, the texture's width and
   * height are swapped.
   */
   ...
   * \param pPremultiplyAlpha       Set to \c true if the method of storing
   * alpha is PremultiplyAlpha. \remarks                        If
   * PremultiplyAlpha flag is true, the R, G, and B components you store have
   * already been multiplied in with the alpha.
   */
  void SetPremultiplyAlpha(bool pPremultiplyAlpha);

  /** Returns the PremultiplyAlpha flag.
   * \return             \c True if the method of storing alpha is
   * PremultiplyAlpha. \remarks            If PremultiplyAlpha flag is true, the
   * R, G, and B components you store have already been multiplied in with the
   * alpha.
   */
  bool GetPremultiplyAlpha() const;

  /** \enum EAlphaSource Controls if the Alpha computation of the current
   * texture comes from the Alpha channel, RGB Intensity channel, or if there is
   * No Alpha.
   */
  enum EAlphaSource
  {
   ...
   * \remarks            The defined rectangle is not checked for invalid
   * values. The caller must verify that the rectangle is meaningful for this
   * texture.
   */
  ...
  {
    ePlanarNormalX, //! Normals are in the direction of the X axis, mapping plan
                    //! is in the YZ axis.
    ePlanarNormalY, //! Normals are in the direction of the Y axis, mapping plan
                    //! is in the XZ axis.
    ePlanarNormalZ  //! Normals are in the direction of the Z axis, mapping plan
                    //! is in the XY axis.
  };

  /** Sets the normal orientations for planar mapping.
   * \param pPlanarMappingNormal The identifier for planar mapping normal
   * orientation.
   */
  void SetPlanarMappingNormal(EPlanarMappingNormal pPlanarMappingNormal);

  /** Returns the normal orientations for planar mapping.
   * \return                     The identifier for planar mapping normal
   * orientation.
   */
  ...
  enum ETextureUse
  {
    ...
    eLightMap,               //! Light map
    eSphericalReflectionMap, //! Spherical reflection map: Object reflects the
                             //! contents of the scene
    eSphereReflectionMap,    //! Sphere reflection map: Object reflects the
                          //! contents of the scene from only one point of view
    eBumpNormalMap //! Bump map: Texture contains two direction vectors, that
                   //! are used to convey relief in a texture.
  };
   ...
   * This set of functions provides direct access to the default values in
   * vector base.
   */
  //@{
   ...
   *                     the texture. A displacement of one unit is equal to the
   * texture width after the U scaling is applied. The second element is the V
   * translation applied to the texture. A displacement of one unit is equal to
   * the texture height after the V scaling is applied. The third and fourth
   * elements have no effect on texture translation.
   */
   ...
   * translation applied to the texture. A displacement of one unit is equal to
   * the texture height after the V scaling is applied. The third and fourth
   * elements have no effect on texture translation. \return             The
   * input parameter completed with appropriate data.
   */
   ...
   *                     U axis in degrees. The second element is the texture
   * rotation around the V axis in degrees. The third element is the texture
   ...
   *                     U axis in degrees. Second element is the texture
   * rotation around the V axis in degrees. Third element is the texture
   *                     rotation around the W axis in degrees.
   ...
   * \param pS           The first element is scale applied to the texture
   * width. The second element is scale applied to the texture height. The third
   ...
   *	\param pAlpha       A value on a scale from 0 to 1, with 0 being
   *transparent.
   */
  void SetDefaultAlpha(double pAlpha);

  /** Returns the default alpha.
   *	\return             A value on a scale from 0 to 1, with 0 being
   *transparent.
   */
   ...
   * This set of functions provides direct access to the default values in
   * number base. U, V and W coordinates are mapped to the X, Y and Z
   * coordinates of the default vectors found in the "Default Values By Vector"
   * section.
   */
  //@{
   ...
   *                 of one unit is equal to the texture's width after applying
   * U scaling. \param pV       Vertical translation applied to a texture. A
   * displacement of one unit is equal to the texture's height after applying V
   * scaling.
   */
   ...
   * \remarks        The W axis is oriented toward the result of the vector
   * product of the U and V axes that is W = U x V.
   */
...
protected:
  ...
  EAlphaSource
      mAlphaSource; // now unused in MB (always set to None); not a prop
  EMappingType mMappingType;                 // CurrentMappingType
  EPlanarMappingNormal mPlanarMappingNormal; // CurrentMappingType

  // Unsupported parameters in the FBX SDK, these are declared but not
  // accessible. They are used to keep imported and exported data identical.
