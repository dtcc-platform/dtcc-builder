...
/** FbxLayeredTexture is a combination of multiple textures(FbxTexture) blended
 * sequentially. For example, you can access individual texture by: \code
 * FbxTexture* pIndiTexture = lLayeredTexture->GetSrcObject(FbxTexture::ClassId,
 * pTextureIndex); \endcode Another example to construct a layered texture with
 * two sub textures. \code FbxFileTexture *background, *file1;
...
public:
   ...
   * - \e eTranslucent,        The new texture layer is transparent (depending
   * on the Alpha value).
   * - \e eAdditive,           Add the color of the new texture to the previous
   * texture.
   * - \e eModulate,           Multiples the color value of the new texture by
   * the color values of all previous layers of texture.
   * - \e eModulate2,          Multiples the color value of the new texture by
   * two and then by the color values of all previous layers of texture.
   ...
   * intense results.
   * - \e eLighterColor,       This blend mode has the opposite effect of the
   * Darker Color mode. It compares all the values in both layers, then displays
   * the lightest values.
   * - \e eSoftLight,          Soft Light will multiply the dark tones and
   * screen the light tones.
   * - \e eHardLight,          Hard Light multiplies the dark colors and screens
   * the light colors.
   ...
   * - \e eLinearLight,        Linear Light is the same as Vivid light but it
   ...
   * mask the difference between upper and lower layers.
   * - \e eSubtract,           The result color is the foreground color
   * subtracted from the background color. The result color is then applied over
   * the background color using the foreground alpha to define the opacity of
   * the result.
   ...
   * alone.
   * - \e eLuminosity,         Luminosity changes the luminosity of the lower
   * layer to the luminosity of the upper layer while leaving hue and saturation
   * the same.
   * - \e eOverlay,            Multiplies (darkens) when the layer on which the
   * mode is set is dark and screens (brightens) when the layer on which the
   * mode is applied is lighter.
   ...
   * \return                            \c True if pOther is equivalent to this
   ...
   * \return                            \c True if successful, returns \c false
   * otherwise.
   */
   ...
   * \param pIndex                      The texture index.
   * \param pMode                       The parameter that will hold the
   * returned blend mode. \return                            \c True if
   * successful, returns \c false otherwise.
   */
   ...
   * \param pIndex                      The texture index.
   * \param pAlpha                      The parameter that will hold the
   * returned alpha. \return                            \c True if successful,
