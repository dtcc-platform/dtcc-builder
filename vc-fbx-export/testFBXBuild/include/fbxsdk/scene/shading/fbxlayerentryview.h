...
/** FbxLayerEntryView encodes a layer element representation (the index of the
 * layer, the type of the layer element and whether the layer element is a UVSet
 * layer element) to a string stored in FbxBindingTableEntry and decodes the
 * string back to a layer element representation. This class is used in
 * combination with FbxBindingTableEntry to represent a source or a destination
 * FbxLayerElement.
...
public:
   ...
   */
  void
  SetLayerElement(int pLayerIndex, FbxLayerElement::EType pType, bool pUVSet);
  ...
  /** Get the layer element for binding entry.
   * \param pContainer           FbxLayerContainer to get the layer element
   * from. \return                     The layer element for binding entry.
   */
