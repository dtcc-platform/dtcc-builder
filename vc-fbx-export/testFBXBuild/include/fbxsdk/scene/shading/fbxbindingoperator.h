 ...
 *
 *  //Create a NodePositionConvert binding operator and add it as source of the
 * lEntry. FbxOperatorEntryView lSrc(&lEntry, true, true); lSrc.SetOperatorName(
 * "NodePositionConvert"); FbxBindingOperator* lOp =
 * pImpl.AddNewBindingOperator( "NodePositionConvert",
 * FbxNodePositionBOF::FunctionName);
 *
 ...
 *  //Set the shader parameter (the property's name) as source of the
 * lEntryPropParam. lPropSrc.SetProperty(lProp.GetHierarchicalName());
 *  //Set the operator function FbxNodePositionBOF as destination of the
 * lEntryPropParam. lEntryPropParam.SetDestination(
 * FbxNodePositionBOF::FunctionName );
 *
...
public:
                         ...
                         int index = 0) const
  {
    ...
    bool lSuccess =
        ReverseEvaluate(pObject, lIn, &lOut, &lOutType, setObj, index);
   ...
   *                              This is used to get the property or operator
   * that is related to this parameter, then to evaluate the property or
   * operator. \param pResult A pointer to the result. \return \c true on
   * success, \c false otherwise. \remarks This method can handle different
   * types of entries. For property entry and constant entry, this method will
   * find out the property via the pEntryDestinationName and then evaluate its
   * value; for operator entry, this method will find out the operator via the
   * pEntryDestinationName and evaluate the operator function to get the
   * property's value; for any other types of entry, this method is meaningless.
   */
  template <class FBXTYPE>
  bool EvaluateEntry(const FbxObject *pObject,
                     const char *pEntryDestinationName,
                     FBXTYPE *pResult) const
  {
    EFbxType lResultType;
    void *lResult = NULL;

    bool lSuccess =
        EvaluateEntry(pObject, pEntryDestinationName, &lResultType, &lResult);
  ...
  /** It represents a binding relationship between current object and the
   * target. Any binding operation need to specify a certain kind of binding
   * function. \nosubgrouping
   */
  ...
  public:
     ...
     * \param pResult Will be filled by a pointer to a buffer that hold the
     * result. The caller must call FreeEvaluationResult() when it is done with
     * this pointer. \return \c true on success, \c false otherwise.
     */
    virtual bool Evaluate(const FbxBindingOperator *pOperator,
                          const FbxObject *pObject,
                          EFbxType *pResultType,
                          void **pResult) const = 0;
     ...
     * \param setObj Control to set the property (only to query by the default
     * ). \param index Used only in FbxMultiplyDistBOF. \return \c true on
   ...
   * \nosubgrouping
   */
  template <class FUNCTION> class FunctionCreator : public FunctionCreatorBase
  {
  public:
     ...
     */
    virtual Function *CreateFunction() const { return FbxNew<FUNCTION>(); }
  };

  /** This utility class is used to register and unregister the binding function
   * creators. \nosubgrouping
   */
  ...
  private:
    typedef FbxMap<const char *, const FunctionCreatorBase *, FbxCharPtrCompare>
        RegistryType;
...
*****************************************************************************************************************************/
  ...
  bool EvaluateEntry(const FbxObject *pObject,
                     ...
                     EFbxType *pResultType,
                     void **pResult) const;
  bool GetEntryProperty(const FbxObject *pObject,
                        const char *pEntryDestinationName,
                        FbxProperty &pProp) const;
...
protected:
  ...
  bool Evaluate(const FbxObject *pObject,
                EFbxType *pResultType,
                void **pResult) const;
  bool ReverseEvaluate(const FbxObject *pTarget,
                       const void *pIn,
                       void **pOut,
                       EFbxType *pOutType,
                       bool setObj,
 ...
 * this operator via a certain property. The position of the node is represented
 * by translation.
 */
...
public:
  ...
  /** Evaluate the position of the node that is bound with this operator via a
   * certain property. The position of the node is represented by translation.
   *
   * \param pOperator Operator running on the object.
   * \param pObject The object that will be evaluated.
   * \param pResultType The type of the result to be returned, eFbxDouble4 in
   * this case. \param pResult A pointer to a buffer that can hold the result.
   * \return \c true on success, \c false otherwise.
   */
  virtual bool Evaluate(const FbxBindingOperator *pOperator,
                        const FbxObject *pObject,
 ...
 * this operator via a certain property. The direction of the node is
 * represented by Euler rotation.
 */
...
public:
  ...
  /** Evaluate the direction of the node that is bound with this operator via a
   * certain property. The direction of the node is represented by Euler
   * rotation.
   *
   * \param pOperator Operator running on the object.
   * \param pObject The object that will be evaluated.
 ...
 * corresponding unique id, based on the value of a predicate property.
 */
...
public:
  ...
  /** Check in the symbol table the string and returns its unique ID as an
   * integer
   *
   * \param pOperator Operator running on the object.
   * \param pObject The object that will be evaluated.
 ...
 * value, based on the value of a predicate property.
 */
