 ...
 * It defines basic information about the shader and the binding
 * table(FbxBindingTable). For example, you can create a new FbxImplementation
 * like this: \code FbxImplementation* lImpl = FbxImplementation::Create(
 * &pMyScene, "MyImplementation" ); pMyObject.AddImplementation( lImpl );
 * pMyObject.SetDefaultImplementation( lImpl );
 * lImpl->RenderAPI = FBXSDK_RENDERING_API_DIRECTX;
 * //FBXSDK_RENDERING_API_DIRECTX, FBXSDK_RENDERING_API_OPENGL,
 * FBXSDK_RENDERING_API_MENTALRAY or FBXSDK_RENDERING_API_PREVIEW
 * lImpl->RenderAPIVersion = "9.0"; //API Version
 *
 * lImpl->Language = FBXSDK_SHADING_LANGUAGE_HLSL;
 * //FBXSDK_SHADING_LANGUAGE_HLSL, FBXSDK_SHADING_LANGUAGE_GLSL,
 * FBXSDK_SHADING_LANGUAGE_CGFX or FBXSDK_SHADING_LANGUAGE_MRSL
 * lImpl->LanguageVersion = "1.0";  //Language Version
 * \endcode
 *
 * After the new FbxImplementation is created, you can access FbxBindingTable
 * like this: \code FbxBindingTable* lTable =
 * lImpl->GetTableByTargetName("root"); \endcode Also, you can access the exist
 * FbxImplementation in FbxObject by this: \code const FbxImplementation* lImpl
 * = GetImplementation( pMyObject, FBXSDK_IMPLEMENTATION_CGFX ); //
 * FBXSDK_IMPLEMENTATION_PREVIEW, FBXSDK_IMPLEMENTATION_MENTALRAY,
 * FBXSDK_IMPLEMENTATION_CGFX, FBXSDK_IMPLEMENTATION_HLSL,
 * FBXSDK_IMPLEMENTATION_OGS or FBXSDK_IMPLEMENTATION_NONE \endcode
 * \nosubgrouping
...
public:
  ...
  /** Shader Language.
   * \see FBXSDK_SHADING_LANGUAGE_HLSL, FBXSDK_SHADING_LANGUAGE_GLSL,
   * FBXSDK_SHADING_LANGUAGE_CGFX and FBXSDK_SHADING_LANGUAGE_MRSL in
   * conventions.h
   */
  ...
  //! Property to store the shader parameters(constants) values in this
  //! implementation.
   ...
   * \param pIndex The index of the table to retrieve. Valid values are [ 0,
   * GetTableCount() ). \return A const pointer to the pIndex-th table or NULL
   ...
   * \return A const pointer to the binding table with the given target name, or
   * NULL if there is no such binding table.
   */
  const FbxBindingTable *GetTableByTargetName(const char *pName) const;

  /** Returns the binding table that has the given target name.
   * \param pName The target name of the table to look for.
   * \return A pointer to the binding table with the given target name, or NULL
   * if there is no such binding table.
   */
   ...
   * \return A const pointer to the binding table with the given target type, or
   * NULL if there is no such binding table.
   */
  ...
  /** Returns the binding table that has the given target type.
   * \param pTargetName The target type to look for.
   * \return A pointer to the binding table with the given target type, or NULL
   * if there is no such binding table.
   */
   ...
   * \return A const pointer to the binding operator with the given name, or
   * NULL if there is no such binding table.
   */
  const FbxBindingOperator *
  GetOperatorByTargetName(const char *pTargetName) const;
  //@}
  ...
  /** Name of property to store the shader parameters(constants) values in this
   * implementation. \see GetConstants
   */
