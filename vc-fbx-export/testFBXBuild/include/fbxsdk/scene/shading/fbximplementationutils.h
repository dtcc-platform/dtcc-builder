 ...
 */
FBXSDK_DLL const FbxImplementation *
 ...
 * \param pImplementation          FbxImplementation of the bound property value
 * to get if the Entry type is FbxOperatorEntryView::sEntryType. \param
 * pBoundObject             FbxObject of the bound property value to get if the
 * Entry type is FbxPropertyEntryView::sEntryType. \param pValue Pointer to
 * bound property value from FbxBindingTable. \return Whether get bound property
 * value success or not.
 */
template <typename T>
bool GetBoundPropertyValue(const FbxBindingTable *pBindingTable,
                           ...
                           T &pValue)
{
  if ((NULL != pImplementation) && (NULL != pBindingTable) &&
      (NULL != pBoundObject) && (NULL != pEntryName))
  {
    const FbxBindingTableEntry *lEntry =
        pBindingTable->GetEntryForDestination(pEntryName);

    if (NULL != lEntry)
    {
      if (strcmp(lEntry->GetEntryType(true),
                 FbxPropertyEntryView::sEntryType) == 0)
      {
      ...
      else if (strcmp(lEntry->GetEntryType(true),
                      FbxOperatorEntryView::sEntryType) == 0)
      {
        const char *lOperatorName = lEntry->GetSource();
        const FbxBindingOperator *lOp =
            pImplementation->GetOperatorByTargetName(lOperatorName);
        if (lOp)
        {
