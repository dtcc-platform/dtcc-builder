...
public:
   ...
   * Besides storing video clips, the FbxVideo object can also store image
   * sequences. This section contains the manipulation methods used in this
   * specialized mode. Note that, except for the GetFileName(), SetFileName(),
   * GetRelativeFileName(), SetRelativeFileName() and the methods in this
   * section, all the other ones are not mandatory therefore could contain
   * uninitialized or default data values.
   *
   */
  //@{
  /** Specify if this video object is holding the starting point of an image
   * sequence. \param pImageSequence If \c true, this object is holding an image
   * sequence. \remarks When this object is used as image sequence, the FBX SDK
   ...
   * \remarks               The parameter value is not checked. It is the
   * responsibility of the caller to deal with bad frame numbers.
   */
  ...
  enum EInterlaceMode
  {
    ...
    eFullEvenOdd, //!< Extract eFields and make full frame with each one
                  //!< beginning with Odd (60fps).
    eFullOddEven  //!< Extract eFields and make full frame with each one
                  //!< beginning with Even (60fps).
  };
