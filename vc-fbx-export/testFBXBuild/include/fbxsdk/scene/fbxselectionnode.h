 ...
 * \nosubgrouping
 * Used to group objects with their components (e.g. vertex, edge, and face),
 * when adding it to a selection set (FbxSelectionSet). \see FbxSelectionSet
 */
...
public:
  ...
  /** Set an object whose components or itself is contained in the
   * SelectionNode. \param pObject      The object whose components or itself is
   * contained in the SelectionNode. \return \c true if the object is set
   * successfully. \c false otherwise. \remarks It is possible a SDK user will
   * try to set multiple objects to one SelectionNode, but only the last one
   * will be kept.
   */
  bool SetSelectionObject(FbxObject *pObject);

  /** Get the object whose components or itself or both are contained in the
   * SelectionNode. \return The object whose components or itself or both are
   ...
   *   \c false means only the object's components are in the selection set, the
   * object is not.
   */
