...
/** This FbxTakeLayerInfo structure is used to identify a layer by name and id
 * number.
 */
 ...
 * the FBX file format can support many "animation takes" in an FBX file to
 * mimic how a movie is produced by making many takes of the same scene.
 *
 ...
 * When a FbxImporter is initialized, the take information can be read and can
 * be available before the long Import() step, this way, we can get the take
 * info data very fast since we don't need to load all the animation scene data.
 * \code
 * // Ex: to get all take names in a FBX file
 * for(int lAnimStackCount=0; lAnimStackCount < lImporter->GetAnimStackCount();
 * lAnimStackCount++)
 * {
...
public:
   ...
   * You can modify it if it must be different from the take name in the
   * imported file. \remarks                This field is only used when
   * importing a scene.
   */
  ...
  /** Import/export flag.
   * Set to \c true by default, set to \c false if the take must not be imported
   * or exported.
   */
  bool mSelect;

  //! Local time span, set to animation interval if it is left at the default
  //! value.
  FbxTimeSpan mLocalTimeSpan;

  //! Reference time span, set to animation interval if it is left at the
  //! default value.
  FbxTimeSpan mReferenceTimeSpan;

  /** Time value for offsetting the animation keys once they are imported in a
   * scene. You can modify it if you need the animation of a take to be offset.
