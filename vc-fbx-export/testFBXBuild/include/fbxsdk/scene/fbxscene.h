...
/** This class contains the description of a 3D scene. It contains the nodes
 * (including the root node) (FbxNode), materials, textures, videos, gobos,
 ...
 *
 * When an object is created using the FBX SDK, a scene is usually passed as
 * argument to the object creation function to specify that the object belongs
 * to this scene. At this point, a connection is made with the object as source
 * and the scene as destination.
 *
 * All objects in the scene can be queried by connection index. In addition,
 * generic nodes, materials, and textures can also be queried by name. In this
 * latter case, the first object with the queried name will be returned.
 *
...
public:
  ...
  //@{
  //! Clear the scene content by deleting the node tree below the root node and
  //! restoring default settings.
  void Clear();

  /** Get the root node of the scene.
   * \return Pointer to the root node.
   * \remarks This node is not saved. Do not use it to apply a global
   * transformation to the node hierarchy. If a global transformation must be
   * applied, insert a new node below this one. */
  FbxNode *GetRootNode() const;
  //@}

  //! \name Texture Material and Video Access
  //@{
  /** Clear, then fill, a texture array with all existing textures included in
   * the scene. \param pTextureArray An array of texture pointers. */
  void FillTextureArray(FbxArray<FbxTexture *> &pTextureArray);

  /** Clear, then fill, a material array with all existing materials included in
   * the scene. \param pMaterialArray An array of material pointers. */
   ...
   * \return Pointer to the generic node or \c NULL if the index is out of
   * bounds.
   */
   ...
   * \return If the passed parameter is \c NULL, this method will return \c
   * false, otherwise \c true. */
   ...
   * \return If the passed parameter is \c NULL, this method will return \c
   * false, otherwise \c true.
   * \remarks The pointed object is not referenced by the scene anymore but is
   * not deleted. */
   ...
   * \remarks Character Poses and Poses are two distinct entities having their
   * own lists. */
   ...
   * \remarks Poses and Character Poses are two distinct entities having their
   * own lists. */
   ...
   * \param pPose The pose (for example: bind pose, rest pose) to be added to
   * the scene. \return If the pose is correctly added to the scene, return \c
   * true. Otherwise, if the pose is already in the scene, return \c false. */
  bool AddPose(FbxPose *pPose);

  /** Remove the specified pose from the scene.
   * \param pPose The pose (for example: bind pose, rest pose) to be removed
   * from the scene. \return If the pose was successfully removed from the
   * scene, return \c true. Otherwise, if the pose could not be found return \c
   * false. */
   ...
   * \return If the pose was successfully removed from the scene, return \c
   * true. Otherwise, if the pose could not be found return \c false. */
  ...
  * The scene's animation evaluator is used to compute animation values for
  animated scenes. A typical
  * usage would be to compute the global transform matrix of a node \c lNode at
  a given time \c lTime.
  * \code
  FbxAMatrix& lGlobalMatrix =
  lNode->GetScene()->GetAnimationEvaluator()->GetNodeGlobalTransform(lNode,
  lTime);
  ...
  * \endcode
  *
  * The user can create one or more evaluators in the scene. The default
  evaluator is set using SetEvaluator. When
  * GetEvaluator is called, if the scene has no evaluator, an evaluator is
   ...
   * \remark Changing the current animation stack will also cause the animation
   * evaluator to reset its state. */
  void SetCurrentAnimationStack(FbxAnimStack *pAnimStack);

  /** Retrieve the current animation stack.
   * \return the current animation stack. If none were set previously, the
   * default one will be returned if it exists. */
  FbxAnimStack *GetCurrentAnimationStack();

  /** Set the global evaluator used by this scene evaluation engine.
   * \param pEvaluator The evaluator to be used for evaluation processing of
   * this scene. */
   ...
   * \return Number of textures in this scene. Includes types FbxFileTexture,
   * FbxLayeredTexture and FbxProceduralTexture. \remarks To get the number of
   * textures of a specific type, use GetSrcCount(). For example: \code int
   * lNbFileTextures = lScene->GetSrcObjectCount<FbxFileTexture>(); int
   * lNbLayeredTextures = lScene->GetSrcObjectCount<FbxLayeredTexture>(); int
   * lNbProceduralTextures = lScene->GetSrcObjectCount<FbxProceduralTexture>();
   * \endcode */
  int GetTextureCount() const;

  /** Get the texture at the given index. pIndex must be between 0 and
   * GetTextureCount(). \param pIndex Position in the list of textures. \return
   * Pointer to the texture or \c NULL if the index is out of bounds. \remarks
   * To get the texture of a specific texture type, use GetSrcObject(). For
   * example: \code FbxFileTexture* lFileTexture =
   * lScene->GetSrcObject<FbxFileTexture>(i); FbxLayeredTexture* lLayeredTexture
   * = lScene->GetSrcObject<FbxLayeredTexture>(i); FbxProceduralTexture*
   * lProceduralTexture = lScene->GetSrcObject<FbxProceduralTexture>(i);
   * \endcode */
  ...
  /** Helper method for determining the number of nodes that have curves on
   * surface attributes in the scene. Since the curve-on-surface nodes are
   * connected to nurbs geometry and not any FbxNode in the scene, they won't
   * normally be picked up in a graph traversal. \return The number of
   * curve-on-surface nodes in the scene */
   ...
   * Walks all the node attributes defined in the scene and synchronize the Show
   * property of all the nodes that reference the node attribute so that they
   * all contain the same value. This method should be called after the FBX
   * scene is completely created, typically right after the calls to the
   * FbxImporter::Import() or just before the calls to the
   * FbxExporter::Export(). \remarks Applications only need to call this method
   * if their interpretation of the Show property implies that setting the Show
   * state on one instance affect all of them. \see FbxNode::Visibility
   * property, FbxNode::Show property */
   ...
   * upon successful return. \param pSelected If \c true, only take into account
   * selected geometry, otherwise take all geometry into account. \param pTime
   * If different from FBXSDK_TIME_INFINITE, time used to compute the bounding
   * box for deformed geometry. \return \c true if successful, otherwise \c
   * false. \remark If geometry have been unloaded from memory, their bounding
   * box cannot be calculated and will use any value set previously. */
  bool
  ComputeBoundingBoxMinMaxCenter(FbxVector4 &pBBoxMin,
                                 FbxVector4 &pBBoxMax,
...
*****************************************************************************************************************************/
  ...
  FbxClassId
  ConvertAttributeTypeToClassID(FbxNodeAttribute::EType pAttributeType);

  // These data structures are only used for legacy FBX files (version 6.x and
  // earlier). The validity of their content is not guaranteed with the most
  // recent versions.
  ...
  //  Clone this scene object (and everything else it contains if clone type is
  //  eDeepClone)
  ...
  void ReindexMaterialConnections(); // called to make sure that eIndex is
                                     // remapped to eIndexToDirect

  FbxMultiMap *AddTakeTimeWarpSet(char *pTakeName);
  FbxMultiMap *GetTakeTimeWarpSet(char *pTakeName);

  // This function will destroy the scene (and all the objects directly
  // connected to it) without sending the Connect notifications nor trying to
  // disconnect the objects first. This is a bypass of the intended workflow and
...
private:
  ...
  void ConnectTextureLayerElement(FbxLayerContainer *pLayerContainer,
                                  ...
                                  FbxNode *pParentNode);
  void
  BuildTextureLayersDirectArrayForLayerType(FbxLayerContainer *pLayerContainer,
                                            FbxLayerElement::EType pLayerType);
