 ...
 * A document is a collection (FbxCollection) of objects (FbxObject), called the
 * root member objects. This is because these objects each form the root of an
 * object graph. The manager (FbxManager) has access to all documents, scenes
 * and libraries.
 *
 * A document can be contained in another document, thus, a hierarchy of
 * documents can be built. The root of all documents is simply called the root
 * document.
 *
 * A document manages animation stacks (FbxAnimStack). It also provides access
 * to animation stack information (FbxTakeInfo).
 *
 * A document carries information in its FbxDocumentInfo.
 *
 * Documents manage peripherals to load and unload objects (see class
 * FbxPeripheral), as well as references to other objects or documents.
 *
...
public:
  ...
  /** Return the number of objects of the document that satisfy the given
   * criteria. \param pCriteria Criteria for selecting objects. \return The
   * number of objects satisfying the given criteria.
   */
  ...
  /** Return the document member which satisfies given criteria, for given
   * index. \param pCriteria Criteria for selecting objects. \param pIndex
   * Selection index.
   */
   ...
   * \return \c true if pMember is an object part of the document, \c false
   * otherwise.
   */
   ...
   */
  //@{
  /** Set the current peripheral to be used to load or unload objects from this
   * document. \param pPeripheral The peripheral to be set.
   */
   ...
   * currently set peripheral. \param pStatus The FbxStatus object to hold error
   * codes. \return The number of objects that the document has been able to
   * unload.
   */
   ...
   * currently set peripheral. \param pStatus The FbxStatus object to hold error
   * codes. \return The number of loaded objects.
   */
  ...
  /**
   * Fills an array of pointers to documents that reference objects in this
   * document.
   *
  ...
  int GetReferencingDocuments(
      FbxArray<FbxDocument *> &pReferencingDocuments) const;
  ...
  int GetReferencingObjects(const FbxDocument *pFromDoc,
                            FbxArray<FbxObject *> &pReferencingObjects) const;

  /**
   * Fills an array of pointers to documents that are referenced by objects in
   * this document.
   *
   * \param pReferencedDocuments Array of pointers to documents.
   * \returns Number of documents that are referenced by objects in this
   * document.
   */
  int GetReferencedDocuments(
      FbxArray<FbxDocument *> &pReferencedDocuments) const;
  ...
  int GetReferencedObjects(const FbxDocument *pToDoc,
                           FbxArray<FbxObject *> &pReferencedObjects) const;

  /**
    * Gets the path string to the root document, if the current document is
    contained in another document. \returns Path to the root document.
    */
  FbxString GetPathToRootDocument(void) const;
  /**
   * Gets the document path to the root document as an array of documents, if
   * the current document is contained in another document. \param pDocumentPath
   * Array of FbxDocument to store the document path. \param pFirstCall
   * Recursive flag: always use pFirstCall = \c true.
   */
  void GetDocumentPathToRootDocument(FbxArray<FbxDocument *> &pDocumentPath,
                                     bool pFirstCall = true) const;

  /**
    * Tells if this document is a root document.
            \return \c false if the current document is contained in another
    document, \c true otherwise.
    */
   ...
   */
  //@{
  /** Holds the name of the FbxAnimStack that the application uses for animation
   * in this document.
   */
   ...
   * \return \c true if the FbxAnimStack has been destroyed and \c false
   * otherwise.
   */
   ...
   * \remark Although takes are deprecated, class FbxTakeInfo is not deprecated
   * and now contains animation stack information.
   */
  //@{
  /** Set information about an animation stack.
   * \param pTakeInfo Animation stack information. Field FbxTakeInfo::mName
   * specifies the targeted animation stack. \return \c true if animation stack
   * is found with this name, and if information is set.
   */
   ...
   * \return Animation stack information, or \c NULL if animation stack isn't
   * found or has no information set for this document.
   */
