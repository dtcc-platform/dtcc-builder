...
public:
  ...
  enum ETangentMode
  {
    ...
    eTangentUser =
        0x00000400, //!< Next slope at the left equal to slope at the right.
    eTangentGenericBreak = 0x00000800, //!< Independent left and right slopes.
    eTangentBreak =
        eTangentGenericBreak |
        eTangentUser, //!< Independent left and right slopes, with next slope at
                      //!< the left equal to slope at the right.
    eTangentAutoBreak =
        eTangentGenericBreak |
        eTangentAuto, //!< Independent left and right slopes, with auto key.
    eTangentGenericClamp =
        0x00001000, //!< Clamp: key should be flat if next or previous key has
                    //!< the same value (overrides tangent mode).
    eTangentGenericTimeIndependent =
        0x00002000, //!< Time independent tangent (overrides tangent mode).
    eTangentGenericClampProgressive =
        0x00004000 |
        eTangentGenericTimeIndependent //!< Clamp progressive: key should be
                                       //!< flat if tangent control point is
                                       //!< outside [next-previous key] range
                                       //!< (overrides tangent mode).
  };
  ...
  enum EWeightedMode
  {
    eWeightedNone = 0x00000000,  //!< Tangent has default weights of 0.333; we
                                 //!< define this state as not weighted.
    eWeightedRight = 0x01000000, //!< Right tangent is weighted.
    eWeightedNextLeft = 0x02000000, //!< Left tangent is weighted.
    eWeightedAll =
        eWeightedRight |
        eWeightedNextLeft //!< Both left and right tangents are weighted.
  };
  ...
  {
    eConstantStandard =
        0x00000000, //!< Curve value is constant between this key and the next
    eConstantNext =
        0x00000100 //!< Curve value is constant, with next key's value
  };

  //! Velocity mode. Velocity settings speed up or slow down animation on either
  //! side of a key without changing the trajectory of the animation. Unlike
  //! Auto and Weight settings, Velocity changes the animation in time, but not
  //! in space.
  enum EVelocityMode
  {
    ...
    eVelocityAll =
        eVelocityRight |
        eVelocityNextLeft //!< Both left and right tangents have velocity.
  };
  ...
  enum ETangentVisibility
  {
    ...
    eTangentShowBoth =
        eTangentShowLeft |
        eTangentShowRight //!< Both left and right tangents are visible.
  };

  //! FbxAnimCurveKey data indices for cubic interpolation tangent information.
  enum EDataIndex
  {
    eRightSlope = 0, //!< Index of the right derivative, User and Break tangent
                     //!< mode (data are float).
    eNextLeftSlope = 1, //!< Index of the left derivative for the next key, User
                        //!< and Break tangent mode.
    eWeights =
        2, //!< Start index of weight values, User and Break tangent break mode
           //!< (data are FbxInt16 tokens from weight and converted to float).
    eRightWeight = 2,    //!< Index of weight on right tangent, User and Break
                         //!< tangent break mode.
    eNextLeftWeight = 3, //!< Index of weight on next key's left tangent, User
                         //!< and Break tangent break mode.
    ...
    eNextLeftVelocity =
        5, //!< Index of velocity on next key's left tangent, Velocity mode
 ...
 * \nosubgrouping
 *
 * \remarks Users should not use this class directly, but always use
 * FbxAnimCurveKey. A FbxAnimCurveKey has a FbxAnimCurveKey_Impl. But
 * FbxAnimCurveKey_Impl is just an implementation interface,
 */
...
public:
   ...
   */
  virtual FbxAnimCurveKey_Impl &
  operator=(const FbxAnimCurveKey_Impl &pFKey) = 0;
   ...
   *	\param pData0	Tension. Controls the amount of curvature in the
   *animation curve. The higher the tension is, the more linear the curve looks.
   *When the tension is low, the curve looks rounder or wider. \param pData1
   *Continuity. Controls the smoothness or singularity of the curve on the key.
   *	\param pData2	Bias. Controls if the effect of tension and continuity
   *affect the curve before or after the key.
   */
   ...
   */
  virtual void
  SetInterpolation(FbxAnimCurveDef::EInterpolationType pInterpolation) = 0;

  /** Get key's tangent mode.
   *   \param pIncludeOverrides Include override flags: Break, Clamp,
   *Time-Independent. \return Tangent mode of the key. \remarks This method is
   *meaningful for cubic interpolation only. Using this method for non cubic
   *interpolated key will return unpredictable value.
   */
  ...
  /** Set key's tangent weight mode as double value (cubic interpolation, non
   *TCB tangent mode). \param pTangentWeightMode	Weight mode. \param
   *pMask				Used to select the affected tangents.
   *	\remarks This method is meaningful for cubic interpolation only.
   *   The pMask will be used to cancel out the current tangent weight mode
   *first, and then be used to define which tangent to select to affect.
   *
   *   Sample01:
   *   \code
   ...
   *   lAnimCurveKey->SetTangentWeightMode(FbxAnimCurveDef::eWeightedRight,
   *FbxAnimCurveDef::eWeightedRight); \endcode pMask is eWeightedRight, it will
   *first be used to cancel out the current tangent weight mode
   *eWeightedNextLeft, since they are not the same, it fails to cancel it out.
   *   Then the mask eWeightedRight will be used to define which tangent should
   *be affected, since it is the same as pTangentWeightMode (eWeightedRight), so
   *the eWeightedRight should be affected. In total, after above calls, both
   *eWeightedNextLeft and eWeightedRight of this key are affected, so
   *   lAnimCurveKey->GetTangentWeightMode() will be
   ...
   *   lAnimCurveKey->SetTangentWeightMode(FbxAnimCurveDef::eWeightedRight,
   *FbxAnimCurveDef::eWeightedNextLeft); \endcode pMask is eWeightedNextLeft, it
   *will first be used to cancel out the current tangent weight mode
   *eWeightedAll, it will cancel out affect on eWeightedNextLeft, but leave
   *affect on eWeightedRight. Then the mask eWeightedNextLeft will be used to
   *define which tangent should be affected, since it is not the same as
   *pTangentWeightMode (eWeightedRight), so the pMask won't affect anything in
   *this step. In total, after above calls, only eWeightedRight of this key is
   *still affected, so lAnimCurveKey->GetTangentWeightMode() will be
   *FbxAnimCurveDef::eWeightedRight.
   */
  virtual void SetTangentWeightMode(
      FbxAnimCurveDef::EWeightedMode pTangentWeightMode,
      FbxAnimCurveDef::EWeightedMode pMask = FbxAnimCurveDef::eWeightedAll) = 0;

  /** Adjust the actual tangent of the key so that the tangent control point
   * (tangent extremity) stays closer to where it should be. This is required
   * because the weight value gets imprecise when it is small (it is stored as a
   * fixed point value). This method must be called when setting the weight
   * coming from a source where the precision is the same. It must be called
   *   after the tangent value has been set.
   *   \remark Do not use this call repetitively (from an interactive editor for
   * example) because this function will create imprecision on the tangent
   * value. \param pIndex FbxAnimCurveDef::EDataIndex \param pWeight New tangent
   * weight value.
   */
  virtual void
  SetTangentWeightAndAdjustTangent(FbxAnimCurveDef::EDataIndex pIndex,
                                   double pWeight) = 0;
  ...
  /** Set key's tangent velocity mode as double value (cubic interpolation, non
   *TCB tangent mode). \param pTangentVelocityMode	Velocity mode. \param
   *pMask				Used to select the affected tangents
   *	\remarks This method is meaningful for cubic interpolation only.
   *   The pMask will be used to cancel out the current tangent velocity mode
   *first, and then be used to define which tangent to select to affect.
   *
   *   \see The documentation of SetTangentWeightMode for more details and
   *samples about how the pMask works.
   */
  virtual void SetTangentVelocityMode(
      FbxAnimCurveDef::EVelocityMode pTangentVelocityMode,
      FbxAnimCurveDef::EVelocityMode pMask = FbxAnimCurveDef::eVelocityAll) = 0;
   ...
   * \param pIndex Data index to specify which data to get value, the index is
   * dependent on the key tangent mode. \return The value of the specified data.
   *
   * \code
   ...
   * \param pIndex Data index to specify which data to get value, the index is
   * dependent on the key tangent mode. \param pValue The data value to set.
   *
   * \code
   * FbxAnimCurveKey* lKey; // we suppose this is a valid pointer
  ...
  /** Set tangent visibility mode. This would indicate what part of the tangent
   *is visible in a graphical interface. \param pVisibility	Tangent
   *visibility mode. \remarks This method is meaningful for cubic interpolation
   *only.
   */
  virtual void
  SetTangentVisibility(FbxAnimCurveDef::ETangentVisibility pVisibility) = 0;
   ...
   * When this flag is on (FbxAnimCurveDef::eTANGEAT_BREAK will be set), the
   * key's left and right slopes are independent. When this flag is off, the
   * key's left and right slope are equal. \param pVal Break flag (\c true or \c
   * false). \remarks This method is meaningful for User
   * (FbxAnimCurveDef::eTangentUser) and Auto (FbxAnimCurveDef::eTangentAuto)
   * tangent modes only.
   */
  virtual void SetBreak(bool pVal) = 0;

  /** Get if the tangent has a break.
   * When this flag is set (FbxAnimCurveDef::eTANGEAT_BREAK), the key's left and
   * right slopes are independent. When this flag is off, the key's left and
   * right slope are equal. \return Break flag (\c true or \c false). \remarks
   * This method is meaningful for User (FbxAnimCurveDef::eTangentUser) and Auto
   * (FbxAnimCurveDef::eTangentAuto) tangent modes only.
   */
  virtual bool GetBreak() const = 0;
};

/** This is the interface for the FBX animation curve keys.
 * A key is defined by a time and a value. It also has tangents that control how
 *the animation curve enters and exits the key. \nosubgrouping
 *
...
public:
   ...
   * \return Time value.
   */
  FbxTime GetTime() const { return FbxAnimCurveKeyBase::GetTime(); }

  /** Set time value.
   * \param pTime Time value to set.
   */
  void SetTime(const FbxTime &pTime) { FbxAnimCurveKeyBase::SetTime(pTime); }
   ...
   * \return The value of the key.
   */
  float GetValue() const { return mImpl->GetValue(); }

  /** Set the key value.
   * \param pValue The value to set.
   */
  void SetValue(float pValue) { mImpl->SetValue(pValue); }
   ...
   * tangent modes only.
   */
  void SetBreak(bool pVal) { mImpl->SetBreak(pVal); }
   ...
   * (FbxAnimCurveDef::eTangentAuto) tangent modes only.
   */
  bool GetBreak() const { return mImpl->GetBreak(); }

  /** Get key implementation.
   * \return Pointer to implemented instance.
   */
  FbxAnimCurveKey_Impl *GetImpl() const { return mImpl; }
   ...
   */
  static void SetCopyAllocatorFct(
      FbxAnimCurveKey_Impl *(*pCopyAllocatorFct)(FbxAnimCurveKey_Impl *));

  /** Set deallocator function
   * \param pDeallocatorFct Deallocator function
   */
  static void
  SetDeallocatorFct(void (*pDeallocatorFct)(FbxAnimCurveKey_Impl *));
...
/** An animation curve, defined by a collection of keys (FbxAnimCurveKey), and
indicating how a value changes over time.
* Since an animation curve is a function, on a given animation curve, only one
key per time is
* allowed. The keys are sorted
* in time order. They can be accessed by their index on the curve, from 0 to
FbxAnimCurve::KeyGetCount-1.
...
* no velocity. Velocity settings speed up or slow down animation on either side
of
* a key without changing the trajectory of the animation. Unlike Auto and Weight
settings,
* Velocity changes the animation in time, but not in space.
*
* \nosubgrouping
* \remarks FbxAnimCurve is now the main animation animation curve object of the
SDK.
...
* lAnimCurve->KeySet(lKeyIndex, lTime, 0.0,
FbxAnimCurveDef::eInterpolationLinear);

* // Second key: time 20s, value -3600
* // Since this curve will describe rotation, each cube will rotate 10 times
around itself during 20 seconds.
* lTime.SetSecondDouble(20.0);
* lKeyIndex = lAnimCurve->KeyAdd(lTime);
* lAnimCurve->KeySet(lKeyIndex, lTime, -3600,
FbxAnimCurveDef::eInterpolationLinear);
...
public:
  ...
  /** Call this function prior to adding, removing or editing keys of an
   * animation curve. Call function FbxAnimCurve::KeyModifyEnd() after
   * modification.
   */
  virtual void KeyModifyBegin() = 0;

  /** Call this function after adding, removing or editing keys of an animation
   * curve. Function FbxAnimCurve::KeyModifyBegin() must have been called prior
   * to modify the keys.
   */
  ...
  /** Add a given key at given time. The new key is appended after all the other
   * animation curve's keys. Function FbxAnimCurve::KeyInsert() should be used
   * instead if the key is to be added in the curve and not at the end. This
   * function does not respect the interpolation type and tangents of the
   * neighboring keys. If there is already a key at the given time, the key is
   * modified and no new key is added.
   *
   ...
   * function is called in a loop, initialize this value to 0 and let it be
   * updated by each call. \return Index of the key at given time, no matter if
   * it was added or already present.
   *
   * \remark Key value, interpolation type and tangent mode must be set
   *  explicitly afterwards.
   */
  virtual int
  KeyAdd(FbxTime pTime, FbxAnimCurveKeyBase &pKey, int *pLast = NULL) = 0;

  /** Add a key at given time. The new key is appended after all the other
   * animation curve's keys. Function FbxAnimCurve::KeyInsert() should be used
   * instead if the key is to be added in the curve and not at the end. This
   * function does not respect of the interpolation type and tangents of the
   * neighboring keys. If there is already a key a the given time, no key is
   * added.
   *
   * \param pTime Time of the new key.
   * \param pLast Index of the last processed key to speed up search. If this
   * function is called in a loop, initialize this value to 0 and let it be
   * updated by each call. \return Index of the key at given time, no matter if
   * it was added or already present. \remark Key value, interpolation type and
   * tangent mode must be set explicitly afterwards.
   */
   ...
   *
   * For example (using seconds for clarity), if there is a key at time 10s with
   * index 5, and a key at time 11s with index 6, KeyFind(10.3s) would
   * return 5.3.
   */
   ...
   * The key at index pKeyIndex is retrieved and modified according to the other
   *parameters. The key must have been previously created, for example using
   *KeyAdd. Use FbxAnimCurve::SetTCB() in the specific case of setting a key
  ...
  KeySet(int pKeyIndex,
         ...
         float pVelocity1 = FbxAnimCurveDef::sDEFAULT_VELOCITY) = 0;

  /**	Set a key with cubic interpolation, TCB tangent mode.
   * The key at index pKeyIndex is retrieved and modified according to the other
   *parameters. The TCB mode controls the tension, continuity, and bias of the
   *curve. \param pKeyIndex  Index of the key. \param pTime	Key time. \param
   *pValue	Key value. \param pData0	Tension. Controls the amount of
   *curvature in the animation curve. The higher the tension is, the more linear
   * the curve looks. When the tension is low, the curve looks rounder or wider.
   *	\param pData1	Continuity. Controls the smoothness or singularity of
   *the curve on the key. \param pData2	Bias. Controls if the effect of tension
   *and continuity affect the curve before or after the key.
   */
   ...
   */
  virtual void
  KeySetInterpolation(int pKeyIndex,
                      FbxAnimCurveDef::EInterpolationType pInterpolation) = 0;

  /** Get key's constant mode.
   *	\note This method is only relevant if the key's interpolation type is
   *constant (eInterpolationConstant). Using this method on a key with an other
   *interpolation type will return unpredictable value. \param pKeyIndex Index
   *of the queried key. \return Key constant mode.
   */
  ...
  virtual void KeySetConstantMode(int pKeyIndex,
                                  FbxAnimCurveDef::EConstantMode pMode) = 0;
  ...
  virtual void KeySetTangentMode(int pKeyIndex,
                                 FbxAnimCurveDef::ETangentMode pTangent) = 0;
  ...
  /** Set or unset the tangent break. When this flag is set
   * (FbxAnimCurveDef::eTangentBreak), the key's left and right slopes are
   * independent. When this flag is off, the key's left and right slope are
   * equal. This method is relevant for User (FbxAnimCurveDef::eTangentUser) and
   * Auto (FbxAnimCurveDef::eTangentAuto) tangent modes only. \param pKeyIndex
   * Index of the key. \param pVal Break flag (\c true or \c false).
   */
  virtual void KeySetBreak(int pKeyIndex, bool pVal) = 0;

  /** Get if the tangent has a break. When this flag is set
   * (FbxAnimCurveDef::eTangentBreak), the key's left and right slopes are
   * independent. When this flag is off, the key's left and right slope are
   * equal. This method is relevant for User (FbxAnimCurveDef::eTangentUser) and
   * Auto (FbxAnimCurveDef::eTangentAuto) tangent modes only. \param pKeyIndex
   * Index of the queried key. \return Break flag (\c true or \c false).
   */
   ...
   * FbxAnimCurveDef::eTangentUser, FbxAnimCurveDef::eTangentBreak or
   * FbxAnimCurveDef::eTangentAuto.
   */
  virtual void KeySetLeftDerivative(int pIndex, float pValue) = 0;

  /** Get the left auto parametric of a key. This is used to compute the slope
   * of Auto and User keys. \param pIndex Index of the key. \param
   * pApplyOvershootProtection Clamp flag (eGENERIC_CLAMP) is taken into
   * account. \return Left auto parametric. \remark Result is undetermined if
   * animation curve has no key or if index is out of bounds.
   */
  virtual float KeyGetLeftAuto(int pIndex,
                               bool pApplyOvershootProtection = false) = 0;
   ...
   * \param pForceDerivative If \c true, assign the tangent info's derivative
   * value to the key derivative. If \c false, use the tangent info's auto
   * parametric value to recompute the key derivative. \remark Result is
   * undetermined if animation curve has no key or if index is out of bounds.
   * This function is only relevant if previous key interpolation
   * type is eInterpolationCubic and tangent mode is
  ...
  /** Get the right auto parametric of a key. This is used to compute the slope
   * of Auto and User keys. \param pIndex Index of the key. \param
   * pApplyOvershootProtection Clamp flag (eGENERIC_CLAMP) is taken into
   * account. \return Right auto parametric. \remark Result is undetermined if
   * animation curve has no key or if index is out of bounds.
   */
   ...
   * \return \c true if the key is left weighted (Weight mode is
   * eWEIGHT_WEIGHTED_RIGHT or eWeightedAll). \c false otherwise. \remark Result
   * is undetermined if animation curve has no key or if index is out of bounds.
   */
  virtual bool KeyIsLeftTangentWeighted(int pIndex) const = 0;

  /** Get the right tangent weight mode of a key.
   * \param pIndex Index of queried key.
   * \return \c true if the key is right weighted (Weight mode is eWeightedRight
   * or eWeightedAll). \c false otherwise. \remark Result is undetermined if
   * animation curve has no key or if index is out of bounds.
   */
   ...
   * \param pIndex Index of the key.
   * \return Left tangent weight, or eDEFAULT_WEIGHT (0.333...) if left tangent
   * is not weighted. \remark This function is only relevant if key
   * interpolation type is eInterpolationCubic.
   */
  virtual float KeyGetLeftTangentWeight(int pIndex) const = 0;

  /** Get the weight value component of the right tangent of a key.
   * \param pIndex Index of the key.
   * \return Right tangent weight, or eDEFAULT_WEIGHT (0.333...) if right
   * tangent is not weighted. \remark This function is only relevant if key
   * interpolation type is eInterpolationCubic.
   */
   ...
   * \param pAdjustTan If true, recompute the tangent height to compensate for
   * very small weights. \remarks This function is only relevant if previous key
   * interpolation type is eInterpolationCubic and tangent mode is
   * FbxAnimCurveDef::eTangentUser or FbxAnimCurveDef::eTangentBreak. The
   * tangent is automatically set in weighted mode. The pAdjustTan option will
   * only produce correct results provided that the tangent has already been set
   * before calling this function.
   */
  virtual void KeySetLeftTangentWeight(int pIndex,
                                       float pWeight,
                                       bool pAdjustTan = false) = 0;

  /** Set the right tangent weight of a key.
   * \param pIndex Index of the key.
   * \param pWeight Weight to set on the right tangent.
   * \param pAdjustTan If true, recompute the tangent height to compensate for
   * very small weights. \remarks This function is only relevant if key
   * interpolation type is eInterpolationCubic and tangent mode is
   * FbxAnimCurveDef::eTangentUser or FbxAnimCurveDef::eTangentBreak. The
   * tangent is automatically set in weighted mode. The pAdjustTan option will
   * only produce correct results provided that the tangent has already been set
   ...
   * function is called in a loop, initialize this value to 0 and let it
   * be updated by each call.
   * \return Animation curve value on given time, or animation curve's default
   * value if animation curve has no key. \remarks This function takes
   ...
   * keys from source animation curve. If \c false, keys in current animation
   * curve are left as is.
   */
  virtual void CopyFrom(FbxAnimCurve &pSource, bool pWithKeys = true) = 0;

  /** Retrieve the value of the parent curve node channel.
   * \param pCurveNodeIndex The index of the parent curve node, if more than one
   * exist. \return The value of the parent curve node channel of this curve.
   * \remark In most case, the curve will have a single curve node channel as
   * destination. However,
   * it is possible that more are connected, hence why we provide the curve node
   * index parameter. */
  virtual float GetValue(int pCurveNodeIndex = 0) = 0;

  /** Set the value to the parent curve node channel.
   * \param pValue The value to set to the parent curve node channel of this
   * curve. \param pCurveNodeIndex The index of the parent curve node, if more
   * than one exist. \remark In most case, the curve will have a single curve
   * node channel as destination. However, it is possible that more are
   * connected, hence why we provide the curve node index parameter. */
