...
public:
  ...
  /** Constructor.
   */
  FbxAnimCurveKeyBase() { mTime = FBXSDK_TIME_ZERO; }
   ...
   * \return Time value.
   */
  virtual FbxTime GetTime() const { return mTime; }

  /** Set time value.
   * \param pTime Time value to set.
   */
  virtual void SetTime(const FbxTime &pTime) { mTime = pTime; }
};
...
public:
   ...
   * let it be updated by each call. \return Index of the key at given time, no
   * matter if it was added or already present.
   */
  virtual int
   ...
   *     <li>MIRROR_REPETITION means the entire function curve is looped once
   * backward, once forward and so on. <li>KEEP_SLOPE means a linear function
   * with a slope matching the first/last key. <li>RELATIVE_REPETITION means
   * entire function curve is looped and one loop is relative to the last loop
   * in value.</ul>
   */
  //@{
   ...
   * \param pLast Index of the last processed key to speed up search. If this
   *              function is called in a loop, initialize this value to 0 and
   * let it be updated by each call. \return Evaluated curve value. \remarks
