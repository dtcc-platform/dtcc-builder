...
/** This class hold results from animation evaluations. To clear an evaluation
 * state for re-use, it is possible to invalidate or to reset it. For the same
 * scene with the same objects, invalidating an evaluation state is the quickest
 * way to clear an evaluation state object for re-use because it only zeroes all
 * the entries. A reset will delete all the entries. Unless the scene changes,
 * for performance purposes it is recommended to invalidate evaluation states
...
public:
  ...
  /** Reset an evaluation state by deleting the cache it contains. This will
   * remove all entries in the cache. */
  void Reset();

  /** Start a new evaluation state frame by zeroing the cache it contains, and
   * changing its associated time. All node and property entries will remain in
   * the list, but their evaluation state will not be up-to-date. \param pTime
...
public:
  ...
  bool mUpToDate; //!< If \c true, the evaluation state element is up-to-date
...
public:
  ...
  FbxVector4 mLT; //!< Used to hold result value of LclTranslation property from
                  //!< node evaluation.
  FbxVector4 mLR; //!< Used to hold result value of LclRotation property from
                  //!< node evaluation.
  FbxVector4 mLS; //!< Used to hold result value of LclScaling property from
                  //!< node evaluation.
  FbxAMatrix mLX; //!< Used to hold result local transform matrix from node
                  //!< evaluation. Pivots, offsets, pre/post rotation and all
                  //!< other transforms are taken into consideration.
  FbxAMatrix mGX; //!< Used to hold result global transform matrix from node
                  //!< evaluation. Pivots, offsets, pre/post rotation and all
                  //!< other transforms are taken into consideration.

  /** mTransform is used to hold the corresponding FbxTransform of the node.
   * This FbxTransform takes all transform-related info, including pivots,
   * offsets, pre/post rotation, rotation order, limits, etc.
   * The evaluation is actually done through the utility functions of
   * FbxTransform. */
