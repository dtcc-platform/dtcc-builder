...
public:
   ...
   * \return \c True if it is a member of the collection, returns \c false if it
   * is not a member.
   */
   ...
   */
  //@{
  /** Returns the number of objects contained within the collection that meet
   * the specified criteria. \param pCriteria Defines a set of criteria that
   * each object must meet in order to be included in the results. \return The
   * number of objects the collection contains that meet the specified criteria.
   */
  ...
  /** Returns the member at the given index in the collection if it meets the
   * specified criteria. \param pCriteria Defines a set of criteria that the
   * returned object must meet. \param pIndex The given index. \return The
   * member at the given index if it meets the criteria; NULL otherwise.
   */
  inline FbxObject *GetMember(const FbxCriteria &pCriteria,
                              int pIndex = 0) const
  {
    return GetSrcObject(pCriteria, pIndex);
  }

  /** Searches for a member with the given name that also meets the given
   * criteria. \param pCriteria Defines a set of criteria that the returned
   * object must meet. \param pName Member name. \return The member with the
   * given name if it meets the criteria; NULL if no match could be found.
   */
  inline FbxObject *FindMember(const FbxCriteria &pCriteria,
   ...
   * \param pSelection If \c true, all objects are selected, if \c false, all
