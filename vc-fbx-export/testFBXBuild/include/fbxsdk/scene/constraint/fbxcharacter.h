 ...
 *
 * \brief This class represents a link between a given FBX node and the
 * associated node in the character hierarchy. It also contains the transform
 * matrix (offset) for the linked character's node.
 */
...
public:
  ...
  FbxNode *mNode; //! The character's node in hierarchy linked to this character
                  //! link.
  FbxString mTemplateName; //! A template name is a naming convention that is
                           //! used to automatically map the nodes of other
                           //! skeletons that use the same naming convention.
  ...
  FbxVector4
      mParentROffset; //! Get the parent offset rotation of this character link
  bool mHasRotSpace;  //! \c true if this character link has a defined rotation
                      //! space
 ...
 * skeleton system is composed of multiple named node (skeleton). This class
 * contains all methods to setup an exported character or query information on
 * an imported character. This class also contains some methods for manipulating
 * the FbxCharacterLink, FbxControlSet
 *
 * The most important part of a FbxCharacter is the FbxCharacterLink. There is
 * one FbxCharacterLink for each characterized node. For more information see
 * FbxCharacterLink class documentation.
 *
...
public:
  ...
  /** \enum EGroupId Define ID for character groups that contains multiple
   * character nodes. */
  enum EGroupId
  {
    eGroupBase,
   ...
   * \param pInputObject     Pointer to input character if input type equals
   * eInputCharacter, otherwise \c NULL.
   */
   ...
   *                  - If the input type is set to eInputCharacter. The
   * returned pointer can be casted to a pointer of type FbxCharacter.
   *                  - \c Null pointer if the input object has not been set, or
   * if the input type is not set to eInputCharacter.
   */
  FbxObject *GetInputObject() const;

  /** Associate a character link to a given character node ID. If a character
   * link already exists for this character node ID, the character link will be
   * removed. \param pCharacterNodeId      Character node ID. \param
   * pCharacterLink        Character link. \param pUpdateObjectList     Set to
   * \c true to update the object list (default value). \return \c true if
   ...
   * \param pCharacterLink       Optional pointer to receive the character link
   * if function succeeds. \return                     \c true if successful, \c
   * false otherwise.
   */
  bool GetCharacterLink(ENodeId pCharacterNodeId,
                        FbxCharacterLink *pCharacterLink = NULL) const;
   ...
   * pCharacterGroupId character group.
   */
  ...
  /** Find the character group index associated with a given character node
   * name. \param pName                 Character node name. \param
   * pForceGroupId         Set to \c true to force the character group ID.
   ...
   * \param pCharacterGroupId    if the Character node ID is found, the method
   * returns the group ID through this parameter \param pIndex               if
   * the Character node ID is found, the method returns the index through this
   * parameter \remarks                    Only works for a character node ID
   * that is part of a group. \return                     \c true if successful,
   * \c false otherwise.
   */
   ...
   * \param pVersion             if the node ID is found, the method returns the
   * version through this parameter \remarks                    Only works for a
   * character node ID is part of a group. \return                     \c true
   ...
   * \param pName                if the node ID is found, the method returns the
   * node name through this parameter Since the Pointer points to internal data,
   * it is not necessary to allocate a string buffer before calling this
   * function. \return                     \c true if a name exists for the
   * given node ID.
   */
   ...
   * \param pCharacterNodeId     if the node name is found, this method returns
   * the node ID through this parameter \return                     \c true if a
   * node ID exists for the given node name.
   */
...
*****************************************************************************************************************************/
  ...
  void GetPropertyInfo(char *&pCharacterPropertyName,
                       ...
                       int pIndex) const;
  void
  GetFbxCharacterPropertyFromHIKProperty(char *&pCharacterPropertyName,
                                         char *&pCharacterPropertyModeName,
                                         EPropertyUnit &pUnit,
                                         int &pPropertyIndex,
