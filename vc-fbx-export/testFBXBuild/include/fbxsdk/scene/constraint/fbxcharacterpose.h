 ...
 *
 * Only the default position of the nodes is considered, the animation data is
 * ignored.
 *
 * You can access the content of a character pose, using the functions
 * FbxCharacterPose::GetOffset(), FbxCharacterPose::GetLocalPosition(), and
 * FbxCharacterPose::GetGlobalPosition(). Their source code is provided inline
 * as examples on how to access the character pose data.
 *
 * To create a character pose, You must first create a hierarchy of nodes under
 * the root node provided by function FbxCharacterPose::GetRootNode(). Then,
 * feed this hierarchy of nodes into the character returned by function
 * FbxCharacterPose::GetCharacter(). Offsets are set in the character links.
 * Local positions are set using FbxNode::SetDefaultT(), FbxNode::SetDefaultR(),
 * and FbxNode::SetDefaultS().
 *
 ...
 *     -# Call FbxNode::SetGlobalState() for all nodes found in the hierarchy
 * under lCharacterPose->GetRootNode();
 *     -# Call lCharacterPose->GetRootNode()->ComputeLocalState(1, true);
 *     -# Call
 * lCharacterPose->GetRootNode()->SetCurrentTakeFromLocalState(FBXSDK_TIME_ZERO,
 * true).
 */
...
public:
   ...
   * \return The pose's scene (this is a different scene than the scene the
   * character pose is in). */
...
*****************************************************************************************************************************/
  ...
  void Clone(FbxScene *pPoseScene,
             FbxObject::ECloneType pCloneType = eDeepClone,
             FbxObject *pContainer = NULL,
             void *pSet = NULL);
