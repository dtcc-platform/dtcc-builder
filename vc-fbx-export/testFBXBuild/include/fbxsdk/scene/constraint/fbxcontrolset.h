 ...
 *
 * \brief This class represents a link between a given character's FK node and
 * the associated node in the character hierarchy.
 *
 */
...
public:
  ...
  /** Assignment operator.
   * \param pControlSetLink Another FbxControlSetLink object assigned to this
   * one.
   */
  ...
  //! the nodes of other skeletons that use the same naming convention for
  //! automatic characterization.
  FbxString mTemplateName;
};

/**
 * An effector wraps a character node (FbxNode) used to animate its control rig
 * (FbxControlSet) via inverse kinematics.
 */
...
/** \class FbxControlSet
 *
 * This class contains all methods to either set-up an exported control rig or
 * query information on an imported control rig. A Control rig is a character
 * manipulation tool that lets you change the position and orientation of a
 * character to create or alter animation.
 *
 * This class also contains some methods to manipulate the FbxEffector and
 * FbxControlSetLink.
 *
 * The FbxControlSet class contains FK rig (Forward Kinematics) and IK rig
 * (Inverse Kinematics) animation. The FK rig is represented by a list of nodes
 * while the IK rig is represented by a list of effectors.
 *
 * You can access the FK rig with the FbxControlSetLink class, using the
 * functions FbxControlSet::SetControlSetLink() and
 * FbxControlSet::GetControlSetLink().
 *
 * You can access the IK rig with the FbxEffector class, using the functions
 * FbxControlSet::SetEffector() and FbxControlSet::GetEffector().
 *
...
public:
   ...
   * \param pControlSetLink      Control set link to be associated with the
   * Character node ID. \return                     \c true if successful, \c
   * false otherwise. \remarks                    You should avoid setting a
   * control set link for eCharacterLeftFloor, eCharacterRightFloor,
   * eCharacterLeftHandFloor, eCharacterRightHandFloor, eCharacterProps0,
   * eCharacterProps1, eCharacterProps2, eCharacterProps3 or eCharacterProps4.
   ...
   * \param pControlSetLink      Optional pointer that returns the control set
   * link if the function succeeds. \return                     \c true if
   * successful, \c false otherwise. \remarks                    You should
   * avoid getting a control set link for eCharacterLeftFloor,
   * eCharacterRightFloor, eCharacterLeftHandFloor, eCharacterRightHandFloor,
   *                             eCharacterProps0, eCharacterProps1,
   * eCharacterProps2, eCharacterProps3 or eCharacterProps4. None of these nodes
   * are part of a control set.
   */
  bool GetControlSetLink(FbxCharacter::ENodeId pCharacterNodeId,
                         FbxControlSetLink *pControlSetLink = NULL) const;

  /** Set an effector node for an effector node ID.
   * \param pEffectorNodeId     Effector node ID.
   * \param pEffector           Effector to be associated with the effector node
   * ID. \return                    \c true if successful, \c false otherwise.
   */
   ...
   * \param pEffector           Optional pointer that returns the effector if
   * the function succeeds. \return                    \c true if successful, \c
   * false otherwise.
   */
   ...
   * \param pEffectorNodeId     Effector node ID.
   * \param pNode               Auxiliary effector node to be associated with
   * the effector node ID. \param pEffectorSetId      Effector set ID. Set to
   * FbxEffector::eAux1Set by default. \return                    \c true if
   * successful, \c false otherwise.
   */
  bool
  SetEffectorAux(FbxEffector::ENodeId pEffectorNodeId,
                 FbxNode *pNode,
   ...
   * \param pNode               Optional pointer that returns the auxiliary
   * effector node if the function succeeds. \param pEffectorSetId      Effector
   * set ID. Set to FbxEffector::eAux1Set by default. \return \c true if
   * successful, \c false otherwise.
   */
  bool GetEffectorAux(
      FbxEffector::ENodeId pEffectorNodeId,
      FbxNode **pNode = NULL,
      FbxEffector::ESetId pEffectorSetId = FbxEffector::eAux1Set) const;
   ...
   * \return                      Effector node ID associated with the given
   * effector node name, or FbxEffector::eNodeIdInvalid (-1) if no effector node
   * with pEffectorNodeName exists.
   */
...
private:
  ...
  bool mLock3DPick;
  FbxControlSetLink
      mControlSetLink[FbxCharacter::eNodeIdCount]; // Except floor node IDs!
  FbxEffector mEffector[FbxEffector::eNodeIdCount];
  FbxNode
      *mEffectorAux[FbxEffector::eNodeIdCount][FbxEffector::eSetIdCount - 1];
