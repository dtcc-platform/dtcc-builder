 ...
 * Constraints are primarily used to impose limits on properties of objects
 * (e.g. position, orientation, scale) and to automate animation processes. A
 * <b>constrained object</b> is an object with properties constrained by one or
 * more weighted <b>constraint source</b>s. \nosubgrouping
 */
...
public:
   ...
   */
  //@{
  /** This property represents the degree of influence of a constraint from 0.0
   * (no influence) to 100.0 (full influence).
   *
   ...
   * If the value is \c false the constraint will have no effect. The default
   * value is \c true.
   *
   ...
   * constrained location when the constraint is active.
   *
  ...
  enum EType
  {
    ...
    ePosition, //! Position constraint (referred to as a point constraint in
               //! Maya).
    eRotation, //! Rotation constraint (referred to as an orient constraint in
               //! Maya).
