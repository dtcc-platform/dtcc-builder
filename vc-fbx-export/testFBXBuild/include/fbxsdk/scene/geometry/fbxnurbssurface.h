...
/** A NURBS surface is a type of parametric geometry. A NURBS surface is defined
  by the order, form, knot vector and control points in the U and V directions.

    For more information on the meaning of the form, knot vector and control
  points, see the documentation for the FbxNurbsCurve. The same concepts for
  NURBS curves apply to NURBS surfaces. NURBS surfaces simply have two
  dimensions (U and V).
...
public:
   ...
   * \remarks            Always call this function after
   * FbxNurbsSurface::SetOrder().
   */
  ...
  /** Returns the number of elements in the U-dimension knot vector. See
   * FbxNurbsCurve for more information. \return             The number of
   * elements in the U-dimension knot vector.
   */
  ...
  /** Returns the number of elements in the V-dimension knot vector. See
   * FbxNurbsCurve for more information. \return             The number of
   * elements in the V-dimension knot vector.
   */
  ...
  /** Returns the number of divisions between adjacent control points in U
   * dimension. \return             Steps in U dimension.
   */
  inline int GetUStep() const { return mUStep; }

  /** Returns the number of divisions between adjacent control points in V
   * dimension. \return             Steps in V dimension.
   */
   ...
   * \return             The number of spans in the U dimension if the surface
   * has been initialized. If the spans have not been initialized, returns -1.
   */
  int GetUSpanCount() const;

  /** Calculates the number of surface spans in the V dimension.
   * See FbxNurbsCurve::GetSpanCount() for more information.
   * \return             The number of spans in the V dimension if the surface
   * has been initialized. If the spans have not been initialized, returns -1.
   */
   ...
   * \param pFlag        If \c true, the links control points indices are
   * flipped.
   */
   ...
   * \param pIndex        Index of the curve to retrieve (Valid range is 0 to
   * GetCurveOnSurfaceCount() - 1). \return              The curve at the
   * specified index, or returns NULL if pIndex is out of range.
   */
   ...
   * \return              \c True if the curve is removed successfully, if
   * unsuccessful, returns \c false.
   */
