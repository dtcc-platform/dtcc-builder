...
/** This node attribute contains methods for accessing the properties of a
 * camera. \nosubgrouping A camera can be set to automatically point at and
 * follow another node in the hierarchy. To do this, the focus source must be
 * set to EFocusDistanceSource::eFocusSrcCameraInterest and the followed node
 * associated with function FbxNode::SetTarget(). \see FbxCameraStereo and
 * FbxCameraSwitcher.
 */
...
public:
   ...
   *              If the camera projection type is set to eOrthogonal, the
   * following options are not relevant:
   *                   - aperture format
   *                   - aperture mode
  ...
  enum EFormat
  {
    eCustomFormat, //!< The format's width, height, or pixel ratio has been
                   //!< user-specified, and matches none of the other picture
                   //!< formats.
    eD1NTSC,       //!< Standard format for D1 NTSC (720 by 486).
    eNTSC,    //!< NTSC standard for North American television broadcast (640 by
              //!< 480).
   ...
   *                    ratio mode to eFixedResolution and modifies the aspect
   * width size, height size, and pixel ratio accordingly.
   */
   ...
   * \see SetAspect, GetAspectRatioMode, AspectWidth, AspectHeight and
   * AspectRatioMode.
   */
  enum EAspectRatioMode
  {
    eWindowSize,      //!< Both width and height values aren't relevant.
    eFixedRatio,      //!< The height value is set to 1.0 and the width value is
                      //!< relative to the height value.
    eFixedResolution, //!< Both width and height values are in pixels.
    eFixedWidth,      //!< The width value is in pixels and the height value is
                      //!< relative to the width value.
    eFixedHeight      //!< The height value is in pixels and the width value is
                      //!< relative to the height value.
  };
   ...
   * \remarks              Changing the camera aspect sets the camera format to
   * eCustomFormat. \see EAspectRatioMode.
   */
   ...
   * \remarks          The value must be a positive number. Comprised between
   * 0.05 and 20.0. Values outside these limits will be clamped. Changing the
   * pixel ratio sets the camera format to eCustomFormat.
   */
   ...
   * The near plane is the minimum distance to render a scene on the camera
   * display. A synonym for the near plane is "front clipping plane". \param
   * pDistance     The near plane distance value. \remarks             The near
   * plane value is limited to the range [0.001, 600000.0] and must be inferior
   * to the far plane value.
   */
  void SetNearPlane(double pDistance);

  /** Get the near plane distance from the camera.
   * The near plane is the minimum distance to render a scene on the camera
   * display. A synonym for the near plane is "front clipping plane". \return
   * The near plane value.
   */
  double GetNearPlane() const;

  /** Set the far plane distance from camera.
   * The far plane is the maximum distance to render a scene on the camera
   * display. A synonym for the far plane is "back clipping plane". \param
   * pDistance     The far plane distance value. \remarks             The far
   * plane value is limited to the range [0.001, 600000.0] and must be superior
   * to the near plane value.
   */
  void SetFarPlane(double pDistance);

  /** Get the far plane distance from camera.
   * The far plane is the maximum distance to render a scene on the camera
   * display. A synonym for the far plane is "back clipping plane". \return The
   * far plane value.
   */
   ...
   * In photography, the aperture is the size of hole allowing light from the
   * lens to get through to the film. The aperture mode determines which values
   * drive the camera aperture. When the aperture mode is \e eHorizAndVert, \e
   * eHorizontal or \e eVertical, the field of view is used. When the aperture
   * mode is \e eFocalLength, the focal length is used.
   *
   * It is possible to convert the aperture mode into field of view or vice
   * versa using functions ComputeFieldOfView and ComputeFocalLength. These
   * functions use the camera aperture width and height for their computation.
   */
  //@{

  /** Camera's aperture formats.
   * \remarks This is designed as the same as in MotionBuilder.
   * \see SetApertureFormat, GetApertureFormat, FilmFormat, FilmWidth,
   * FilmHeight, FilmSqueezeRatio and FilmAspectRatio.
   */
  enum EApertureFormat
  {
    eCustomAperture, //!< The film size, squeeze ratio and aspect ratio has been
                     //!< user-specified, and matches none of the other aperture
                     //!< formats.
    e16mmTheatrical, //!< Film Size: 0.404, 0.295 inches. Film Squeeze
                     //!< Ratio: 1.0. Film Aspect Ratio: 1.369.
    eSuper16mm,   //!< Film Size: 0.493, 0.292 inches. Film Squeeze Ratio: 1.0.
                  //!< Film Aspect Ratio: 1.688.
    e35mmAcademy, //!< Film Size: 0.864, 0.630 inches. Film Squeeze Ratio: 1.0.
                  //!< Film Aspect Ratio: 1.371.
    e35mmTVProjection,  //!< Film Size: 0.816, 0.612 inches. Film Squeeze
                        //!< Ratio: 1.0. Film Aspect Ratio: 1.333.
    e35mmFullAperture,  //!< Film Size: 0.980, 0.735 inches. Film Squeeze
                        //!< Ratio: 1.0. Film Aspect Ratio: 1.333.
    e35mm185Projection, //!< Film Size: 0.825, 0.446 inches. Film Squeeze
                        //!< Ratio: 1.0. Film Aspect Ratio: 1.850.
    e35mmAnamorphic,    //!< Film Size: 0.864, 0.732 inches. Film Squeeze
                        //!< Ratio: 2.0. Film Aspect Ratio:1.180.
    e70mmProjection,    //!< Film Size: 2.066, 0.906 inches. Film Squeeze
                        //!< Ratio: 1.0. Film Aspect Ratio: 2.280.
    eVistaVision, //!< Film Size: 1.485, 0.991 inches. Film Squeeze Ratio: 1.0.
                  //!< Film Aspect Ratio: 1.498.
    eDynaVision,  //!< Film Size: 2.080, 1.480 inches. Film Squeeze Ratio: 1.0.
                  //!< Film Aspect Ratio: 1.405.
    eIMAX //!< Film Size: 2.772, 2.072 inches. Film Squeeze Ratio: 1.0. Film
          //!< Aspect Ratio: 1.338.
  };

  /** Set the camera aperture format.
   * \param pFormat     The camera aperture format identifier.
   * \remarks           Changing the aperture format modifies the aperture
   * width, height, and squeeze ratio accordingly.
   */
   ...
   * If the aperture mode is \e eHorizAndVert, \e eHorizontal, or \e eVertical,
   * then the field of view is used. If the aperture mode is \e eFocalLength,
   * then the focal length is used.
   */
  enum EApertureMode
  {
    eHorizAndVert, //!< Set the angle values for both the horizontal and
                   //!< vertical settings.
   ...
   * \remarks          Must be a positive value. The minimum accepted value is
   * 0.0001. Changing the aperture width sets the camera aperture format to
   * eCustomFormat.
   */
   ...
   * \remarks           Must be a positive value. The minimum accepted value is
   * 0.0001. Changing the aperture height sets the camera aperture format to
   * eCustomFormat.
   */
   ...
   * \remarks           Must be a positive value. The minimum accepted value is
   * 0.0001. Changing the squeeze ratio sets the camera aperture format to
   * eCustomFormat.
   */
   ...
   * Film gate is a border indicating the area of the camera's view as a
   * real-world camera records on film. The dimensions of the film gate
   * represent the dimensions of the camera aperture. But the film gate does not
   * represent the render region. It is the resolution gate that represents the
   * rendering resolution. The gate fit mode controls the size of the resolution
   * gate relative to the film gate.
   */
  enum EGateFit
  {
    eFitNone,     //!< No resolution gate fit.
    eFitVertical, //!< Fit the resolution gate vertically within the film gate.
    eFitHorizontal, //!< Fit the resolution gate horizontally within the film
                    //!< gate.
  ...
  enum EFilmRollOrder
  {
    eRotateFirst,   //!< The film back is first rotated then translated by the
                    //!< pivot point value.
    eTranslateFirst //!< The film back is first translated then rotated by the
                    //!< film roll value.
  };
   ...
   *
   * In the FbxSdk terminology, the Back/Front plane is the support of the
   * plate. And the plate is the support of the texture used for
   * backgrounds/foregrounds. Functions and properties identified by the "Plate"
   * name are affecting the display of the texture on the plate. The functions
   * and properties identified with the "Back/FrontPlane" are affecting the
   * plate.
   *
   * Typically a client application would place the BackPlate a small distance
   * in front of the FarPlane and the FrontPlate just behind the NearPlane to
   * avoid them to be hidden by the clipping. Unless otherwise noted, there are
   * no restrictions on the values stored by the camera object therefore it is
   * the responsibility of the client application to process the information in
   * a meaningful way and to maintain consistency between the different
   ...
   * \remarks             This method is still provided just for legacy files
   * (Fbx version 5.0 and earlier) and must not be used in any other cases.
   */
  void SetBackgroundFileName(const char *pFileName);

  /** Get the background image file name.
   * \return     Pointer to the background filename string or \c NULL if not
   * set. \remarks             This method is still provided just for legacy
   * files (Fbx version 5.0 and earlier) and must not be used in any other
   * cases.
   */
   ...
   * \remarks             The media name is a unique name used to identify the
   * background image file. \remarks             This method is still provided
   * just for legacy files (Fbx version 5.0 and earlier) and must not be used in
   * any other cases.
   */
   ...
   * \return     Pointer to the foreground filename string or \c NULL if not
   * set. \remarks             This method is still provided just for legacy
   * files (Fbx version 5.0 and earlier) and must not be used in any other
   * cases.
   */
   ...
   * \remarks             The media name is a unique name used to identify the
   * foreground image file. \remarks             This method is still provided
   * just for legacy files (Fbx version 5.0 and earlier) and must not be used in
   * any other cases.
   */
  ...
  enum EPlateDrawingMode
  {
    ...
    ePlateForeground,  //!< Image is drawn in front of models based on alpha
                       //!< channel.
    ePlateBackAndFront //!< Image is drawn behind and in front of models
                       //!< depending on alpha channel.
  };

  /** Set front plate matte threshold.
   * \param pThreshold     Threshold value on a range from 0.0 to 1.0.
   * \remarks              This option is only relevant if the image plate
   * drawing mode is set to ePlateForeground or ePlateBackAndFront.
   */
   ...
   * \remarks     This option is only relevant if the image plate drawing mode
   * is set to ePlateForeground or ePlateBackAndFront.
   */
   ...
   * Turn on both the keep ration flag and the fit image flag to scale the back
   * plate image proportionately. \param pKeepRatio     New value for the
   * BackPlateKeepRatio property.
   */
   ...
   * If this flag is on, keep the aspect ratio of the back plate image.
   * Turn on both the keep ration flag and the fit image flag to scale the back
   * plate image proportionately. \return               The value of the
   * BackPlateKeepRatio property.
   */
  bool GetBackPlateKeepRatio() const;

  /** Enable or disable the display of the texture without the need to
   * disconnect it from its plate. \param pEnable     If \c true the texture is
   * displayed, \c false otherwise. \remarks           It is the responsibility
   * of the client application to perform the required tasks according to the
   * state of this flag.
   */
  void SetShowFrontPlate(bool pEnable);

  /** Get the current state of the flag to display the front plate or not.
   * \return            \c true if show front plate is enabled, otherwise \c
   * false. \remarks           It is the responsibility of the client
   * application to perform the required tasks according to the state of this
   * flag.
   */
   ...
   * \param pFrontPlateCenter		  New value for the FrontPlateCenter
   * property.
   */
   ...
   * Turn on both the keep ration flag and the fit image flag to scale the front
   * plate image proportionately. \param pFrontPlateKeepRatio     New value for
   * the FrontPlateKeepRatio property.
   */
  ...
  {
    eRelativeToInterest, //!< The back plane distance is measured in relation to
                         //!< the camera interest.
    eRelativeToCamera    //!< The back plane distance is measured in relation to
                         //!< the camera.
  };
  ...
  {
    ePlanesDisabled, //!< Disables the front/back plane whether a texture is
                     //!< being projected or not.
    ePlanesAlways,   //!< Always shows the front/back plane, even if no texture
                     //!< has been added.
    ePlanesWhenMedia //!< Shows the front/back plane only if a texture has been
                     //!< added.
  };

  /** Set the front plane display mode. This mode can be used by the client
   * application to decide under which circumstance the front plane should be
   * drawn in the viewport. \param pMode        The front/back plane display
   * mode.
   */
  ...
  /** Set the back plane display mode. This mode can be used by the client
   * application to decide under which circumstance the back plane should be
   * drawn in the viewport. \param pMode        The front/back plane display
   * mode.
   */
   ...
   * It is the responsibility of the client application to perform the required
   * tasks according to the state of the options that are either set or returned
   * by these methods.
   */
  //@{

  /** Change the camera interest visibility flag.
   * \param pEnable     Set to \c true if the camera interest is shown, \c false
   * otherwise.
   */
   ...
   * \param pEnable      Set to \c true if the near and far planes are shown, \c
   * false otherwise.
   */
  void SetViewNearFarPlanes(bool pEnable);

  /** Get current visibility state of the camera near and far planes.
   * \return     \c true if the near and far planes are shown, \c false
   * otherwise.
   */
  ...
  enum ESamplingType
  {
    eSamplingUniform,   /*!<	The Uniform method samples each pixel at the
                           same location.   The pixel is divided into equal parts,
                           and each part is sampled.   The number of samples
                           determines the number of times the pixel is divided. */
    eSamplingStochastic /*!<	The Stochastic method randomly samples each
                           pixel.
                                                        This produces an
                           accurate color using a small number of samples. */
  };
  ...
  {
    eFocusSrcCameraInterest, /*!<	Base the depth of field on the camera
                                interest. Models at the camera interest are in
                                focus. As you move toward or away from the
   ...
   * \param pTime The time at which the camera should be evaluated.
   * \return The camera target position evaluated from property value and
   * animation. */
  FbxVector4
  EvaluateLookAtPosition(const FbxTime &pTime = FBXSDK_TIME_ZERO) const;

  /** Evaluate the camera up direction, taking target up objects into
   * consideration. \param pCameraPosition The camera current position. You can
   * retrieve this with FbxCamera::EvaluatePosition(). \param pLookAtPosition
   * The camera target position. you can retrieve this with
   * FbxCamera::EvaluateLookAtPosition(). \param pTime The time at which the
  ...
  FbxVector4 EvaluateUpDirection(const FbxVector4 &pCameraPosition,
                                 ...
                                 const FbxTime &pTime = FBXSDK_TIME_ZERO) const;

  /** Compute the camera projection matrix.
   * \param pWidth The width of the output frame.
   * \param pHeight The height of the output frame.
   * \param pVerticalFOV Calculate FOV vertically (based on height) if true or
   * horizontally (based on width) if false (Note: Only applicable in
   * perspective proj). \return The camera projection matrix, or the default
   * identity matrix in case of wrong camera parameters. */
  FbxMatrix ComputeProjectionMatrix(const int pWidth,
                                    const int pHeight,
                                    const bool pVerticalFOV = true) const;

  /** Determine if the given bounding box is in the camera's view. The input
   * points do not need to be ordered in any particular way. \param
   * pWorldToScreen The world to screen transformation. Please refer to
   * FbxCamera::ComputeWorldToScreen. \param pWorldToCamera The world to camera
   * transformation. Inverse of the matrix returned from
   * FbxAnimEvaluator::GetNodeGlobalTransform is suitable. Please refer to
   * FbxScene::GetEvaluator and FbxAnimEvaluator::GetNodeGlobalTransform. \param
   * pPoints 8 corners of the bounding box.
   * \return \c true if any of the given points are in the camera's view, \c
   * false otherwise. */
   ...
   * \param pWorldToScreen The world to screen transformation. Please refer to
   * FbxCamera::ComputeWorldToScreen. \param pWorldToCamera The world to camera
   * transformation. Inverse of the matrix returned from
   * FbxAnimEvaluator::GetNodeGlobalTransform is suitable. Please refer to
   * FbxScene::GetEvaluator and FbxAnimEvaluator::GetNodeGlobalTransform. \param
   * pPoint World-space point to test.
   * \return \c true if the given point is in the camera's view, \c false
   * otherwise. */
  bool IsPointInView(const FbxMatrix &pWorldToScreen,
                     const FbxMatrix &pWorldToCamera,
                     const FbxVector4 &pPoint) const;
  ...
  FbxMatrix ComputeWorldToScreen(int pPixelWidth,
                                 int pPixelHeight,
                                 const FbxAMatrix &pWorldToCamera) const;
  ...
  FbxVector4
  ComputeScreenToWorld(float pX,
                       float pY,
                       float pWidth,
                       float pHeight,
                       const FbxTime &pTime = FBXSDK_TIME_INFINITE) const;
  //@}
  ...
  /** This property handles the default point (XYZ coordinates) the camera is
   * looking at.
   *
   ...
   * When the camera's aperture mode is set to \e eVertical, this property has
   * no effect.
   *
   ...
   * When the camera's aperture mode is set to \e eHorizontal, this property has
   * no effect.
   *
  ...
  /** When modeling 3D objects, you often need to review or evaluate your models
   * during the creation process. You may create a camera with turn table
   * animation to view your models in 360 or certain degrees. This property
   * handles the camera's turn table angle in degrees.
   *
   ...
   *
   * \remarks This property is read-only.
   * \remarks Please use function SetPixelRatio() if you want to change its
   * value.
   *
   * Default value is 1.
   ...
   * If the resolution gate and the film gate have the same aspect ratio, then
  ...
  /** This property stores a flag that indicates to draw a border with color
   * around the camera's viewable area or not. To access this property do:
   * UseFrameColor.Get(). To set this property do: UseFrameColor.Set(FbxBool).
   *
   * Default value is false.
   */
  FbxPropertyT<FbxBool> UseFrameColor;

  /** This property is used to define the color of the border around the camera
   * view.
   *
   ...
   * When the camera is revolved about the center of interest in the perspective
   * view, the angle of a camera's sight line relative to a plane perpendicular
   * to the ground plane is referred to as its azimuth; and the angle of a
   * camera's sight line relative to the ground plane is referred to as its
   * elevation;
   *
   ...
   *
   * \remarks This property is read-only.
   * \remarks Please use function SetNearPlane() if you want to change its
   * value.
   *
   * Default value is 10.
  ...
  /** This property indicates that the clip planes should be automatically
   * computed or not.
   *
   ...
   *
   * \remarks This property is read-only.
   * \remarks Please use function SetApertureFormat() if you want to change its
   * value.
   *
  ...
  /** This property handles the horizontal offset from the center of the film
   * aperture, defined by the film height and film width. The offset is measured
   * in inches.
   *
  ...
  /** This property handles the vertical offset from the center of the film
   * aperture, defined by the film height and film width. The offset is measured
   * in inches.
   *
  ...
  /** This property handles the horizontal pivot point used for rotating the
   * film back.
   *
   ...
   *
   * Default value is 0.0.
   * \remarks FilmRollPivot value is used to compute the film roll matrix, which
   * is a component of the post projection matrix.
   */
  FbxPropertyT<FbxDouble> FilmRollPivotX;

  /** This property handles the vertical pivot point used for rotating the film
   * back.
   *
   ...
   * this value is used to compute a film roll matrix, which is a component of
   * the post-projection matrix.
   */
  FbxPropertyT<FbxDouble> FilmRollValue;

  /** This property handles how the roll is applied with respect to the pivot
   * value. eRotateFirst    The film back is first rotated then translated by
   * the pivot point value. eTranslateFirst    The film back is first translated
   * then rotated by the film roll value.
   *
   ...
   * To set this property do:
   * ViewFrustumBackPlaneMode.Set(EFrontBackPlaneDisplayMode).
   *
   ...
   * To set this property do:
   * BackPlaneDistanceMode.Set(EFrontBackPlaneDistanceMode).
   *
   ...
   * To set this property do:
   * ViewFrustumFrontPlaneMode.Set(EFrontBackPlaneDisplayMode).
   *
   * Default value is ePlanesWhenMedia.
   */
   ...
   * To set this property do:
   * FrontPlaneDistanceMode.Set(EFrontBackPlaneDistanceMode).
   *
   * Default value is eRelativeToInterest.
   */
   ...
   *
   * Default value is 1.0.
   * \remarks The application manipulating the camera has to take into
   * consideration of the BackPlateKeepRatio value too.
   */
  ...
  /** This property handles the flag to use real time Depth of Field and
   ...
   * Focus distance is the distance between the camera and the object on which
   * the camera is focused. There are two possible sources for this distance.
...
private:
  double ComputePixelRatio(FbxUInt pWidth,
                           FbxUInt pHeight,
