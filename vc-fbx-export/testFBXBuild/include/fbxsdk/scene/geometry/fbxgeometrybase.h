...
/** This class is the base class for geometric object such as meshes, NURBS and
 * patches. Use the FbxGeometryBase class to manage the control points, normals,
 * binormals and tangents of the geometries. The meaning of "control point" is
 * dependent of the geometry object type. For meshes, the "control point" is the
 * physical 3D coordinate of polygon vertices while, for NURBS, it is the the
 * actual control point on the curves defining the surface. This class also
 * allow you to define normals, binormals and tangents regardless of the type of
 * geometric object. However, in reality, applying such definitions to NURBS and
 * patches does not make much sense since these definitions would only exist at
 * the control points and inbetween them, the interpolation would certainly not
 * follow the curve.
 *
 * Geometric objects are using a system of layered data to extend their
 * construction definition. For example, a typical layer for a Mesh contains
 * Normals, UVs and Materials but client applications can decide to define
 * another set of Normals and UVs and swap them during the rendering phase to
 * produce some different results. The combinations are limitless and it would
 * be impossible to discuss them all. This example has been presented to show
 * one possible context where layers can be used. More information can be found
 * in the FbxLayerContainer and FbxLayer classes description. \nosubgrouping
 */
...
public:
   ...
   * \remarks          Any previously allocated array of control points will be
   * cleared.
   */
  virtual void InitControlPoints(int pCount);

  /** Allocates memory space for the array of normals.
   * \param pCount     The desired size for the normal array. If pCount is
   * specified, the array will be the same size as pCount. If pCount is not
   * specified, the array will be the same length as the array of control
   * points. \remarks          This function must be called after function
   * FbxLayerContainer::InitControlPoints(). \remarks          The normals
   * initialized with this function will have the ReferenceMode set to eDirect.
   * Also, the array will always be defined on layer 0.
   */
  void InitNormals(int pCount = 0);

  /** Allocates memory space for the array of normals cloned from the pSrc.
   * \param pSrc       The source geometry from which the normals information is
   * cloned. \remarks          This function must be called with the argument,
   * otherwise it does not do anything. Also, it will only process the normals
   * array defined on layer 0 of the pSrc.
   */
  void InitNormals(FbxGeometryBase *pSrc);

  /** Allocates memory space for the array of tangents on specified layer.
   * \param pCount      The desired size of the tangent array. If pCount is
   * specified, the array will be the same size as pCount. If pCount is not
   * specified, the array will be the same length as the array of control
   * points. \param pLayerIndex The specified layer index to allocate memory
   * space for the array of tangents. \param pName       The specified name for
   * the allocated tangents array. \remarks           This function must be
   * called after function FbxLayerContainer::InitControlPoints(). The tangents
   * initialized with this function will have the reference mode set to eDirect.
   */
  void InitTangents(int pCount = 0,
                    const int pLayerIndex = 0,
                    const char *pName = "");

  /** Allocates memory space for the array of tangents cloned from the pSrc on
   * the specified layer. \param pSrc        The source geometry from which the
   * tangents information is cloned. \param pLayerIndex The specified layer
   * index to allocate memory space for cloned array of tangents from the pSrc.
   * \remarks           This function must be called with the argument,
   ...
   * \param pCount      The desired size of the binormal array. If pCount is
   * specified, the array will have the same size as pCount. If pCount is not
   * specified, the array will be the same length as the array of control
   * points. \param pLayerIndex The specified layer index to allocate memory
   * space for the array of binormals. \param pName       The specified name for
   * the allocated binormals array. \remarks           This function must be
   * called after function FbxLayerContainer::InitControlPoints(). The binormals
   * initialized with this function will have the reference mode set to eDirect.
   */
   ...
   * \param pSrc        The source geometry from which the binormals information
   * is cloned. \param pLayerIndex The specified layer index to allocate memory
   * space for cloned array of binormals from the pSrc. \remarks           This
   * function must be called with the argument, otherwise it does not do
   * anything.
   */
   ...
   * \param pI2DSearch     When \c true AND the normals array reference mode is
   * eIndexToDirect, search pNormal in the existing array to avoid inserting if
   * it already exist. NOTE: This feature uses a linear search algorithm,
   * therefore it can be time consuming if the DIRECT array of normals contains
   *                       a huge number of elements.
   * \remarks              If the arrays (control points and normals) are not
   * big enough to store the values at the specified index, they will be
   * automatically resized to accommodate the new entries.
   */
   ...
   *
   * \remarks              If the array is not big enough to store the value at
   *the specified index, it will be automatically resized to accommodate the new
   *entry.
   */
   ...
   *
   * \remarks              If index is out of range, FbxVector4(0, 0, 0) is
   * returned.
   */
   ...
   * \param pI2DSearch     When \c true AND the normals array reference mode is
   *eIndexToDirect, search pNormal in the existing array to avoid inserting it
   *if it already exist. NOTE: this feature uses a linear search algorithm,
   *therefore it can be time consuming if the DIRECT array of normals contains
   *                       a huge number of elements.
   * \remarks              If the array is not big enough to store the value at
   *the specified index, it will be automatically resized to accommodate the new
   ...
   * \return         Pointer to the array of control points, or \c NULL if the
   * array has not been allocated. \remarks        Use the function
   * FbxGeometryBase::InitControlPoints() to allocate the array.
   */
  virtual FbxVector4 *GetControlPoints(FbxStatus *pStatus = NULL) const;

  /** Allocates memory space for the array of control points.
   * \param pCount     The number of control points.
   * \remarks          Any previously allocated array of control points will NOT
   * be cleared.
   */
   ...
   */
  //@{
  //! Control the geometry render state. Geometry can still cast shadows even if
  //! this is turned off.
   ...
   *  A FbxGeometryElement describes how the geometry element (normals, UVs and
   * etc.) is mapped to a geometry surface and how the mapping information is
   * arranged in memory. FbxGeometryElement is exactly the same as
   * FbxLayerElement but does not expose the geometry's layer information. Use
   * the geometry element classes to decompose the geometry without dealing with
   * layers.
   */
  //@{
   ...
   * \remarks                The created geometry element is associated with
   * this geometry automatically.
   */
   ...
   * \return                 \c True if the geometry element is removed, \c
   * false otherwise.
   */
   ...
   * \return                 A pointer to the geometry element or \c NULL if \e
  ...
  /** Returns this geometry's normal element.
   * \param pIndex           The normal geometry element index.
   * \return                 A const pointer to the geometry element or \c NULL
   * if \e pIndex is out of range.
   */
   ...
   * \return                 Total number of normal geometry elements for this
   * geometry.
   */
   ...
   * \return                 Total number of binormal geometry elements for this
   * geometry.
   */
   ...
   * \return                 Total number of tangent geometry elements for this
   * geometry.
   */
   ...
   * \return                 Total number of material geometry elements for this
   * geometry.
   */
   ...
   * \param pElementPolygonGroup   A pointer to the polygon group element to be
   * removed. \return                 \c True if the geometry element is
   * removed, \c false otherwise.
   */
  bool RemoveElementPolygonGroup(
      FbxGeometryElementPolygonGroup *pElementPolygonGroup);
   ...
   * \return                 Total number of polygon group geometry elements for
   * this geometry.
   */
   ...
   * \param pElementVertexColor   A pointer to the vertex color element to be
   * removed. \return                 \c True if the geometry element is
   * removed, \c false otherwise.
   */
  bool
  RemoveElementVertexColor(FbxGeometryElementVertexColor *pElementVertexColor);
   ...
   * \param pElementSmoothing   A pointer to the smoothing element to be
   * removed. \return                 \c True if the geometry element is
   * removed, \c false otherwise.
   */
   ...
   * \return                 Total number of smoothing geometry elements for
   * this geometry.
   */
   ...
   * \param pElementCrease   A pointer to the vertex crease element to be
   * removed. \return                 \c True if the geometry element is
   * removed, \c false otherwise.
   */
   ...
   * \return                 Total number of vertex crease geometry elements for
   * this geometry.
   */
   ...
   * \return                 Total number of edge crease geometry elements for
   * this geometry.
   */
   ...
   * \return                 Total number of hole geometry elements for this
   * geometry.
   */
   ...
   * \return                 Total number of user data geometry elements for
   * this geometry.
   */
   ...
   * \param pElementVisibility   A pointer to the visibility element to be
   * removed. \return                 \c True if the geometry element is
   * removed, \c false otherwise.
   */
  bool
  RemoveElementVisibility(FbxGeometryElementVisibility *pElementVisibility);
   ...
   * \param pTypeIdentifier  The texture channel the UVIndex refers to.
   * \return                 A pointer to the geometry element or \c NULL if \e
   * pIndex is out of range. \remarks                If \e pTypeIdentifier is
   * not specified, the function will return the geometry element regardless of
   * its texture type.
   */
  FbxGeometryElementUV *GetElementUV(
      int pIndex = 0,
   ...
   * \param pTypeIdentifier  The texture channel the UVIndex refers to.
   * \return                 A const pointer to the geometry element or \c NULL
   * if \e pIndex is out of range. \remarks                If \e pTypeIdentifier
   * is not specified, the function will return the geometry element regardless
   * of its texture type.
   */
  const FbxGeometryElementUV *GetElementUV(
      int pIndex = 0,
      FbxLayerElement::EType pTypeIdentifier = FbxLayerElement::eUnknown) const;

  /** Get the number of this geometry's UV geometry element.
   * \param pTypeIdentifier  The texture channel the UVIndex refers to.
   * \return                 Total number of UV geometry elements for this
   * geometry. \remarks                If \e pTypeIdentifier is not specified,
   * the function will return the geometry element regardless of its texture
   * type.
   */
   ...
   * \return                 A pointer to the UV geometry element or \c NULL if
   * no UV geometry element with this name exists.
   */
  FbxGeometryElementUV *GetElementUV(const char *pUVSetName);

  /** Returns this geometry's UV element.
   * \param pUVSetName       The UV set name of the UV geometry element.
   * \return                 A const pointer to the UV geometry element or \c
   * NULL if no UV geometry element with this name exists.
   */
  const FbxGeometryElementUV *GetElementUV(const char *pUVSetName) const;

  /** Returns this geometry's all UV set names.
   * \param pUVSetNameList   A reference to \c FbxStringList that will be filled
   * with this geometry's all UV set names.
   */
   ...
   * (FbxPeripheral). There should be no real interest in calling them directly.
   * The functions will write/read the memory dump of the data contained in this
   * class. Each data block written/read will start with an (int) value
   * representing the number of items in the array. If this value (v) is not
   * zero, it will be followed by the array content. A block of data that is (v
   * * sizeof(array item size)) bytes big. The methods will also call the parent
   * class ones to dump the Layers content.
   */
  //@{
   ...
   * \return The memory size in bytes (includes the amount use by the data
...
*****************************************************************************************************************************/
  ...
  bool
  GetNormals(FbxLayerElementArrayTemplate<FbxVector4> **pLockableArray) const;
  bool
  GetNormalsIndices(FbxLayerElementArrayTemplate<int> **pLockableArray) const;
  bool GetTangents(FbxLayerElementArrayTemplate<FbxVector4> **pLockableArray,
                   const int pLayerIndex = 0) const;
