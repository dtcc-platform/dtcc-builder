 ...
 *  A FbxLayerElement describes how the layer element is mapped to a geometry
 * surface and how the mapping information is arranged in memory. A
 * FbxLayerElement contains Normals, UVs or other kind of information.
 *
 * \see FbxLayer
...
public:
   ...
   * - \e eNormal                           Layer Element of type
   * FbxLayerElementNormal.
   * - \e eBiNormal                         Layer Element of type
   * FbxLayerElementBinormal.
   * - \e eTangent                          Layer Element of type
   * FbxLayerElementTangent.
   * - \e eMaterial                         Layer Element of type
   * FbxLayerElementMaterial.
   * - \e eTextureDiffuse                 Layer Element of type
   * FbxLayerElementTexture.
   * - \e ePolygonGroup                    Layer Element of type
   * FbxLayerElementPolygonGroup.
   * - \e eUV                               Layer Element of type
   * FbxLayerElementUV.
   * - \e eVertexColor                     Layer Element of type
   * FbxLayerElementVertexColor.
   * - \e eSmoothing                        Layer Element of type
   * FbxLayerElementSmoothing.
   * - \e eVertexCrease                    Layer Element of type
   * FbxLayerElementCrease.
   * - \e eEdgeCrease                      Layer Element of type
   * FbxLayerElementCrease.
   * - \e eHole                             Layer Element of type
   * FbxLayerElementHole.
   * - \e eUserData                        Layer Element of type
   * FbxLayerElementUserData.
   * - \e eVisibility                       Layer Element of type
   * FbxLayerElementVisibility.
   * - \e eTextureEmissive                Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureEmissiveFactor         Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureAmbient                 Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureAmbientFactor          Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureDiffuseFactor          Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureSpecular                Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureNormalMap               Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureSpecularFactor         Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureShininess               Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureBump                    Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureTransparency             Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureTransparencyFactor     Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureReflection              Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureReflectionFactor       Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureDisplacement            Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTextureDisplacementVector     Layer Element of type
   * FbxLayerElementTexture.
   * - \e eTypeCount
   */
  enum EType
  ...
  const static int sTypeTextureStartIndex =
      int(eTextureDiffuse); //!< The start index of texture type layer elements.
  const static int sTypeTextureEndIndex =
      int(eTypeCount) - 1; //!< The end index of texture type layer elements.
  const static int sTypeTextureCount =
      sTypeTextureEndIndex - sTypeTextureStartIndex +
      1; //!< The count of texture type layer elements.
  const static int sTypeNonTextureStartIndex =
      int(eNormal); //!< The start index of non-texture type layer elements.
  const static int sTypeNonTextureEndIndex =
      int(eVisibility); //!< The end index of non-texture type layer elements.
  const static int sTypeNonTextureCount =
      sTypeNonTextureEndIndex - sTypeNonTextureStartIndex +
      1; //!< The count of non-texture type layer elements.
  static const char *const
      sTextureNames[]; //!< Array of names of texture type layer elements.
  static const char
      *const sTextureUVNames[]; //!< Array of names of UV layer elements.
  static const char
      *const sNonTextureNames[]; //!< Array of names of non-texture type layer
                                 //!< elements.
  static const FbxDataType sTextureDataTypes[]; //!< Array of texture types.
  static const char
      *const sTextureChannelNames[]; //!< Array of texture channels.

  /**	\enum EMappingMode     Determines how the element is mapped to a
    surface.
    * - \e eNone                  The mapping is undetermined.
    * - \e eByControlPoint      There will be one mapping coordinate for each
    surface control point/vertex.
    * - \e eByPolygonVertex     There will be one mapping coordinate for each
    vertex, for every polygon of which it is a part. This means that a vertex
    will have as many mapping coordinates as polygons of which it is a part.
    * - \e eByPolygon            There can be only one mapping coordinate for
    the whole polygon.
    * - \e eByEdge               There will be one mapping coordinate for each
    unique edge in the mesh. This is meant to be used with smoothing layer
    elements.
    * - \e eAllSame              There can be only one mapping coordinate for
    the whole surface.
    */
  enum EMappingMode
  {
  ...
  /** \enum EReferenceMode     Determines how the mapping information is stored
    in the array of coordinates.
    * - \e eDirect              This indicates that the mapping information for
    the n'th element is found in the n'th place of
                                FbxLayerElementTemplate::mDirectArray.
    * - \e eIndex,              This symbol is kept for backward compatibility
    with FBX v5.0 files. In FBX v6.0 and higher, this symbol is replaced with
    eIndexToDirect.
    * - \e eIndexToDirect     This indicates that the
    FbxLayerElementTemplate::mIndexArray contains, for the n'th element, an
    index in the FbxLayerElementTemplate::mDirectArray array of mapping
    elements. eIndexToDirect is usually useful for storing eByPolygonVertex
    mapping mode elements coordinates. Since the same coordinates are usually
                                repeated many times, this saves spaces by
    storing the coordinate only one time and then referring to them with an
    index. Materials and Textures are also referenced with this mode and the
    actual Material/Texture can be accessed via the
    FbxLayerElementTemplate::mDirectArray
    */
  enum EReferenceMode
  {
   ...
   * \param pMappingMode     Specifies the way that layer element is mapped to a
   * surface.
   */
  ...
  {
    return (mName == pOther.mName) && (mMappingMode == pOther.mMappingMode) &&
...
*****************************************************************************************************************************/
  ...
  // Reference count added in case of shared objects or corrupted files that
  // uses the same object. This will prevent the deletion of an object that is
  // stil used.
...
public:
  /** \internal
   * \enum ELockAccessStatus	        Identifies what error occurs when the
   * data arrays are manipulated.
   * - \e eSuccess					Operation Successful.
   * - \e eUnsupportedDTConversion   Attempts to convert to an unsupported
   * DataType.
   * - \e eCorruptedCopyback         The Release of a converted buffer fails and
   * corrupts the main data.
   * - \e eBadValue                  Invalid value.
   * - \e eLockMismatch              Attempts to change to an incompatible lock.
   * - \e eNoWriteLock               A write operation is attempted but no
   * WriteLock is available.
   * - \e eNoReadLock                A read operation is attempted but the
   * WriteLock is active.
   * - \e eNotOwner                  Attempts to release a lock on an invalid
   * data buffer pointer.
 ...
 * it provides lock handling and data array manipulation of the data buffer for
 * FbxLayerElement. \nosubgrouping
 */
...
public:
   ...
   * \return           The current number of read locks (including the one just
   * grabbed) or 0 if a write lock is active.
   */
  int ReadLock() const;
   ...
   * \return           \c True if a write lock has been successfully granted, \c
   * false if one or more read locks are active.
   */
  ...
  /** Locks this array for writing. The data that already exists in the array is
   * kept and is valid. \return           \c True if a write lock has been
   * successfully granted, \c false if one or more read locks are active.
   */
  ...
  /** \enum ELockMode	           Identifies the access mode to the data
   * buffer.
   ...
   * \param pDataType                 If defined, tries to return the data as
   *this type. \return                          A pointer to the data buffer or
   *NULL if a failure occurs. \remarks                         In the case of a
   *failure, the Status is updated with the reason for the failure. Also, when a
   *type conversion occurs, a second buffer of the new type is allocated. In
   *this case, the LockMode does not apply to the returned buffer since it is a
   *copy but it does apply to the internal data of this object. The returned
   *buffer still remains a property of this object and is deleted when the
   *pointer is released or the object is destroyed. At the moment of release or
   *destruction, the values in this buffer are copied back into this object.
   */
  virtual void *GetLocked(ELockMode pLockMode, EFbxType pDataType);

  /** Grants a locked access to the data buffer.
   * \param pLockMode                 Access mode to the data buffer.
   * \return                          A pointer to the data buffer or NULL if a
   *failure occurs. \remarks                         In the case of a failure,
   *the Status is updated with the reason for the failure. Also, when a type
   *conversion occurs, a second buffer of the new type is allocated. In this
   *case, the LockMode does not apply to the returned buffer since it is a copy
   *but it does apply to the internal data of this object. The returned buffer
   *still remains a property of this object and is deleted when the pointer is
   *released or the object is destroyed. At the moment of release or
   *destruction, the values in this buffer are copied back into this object.
   */
   ...
   * \remarks                         The passed pointer must be the one
   *obtained by the call to GetLocked(). Any other pointer causes this method to
   *fail and the Status is updated with the reason for the failure. If the
   *passed pointer refers a converted data buffer (see comment of GetLocked),
   *this method copies the GetCount() items of the received buffer back into
   *this object. Any other items that may have been added using a realloc call
   *are ignored.
   */
  ...
  /** Unlock the data buffer.
   * \param pDataPtr                  The buffer to be released.
   * \param dummy                     The data type of dummy is used to
   *specialize this function. \remarks                         The passed
   *pointer must be the one obtained by the call to GetLocked(). Any other
   *pointer causes this method to fail and the Status is updated with the reason
   *for the failure. If the passed pointer refers a converted data buffer (see
   *comment of GetLocked), this method copies the GetCount() items of the
   *received buffer back into this object. Any other items that may have been
   *added using a realloc call are ignored.
   */
  ...
  /** Returns the Stride size which equals the size of the data type of the data
   * buffer.
   */
   ...
   * \remarks The input index must be within valid range and no error will be
   ...
   * \param pItem                Pointer of the item whose value is copied to
   * pIndex'th item \param pValueType           Data type of the item \remarks
   * The input index must be within valid range and no error will be thrown if
   * it is invalid.
   */
   ...
   * \param pItem                Pointer of the item whose value is copied to
   * the last item \param pValueType           Data type of the item \remarks
   * The array should contain at least one item and no error will be thrown if
   * it is empty.
   */
   ...
   * \param pItem                Place to hold the value of the removed item.
   * \param pValueType           Data type of the item
   * \remarks The array should contain at least one item and no error will be
   * thrown if it is empty.
   */
  void RemoveLast(void **pItem, EFbxType pValueType);

  /** Removes one item from the data buffer.
   * \param pItem                The first item who equals pItem is to be
   * removed \param pValueType           Data type of the item \return \c True
   * if the item is removed successfully, \c false otherwise
   */
   ...
   * \param pValueType           Data type of the item
   * \return                     \c True if the item's value is returned
   * successfully, \c false otherwise \remarks                    If the index
   * is invalid, pItem is set to zero.
   */
   ...
   * \remarks                    The index of the first item whose value equals
   * pItem is returned.
   */
  int Find(const void *pItem, EFbxType pValueType) const;

  /** Searches for an item after the specified index in the data buffer.
   * \param pAfterIndex          The specified index after which the searching
   * begins \param pItem                The value of the item for which to
   * search, the searching begins after pAfterIndex. \param pValueType Data type
   * of the item \return                     The index of the item found, -1 if
   * not found. \remarks                    The index of the first item whose
   * value equals pItem is returned.
   */
   ...
   * \param pBeforeIndex         The specified index before which the searching
   * begins \param pItem                The value of the item for which to
   * search, the searching begins before pBeforeIndex. \param pValueType The
   * item's data type. \return                     The index of the item found,
   * -1 if not found. \remarks                    The index of the first item
   * whose value equals pItem is returned.
   */
  int FindBefore(int pBeforeIndex,
                 const void *pItem,
                 EFbxType pValueType) const;
   ...
   * \param pIndex               The index of the item to be updated.
   * \param pItem                The item whose value is copied to pIndex'th
   * item \remarks The input index must be within valid range and no error will
  ...
  /** Removes one item from the data buffer.
   * \param pItem                The first item who equals pItem is to be
   * removed \return                     \c True if the item is removed
   * successfully, \c false otherwise
   */
   ...
   * \param pAfterIndex          The specified index after which the searching
   * begins \param pItem                The value of the item for which to
   * search, the searching begins after pAfterIndex. \return The index of the
   * item found, -1 if not found. \remarks                    The index of the
   * first item whose value equals pItem is returned.
   */
   ...
   * \param pBeforeIndex         The specified index before which the searching
   * begins \param pItem                The value of the item for which to
   * search, the searching begins before pBeforeIndex. \return The index of the
   * item found, -1 if not found. \remarks                    The index of the
   * first item whose value equals pItem is returned.
   */
  template <class T>
   ...
   * \param pDst                 The destination array where the items are to be
   * copied.
   */
...
protected:
  ...
  virtual void
  ConvertDataType(EFbxType pDataType, void **pDataPtr, size_t *pStride);
...
/** \internal
 * This class provides simple RAII-style read locking of a FbxLayerElementArray
 * object.
 */
template <typename T> struct FbxLayerElementArrayReadLock
{
  /** \internal
   ...
   */
  const T *GetData() const { return mLockedData; }
...
/** FbxLayerElementArrayTemplate provides data array manipulation of the data
...
public:
  ...
  /** Assignment operator.
   * \param pArrayTemplate       The source array whose items are copied to this
   * array.
   */
  ...
  /** Assignment operator.
   * \param pArrayTemplate           The source array whose items are copied to
   * this array.
   */
  FbxLayerElementArrayTemplate<T> &
  operator=(const FbxLayerElementArrayTemplate<T> &pArrayTemplate)
  {
...
private:
  // This one is not the best thing to do, but at least I don't get deprecated
  // calls inside this file. Note that FbxLayerElementUserData is kind of a
  // weird class in the first place anyway. So either we clean it up, or we live
 ...
 *         0 if layer element or index array is \c NULL and 1 if the remap is
 * successful
 */
extern FBXSDK_DLL int
RemapIndexArrayTo(FbxLayerElement *pLayerEl,
 ...
 * It provides interfaces to access the direct array and index array of
...
public:
   ...
   * \remarks     You cannot put elements in the direct array when the reference
   * mode is set to eIndex.
   */
  FbxLayerElementArrayTemplate<Type> &GetDirectArray() const
  {
    FBX_ASSERT(mReferenceMode == FbxLayerElement::eDirect ||
   ...
   * \remarks     You cannot put elements in the index array when the mapping
   * mode is set to eDirect.
   */
  FbxLayerElementArrayTemplate<int> &GetIndexArray() const
  {
    FBX_ASSERT(mReferenceMode == FbxLayerElement::eIndex ||
               mReferenceMode == FbxLayerElement::eIndexToDirect);
    return *mIndexArray;
  }
...
public:
  ...
  bool operator==(const FbxLayerElementTemplate &pOther) const
  {
    ...
    {
      const FbxLayerElementArrayTemplate<Type> &directArray =
          pOther.GetDirectArray();
   ...
   *                             If an error occurs, returns 0. In case the
   * function cannot remap to the desired mode because of incompatible modes or
 ...
 * \remarks To be correctly saved in FBX file, this type of Layer element should
 * have its reference mode set to \e eIndexToDirect. \nosubgrouping
 */
class FBXSDK_DLL FbxLayerElementNormal
    : public FbxLayerElementTemplate<FbxVector4>
{
public:
  /** Allocation method.
   * \return             A pointer to the layer element or \c NULL if creation
   * fails.
   */
 ...
 * \nosubgrouping
 */
class FBXSDK_DLL FbxLayerElementBinormal
    : public FbxLayerElementTemplate<FbxVector4>
{
public:
  /** Allocation method.
 ...
 * \nosubgrouping
 */
class FBXSDK_DLL FbxLayerElementTangent
    : public FbxLayerElementTemplate<FbxVector4>
{
public:
  /** Allocation method.
 ...
 *
 * FBX SDK 2011 and later connects materials (FbxSurfaceMaterial) to nodes
 * (FbxNode). The direct array of this class is no longer used. The integer "n"
 * in the index array of this class represents the n-th material (zero-based)
 * connected to the node.
 *
 * For example:
 *
 *   Mapping mode eAllSame and index array {0} means the whole geometry is
 * assigned with the 0-th material connected to the node.
 *
 *   Mapping mode eByPolygon and index array {0, 1} means the first polygon is
 * assigned with the 0-th material and the second polygon is assigned with the
 * 1-th material.
 *
 *
 * You can access the materials from a node by using FbxNode::GetMaterialCount()
 * and FbxNode::GetMaterial(int pIndex) or the more generic calls to
 * GetSrcObjectCount<FbxSurfaceMaterial>() and
 ...
 *
 * \remarks
 * The DirectArray() methods still exist for legacy reasons but has been made
 * private and should not be used. Therefore, to be correctly saved in FBX file,
 * this type of Layer element should have its reference mode set to \e
 * eIndexToDirect.
 *
 * \see FbxSurfaceMaterial
 * \see FbxNode
 */
class FBXSDK_DLL FbxLayerElementMaterial
    : public FbxLayerElementTemplate<FbxSurfaceMaterial *>
{
public:
  ...
  /** \internal
   * Internal class to maintain backward compatibility with old FBX code (prior
   * to FBX SDK 2011). This class synchronizes the direct array with FbxNode
   * connections. Thus, changes on the direct array will reflect on FbxNode.
   */
  class LayerElementArrayProxy
      : public FbxLayerElementArrayTemplate<FbxSurfaceMaterial *>
  {
  public:
 ...
 * have its reference mode set to \e eIndexToDirect. \nosubgrouping
 */
class FBXSDK_DLL FbxLayerElementPolygonGroup
    : public FbxLayerElementTemplate<int>
{
public:
  /** Allocation method.
   * \return             A pointer to the layer element or \c NULL if creation
 ...
 *
 * This class represents a UV set belongs to a geometry. Each UV set in a
 * geometry has a name to identify itself. The string property FbxTexture.UVSet
 * indicates the UV set to use.
 *
 * \remarks    if the Mapping mode of this LayerElement is \e eNone, the stored
 * data should be treated as irrelevant. In some circumstances, you can still
 * send this data to systems that cannot function without UV coordinates, but
 * ensure that you have enough coordinates to do so.
 *
 * \see FbxTexture
 * \nosubgrouping
 */
 ...
 * \nosubgrouping
 */
class FBXSDK_DLL FbxLayerElementVertexColor
    : public FbxLayerElementTemplate<FbxColor>
{
public:
  /** Allocation method.
   * \return             A pointer to the layer element or \c NULL if creation
...
template <class T>
inline FbxLayerElementArrayTemplate<T> &FbxGetDirectArray(
    FbxLayerElementUserData *pLayerElement, int pIndex, bool *pStatus = NULL);
template <class T>
inline FbxLayerElementArrayTemplate<T> const &
FbxGetDirectArray(FbxLayerElementUserData const *pLayerElement,
                  int pIndex,
                  bool *pStatus = NULL);
template <class T>
inline FbxLayerElementArrayTemplate<T> &
FbxGetDirectArray(FbxLayerElementUserData *pLayerElement,
                  const char *pName,
                  bool *pStatus = NULL);
template <class T>
inline FbxLayerElementArrayTemplate<T> const &
 ...
 * This layer element is different from the other types of layer elements in
 * that it has multiple direct arrays. There is one array for each user data
 * attribute. Each array is indexed by the index array. \nosubgrouping
 */
class FBXSDK_DLL FbxLayerElementUserData
    : public FbxLayerElementTemplate<void *>
{
public:
  FBXSDK_FRIEND_NEW();
   ...
   * \param pDataTypes   Attribute data types of this layer element, one direct
   * array is allocated for each Attribute data type. \param pDataNames
   * Attribute names of this layer element. \return             A pointer to the
   * layer element or \c NULL if creation fails. \remarks            Only
   * "bool", "int", "float" and "double" are supported.
   */
   ...
   * \param pStatus                  A flag to indicate whether the direct array
   * is returned successfully or not. \return                         The
   * specified attribute's direct array.
   */
                                                           ...
                                                           bool *pStatus = NULL)
  {
    if (pIndex >= 0 || pIndex < GetDirectArray().GetCount())
    {
      if (pStatus)
        *pStatus = true;
      return (FbxLayerElementArrayTemplate<void *> *)GetDirectArray()
          .AsReference(pIndex);
    }
    else
    {
   ...
   * specified attribute's direct array.
   */
  const FbxLayerElementArrayTemplate<void *> *
   ...
   * \return           The data type, or FbxUndefinedDT if pIndex is out of
   * range
   */
   ...
   * \return          The data type, or FbxUndefinedDT if no attribute has the
   * given name
   */
  ...
  void ResizeAllDirectArrays(int pSize)
  {
    for (int i = 0; i < GetDirectArray().GetCount(); ++i)
    {
      ...
      case eFbxDouble:
        ...
        break;
      // case eFbxDouble3:	GetDirectArray< FbxDouble3 >(i).Resize( pSize );
      // break;
      // case eFbxDouble4:	GetDirectArray< FbxDouble4 >(i).Resize( pSize );
      // break; case eFbxDouble4x4:	GetDirectArray< FbxDouble4x4>(i).Resize(
      // pSize );	break;
      default:
  ...
  void RemoveFromAllDirectArrays(int pIndex)
  {
    for (int i = 0; i < GetDirectArray().GetCount(); ++i)
    {
      ...
      case eFbxDouble:
        ...
        break;
      // case eFbxDouble3:	GetDirectArray< FbxDouble3 >(i).RemoveAt( pIndex
      // );	break; case eFbxDouble4:	GetDirectArray< FbxDouble4
      // >(i).RemoveAt( pIndex );	break; case eFbxDouble4x4:
      // GetDirectArray< FbxDouble4x4>(i).RemoveAt( pIndex );	break;
      default:
        FBX_ASSERT_NOW("unknown type");
        break;
      }
  ...
  int GetArrayCount(int pIndex) const
  {
    if (pIndex >= 0 && pIndex < GetDirectArray().GetCount())
    {
      ...
      // case eFbxDouble3:	return GetDirectArray< FbxDouble3
      // >(pIndex).GetCount(); case eFbxDouble4:	return GetDirectArray<
      // FbxDouble4 >(pIndex).GetCount(); case eFbxDouble4x4:	return
      // GetDirectArray< FbxDouble4x4>(pIndex).GetCount();
      default:
        FBX_ASSERT_NOW("Unknown type");
        break;
      }
   ...
   * \remarks    This count should be equal to the count of user data
   * attributes.
   */
  int GetDirectArrayCount() const { return GetDirectArray().GetCount(); }

  /** Assignment operator which performs a deep copy.
   * \param pOther      Other FbxLayerElementUserData from which to perform a
   * deep copy. \return            This FbxLayerElementUserData.
   */
  FbxLayerElementUserData &operator=(FbxLayerElementUserData const &pOther)
  {
    ...
    for (int i = 0; i < pOther.GetDirectArrayCount(); ++i)
    {
      ...
      case eFbxFloat:
        FbxGetDirectArray<float>(this, i) =
            FbxGetDirectArray<float>(&pOther, i);
        break;

      case eFbxDouble:
        FbxGetDirectArray<double>(this, i) =
            FbxGetDirectArray<double>(&pOther, i);
        break;
  ...
  void Init()
  {
    ...
    for (i = 0; i < mDataTypes.GetCount(); ++i)
    {
      ...
      if (dst)
      {
        ...
        {
        case eFbxBool:
          dst[i] = (FbxHandle *)FbxNew<FbxLayerElementArrayTemplate<bool>>(
              mDataTypes[i].GetType());
          break;
        case eFbxInt:
          dst[i] = (FbxHandle *)FbxNew<FbxLayerElementArrayTemplate<int>>(
              mDataTypes[i].GetType());
          break;
        case eFbxFloat:
          dst[i] = (FbxHandle *)FbxNew<FbxLayerElementArrayTemplate<float>>(
              mDataTypes[i].GetType());
          break;
        case eFbxDouble:
          dst[i] = (FbxHandle *)FbxNew<FbxLayerElementArrayTemplate<double>>(
              mDataTypes[i].GetType());
          break;
        default:
 ...
 * must match the attribute type at pIndex. \param pLayerElement     The layer
 * element whose direct array to return. \param pIndex            The direct
 * array index \param pStatus           Will be set to \c false if accessing the
 * direct array encounters an error. \return                  If pStatus
 * receives \c true, the direct array at the given index is returned. Otherwise
 * the return value is \c undefined.
 */
template <class T>
inline FbxLayerElementArrayTemplate<T> &FbxGetDirectArray(
    FbxLayerElementUserData *pLayerElement, int pIndex, bool *pStatus)
{
  return *(FbxLayerElementArrayTemplate<T> *)pLayerElement->GetDirectArrayVoid(
      pIndex, pStatus);
}
 ...
 */
template <class T>
inline FbxLayerElementArrayTemplate<T> const &FbxGetDirectArray(
    FbxLayerElementUserData const *pLayerElement, int pIndex, bool *pStatus)
{
  return *(const FbxLayerElementArrayTemplate<T> *)
              pLayerElement->GetDirectArrayVoid(pIndex, pStatus);
}

/** Returns the direct array with the given attribute name.The template type
 * must match the attribute type with pName. \param pLayerElement     The layer
 * element whose direct array to return. \param pName             The given
 * attribute name. \param pStatus           Will be set to false if accessing
 * the direct array encounters an error. \return                  If pStatus
 * receives \c true, the direct array at the given index is returned. Otherwise
 * the return value is \c undefined.
 */
template <class T>
inline FbxLayerElementArrayTemplate<T> &FbxGetDirectArray(
    FbxLayerElementUserData *pLayerElement, const char *pName, bool *pStatus)
{
  return *(FbxLayerElementArrayTemplate<T> *)pLayerElement->GetDirectArrayVoid(
      pName, pStatus);
}
...
{
  return *(const FbxLayerElementArrayTemplate<T> *)
              pLayerElement->GetDirectArrayVoid(pName, pStatus);
}

/** Layer element for indicating smoothness of components of a geometry.
 * \remarks To be correctly saved in FBX file, this type of Layer element should
 * have its reference mode set to \e eDirect.
 *
 * \nosubgrouping
 */
 ...
 */
class FBXSDK_DLL FbxLayerElementVisibility
    : public FbxLayerElementTemplate<bool>
{
public:
  /** Allocation method.
   * \return             A pointer to the layer element or \c NULL if creation
 ...
 * deprecated.
 *
 * Deprecated since FBX SDK 2011. Textures (FbxTexture derived classes) should
 * be connected to material properties.
 *
 * For example:
 *
 * \code
 ...
 * have its reference mode set to \e eIndexToDirect. \nosubgrouping
 */
class FBXSDK_DLL FbxLayerElementTexture
    : public FbxLayerElementTemplate<FbxTexture *>
{
public:
  /** Allocation method.
   * \return             A pointer to the layer element or \c NULL if creation
  ...
  /** \enum EBlendMode      Lets you control how textures are combined when you
   * apply multiple layers of texture to a surface.
   * - \e eTranslucent     The new texture layer is transparent (depending on
   * the Alpha value).
   * - \e eAdd             Add the color of the new texture to the previous
   * texture.
   * - \e eModulate        Multiples the color value of the new texture by the
   * color values of all previous layers of texture.
   * - \e eModulate2       Multiples the color value of the new texture by two
   * and then by the color values of all previous layers of texture.
   * - \e eOver            Equivalent to eTranslucent. Blends the new texture
   * over top of the old texture, according to the new texture's alpha channel.
   * - \e eNormal,         The colors of the two layers will not interact in any
   ...
   * results.
   * - \e eLighterColor,   This blend mode has the opposite effect of the Darker
   * Color mode. It compares all the values in both layers, then displays the
   * lightest values.
   * - \e eSoftLight,      Soft Light will multiply the dark tones and screen
   * the light tones.
   * - \e eHardLight,      Hard Light multiplies the dark colors and screens the
   * light colors.
   * - \e eVividLight,     Vivid Light will dodges or burn the lower layer
   * pixels depending on whether the upper layer pixels are brighter or darker
   * than neutral gray. It works on the contrast of the lower layer.
   * - \e eLinearLight,    Linear Light is the same as Vivid light but it works
   * on the brightness of the lower layer.
   * - \e ePinLight,       Pin Light changes the lower layer pixels depending on
   * how bright the pixels are in the upper layer.
   * - \e eHardMix,		  Produces either white or black, depending on
   * similarities between A and B.
   * - \e eDifference, 	  Difference reacts to the differences between the upper
   * and lower layer pixels.
   * - \e eExclusion, 	  Exclusion uses the darkness of the lower layer to mask
   * the difference between upper and lower layers.
   * - \e eSubtract,       The result color is the foreground color subtracted
   * from the background color. The result color is then applied over the
   * background color using the foreground alpha to define the opacity of the
   * result.
   ...
   * layer to the hue and saturation of the upper layer but leaves luminosity
   * alone.
   * - \e eLuminosity,     Luminosity changes the luminosity of the lower layer
   * to the luminosity of the upper layer while leaving hue and saturation the
   * same.
   * - \e eOverlay,        Multiplies (darkens) when the layer on which the mode
   * is set is dark and screens (brightens) when the layer on which the mode is
   * applied is lighter.
   ...
   * \param pAlpha     Set to a value between 0.0 and 1.0, where 0.0 is totally
   * transparent and 1.0 is totally opaque. \remarks          Values smaller
   * than 0.0 are clipped to 0.0, while values greater than 1.0 are clipped
   * to 1.0.
   */
   ...
   * \return     An alpha value between 0.0 and 1.0, where 0.0 is totally
 ...
 *
 * A typical layer for a Mesh contains Normals, UVs and Materials. A typical
 * layer for NURBS contains only Materials. In the case of the NURBS, the NURBS'
 * parameterization is used for the UVs; no UVs should be specified.
 *
 * In most cases, you only need a single layer to describe a geometry. Many
 * applications only support what is defined on the first layer. Take this into
 * account when you fill the layer. For example, it is legal to define the Layer
 * 0 with the UVs and then define the model's Normals on layer 1. However if you
 * construct a file this way, it may not be imported correctly in other
 * applications. Store the Normals in Layer 0 to avoid problems.
 *
 * Since FBX SDK 2011, Textures are connected to the properties of
 * FbxSurfaceMaterial derived classes. FbxLayerElementTexture is no longer used.
 * See the code example in FbxLayerElementTexture for how to connect a texture.
 *
 * Since FBX SDK 2011, texture layering is achieved by FbxLayeredTexture. See
 * the code example in FbxLayeredTexture for how to blend textures.
 *
...
public:
   ...
   * \return      A pointer to the Normals layer element, or \c NULL if no
   * Normals layer element is defined in this layer. \remarks     FbxNurbs or
   * FbxPatch geometry should not have Normals defined.
   */
   ...
   * \return      A pointer to the Tangents layer element, or \c NULL if no
   * Tangents layer element is defined in this layer. \remarks     FbxNurbs or
   * FbxPatch geometry should not have Tangents defined.
   */
   ...
   * \return      A pointer to the Binormals layer element, or \c NULL if no
   * Binormals layer element is defined in this layer. \remarks     FbxNurbs or
   * FbxPatch geometry should not have Binormals defined.
   */
   ...
   * \return     A pointer to the Materials layer element, or \c NULL if no
   * Materials layer element is defined in this layer.
   */
   ...
   * \return     A pointer to the Polygon Groups layer element, or \c NULL if no
   * Polygon Groups layer element is defined in this layer.
   */
   ...
   * \param pTypeIdentifier          Layer element type identifier, should be a
   * texture type identifier. \return                         A pointer to the
   * UVs layer element, or \c NULL if no UV is defined in this layer. \remarks
   * FbxNurbs or FbxPatch geometry should not have UVs defined. The NURBS/Patch
   * parameterization is used as UV parameters to map a texture.
   */
  FbxLayerElementUV *GetUVs(FbxLayerElement::EType pTypeIdentifier =
   ...
   * parameterization is used as UV parameters to map a texture.
   */
  const FbxLayerElementUV *GetUVs(FbxLayerElement::EType pTypeIdentifier =
                                      FbxLayerElement::eTextureDiffuse) const;
   ...
   * \return      A pointer to the Vertex Colors layer element, or \c NULL if no
   * Vertex Color layer element is defined in this layer. \remarks     FbxNurbs
   * or FbxPatch geometry should not have Vertex Colors defined, since no vertex
   * exists.
   */
   ...
   * \return      A pointer to the Smoothing layer element, or \c NULL if no
   * Smoothing layer element is defined in this layer. \remarks     FbxNurbs or
   * FbxPatch geometry should not have Smoothing defined.
   */
   ...
   * \return      A pointer to the Crease layer element, or \c NULL if no Crease
   * layer element is defined in this layer. \remarks     Crease info should
   * only be defined when the geometry is FbxSubDiv.
   */
   ...
   * \return      A pointer to the Hole layer element, or \c NULL if no Hole
   * layer element is defined in this layer. \remarks     Hole info should only
   * be defined when the geometry is FbxMesh.
   */
   ...
   * \return     A pointer to the User Data layer element, or \c NULL if no User
   * Data layer element is defined in this layer.
   */
   ...
   * \return     A pointer to the visibility layer element, or \c NULL if no
   * visibility layer element is defined in this layer.
   */
   ...
   * \param pType            Layer element type, should be a texture type
   * identifier. \return                 A pointer to the Textures layer
   * element, or \c NULL if no Textures layer element is defined in this layer.
   */
   ...
   * \param pTextures        A pointer to the Textures layer element, or \c NULL
   ...
   *                             - Calling with eNormal is the equivalent of
   * calling GetNormals().
   *                             - Calling with eBiNormal is the equivalent of
   * calling GetBinormals().
   *                             - Calling with eTangent is the equivalent of
   * calling GetTangents().
   *                             - Calling with eMaterial is the equivalent of
   * calling GetMaterials().
   *                             - Calling with ePolygonGroup is the equivalent
   * of calling GetPolygonGroups().
   *                             - Calling with eUV is the equivalent of calling
   * GetUVs().
   *                             - Calling with eVertexColor is the equivalent
   * of calling GetVertexColors().
   *                             - Calling with eSmoothing is the equivalent of
   * calling GetSmoothing().
   *                             - Calling with eVertexCrease is the equivalent
   * of calling GetVertexCrease().
   *                             - Calling with eEdgeCrease is the equivalent of
   * calling GetEdgeCrease().
   *                             - Calling with eUserData is the equivalent of
   * calling GetUserData().
   *                             - Calling with eVisibility is the equivalent of
   * calling GetVisibility().
   *                             - Calling with texture type is the equivalent
   * of calling GetTextures(FbxLayerElement::EType pType). \param pIsUV If \c
   * true, requests the UV layer element that corresponds with the specified
   * texture type. \return                 A pointer to the requested layer
   * element, or \e NULL if the layer element is not defined in this layer.
   */
  ...
  const FbxLayerElement *GetLayerElementOfType(FbxLayerElement::EType pType,
                                               bool pIsUV = false) const;

  /** Sets this layer's Normals description.
   * \param pNormals         A pointer to the Normals layer element, or \c NULL
   * to remove the Normals definition. \remarks                FbxNurbs or
   * FbxPatch geometry should not have Normals defined.
   */
  void SetNormals(FbxLayerElementNormal *pNormals);

  /** Sets this layer's Binormals description.
   * \param pBinormals       A pointer to the Binormals layer element, or \c
   * NULL to remove the Binormals definition. \remarks                FbxNurbs
   * or FbxPatch geometry should not have Binormals defined.
   */
  void SetBinormals(FbxLayerElementBinormal *pBinormals);

  /** Sets this layer's Tangents description.
   * \param pTangents        A pointer to the Tangents layer element, or \c NULL
   * to remove the Tangents definition. \remarks                FbxNurbs or
   * FbxPatch geometry should not have Tangents defined.
   */
  void SetTangents(FbxLayerElementTangent *pTangents);

  /** Sets this layer's Materials description.
   * \param pMaterials       A pointer to the Materials layer element, or \c
   * NULL to remove the Material definition.
   */
  void SetMaterials(FbxLayerElementMaterial *pMaterials);

  /** Sets this layer's Polygon Groups description.
   * \param pPolygonGroups     A pointer to the Polygon Groups layer element, or
   * \c NULL to remove the Polygon Group definition.
   */
  void SetPolygonGroups(FbxLayerElementPolygonGroup *pPolygonGroups);

  /** Sets this layer's UVs description.
   * \param pUVs             A pointer to the UVs layer element, or \c NULL to
   * remove the UV definition. \param pTypeIdentifier  Layer element type,
   * should be texture type. \remarks                FbxNurbs or FbxPatch
   * geometry should not have UVs defined. The NURBS/Patch parameterization is
   * used as UV parameters to map a texture.
   */
   ...
   * \param pVertexColors     A pointer to the Vertex Colors layer element, or
   * \c NULL to remove the Vertex Color definition. \remarks FbxNurbs or
   * FbxPatch geometry should not have Vertex Colors defined, since no vertex
   * exists.
   */
  void SetVertexColors(FbxLayerElementVertexColor *pVertexColors);

  /** Sets this layer's Smoothing description.
   * \param pSmoothing       A pointer to the Smoothing layer element, or \c
   * NULL to remove the Smoothing definition. \remarks                FbxNurbs
   * or FbxPatch geometry should not have Smoothing defined.
   */
  void SetSmoothing(FbxLayerElementSmoothing *pSmoothing);

  /** Sets this layer's Vertex Crease description.
   * \param pCrease          A pointer to the Vertex Crease layer element, or \c
   * NULL to remove the Crease definition. \remarks                Crease should
   * only be defined when the geometry is FbxSubDiv.
   */
  void SetVertexCrease(FbxLayerElementCrease *pCrease);

  /** Sets this layer's Edge Crease description.
   * \param pCrease          A pointer to the Edge Crease layer element, or \c
   * NULL to remove the Crease definition. \remarks                Crease should
   * only be defined when the geometry is FbxSubDiv.
   */
   ...
   * \param pHole          A pointer to the Hole layer element, or \c NULL to
   * remove the Hole definition. \remarks                Hole should only be
   * defined when the geometry is FbxMesh.
   */
  void SetHole(FbxLayerElementHole *pHole);

  /** Sets this layer's User Data.
   * \param pUserData        A pointer to the User Data layer element, or \c
   * NULL to remove the User Data.
   */
  void SetUserData(FbxLayerElementUserData *pUserData);

  /** Sets  this layer's the visibility.
   * \param pVisibility      A pointer to the visibility layer element, or \c
   * NULL to remove the visibility.
   */
   ...
   * \param pLayerElement    A pointer to the layer element, or \c NULL to
   * remove the layer element. \param pType            The required Layer
   * element type.
   *                             - Calling with eNormal is the equivalent of
   * calling GetNormals().
   *                             - Calling with eBiNormal is the equivalent of
   ...
   * \param pType            The required Layer element type.
   * \param pIsUV            When \c true, requests the UV LayerElement that
   * corresponds with the specified Layer Element type (only applies to TEXTURE
   * type layer elements). \return                 A pointer to the newly
   * created layer element, or \e NULL if the layer element has not been created
   * for this layer.
   */
 ...
 */
#define FBXSDK_FOR_EACH_TEXTURE(lLayerIndex)                                   \
  for ((lLayerIndex) = 0; (lLayerIndex) < FbxLayerElement::sTypeTextureCount;  \
       (lLayerIndex)++)

/**
 *  Utility macro for iterating over non-texture layer elements
 */
#define FBXSDK_FOR_EACH_NON_TEXTURE(lLayerIndex)                               \
  for ((lLayerIndex) = 0;                                                      \
       (lLayerIndex) < FbxLayerElement::sTypeNonTextureCount; (lLayerIndex)++)

/**
 *  Utility macro for getting texture layer element index by type
 */
#define FBXSDK_TEXTURE_INDEX(ElementType)                                      \
  (int(ElementType) - FbxLayerElement::sTypeTextureStartIndex)

/**
 *  Utility macro for getting texture layer element type by index
 */
#define FBXSDK_TEXTURE_TYPE(TextureIndex)                                      \
  (FbxLayerElement::EType((TextureIndex) +                                     \
                          FbxLayerElement::sTypeTextureStartIndex))

/**
 *  Utility macro for getting non-texture layer element index by type
 */
#define FBXSDK_NON_TEXTURE_INDEX(ElementType)                                  \
  (int(ElementType) - FbxLayerElement::sTypeNonTextureStartIndex)

/**
 *  Utility macro for getting non-texture layer element type by index
 */
#define FBXSDK_NON_TEXTURE_TYPE(Index)                                         \
  (FbxLayerElement::EType((Index) + FbxLayerElement::sTypeNonTextureStartIndex))

/**  Defines geometry element classes.
 *  A geometry element describes how the geometry element is mapped to a
 * geometry surface and how the mapping information is arranged in memory.
 *  \remarks Geometry elements are independent of layer elements and hide the
 * complexity of layers. \code FbxGeometryElementUV* lUVs =
 * lMesh->GetElementUV("map1"); FbxGeometryElementUV::DirectArrayType
 * lDirectArray = lUVs->GetDirectArray(); int lDirectUVCount =
 * lDirectArray.GetCount(); FbxVector2 lFirstUV = lDirectArray[0]; \endcode \see
 * FbxGeometryBase
 */
