...
/** A shape describes the deformation on a set of control points, which is
 * similar to the cluster deformer in Maya. For example, we can add a shape to a
 * created geometry. And the shape and the geometry have the same topological
 * information but different position of the control points. With varying
 * amounts of influence, the geometry performs a deformation effect.
 * \nosubgrouping
 * \see FbxGeometry
 */
...
public:
   ...
   * \return                        a pointer to the blend shape channel if set
   * or NULL.
   */
  FbxBlendShapeChannel *GetBlendShapeChannel() const;

  /** Get the base geometry of this target shape.
   * \return                        a pointer to the base geometry if set or
   * NULL. \remarks Since target shape can only connected to its base geometry
   * through blend shape channel and blend shape deformer. So only when this
   * target shape is connected to a blend shape channel, and the blend shape
   * channel is connected to a blend shape deformer, and the blend shape
   * deformer is used on a base geometry, then to get base geometry will
   * success.
   */
