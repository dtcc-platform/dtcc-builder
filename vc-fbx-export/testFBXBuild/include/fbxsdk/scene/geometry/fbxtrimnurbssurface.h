...
public:
   ...
   * \return             \c True if the point is in the boundary's control hull,
   * returns \c false if it is not in the control hull.
   */
...
public:
   ...
   *                         If the boundary is not added successfully, returns
   * \c false.
   */
  bool AddBoundary(FbxBoundary *pBoundary);

  /** Returns the boundary at a given index for the specified region
   * \param pIndex           The index of the boundary to retrieve, no bound
   * checking is done. \param pRegionIndex     The index of the region which is
   * bordered by the boundary. \return                 The trimming boundary at
   * index pIndex, if pIndex is in the range [0, GetBoundaryCount() ), otherwise
   ...
   * \param pFlip            If \c true, the surface is reversed. If it is
   * false, the surface is not reversed.
   */
  inline void SetFlipNormals(bool pFlip) { mFlipNormals = pFlip; }

  /** Checks if the normals are flipped.
   * \return                 \c True if normals are flipped, returns \c false if
