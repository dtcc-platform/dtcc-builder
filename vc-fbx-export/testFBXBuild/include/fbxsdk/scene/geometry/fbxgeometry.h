...
/** The base class of geometric objects that support control point deformations
 * (e.g. FbxMesh, FbxNurbs, and FbxPatch). The FbxGeometry provides support for
 * the following kinds of deformations.
 *
 ...
 * \li Shapes
 *
 * Most of the methods of FbxGeometry are wrappers to simplify the
 * access/manipulation of the connections to the deformers. For example, calling
 * the GetDeformerCount() method is the same thing as calling:
 *
 * \code
...
public:
   ...
   * This method is derived in the more high level classes (FbxMesh, FbxNurbs,
  ...
  /** Adds a deformer to this geometry (as mentioned in the description of this
   * class, adding a deformer is a synonym for "connect a deformer"). \param
   * pDeformer     Pointer to the deformer to be added. \return Index of the
   * added deformer.
   */
   ...
   * \return Pointer to the removed deformer (or \c NULL if pIndex is out of
   * range).
   */
   ...
   * \param pStatus    The FbxStatus object to hold error codes.
   * \return           Pointer to the deformer (or \c NULL if pIndex is out of
   * range).
   */
  ...
  /** Returns the source geometry weighted map that is connected to this
   * geometry. \return     Pointer to the source geometry weighted map that is
   * connected to this geometry if any.
   */
  FbxGeometryWeightedMap *GetSourceGeometryWeightedMap();

  /** Returns the number of destination geometry weighted map(s) that are
   * connected to this geometry. \return     The number of destination geometry
   ...
   * \return           Pointer to the destination geometry weighted map at the
   * specified index (if any).
   */
  ...
  /** Add a shape to the specified blend shape deformer and blend shape channel
   * of this geometry. \param pBlendShapeIndex           The blend shape
   * deformer index. \param pBlendShapeChannelIndex    The blend shape channel
   * index. \param pShape                     Pointer to the shape object to be
   * added. \param pPercent                   The full deform percentage of this
   * shape. \param pStatus                    The FbxStatus object to hold error
   * codes. \return  \c true if success, \c false otherwise.
   */
   ...
   * If shapes aren't explicitly destroyed before calling this function, they
   * will be destroyed along with the SDK manager that created them.
   */
  void ClearShape();
   ...
   * \param pStatus                       The FbxStatus object to hold error
   * codes. \return     The number of shapes that have been added to this blend
   * shape channel of this blend shape deformer.
   */
  int GetShapeCount(int pBlendShapeIndex,
                    int pBlendShapeChannelIndex,
                    FbxStatus *pStatus = NULL) const;

  /** Returns the shape found at the specified index on a blend shape channel of
   * a blend shape deformer. \param pBlendShapeIndex              The blend
   * shape deformer index. \param pBlendShapeChannelIndex       The blend shape
   * channel index. \param pShapeIndex                   The specified shape
   * index. \param pStatus                       The FbxStatus object to hold
   * error codes. \return                              Pointer to the shape (or
   * \c NULL if pIndex is out of range).
   */
   ...
   * The shape channel is an animatable property with a value range from 0 to
   * 100 (with 100 being full shape deformation). The default value is 0. \param
   * pBlendShapeIndex              The blend shape deformer index. \param
   * pBlendShapeChannelIndex       The blend shape channel index. \param pLayer
   * The animation layer from which we want to get the requested animation
   * curve. \param pCreateAsNeeded               If \c true, creates the
   * animation curve if it is not already present. \param pStatus The FbxStatus
   * object to hold error codes. \return Animation curve (or NULL if an error
   * occurred). \remarks If pLayer is left at NULL, the method will use the
   * first layer of the Animation stack.
   */
   ...
   * This information is never directly used inside the FBX SDK. Applications
   * can use these values if they wish to carry the "rendering" details of the
   * NURBS and Patches. The FBX SDK guarantee that the value (member of the
   * FbxNurbs, FbxNurbsSurface and FbxPatch classes) is stored to FBX files and
   * retrieved from them. \remarks The enum has been defined in this class to
   ...
   * \param pXMatrix     The transformation matrix that is assigned to the pivot
   * matrix.
   */
   ...
   * These functions provides direct access to default animation values that are
   * specific to a geometry. These functions only work if the geometry has been
   * associated with a node.
   */
  //@{

  /** Sets the default deformation for a specified shape.
   * The default shape property has a value range from 0 to 100 (with 100 being
   * full shape deformation). The default value is 0. \param pBlendShapeIndex
   * The blend shape deformer index. \param pBlendShapeChannelIndex       The
   * blend shape channel index. \param pPercent                      Deformation
   * percentage (on a scale ranging from 0 to 100). \remarks This function has
   * no effect if pIndex is out of range.
   */
   ...
   * The default shape property has a value range from 0 to 100 (with 100 being
   * full shape deformation). The default value is 0. \param pBlendShapeChannel
   * The blend shape channel. \param pPercent                 Deformation
   * percentage (on a scale ranging from 0 to 100). \remarks This function has
   * no effect if pShapeName is invalid.
   */
   ...
   * full shape deformation). The default value is 0. \param pBlendShapeIndex
   * The blend shape deformer index. \param pBlendShapeChannelIndex       The
   * blend shape channel index. \return           The deformation value for the
   * specified shape, or 0 if pIndex is out of range.
   */
  double GetDefaultShape(int pBlendShapeIndex,
                         int pBlendShapeChannelIndex) const;

  /** Returns the default deformation value for the specified channel.
   * The default shape property has a value range from 0 to 100 (with 100 being
   * full shape deformation). The default value is 0. \param pBlendShapeChannel
   * The blend shape channel. \return                         The deformation
   * value for the specified shape, or 0 if pShapeName is invalid.
   */
