 ...
 * Typically, these percentage values represent the ratio between the group
 *bounding box height (in screen space) and the viewing screen height.
 *
 * To switch to this mode, the client application must set the value of the
 * ThresholdsUsedAsPercentage property to "true" before the calls to the
 *AddThreshold/ SetThreshold methods. Client applications should always check
 *the return value of these methods to validate that the action was successful.
 *
 * Note that, for backward compatibility, the data is always stored as
 *FbxDistance type. The client application should always check the return value
 *of this method to validate that the argument contains a meaningful value (see
 *GetThreshold for more details).
 *
...
public:
   ...
   *
   * \remarks This property needs to be set before any call to the
   * Add\SetThreshold methods since its value is used to validate that the
  ...
  /**
   * \name Distance Mode
   * The properties in this block are meaningful only if
   * ThresholdsUsedAsPercentage is set to false and should be ignored otherwise.
   */
  //@{
   ...
   * This means it is possible to parent the LOD transform below other
   * transforms and still have it work as expected. If this attribute is set to
   * false, the distance computation ignores any parent transforms of the LOD
   * transform.
   *
