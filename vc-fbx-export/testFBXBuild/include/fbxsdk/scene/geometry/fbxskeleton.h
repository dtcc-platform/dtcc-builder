...
/**	This class specializes a node attribute to represent the elements
 * forming "bone" chains. The FbxSkeleton name of the class comes from the
 * analogy with the human body skeletal structure. In fact, an object of this
 * type is nothing more than a transform node with special properties that are
 * useful for its graphical representation and during IK/FK and skin deformation
 * computations. Typically, a scene will contain chains of FbxSkeleton node
 * attributes that, together, form a skeleton segment. For instance, the
 * representation of the leg of a character can be achieved using one parent
 * node with the attribute eRoot, followed by one child (femur) of type eLimb,
 * this child having a child also (tibia) of the same type. Finally, terminated
 * with a last node attribute of type eEffector (ankle).
 *
 * \nosubgrouping
 */
...
public:
   ...
   * \remarks The \e eLimbNode type is a bone defined uniquely by a transform
   * and a size value while \remarks the \e eLimb type is a bone defined by a
   * transform and a length.
   */
  enum EType
  {
   ...
   * \remarks When the attribute is not set, the application can choose to
   * ignore the attribute or use the default value. \remarks The flag is set
   * back to \c false when Reset() is called.
   */
   ...
   * \return \c true if skeleton type is \c eLimb or \c eLimbNode, \c false
   * otherwise. \remarks Limb or limb node color is only set if skeleton type is
   * \c eLimb or \c eLimbNode.
   */
  bool SetLimbNodeColor(const FbxColor &pColor);

  /** Get limb or limb node color.
   * \return Currently set limb color.
   * \remarks Limb or limb node color is only valid if skeleton type is \c eLimb
   * or \c eLimbNode.
   */
   ...
   * \remarks if a skeleton node do not have a parent or its parent is not a
   * skeleton node itself, then this skeleton is root in the hierarchy.
   */
   ...
   *
   * FbxSkeleton is a node attribute and it will be attached to a FbxNode which
   * represents the transform. Given a chain of skeleton nodes the parent and
   * child skeletons will be attached to a parent node and a child node. The
   * orientation of the limb is computed from the vector between the parent and
   * child position (from parent to child). The LimbLength represents the
   * proportion of the parent node's position to the child node's position which
   * is used to compute the actual limb length. The default value of 1.0 means
   * the LimbLength is equal to the length between the parent and child node's
   * position. So if the value is 0.5, it means the LimbLength will be half of
   * the length between the parent and child node's position.
   */
