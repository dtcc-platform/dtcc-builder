 ...
 * More precisely, the cluster acts on a subset of the geometry's control
 *points. For each control point that the cluster acts on, the intensity of the
 *cluster's action is modulated by a weight. The link mode (ELinkMode) specifies
 *how the weights are taken into account.
 *
 ...
 * we create a skin (FbxSkin). The skin has many clusters, each one
 ...
 *influence on some other points (low weight). Some points of the mesh are not
 *affected at all by the bone, so they would not be part of the corresponding
 * cluster.
 *
 *\nosubgrouping
 */
...
public:
  ...
  enum ELinkMode
  {
    eNormalize, /*!<	  In mode eNormalize, the sum of the weights assigned to
                   a control point is normalized to 1.0. Setting the associate
                   model in this mode is not relevant. The influence of the link
                   is a function of the displacement of the link node relative
                   to the node containing the control points.*/
    eAdditive,
    /*!<    In mode eAdditive, the sum of the weights assigned to a control
       point is kept as is. It is the only mode where setting the associate
       model is relevant. The influence of the link is a function of the
       displacement of the link node relative to the node containing the control
       points or, if set, the associate model. The weight gives the proportional
       displacement of a control point. For example, if the weight of a link
       over a control point is set to 2.0, a displacement of the link node of 1
       unit in the X direction relative to the node containing the control
       points or, if set, the associate model, triggers a displacement of the
       control point of 2 units in the same direction.*/
    eTotalOne
    /*!<    Mode eTotalOne is identical to mode eNormalize except that the sum
       of the weights assigned to a control point is not normalized and must
       equal 1.0.*/
  };

  /** Set the link mode.
   * \param pMode     The link mode.
   * \remarks         All the links associated to an instance of class
   * FbxGeometry must have the same link mode.
   */
  ...
  /** Set the link node. The link node is the node which influences the
   * displacement of the control points. Typically, the link node is the bone a
   * skin is attached to. \param pNode     The link node.
   */
  void SetLink(const FbxNode *pNode);

  /** Get the link node. The link node is the node which influences the
   * displacement of the control points. Typically, the link node is the bone a
   * skin is attached to. \return      The link node or \c NULL if
   * FbxCluster::SetLink() has not been called before.
   */
  FbxNode *GetLink();

  /** Get the link node (as const). The link node is the node which influences
   * the displacement of the control points. Typically, the link node is the
   * bone a skin is attached to. \return      The link node or \c NULL if
   * FbxCluster::SetLink() has not been called before.
   */
   ...
   * is of type eAdditive. If set, the associate model is the node used as a
   * reference to measure the relative displacement of the link node. Otherwise,
   * the displacement of the link node is measured relative to the node
   * containing the control points. Typically, the associate model node is the
   * parent of the bone a skin is attached to. \param pNode     The associate
   * model node.
   */
  void SetAssociateModel(FbxNode *pNode);

  /** Get the associate model.
   * The associate model is optional. It is only relevant if the link mode is of
   * type eAdditive. If set, the associate model is the node used as a reference
   * to measure the relative displacement of the link node. Otherwise, the
   * displacement of the link node is measured relative the the node containing
   * the control points. Typically, the associate model node is the parent of
   * the bone a skin is attached to. \return      The associate model node or \c
   * NULL if FbxCluster::SetAssociateModel() has not been called before.
   */
   ...
   * FbxGeometry owning the link. The weights are the influence of the link node
   * over the displacement of the indexed control points.
   */
  //@{
   ...
   *             Returns 0 if no control point indices have been added or the
  ...
  /** Set the array size for the three arrays: the array of control point
   * indices, the array of weights and the array of blend weights. \param pCount
   * The new count.
   */
   ...
   *      \li Transform refers to the global initial transform of the geometry
   * node that contains the link node. \li TransformLink refers to global
   * initial transform of the link node. \li TransformAssociateModel refers to
   * the global initial transform of the associate model.
   *
   * For example, given a mesh binding with several bones(links), Transform is
   * the global transform of the mesh at the binding moment, TransformLink is
   * the global transform of the bone(link) at the binding moment,
   * TransformAssociateModel is the global transform of the associate model at
   * the binding moment.
   */
  //@{
   ...
   * \param pMatrix     Transformation matrix to be filled with appropriate
   * data. \return            Input parameter matrix filled with appropriate
   * data.
   */
   ...
   * \return           \c true if transform matrix associated with parent node
   * is set.
   */
