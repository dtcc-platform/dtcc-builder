 ...
 * A patch object is useful for creating gently curved surfaces, and provides
 * very detailed control for manipulating complex geometry. \nosubgrouping
 */
...
public:
  ...
  /** Returns the number of divisions between adjacent control points in the
   * U-dimension. \return             Step value in the U-dimension.
   */
  inline int GetUStep() const { return mUStep; }

  /** Returns the number of divisions between adjacent control points in the
   * V-dimension. \return             Step value in the V-dimension.
   */
  ...
  /** Sets U-capped flags.
   * \param pUBottom     Set to \c true if the patch is capped at the bottom in
   * the U-dimension. \param pUTop \c     Set to \c true if the patch is capped
   * on the top in the U-dimension. \remarks            Capping options are
   * saved but not loaded by Motionbuilder because they are computed from the
   * patch topography.
   */
  void SetUCapped(bool pUBottom, bool pUTop);

  /** Returns state of the bottom U-capped flag.
   * \return             \c True if the patch is capped at the bottom in the
   * U-dimension.
   */
  inline bool GetUCappedBottom() const { return mUCappedBottom; }

  /** Returns state of the top U-capped flag.
   * \return             \c True if the patch is capped on the top in the
   * U-dimension.
   */
  inline bool GetUCappedTop() const { return mUCappedTop; }

  /** Sets V-capped flags.
   * \param pVBottom     Sets to \c true if the patch is capped at the bottom in
   * the V-dimension. \param pVTop        Sets to \c true if the patch is capped
   * on the top in the V-dimension. \remarks            Capping options are
   * saved but not loaded by Motionbuilder because they are computed from the
   * patch topography.
   */
   ...
   * \return         \c True if the content is successfully processed by the
   * receiving stream. If it is not successful, returns \c false.
   */
  virtual bool ContentWriteTo(FbxStream &pStream) const;

  /** Reads the content of the patch from the given stream.
   * \param pStream  The source stream.
   * \return         \c True if the patch completes with the data received from
   * the stream successfully. If it is not successful, returns \c false.
   */
