...
/** Base class for skin deformer (FbxSkin) and vertex cache deformer
 *(FbxVertexCacheDeformer). The corresponding deformer types are
 *FbxDeformer::eSkin and FbxDeformer::eVertexCache. A deformer can be binded to
 *a geometry (FbxGeometry) to act on its shape. Typically, some objects under
 *the deformer are animated, and via the deformer, the geometry is animated too.
 *
 ...
 *
 * A vertex cache deformer contains a cache (FbxCache). The cache contains
 *animation information for every control point of the geometry.
 *
 *\nosubgrouping
 */
...
public:
   ...
   * version of FBX files and should not be used anymore. All the animation
   * layering system has been moved to the FbxAnimLayer and FbxAnimStack
   * classes.
   */
  //@{
