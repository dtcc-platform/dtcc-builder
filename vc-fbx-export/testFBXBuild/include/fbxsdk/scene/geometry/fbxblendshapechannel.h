 ...
 * each target shape could have its own full deformation percentage. For
 *example, given a channel that has 3 target shapes, whose full deform
 *percentage are 30, to 80 to 100 separately, then when the percent changes from
 *0 to 100, the base geometry will deform from the first target shape to the
 *last one. This is called in-between blend shapes or progressive morph. The
 *property DeformPercent of blend shape channel will control the deform level of
 *each target shape or in-between blend shape on it. \nosubgrouping
 */
...
public:
   ...
   *
   * \remark Although not enforced, it is strongly suggested to limit the value
   * of this property in the range from 0.0 to 100.0 because graphic
   * applications may handle values outside of this interval differently,
   * therefore producing unexpected results.
   */
   ...
   * \return                        A pointer to the blend shape deformer if set
   * or NULL.
   */
   ...
   * \param pFullDeformPercent      The full deform percentage for the target
   * shape. \return                        \c true on success, \c false
   * otherwise.
   */
  bool AddTargetShape(FbxShape *pShape, double pFullDeformPercent = 100);

  /** Remove the given target shape.
   * \param pShape                  Pointer to the target shape to remove from
   * this blend shape channel. \return                        Pointer to the
   * target shape or \c NULL if pShape is not owned by this blend shape channel.
   */
  FbxShape *RemoveTargetShape(FbxShape *pShape);

  /** Get the number of target shapes.
   * \return                        Number of target shapes that have been added
   * to this blend shape channel.
   */
  int GetTargetShapeCount() const;

  /** Get the target shape at given index.
   * \param pIndex                  Index of the target shape.
   * \return                        Pointer to the target shape or \c NULL if
   ...
   * \return                        The array of full weight values of target
   * shape.
   */
