 ...
 *
 * \note The FBX SDK does not test the validity of the constructed scene graph.
 * It is the responsibility of the caller to make sure that it does not generate
 * cyclic graphs in a node hierarchy.
 *
 * Besides the tree management, this class defines all the properties required
 * to describe the position of the object in the scene. This information include
 * the basic Translation, Rotation and Scaling properties and the more advanced
 * options for pivots, limits, and IK joints attributes such the stiffness and
 * dampening.
 *
 * When it is first created, the FbxNode object is "empty" (i.e: it is an object
 * without any graphical representation that only contains the position
 * information). In this state, it can be used to represent parents in the node
 * tree structure but not much more. The normal use of this type of objects is
 * to add them an attribute that will specialize the node (see the "Node
 * Attribute Management" section).
 *
 * The node attribute is an object in itself and is connected to the the
 * FbxNode. This also means that the same node attribute can be shared among
 * multiple nodes. FbxCamera, FbxLight, FbxMesh, etc... are all node attributes
...
public:
  ...
  //@{
  /** Get the parent node.
   * \return Pointer to parent node or \c NULL if the current node has no
   * parent.
   */
  FbxNode *GetParent();
   ...
   * \return \c true on success, \c false if \e pNode is \c NULL or the system
   * is unable to make the connection. \remarks If \e pNode already has a
   * parent, first it is removed from current parent and then added to this one.
   */
   ...
   * \param pRecursive If \c true the method will also count all the descendant
   * children. \return Total number of children for this node.
   */
  int GetChildCount(bool pRecursive = false) const;

  /** Get child by index.
   * \param pIndex The child index.
   * \return Child node or \c NULL if \e pIndex is out of range (i.e: < 0 or >
   * GetChildCount()).
   */
   ...
   */
  FbxNode *
  FindChild(const char *pName, bool pRecursive = true, bool pInitial = false);
  //@}

  /**
   * \name Node Target Management
   * The FbxNode class allows the client to set a "follow" target node. This
   * target forces the node to re-align itself so it points to the target. By
   * default, the node uses its X axis as the aiming constraint. A rotation
   * offset can be added to change this behavior. While the default relative
   * orientation to the target (the X axis) is sufficient for the FBX cameras
   * (with a (0,0,0) rotation vector, they are aiming along the X axis), this
   * rotation offset becomes particularly useful with the lights objects because
   * their default orientation (when they have a 0,0,0 rotation vector) is to
   * point along the -Y axis and they need to be adjusted with a 90-degree
   * offset on the Z axis.
   *
   * The FbxNode class also permits the use of node to define an Up-vector. By
   * default, the node's up vector points towards the Up node. If the Up node is
   * not specified, then the node's Up vector points towards the Y axis. Here
   * too, a rotation offset can be added to change the default behavior.
   *
   * Of course, these offsets can be applied to anything, not only the cameras
   * and lights.
   *
   * \note Objects in the FBX SDK are always created in the right handed, Y-Up
   * system and need to be adjusted for any other axis system by explicitly
   * convert them (the class FbxAxisSystem can help in that process).
   *
   */
  //@{
   ...
   * \remarks This method checks for the validity of the property before
   * attempting to set its value. In fact, the exact same result can be achieved
   * by the following code: \code if( Visibility.IsValid() )
   * {
   ...
   * \return \c false if the Visibility property value is 0.0 and \c true for
   * any other value. \remarks This method expects the Visibility property to
   * exist and to be valid. If this condition is not met, the returned value
   * will be \c false.
   */
  bool GetVisibility() const;

  /** \enum EShadingMode Shading modes.
   * These shading modes are not directly used by the FBX SDK but it is
   * guaranteed that the information is carried to and from the FBX files. The
   * typical context of using these modes is to affect the rendering of
   * geometric objects (this is, of course, performed at the application level)
   * and the possible definition for each mode is:
   */
  enum EShadingMode
  {
    eHardShading,    //!< Solid geometries rendered with smooth surfaces - using
                     //!< the system light.
    eWireFrame,      //!< Geometries displayed in wire frame.
    eFlatShading,    //!< Solid geometries rendered faceted - using the system
                     //!< light.
    eLightShading,   //!< Solid geometries rendered with the scene lights.
    eTextureShading, //!< Solid geometries rendered with smooth textured
                     //!< surfaces - using system light.
    eFullShading //!< Solid geometries rendered with smooth textured surfaces
                 //!< and scene lights.
  };
   ...
   * \remarks If this node has more than one attribute (added via the
   * AddAttribute() method), this call will destroy all, but the default node
   * attribute.
   */
  FbxNodeAttribute *SetNodeAttribute(FbxNodeAttribute *pNodeAttribute);

  /** Get the default node attribute.
   * The default node attribute is the attribute that has been set by the call
   * to SetNodeAttribute(). \return Pointer to the default node attribute or \c
   * NULL if the node doesn't have a node attribute.
   */
  FbxNodeAttribute *GetNodeAttribute();

  /** Get the default node attribute.
   * The default node attribute is the attribute that has been set by the call
   * to SetNodeAttribute(...). \return Pointer to the default node attribute or
   * \c NULL if the node doesn't have a node attribute.
   */
  ...
  /** Get the index, in the list of connected node attributes, of the node
   * attribute that is set to be the default one. \return Index of the default
   * node attribute or \c -1 if there is no default node attribute set.
   */
  int GetDefaultNodeAttributeIndex() const;

  /** Set index of the default node attribute.
   * \param pIndex Identifies which of the connected node attributes is becoming
   * the default one. This value represent the connection number of the node.
   * \param pStatus The FbxStatus object to hold error codes.
   * \return \c true if the operation succeeds or \c false if the passed index
   * is invalid.
   */
  bool SetDefaultNodeAttributeIndex(int pIndex, FbxStatus *pStatus = NULL);

  /** Get the connected node attribute by specifying its index in the connection
   * list. \param pIndex The connection number of the node. \return Pointer to
   * corresponding node attribute or \c NULL if the index is out of range.
   */
   ...
   * This method will do a linear search of all the connected node attributes
   * (from the last to the first connection) until it finds \e pNodeAttribue.
   * \param pNodeAttribute The pointer to the node attribute.
   * \param pStatus The FbxStatus object to hold error codes.
   * \return The connection number of the node attribute or \c -1 if
   * pNodeAttribute is \c NULL or not connected to this node.
   */
  int GetNodeAttributeIndex(FbxNodeAttribute *pNodeAttribute,
                            FbxStatus *pStatus = NULL) const;

  /** Add the new node attribute to this node.
   * If no other node attribute is already set as the default one, this new node
   * attribute is automatically set as the default one. \param pNodeAttribute
   * The pointer to a node attribute. \param pStatus The FbxStatus object to
   * hold error codes. \return \c true if the operation succeeded or \c false if
   * the operation failed. \remarks The failing conditions for this methods are:
   ...
   * \return Pointer to the removed node attribute or \c NULL if the operation
   * failed.
   */
  FbxNodeAttribute *RemoveNodeAttribute(FbxNodeAttribute *pNodeAttribute);

  /** Remove the node attribute, specified by the connection index, from the
   * connection list of this node. \param pIndex Index of the node attribute.
   * \return Pointer to the removed node attribute or \c NULL if the operation
   * failed. \remarks If the specified node attribute is also the default one,
   * its predecessor in the connection list will become the new default node
   * attribute. And if there are no more predecessors, the node
   * DefaultNodeAttributeIndex is reset to -1.
   */
   ...
   * \remarks If the type cast failed because there is not default node
   * attribute set or it cannot be successfully casted, this method will return
   * \c NULL.
   */
   ...
   * \remarks If the type cast failed because there is not default node
   * attribute set or it cannot be successfully casted, this method will return
   * \c NULL. \remarks For this method to succeed, the node attribute's
   * GetAttributeType() must returns one of the following:
   ...
   * \remarks This method will try to process the default node attribute first.
   * If it cannot find it, it will scan the list of connected node attributes
   * and get the first object that is a FbxNodeAttribute::eMesh. \remarks If the
   * above search failed to get a valid pointer or it cannot be successfully
   * casted, this method will return \c NULL.
   */
  FbxMesh *GetMesh();
   ...
   * \remarks This method will try to process the default node attribute first.
   * If it cannot find it, it will scan the list of connected node attributes
   * and get the first object that is a FbxNodeAttribute::eNurbs. \remarks If
   * the above search failed to get a valid pointer or it cannot be successfully
   * casted, this method will return \c NULL.
   */
  FbxNurbs *GetNurbs();
   ...
   * \remarks This method will try to process the default node attribute first.
   * If it cannot find it, it will scan the list of connected node attributes
   * and get the first object that is a FbxNodeAttribute::eNurbsSurface.
   ...
   * \remarks This method will try to process the default node attribute first.
   * If it cannot find it, it will scan the list of connected node attributes
   * and get the first object that is a FbxNodeAttribute::eNurbsCurve. \remarks
   * If the above search failed to get a valid pointer or it cannot be
   * successfully casted, this method will return \c NULL.
   */
   ...
   * \remarks This method will try to process the default node attribute first.
   * If it cannot find it, it will scan the list of connected node attributes
   * and get the first object that is a FbxNodeAttribute::eLine. \remarks If the
   * above search failed to get a valid pointer or it cannot be successfully
   * casted, this method will return \c NULL.
   */
  FbxLine *GetLine();
   ...
   * \remarks This method will try to process the default node attribute first.
   * If it cannot find it, it will scan the list of connected node attributes
   * and get the first object that is a FbxNodeAttribute::eSubDiv. \remarks If
   * the above search failed to get a valid pointer or it cannot be successfully
   * casted, this method will return \c NULL.
   */
  FbxSubDiv *GetSubdiv();
   ...
   * \remarks This method will try to process the default node attribute first.
   * If it cannot find it, it will scan the list of connected node attributes
   * and get the first object that is a FbxNodeAttribute::ePatch. \remarks If
   * the above search failed to get a valid pointer or it cannot be successfully
   * casted, this method will return \c NULL.
   */
  FbxPatch *GetPatch();
   ...
   * This set of functions provides direct access to the transformation
   * propagations settings of the FbxNode. These settings determine how
   * transformations must be applied when evaluating a node's transformation
   * matrix. The possible values are:
   ...
   *     - eInheritRrs  : Scaling of parent does not affect the scaling of
   * children.
   */
  //@{
  /** Sets how child transforms are inherited from parent transforms.
   * \param pInheritType One of the following values eInheritRrSs, eInheritRSrs
   * or eInheritRrs
   */
  ...
  void
  GetTransformationInheritType(FbxTransform::EInheritType &pInheritType) const;
  //@}

  /**
   * \name Pivot Management
   * Pivots are used to specify translation, rotation and scaling centers in
   * coordinates relative to a node's origin. A node has two pivot contexts
   * defined by the EPivotSet enumeration. The node's animation data can be
   * converted from one pivot context to the other. Each context can be set to
   * be either active or passive (reference). By default the two pivot contexts
   * are passive. They need to be active to be processed during the evaluation
   * of the node final transformation matrix. In its passive state, a pivot
   * context can still be accessed to retrieve its content for any other
   * required purpose. Each pivot context stores values (as FbxVector4) for:
   * \code
   ...
   *
   * These values combine in the matrix form to compute the World transform of
   * the node using the formula:
   *
   * 	World = ParentWorld * T * Roff * Rp * Rpre * R * Rpost * Rp-1 * Soff *
   * Sp * S * Sp-1 \endcode
   *
   * The geometric transformation (Gt * Gr * Gs) is applied only to the node
   * attribute and after the node transformations. This transformation is not
   * inherited across the node hierarchy.
   *
   * \note Please refer to the FBX SDK programmers guide for more details.
   *
   * The application of the pivots is performed by calling the method
   * ConvertPivotAnimation(). Typically, you set-up the eDestinationPivot
   * context to match what your system can directly support and leave at (0,0,0)
   * the attributes that are not supported by your system. When the values of a
   * specific attribute in the two contexts (source and destination) are
   * identical, the system considers that no adjustment is required because the
   * attribute is directly supported in the destination world.
   *
   * Below is an example of code that shows how the pivot information could be
   * setup before calling ConvertPivotAnimation(). \code FbxVector4
   * lZero(0,0,0); FbxVector4 lOne(1,1,1);
   ...
   *
   * //For cameras and lights (without targets) let's compensate the
   * postrotation. if( pNode->GetCamera() || pNode->GetLight() )
   * {
   *    if( !pNode->GetTarget() )
   *    {
   ...
   *        FbxVector4 res = prV;
   *
   *        // Rotation order don't affect post rotation, so just use the
   * default XYZ order FbxRotationOrder rOrder; rOrder.V2M(lSourceR, res);
   *
   *        lR = lSourceR * lR;
   *        rOrder.M2V(res, lR);
   *        prV = res;
   ...
   *    }
   *
   *    // Point light do not need to be adjusted (since they radiate in all the
   * directions). if( pNode->GetLight() && pNode->GetLight()->LightType.Get() ==
   * FbxLight::ePoint )
   *    {
   ...
   * FbxMarker::eEffectorFK)
   *    || (pNode->GetMarker() && pNode->GetMarker()->GetType() ==
   * FbxMarker::eEffectorIK) )
   * {
   *    if( pNode->GetRotationActive() )
   *    {
   *       pNode->SetPreRotation(FbxNode::eDestinationPivot,
   * pNode->GetPreRotation(FbxNode::eSourcePivot));
   *    }
   *
   *    // No pivots on bones
   ...
   *    pNode->SetRotationPivot(FbxNode::eDestinationPivot,
   * pNode->GetRotationPivot(FbxNode::eSourcePivot));
   *    pNode->SetScalingPivot(FbxNode::eDestinationPivot,
   * pNode->GetScalingPivot(FbxNode::eSourcePivot));
   *    // Rotation and scaling offset are supported
   *    pNode->SetRotationOffset(FbxNode::eDestinationPivot,
   * pNode->GetRotationOffset(FbxNode::eSourcePivot));
   *    pNode->SetScalingOffset(FbxNode::eDestinationPivot,
   * pNode->GetScalingOffset(FbxNode::eSourcePivot));
   *    //
  ...
  enum EPivotState
  {
    ePivotActive, //!< The pivot context with this state is affecting the node's
                  //!< transform computation.
    ePivotReference //!< The pivot context with this state is not used during
                    //!< the node transform computation but can be accessed for
                    //!< reference purposes.
  };
  ...
  void GetRotationOrder(EPivotSet pPivotSet,
                        EFbxRotationOrder &pRotationOrder) const;

  /** Set rotation space for limit only.
   * \param pPivotSet Specify which pivot context is manipulated.
   * \param pUseForLimitOnly When set to \c true, the current rotation space
   *                         (set with SetRotationOrder) define the rotation
   * space for the limit only; leaving the rotation animation in Euler XYZ
   * space. When set to \c false, the current rotation space defines the
   ...
   * \remarks When this flag is set to false, the RotationOrder, the Pre/Post
   * rotation values and the rotation limits should be ignored.
   */
   ...
   * \remarks When the \e pPivotSet is eSourcePivot, this method also updates
   * the value of the QuaternionInterpolate property.
   */
   ...
   * \param pVector The X,Y and Z translation values (the 4th component of the
   * FbxVector4 is ignored).
   */
   ...
   * \return The X, Y and Z translation offset values (the 4th component of the
   * FbxVector4 is always 1).
   */
   ...
   * the node's origin.
   * \param pPivotSet Specify which pivot set to modify.
   * \param pVector The new position of the rotation pivot (the 4th component of
   * the FbxVector4 is ignored).
   */
   ...
   * \return The current position of the rotation pivot (the 4th component of
   * the FbxVector4 is always 1).
   */
   ...
   * \param pPivotSet Specify which pivot set to modify.
   * \param pVector The X,Y,Z rotation values to set (the 4th component of the
   * FbxVector4 is ignored).
   */
   ...
   * rotation animation data.
   * \param pPivotSet Specify which pivot set to query.
   * \return The X,Y and Z rotation values (the 4th component of the FbxVector4
   * is always 1).
   */
   ...
   * the node's origin.
   * \param pPivotSet Specify which pivot set to modify.
   * \param pVector The new position of the scaling pivot (the 4th component of
   * the FbxVector4 is ignored).
   */
   ...
   * after the node transformations. This translation is not inherited across
   * the node hierarchy. \param pPivotSet Specify which pivot set to modify.
   * \param pVector The X, Y, and Z translation values (the 4th component of the
   * FbxVector4 is ignored).
   */
   ...
   * \param pPivotSet Specify which pivot set to query.
   * \return The current geometric translation (the 4th component of the
   * FbxVector4 is always 1).
   */
   ...
   * node hierarchy.
   * \param pPivotSet Specify which pivot set to modify.
   * \param pVector The X,Y and Z rotation values (the 4th component of the
   * FbxVector4 is ignored).
   */
   ...
   * \param pPivotSet Specify which pivot set to query.
   * \return The current geometric rotation (the 4th component of the FbxVector4
   * is always 1).
   */
   ...
   * node hierarchy.
   * \param pPivotSet Specify which pivot set to modify.
   * \param pVector The X,Y and Z scale values (the 4th component of the
   * FbxVector4 is ignored).
   */
   ...
   * \param pPivotSet Specify which pivot set to query.
   * \return The current geometric scaling (the 4th component of the FbxVector4
   * is always 1).
   */
  FbxVector4 GetGeometricScaling(EPivotSet pPivotSet) const;

  /** Reset a pivot set to the default pivot context.
   * If the node has a geometry, reset the geometry's pivot to the identity
   * matrix. \param pPivotSet Pivot set to reset. \remarks The default pivot
   * context is a context with all the vector attributes set to (0,0,0) except
   * the GeometricScaling attribute that is reset to (1,1,1).
   */
  void ResetPivotSet(FbxNode::EPivotSet pPivotSet);

  /** This version is an improved version of the ConvertPivotAnimation(). It
   * fully supports all the attributes defined in the pivot sets and can process
   * animation data defined on different animation stack. \param pAnimStack The
   * animation stack on which the conversion will take place. If equals \c NULL,
   * convert the animation on all the animation stacks. \param pConversionTarget
   * If set to EPivotSet::eDestinationPivot, convert animation data from the
   * EPivotSet::eSourcePivot pivot context to the EPivotSet::eDestinationPivot
   * pivot context. Otherwise, the conversion is computed the other way around.
   * \param pFrameRate Resampling frame rate in frames per second.
   * \param pKeyReduce Apply or skip key reducing filter.
   * \remarks Due to the intrinsic properties of the mathematical operations
   * performed, sometimes, it is necessary to resample animation curves to
   * maintain the accurate conversion. When this resampling is required, the
   * method will use the \e pFrameRate value to specify the number of samples.
   * To avoid a huge number of keys in the animation curves, a constant key
   * reducer filter (FbxKFCurveFilterConstantKeyReducer) is automatically
   * applied to all the affected curves to remove as much consecutive keys that
   * have the same value. This filter is private and its settings cannot be
   * changed. It is possible that, after the filtering pass, the animations
   * curves do not contain keys anymore. This is a normal result and does not
   * affect the overall results. \note    Although it is possible to call this
   * method several times with a different AnimStack name, users must be aware
   * that some pivot computation can irreversibly modify the geometric nodes
   * with a cumulative effect of the \e GeometricTranslation, \e
   * GeometricRotation and \e GeometricScaling which will produce undesirable
   * results. It is recommended to call ConvertPivotAnimationRecursive with \p
   * pAnimStackName = NULL and let the method convert the animation on all the
   * Anim stacks at once. In the case when there are no geometric nodes in the
   * scene tree, specifying the animation stack is safe and somewhat faster. If
   * any transform limits are active, they are applied during the conversion and
   * disabled.
   */
  ...
  /** Reset all the pivot sets to the default pivot context and convert the
   * animation. \param pFrameRate Resampling frame rate in frames per second.
   * \param pKeyReduce Apply or skip key reducing filter.
   * \param pToNodeCenter: Reset pivots to node center if \c true, or retain
   * pivot places if \c false. \param pForceResetLimits  If \c true, this flag
   * will reset all the Translation, Rotation and Scaling limits and clears the
   * enabled flags. \remarks The resulting animation will be visually equivalent
   * and all the pivots will be cleared. The conversion is performed on all
   * animation stacks. \remarks Will recursively convert the animation of all
   * the children nodes. \remarks The \e pForceResetLimits flag has a
   * destructive behavior and should be used only in very limited cases where
   * the values of the limits are not required after the call to this method.
   * \remarks Currently, this function just works under RSrs inherit type if
   * pToNodeCenter is set to \c false.
   */
  ...
  void SetRotationPivotAsCenterRecursive(
      FbxVector4 pParentGeometricOffset = FbxVector4());
  //@}
   ...
   * \return If the object has no scene, returns the default evaluator,
   * otherwise the object's scene evaluator. */
  FbxAnimEvaluator *GetAnimationEvaluator() const;

  /** Returns this node's global transformation matrix at the specified time.
   * The node's translation, rotation and scaling limits are taken into
   * consideration. \param pTime The time used for evaluate. If
   * FBXSDK_TIME_INFINITE is used, this returns the default value, without
   * animation curves evaluation. \param pPivotSet The pivot set to take into
   * account \param pApplyTarget Applies the necessary transform to align into
   * the target node \param pForceEval Force the evaluator to refresh the
   * evaluation state cache even if its already up-to-date. \return The
   * resulting global transform of the specified node at the specified time.
   * \remarks This function is the equivalent of calling
   * Scene->GetEvaluator()->GetNodeGlobalTransform().
   */
  FbxAMatrix &
  EvaluateGlobalTransform(FbxTime pTime = FBXSDK_TIME_INFINITE,
                          FbxNode::EPivotSet pPivotSet = FbxNode::eSourcePivot,
                          bool pApplyTarget = false,
                          bool pForceEval = false);

  /** Returns this node's local transformation matrix at the specified time. The
   * node's translation, rotation and scaling limits are taken into
   * consideration. \param pTime The time used for evaluate. If
   * FBXSDK_TIME_INFINITE is used, this returns the default value, without
   * animation curves evaluation. \param pPivotSet The pivot set to take into
   * account \param pApplyTarget Applies the necessary transform to align into
   * the target node \param pForceEval Force the evaluator to refresh the
   * evaluation state cache even if its already up-to-date. \return The
   * resulting local transform of the specified node for the specified time.
   * \remarks The local transform matrix is calculated in this way:
   * ParentGlobal.Inverse * Global, all transforms such as pre/post rotation are
   * taken into consideration. This will return a different value than
   * LclTranslation, LclRotation and LclScaling at the specified time. To
   * evaluate these properties separately without taking pre/post rotation,
   * pivots and offsets into consideration, please use
   * GetNodeLocalTranslation(), GetNodeLocalRotation() and
   * GetNodeLocalScaling(). This function is the equivalent of calling
   * Scene->GetEvaluator()->GetNodeLocalTransform().
   */
  FbxAMatrix &
  EvaluateLocalTransform(FbxTime pTime = FBXSDK_TIME_INFINITE,
                         FbxNode::EPivotSet pPivotSet = FbxNode::eSourcePivot,
                         bool pApplyTarget = false,
                         bool pForceEval = false);

  /** Returns this node's LclTranslation property at the specified time.
   * No pivot, offsets, or any other transform is taken into consideration. The
   * translation limit is applied. \param pTime The time used for evaluate. If
   * FBXSDK_TIME_INFINITE is used, this returns the default value, without
   * animation curves evaluation. \param pPivotSet The pivot set to take into
   * account \param pApplyTarget Applies the necessary transform to align into
   * the target node \param pForceEval Force the evaluator to refresh the
   * evaluation state cache even if its already up-to-date. \return The
   * resulting value of LclTranslation property of the specified node at the
   * specified time. \remarks This function is the equivalent of calling
   * Scene->GetEvaluator()->GetNodeLocalTranslation().
   */
  FbxVector4 &
  EvaluateLocalTranslation(FbxTime pTime = FBXSDK_TIME_INFINITE,
                           FbxNode::EPivotSet pPivotSet = FbxNode::eSourcePivot,
                           bool pApplyTarget = false,
                           bool pForceEval = false);

  /** Returns this node's LclRotation property at the specified time.
   * No pre/post rotation, rotation pivot, rotation offset or any other
   * transform is taken into consideration. The rotation limit is applied.
   * \param pTime The time used for evaluate. If FBXSDK_TIME_INFINITE is used,
   * this returns the default value, without animation curves evaluation. \param
   * pPivotSet The pivot set to take into account \param pApplyTarget Applies
   * the necessary transform to align into the target node \param pForceEval
   * Force the evaluator to refresh the evaluation state cache even if its
   * already up-to-date. \return The resulting value of LclRotation property of
   * the specified node at the specified time. \remarks This function is the
   * equivalent of calling Scene->GetEvaluator()->GetNodeLocalRotation().
   */
  FbxVector4 &
  EvaluateLocalRotation(FbxTime pTime = FBXSDK_TIME_INFINITE,
                        FbxNode::EPivotSet pPivotSet = FbxNode::eSourcePivot,
                        bool pApplyTarget = false,
                        bool pForceEval = false);

  /** Returns this node's LclScaling property at the specified time.
   * No scaling pivot, scaling offset or any other transform is taken into
   * consideration. The scaling limit is applied. \param pTime The time used for
   * evaluate. If FBXSDK_TIME_INFINITE is used, this returns the default value,
   * without animation curves evaluation. \param pPivotSet The pivot set to take
   * into account \param pApplyTarget Applies the necessary transform to align
   * into the target node \param pForceEval Force the evaluator to refresh the
   * evaluation state cache even if its already up-to-date. \return The
   * resulting value of LclScaling property of the specified node at the
   * specified time. \remarks This function is the equivalent of calling
   * Scene->GetEvaluator()->GetNodeLocalScaling().
   */
  FbxVector4 &
  EvaluateLocalScaling(FbxTime pTime = FBXSDK_TIME_INFINITE,
                       FbxNode::EPivotSet pPivotSet = FbxNode::eSourcePivot,
                       bool pApplyTarget = false,
                       bool pForceEval = false);

  /** Compute the node's bounding box and its center in global coordinates.
   * \param pBBoxMin The minimum value of the bounding box upon successful
   * return. \param pBBoxMax The maximum value of the bounding box upon
   * successful return. \param pBBoxCenter The center value of the bounding box
   * upon successful return. \param pTime If different from
   * FBXSDK_TIME_INFINITE, time used to compute the bounding box for deformed
   * geometry. \return \c true if successful, otherwise \c false. \remark If
   * geometry have been unloaded from memory, their bounding box cannot be
   * calculated and will use any value set previously. */
  bool EvaluateGlobalBoundingBoxMinMaxCenter(
      FbxVector4 &pBBoxMin,
      FbxVector4 &pBBoxMax,
      FbxVector4 &pBBoxCenter,
      const FbxTime &pTime = FBXSDK_TIME_INFINITE);

  /** Compute closest ray intersection point with mesh attributes of this node
   * (triangle meshes only!). \param pOut The closest intersection point from
   * pRayOrigin location in pRayDir direction. Variable is unchanged if return
   * value is \c false. \param pRayOrigin The origin location to cast the ray
   * from. \param pRayDir The direction the cast ray to test mesh triangles
   * from. \param pCulling If \c true, only test triangles that are front
   * facing, otherwise test both sides. \param pTime The time to use to evaluate
   * mesh deformations. \return \c true if a triangle intersect with the ray,
   * otherwise \c false. \remark This function will automatically fail if the
   * node's meshes are not triangulated. */
  bool
  EvaluateRayIntersectionPoint(FbxVector4 &pOut,
   ...
   * \param pCharacterLinkType Pointer to receive character link type if
   * function succeeds, cast to \c FbxCharacterLink::Type. \param pNodeId
   * Pointer to receive the node ID if function succeeds. This ID should be
   * casted to \c FbxCharacter::ENodeId type when the character link type is \c
   * eCharacterLink or \c eControlSetLink  else to the \c FbxEffector::ENodeId
   * type if the character link type is \c eControlSetEffector or \c
   * eControlSetEffectorAux. \param pNodeSubId For internal use. \return \c
   ...
   * \param pCharacterLinkType Character link type searched. Its value must be
   *one of the \c FbxCharacterLink::Type symbols.. \param pNodeId Node ID
   *searched. If \e pCharacterLinkType is \c eCharacterLink or \c
   *eControlSetLink the \e pNodeId value is casted to the \c
   *FbxCharacter::ENodeId type. If the \e pCharacterLinkType is \c
   *eControlSetEffector or \c eControlSetEffectorAux then the \e pNodeId is
   *casted to the \c FbxEffector::ENodeId type. \param pNodeSubId For internal
   *use. \return Index of found character link if it exists, -1 otherwise.
   */
  int FindCharacterLink(FbxCharacter *pCharacter,
                        int pCharacterLinkType,
                        int pNodeId,
                        int pNodeSubId) const;
  //@}
   ...
   *represent the index of the connection. For example, the call: \code
   *	lNode->GetAnimationInterval(span, myStack, 3);
   * \endcode
   * will scan all the animation curves of this node, and it's children, that
   *are defined on the third animation layer of \c myStack.
   */
   ...
   *          to have a material applied to this node more than once. The
   * material count may not reflect the distinct material count.
   */
   ...
   * The assumed behavior of this property is to affect the visibility of the
   * node, all the nodes attributes connected to it as well as all its
   * descendants. This property can be animated.
   *
   * To access this property do: Visibility.Get().
   * To set this property do: Visibility.Set(FbxDouble).
   *
   * Default value is 1.
   * \remarks  \li This property holds values ranging from 0.0 to 1.0 where the
   * value 0.0 means a totally invisible object, the value 1.0, a full visible
   * object and anything inbetween, a percentage degree of visibility.\n
   *
   *          \li Since not all the applications may support a degree of
   * visibility, it is agreed that a value of 0.0 means invisible and anything
   * else means visible.
   *
   * \see Show property.
   */
  ...
  /** This property contains the visibility inheritance flag that allow
   * applications to modify the Visibility property interpretation. By default,
   * this value is set to \c true because it is assumed (as explained in the
   * Visibility property description) that the node visibility is inherited from
   * its parent. In other words, applications should always process the
   * Visibility property of the node and, depending on its value, decide whether
   * or not the node has to be displayed. After this first assessment, check the
   * node VisibilityInheritance flag. If its value is set to \c false then move
   * to the next object, else use the parent's Visibility value and modify this
   * node display state by performing the logical AND operation between this
   * node Visibility property and its parent's.
   *
   * To access this property do: VisibilityInheritance.Get().
   * To set this property do: VisibilityInheritance.Set(FbxBool).
   *
   * Default value is \c true.
   * \remarks This property is non-animatable and is not used inside the FBX SDK
   * but it is guaranteed to exist in FBX files with version 7.2 and above. \see
   * Visibility property.
   */
   ...
   * When set to \c false the object can translate in any direction without
   * limitations. Else the \ref TranslationMinX, \ref TranslationMinY, \ref
   * TranslationMinZ, \ref TranslationMaxX, \ref TranslationMaxY and \ref
   * TranslationMaxZ flags are used to limit the translation on each individual
   * axis.
   *
  ...
  /** This property sets the minimum translation values the object can occupy on
  ...
  /** This property sets the maximum translation values the object can occupy on
   * each individual axis.
   *
   ...
   * When set to \c true, the object translation is constrained by the value of
   * \ref TranslationMin.
   *
   ...
   * When set to \c true, the Rotation space is applied only on the limit data
   * (provided the \ref RotationActive is also \c true).
   *
   ...
   * When set to \c false the object can rotate in any direction without
   * limitations. Else the \ref RotationMinX, \ref RotationMinY, \ref
   * RotationMinZ, \ref RotationMaxX, \ref RotationMaxY and \ref RotationMaxZ
   * flags are used to limit the rotation on each individual axis. \remarks The
   * PreRotation value is applied before the limit, while the PostRotation is
   * applied after the limit.
   *
  ...
  /** This property sets the minimum rotation values the object can occupy on
   * each individual axis.
   *
  ...
  /** This property sets the maximum rotation values the object can occupy on
   * each individual axis.
   *
   ...
   * When set to \c true, the object rotation is constrained by the value of
   * \ref RotationMin.
   *
   ...
   * When set to \c false the object can scale in any direction without
   * limitations. Else the \ref ScalingMinX, \ref ScalingMinY, \ref ScalingMinZ,
  ...
  /** This property sets the minimum scaling values the object can occupy on
   * each individual axis.
   *
  ...
  /** This property sets the maximum scaling values the object can occupy on
   * each individual axis.
   *
  ...
  /** This property activates or disables the limit on scaling X. When active,
   * the object scaling is constrained by the value of \ref ScalingMin.
   *
   ...
   * When set to \c true, the object scaling is constrained by the value of \ref
   * ScalingMin.
   *
  ...
  /** This property contains the x component of the minimum damp range angles of
   * the node
   *
  ...
  /** This property contains the y component of the minimum damp range angles of
   * the node
   *
  ...
  /** This property contains the z component of the minimum damp range angles of
   * the node
   *
  ...
  /** This property contains the x component of the maximum damp range angles of
   * the node
   *
  ...
  /** This property contains the y component of the maximum damp range angles of
   * the node
   *
  ...
  /** This property contains the z component of the maximum damp range angles of
   * the node
   *
  ...
  /** This property contains the x component of the minimum damp strength of the
   * node
   *
  ...
  /** This property contains the y component of the minimum damp strength of the
   * node
   *
  ...
  /** This property contains the z component of the minimum damp strength of the
   * node
   *
  ...
  /** This property contains the x component of the maximum damp strength of the
   * node
   *
  ...
  /** This property contains the y component of the maximum damp strength of the
   * node
   *
  ...
  /** This property contains the z component of the maximum damp strength of the
   * node
   *
   ...
   * As opposed to the Visibility property, this one cannot be animated. The
   * assumed behavior of this property is to represent the show/hide state of
   * all the nodes attributes connected to this node only.
   *
   ...
   *
   * \remarks  \li Because node attributes can be shared by multiple nodes
   * (instances), the FBX SDK provides an utility function
   * FbxScene::SyncShowPropertyForInstance() to propagates the same Show value
   * across all the nodes referencing the node attribute. The applied logic is
   * that as soon as one of these nodes has the Show property set to \c false,
   * all will be set to \c false (basically it is an AND operation on all the
   *           Show flags).
   *
   *           \li Depending on the support of the Show and Visibility
   * properties that applications will implement, there may be conflicts with
   * these two states. In this case, it is suggested that the Visibility
   * property always overrides the Show.
   *
   * \see Visibility property.
   */
  FbxPropertyT<FbxBool> Show;

  /** This property contains negative percent shape support information of the
   * node
   *
...
#define MACRO_PIVOT_VECTOR_FCTS(name, defVect)                                 \
  ...
  {                                                                            \
    FBX_ASSERT(id == 0 || id == 1);                                            \
    if (mIsDefault[id] && pV[0] == defVect[0] && pV[1] == defVect[1] &&        \
        pV[2] == defVect[2])                                                   \
      return;                                                                  \
    mIsDefault[id] = false;                                                    \
    Get(id).Set##name(pV);                                                     \
...
#define MACRO_PIVOT_BOOL_FCTS(name)                                            \
  ...
  {                                                                            \
    ...
    {
      FBX_ASSERT(id == 0 || id == 1);
      // If pivot has default values, and we want to set default rotation order
      // eEulerXYZ, return to avoid allocating memory for the pivot (in
      // Get(id).)
...
#undef MACRO_PIVOT_BOOL_FCTS
  ...
  int AddCharacterLink(FbxCharacter *pCharacter,
                       int pCharacterLinkType,
                       int pNodeId,
                       int pNodeSubId);
  int RemoveCharacterLink(FbxCharacter *pCharacter,
                          int pCharacterLinkType,
                          int pNodeId,
                          int pNodeSubId);

  // Duplicate this node as well as all its node attributes and the Target and
  // UpTarget objects.
...
protected:
  ...
  void Reset();
  bool GetAnimationIntervalRecursive(FbxTimeSpan &pTimeInterval,
...
private:
  ...
  void ConvertPivotAnimationRecurseLoop(FbxAnimStack *pAnimStack,
                                        const EPivotSet pConversionTarget,
                                        const double pFrameRate,
                                        const bool pKeyReduce,
                                        GeomInstSet &pGeomInstSet);
  void ConvertPivotAnimation(FbxAnimStack *pAnimStack,
                             const EPivotSet pConversionTarget,
                             const double pFrameRate,
                             const bool pKeyReduce,
                             GeomInstSet &pGeomInstSet);
  bool ConvertPivotAnimation_SetupMatrixConverter(
      ...
      const bool pKeyReduce,
      GeomInstSet &pGeomInstSet);
  void ConvertPivotAnimation_ApplyGeometryPivot(const EPivotSet &pSrcSet,
                                                const EPivotSet &pDstSet,
                                                GeomInstSet &pGeomInstSet);
