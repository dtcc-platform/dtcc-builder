 ...
 * of the geometry's control points, with different weights. For example, a mesh
 * of humanoid shape can have a skin attached, that describes the way the
 * humanoid mesh is deformed by bones. When the bones are animated, the clusters
 * act on the geometry to animate it too.
 *
...
public:
   ...
   * 100% is full accuracy and 1% is a rough estimation of the envelope
   * deformation. \param pDeformAccuracy         value for deformation accuracy.
   */
   ...
   * \param pCluster                Pointer to the cluster to remove from this
   * skin deformer. \return                        Pointer to cluster or \c NULL
   * if pCluster is not owned by this skin deformer.
   */
  FbxCluster *RemoveCluster(FbxCluster *pCluster);

  /** Get the number of clusters.
   * \return                        Number of clusters that have been added to
   * this object.
   */
   ...
   * \return                        Pointer to cluster or \c NULL if index is
   * out of range.
   */
   ...
   *      - \e eRigid                       Type eRigid means rigid skinning,
   * which means only one joint can influence each control point.
   *      - \e eLinear                      Type eLinear means the classic
   * linear smooth skinning.
   *      - \e eDualQuaternion              Type eDualQuaternion means the dual
   * quaternion smooth skinning.
   *      - \e eBlend                       Type eBlend means to blend classic
   * linear and dual quaternion smooth skinning according to blend weights.
   */
  enum EType
  {
    eRigid,
  ...
  /**
   * \name Control Points
   * A skin has an array of indices to control points and associated blend
   * weights. The indices refer to the control points in the instance of class
   * FbxGeometry. The blend weights are the influence of the different skinning
   * type over the deformation effect of the indexed control points.
   */
  //@{

  /** Add an element in both arrays of control point indices and blendWeights.
   * \param pIndex          The index of the control point.
   * \param pBlendWeight    The blend weight for this control point. The value
   * should between 0 and 1. Any value that is less than 0 will be set to 0, any
   * value that is greater than 1 will be set to 1. 0 means completely linear
   * skinning, 1 means completely dual quaternion skinning, a value between 0
   * and 1 means the weighted blending of the above two skinning methods.
   */
   ...
   * \return     Length of the arrays of control point indices and blend
   * weights. Returns 0 if no control point indices have been added or the
