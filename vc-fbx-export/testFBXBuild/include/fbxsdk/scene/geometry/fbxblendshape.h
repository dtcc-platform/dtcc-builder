 ...
 * A blend shape deformer takes a base shape (polygonal surface, curve, or
 * surface) and blends it with other target shapes based on weight values. Blend
 * shape deformer organize all target shapes via blend shape channel. One blend
 * shape deformer can contains multiple blend shape channels, then each channel
 * can organize multiple target shapes, \see FbxBlendShapeChannel, FbxShape.
 * \remarks The blend effect of each blend shape channel is additive, so the
 * final blend effect of a blend shape deformer is the sum of blend effect of
 * all blend shape channels it contains, the blend effect of each blend shape
 * channel is controlled by the property DeformPercent of blend shape channel.
...
public:
   ...
   * \remarks            One blend shape deformer can only be used on one base
   * geometry. So when SetGeometry is called, the pGeometry will replace the
   *                     current base geometry connected to this blend shape
   * deformer.
   */
  bool SetGeometry(FbxGeometry *pGeometry);

  /** Get the geometry affected by this blend shape deformer.
   * \return             A pointer to the geometry if it is set or \c NULL if
   * not set yet.
   */
  FbxGeometry *GetGeometry();

  /** Add a blend shape channel.
   * \param pBlendShapeChannel      Pointer to the blend shape channel object to
   * add. \return                        \c true on success, \c false otherwise.
   */
  bool AddBlendShapeChannel(FbxBlendShapeChannel *pBlendShapeChannel);

  /** Remove the given blend shape.
   * \param pBlendShapeChannel      Pointer to the blend shape channel to remove
   * from this blend shape deformer. \return                        Pointer to
   * the blend shape channel or \c NULL if pBlendShapeChannel is not owned by
   * this blend shape deformer.
   */
  FbxBlendShapeChannel *
  RemoveBlendShapeChannel(FbxBlendShapeChannel *pBlendShapeChannel);

  /** Get the number of blend shape channels.
   * \return                        Number of blend shape channels that have
   ...
   * \return                        Pointer to the blend shape channel or \c
   ...
   * \return                         The deformer type identifier of blend shape
   * deformer.
   */
