 ...
 * stereo camera. \nosubgrouping Generally, a set of stereoRig contains the
 * center camera, the left camera and the right camera. FbxCameraStereo is used
 * to represent the center camera. The left and right camera could be FbxCamera.
 * FbxCameraStereo contains stereo properties.
 * The left and right camera can also be get and set via related methods in
 * FbxCameraStereo class. \see FbxCamera and FbxCameraSwitcher.
 */
...
public:
  ...
  enum EStereoType
  {
    ...
    eOffAxis, //!< Computes the convergence plane by shifting the frustum using
              //!< camera film back.
    eParallel //!< A parallel camera setup where there is effectively no
              //!< convergence plane.
  };

  /** Get the left camera which connect to property LeftCamera.
   * \return   A pointer to FbxCamera.
   * \remarks  Current FbxCameraStereo should work with two FbxCamera, left
   * camera and right camera. Use this method to get the left camera.
   */
   ...
   * \return   A pointer to FbxCamera.
   * \remarks  Current FbxCameraStereo should work with two FbxCamera, left
   * camera and right camera. Use this method to get the right camera.
   */
   ...
   * \remarks Current FbxCameraStereo should work with two FbxCamera, left
   * camera and right camera. Use this method to set the left camera.
   */
   ...
   * \return        \c true if it's successful, \c false otherwise.
   * \remarks Current FbxCameraStereo should work with two FbxCamera, left
   * camera and right camera. Use this method to set the right camera.
   */
   ...
   * \remarks Use this method to reevaluate the local transformation of left
   * camera.
   */
   ...
   * \remarks Use this method to reevaluate the global transformation of left
   * camera.
   */
   ...
   * \remarks Use this method to reevaluate the local transformation of right
   * camera.
   */
   ...
   * \remarks Use this method to reevaluate the global transformation of right
   * camera.
   */
  ...
  /** This property handles the distance on the camera view axis where the zero
  ...
  /** This property is to offset the computed toe-in effect when it's in
   * Converged mode.
   *
   ...
   *
   * \remarks Default Value is 0.0.
   * This value is specified in degrees and acts as an offset to the computed
   * toe-in. \see EStereoType.
   */
   ...
   * \remarks It's used to connect the left/right camera property [FocalLength,
   * FarPlane, NearPlane, FilmWidth, FilmHeight, FilmSqueezeRatio] to stereo
   * camera. During FBX SDK reevaluating, if ConnectProperties is called, to get
   * the newest FocalLength property of left camera, please use
   * lLeft_Camera->FocalLength.GetSrcProperty();
   */
