...
public:
  ...
  /** Remove all weighted relations and give new source and destination sets
   * sizes. \param pSourceSize       New source set size. \param
   ...
   * For example, for one object (which index is specified by pElement) in
   * source set (specified by pSet), the function return how many objects (as
   * destination) the object (as source) mapping to. \param pSet     Source or
   * destination set. \param pElement Object index in the set.
   */
   ...
   * \return                  Element gives the index of an element in the other
   * set and the assigned weight.
   */
  Element &GetRelation(ESet pSet, int pElement, int pIndex);

  /** Given the index of an element in the other set, get the index of one of
   * the relations an element of a set is linked to. Returns -1 if there is not
   * relation between these elements. \param pSet             Source or
   * destination set. \param pElementInSet    Object index in the set. \param
   * pElementInOtherSet   Object index in another set. \return The index of one
   * of the relations, -1 if there is not relation between these elements.
   */
  int GetRelationIndex(ESet pSet,
                       int pElementInSet,
                       int pElementInOtherSet) const;

  /** Get the sum of the weights from the relations an element of a set is
   * linked to. \param pSet             Source or destination set. \param
   * pElement         Object index in the set. \param pAbsoluteValue   Flag to
   * convert negative value to positive value. \return                 The sum
   * of the weights  from the relations.
   */
...
typedef class FbxArray<FbxArray<FbxWeightedMapping::Element> *>
