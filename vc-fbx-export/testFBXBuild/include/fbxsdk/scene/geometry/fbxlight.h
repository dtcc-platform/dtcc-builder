...
/** \brief This node attribute contains methods for accessing the properties of
 * a light. \nosubgrouping
 */
...
public:
  ...
  /** \enum EDecayType     Decay types. Used for setting the attenuation of the
   * light.
   * - \e eNone          No decay. The light's intensity will not diminish with
   * distance.
   * - \e eLinear        Linear decay. The light's intensity will diminish
   * linearly with the distance from the light.
   * - \e eQuadratic     Quadratic decay. The light's intensity will diminish
   * with the squared distance from the light. This is the most physically
   * accurate decay rate.
   * - \e eCubic         Cubic decay. The light's intensity will diminish with
   * the cubed distance from the light.
   */
  enum EDecayType
  {
    eNone,
  ...
  /** Get the light state.
   * \return     Pointer to the texture cast by the light shadow, or \c NULL if
   * the shadow texture has not been set.
   */
  ...
  /** This property handles the light inner cone angle (in degrees). Also know
   * as the HotSpot
   *
  ...
  /** This property handles the light outer cone angle (in degrees). Also known
   * as the Falloff
   *
   * Default value is 45.0
   */
