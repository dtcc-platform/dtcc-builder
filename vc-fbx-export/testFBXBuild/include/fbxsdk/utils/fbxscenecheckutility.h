...
/** \brief This class defines functions to check the received scene graph for
  issues.
  * remark This is still an experimental class and it is not expected to
  validate every single data object.
  */
...
public:
  ...
  FbxSceneCheckUtility(const FbxScene *pScene,
                       ...
                       FbxArray<FbxString *> *pDetails = NULL);
  ~FbxSceneCheckUtility();

  /** Check for issues in the scene
    * return	\false if any issue is found in the scene
    * remark  Depending on the check mode settings, the processing time can
    increase dramatically.
    * remark  If a status and/or details object is provided, the error code is
    set and, details info is logged.
    */
...
private:
  ...
  bool CheckMappingMode(FbxLayerElement::EMappingMode pMappingMode,
                        const FbxString &pPrefix);
  bool CheckReferenceMode(FbxLayerElement::EReferenceMode pReferenceMode,
                          const FbxString &pPrefix);
  bool CheckSurfaceMode(FbxGeometry::ESurfaceMode pSurfaceMode,
                        const FbxString &pPrefix);
  bool CheckSurfaceType(FbxNurbs::EType pSurfaceType,
                        const FbxString &pPrefix,
                        const char *pDir);
