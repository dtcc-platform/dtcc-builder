...
/** This class can be used to provide basic camera manipulation in any program
 * using this library. \nosubgrouping
 */
...
public:
  //! All possible manipulation actions that can be performed on a camera using
  //! this manipulator.
  enum EAction
  {
    eNone,   //!< No action.
   ...
   * \param pTimeDelta Elapsed time since the last notify. Only used if
   * Smoothing is enabled. \param pX Horizontal position of the manipulation, in
   * pixels. \param pY Vertical position of the manipulation, in pixels.
   * \param pScale Scaling value of the manipulation. Only used by eFreePan
   * action. */
  ...
  /** Update the camera position. This must be called periodically in order for
   * the camera to update its position. \param pTimeDelta Elapsed time since the
   * last update. If Smooth is disabled, you can leave this value to zero.
  ...
  /** Do a complete manipulation action in a single operation. This is the
   * equivalent of calling Begin, Notify and End successively. \param pAction
   * The action performed for this manipulation scope. \param pX Horizontal
   * position of the manipulation, in pixels. \param pY Vertical position of the
   * manipulation, in pixels. \param pScale Scaling value of the manipulation.
   * Only used by eFreePan action. */
   ...
   * \param pTime Time to use to evaluate mesh deformations. Leave at default
   * value to cancel mesh evaluation. */
  ...
  /** Change camera position and LookAt to frame the selected position on
   * screen. The LookAt will be placed at first closest intersecting geometry,
   * and the distance between camera and LookAt will be preserved. \param pX The
   * horizontal screen coordinate. \param pY The vertical screen coordinate.
   * \param pCulling If \c true, only test triangles that are front-facing,
   * otherwise test both sides. \param pTime Time to use to evaluate mesh
   * deformations. Leave at default value to cancel mesh evaluation. */
  ...
  /** Width of the camera viewport, in pixels. This is used to accurately
   * calculate to movement speed.
   * \remark If this property is not correctly set, movements will be erronous.
   */
  FbxPropertyT<FbxFloat> ViewportWidth;

  /** Height of the camera viewport, in pixels. This is used to accurately
   * calculate to movement speed.
   * \remark If this property is not correctly set, movements will be erronous.
   */
  ...
  /** Camera manipulations smoothing speed. Higher speed will stabilize the
   * camera more quickly. Default is 10.0 */
  FbxPropertyT<FbxDouble> SmoothSpeed;

  /** Invert the camera horizontal manipulation direction if set to true. False
   * by default. */
  FbxPropertyT<FbxBool> InvertX;

  /** Invert the camera vertical manipulation direction if set to true. False by
   * default. */
...
private:
  ...
  double ComputeRotationAxis(FbxVector4 &pFront,
                             FbxVector4 &pUp,
                             FbxVector4 &pRight,
                             const FbxVector4 &pEye,
                             const FbxVector4 &pLookAt,
                             const FbxVector4 &pUpVector) const;
  void ComputeRotationMatrix(FbxAMatrix &pRM,
                             const FbxVector4 &pEye,
                             const FbxVector4 &pLookAt,
                             const FbxVector4 &pUpVector);
  void UpdateCameraRotation();

  bool FrameObjects(bool pSelected, const FbxTime &pTime);
  FbxVector4
  ComputePositionToFitBBoxInFrustum(const FbxVector4 &pBBoxMin,
                                    const FbxVector4 &pBBoxMax,
                                    const FbxVector4 &pBBoxCenter,
                                    const FbxVector4 &pCameraPosition,
                                    const FbxAMatrix &pCameraRM,
                                    const FbxTime &pTime);
