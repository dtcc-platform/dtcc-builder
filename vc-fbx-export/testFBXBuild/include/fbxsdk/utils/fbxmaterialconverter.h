 ...
 * applied to geometries from FBX v5 material system to v6-and-up or the other
 * way around. \nosubgrouping \see FbxSurfaceMaterial, FbxTexture
 */
...
private:
  ...
  void GetTextures(int pComponent,
                   FbxLayer *pLayer,
                   LayerConfig &pLayerConfig) const;
  FbxSurfaceMaterial *GetMaterial(int pComponent,
                                  FbxLayer *pLayer,
                                  FbxNode *pNode,
                                  bool pLookOnNode);
  int GetMaterialOrder(int pComponent,
                       FbxLayer *pLayer,
                       FbxNode *pNode,
                       bool pLookOnNode);

  bool HasGoodMappingModes(FbxNode *pNode, FbxGeometry *pGeom) const;
  void ConnectTextures(FbxSurfaceMaterial *pMat,
                       FbxObject *pTexture,
                       int pTextureType) const;
  ...
  void GetTextureList(FbxArray<TexData> &pTextures,
                      FbxLayeredTexture *pTex) const;

  FbxLayer *
  FindLayerForTexture(FbxTexture *pTex,
  ...
  void InitTextureElement(FbxLayerElementTexture *pTexElm,
                          int pComponentCount,
