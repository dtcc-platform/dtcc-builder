 ...
 * Fbx_Root nodes were used to orient and scale scenes from other graphics
 * applications. They have been replaced by the conversion routines in
 * FbxAxisSystem and FbxSystemUnit. These methods are provided for backward
 * compatibility only and will eventually be removed. Use the conversion
 * routines in FbxSystemUnit and FbxAxisSystem when possible.
 */
...
public:
   ...
   */
  static bool
  InsertFbxRoot(FbxScene *pScene,
                const FbxAxisSystem &pDstAxis,
                const FbxSystemUnit &pDstUnit,
                const FbxSystemUnit::ConversionOptions &pUnitOptions =
