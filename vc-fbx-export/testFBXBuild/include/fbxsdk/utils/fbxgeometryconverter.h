 ...
 * major categories: Triangulation and conversion between NURBS and Patches
 * surfaces. \nosubgrouping
 */
...
public:
   ...
   * \param pReplace If \c true, replace the original meshes with the new
   * triangulated meshes on all the nodes, and delete the original meshes.
   * Otherwise, original meshes are left untouched. \param pLegacy If \c true,
   * use legacy triangulation method that does not support holes in geometry.
   * Provided for backward compatibility. \return \c true if all node attributes
   * that can be triangulated were triangulated successfully. \remark The
   * function will still iterate through all meshes regardless if one fails to
   * triangulate, but will return false in that case. This function currently
   * only supports node attribute of type eMesh, ePatch, eNurbs or
   * eNurbsSurface. */
  bool Triangulate(FbxScene *pScene, bool pReplace, bool pLegacy = false);

  /** Triangulate a node attribute, if supported, and preserve the skins and
   * shapes animation channels. \param pNodeAttribute Pointer to the node
   * containing the geometry to triangulate. \param pReplace If \c true, replace
   * the original geometry with the new triangulated geometry on the nodes, and
   * delete the original geometry. Otherwise, the original geometry is left
   * untouched, the new one is added to the nodes, and becomes the default one.
   * \param pLegacy If \c true, use legacy triangulation method that does not
   * support holes in geometry. Provided for backward compatibility. \return The
   * newly created node attribute if successful, otherwise NULL. If node
   * attribute type is not supported by triangulation, it returns the original
   * node attribute. \remark This function currently only supports node
   * attribute of type eMesh, ePatch, eNurbs or eNurbsSurface. If the node
   * attribute does not support triangulation,
  ...
  /** Compute a "vertex-correspondence" table that helps passing from source to
   * destination geometry. \param pSrcGeom Pointer to the source geometry.
   ...
   * \return \c true on success, \c false if the function fails to compute the
   * correspondence. \remark Skins and shapes are also converted to fit the
   * alternate geometry. */
  ...
  /** Convert a patch contained in a node to a nurb. Use this function to
   * preserve the patch's skins and shapes animation channels. \param pNode
   * Pointer to the node containing the patch. \return          \c true on
   * success, \c false if the node attribute is not a patch. \remarks The patch
   * must be of type eBSpline, eBezier or eLinear.
   */
  ...
  /** Convert a patch contained in a node to a nurb surface. Use this function
   * to preserve the patch's skins and shapes animation channels. \param pNode
   * Pointer to the node containing the patch. \return          \c true on
   * success, \c false if the node attribute is not a patch. \remarks The patch
   * must be of type eBSpline, eBezier or eLinear.
   */
   ...
   * \return          A FbxNurbs that is equivalent to the original nurbs
   * surface.
   */
  FbxNurbs *ConvertNurbsSurfaceToNurbs(FbxNurbsSurface *pNurbs);

  /** Convert a nurb, contained in a node, to a nurbs surface. Use this function
   * to preserve the nurb's skins and shapes animation channels. \param pNode
   * Pointer to the node containing the nurb. \return          \c true on
  ...
  /** Convert a nurb contained in a node to a nurbs surface. Use this function
   * to preserve the nurb's skins and shapes animation channels. \param pNode
   * Pointer to the node containing the nurbs surface. \return          \c true
   ...
   * \param pSwapClusters     Set to \c true to swap the control point indices
   * of clusters. \return                  A flipped FbxNurbs, or \c NULL if the
   * function fails.
   */
   ...
   * \param pSwapUV           Set to \c true to swap the UVs.
   * \param pSwapClusters     Set to \c true to swap the control point indices
   * of clusters. \return                  A flipped FbxNurbsSurface, or \c NULL
   * if the function fails.
   */
  FbxNurbsSurface *
  FlipNurbsSurface(FbxNurbsSurface *pNurbs, bool pSwapUV, bool pSwapClusters);
  //@}
   ...
   * \return          \c true on success, \c false if the number of normals in
   * the mesh and in its associated shapes don't match the number of polygon
   *                  vertices.
   * \remarks         For applications that only supports normals by control
   * points, this function duplicates control points to equal the number of
   * polygon vertices. skins and shapes are also converted. As preconditions:
   *                       -# polygons must have been created
   *                       -# the number of normals in the mesh and in its
   * associated shapes must match the number of polygon vertices.
   */
   ...
   *                  Normals do not need to be on Layer 0, since the first
   * layer with per polygon vertex normals is used.
   */
   ...
   * \return          \c true on success, \c false otherwise.
   * \remarks         The smoothing group is bitwise.  Each bit of the integer
   * represents one smoothing group.  Therefore, there is a maximum of 32
   * smoothing groups.
   */
   ...
   * \param pReplace If \c true, replace the original mesh with new ones and
   * delete the original meshes, but *only* if they got split into multiple
   * meshes, otherwise they are left untouched. \return \c true if all splitable
   * mesh were successfully split, \c false otherwise.
   * \remark The function will still iterate through all meshes regardless if
   * one fails to split, but will return false in that case. */
  bool SplitMeshesPerMaterial(FbxScene *pScene, bool pReplace);

  /** Split mesh per material.
   * \param pMesh The mesh that will be split if it has multiple materials
   * assigned. \param pReplace If \c true, replace the original mesh with new
   * one and delete the original mesh, but *only* if they got split into
   * multiple meshes, otherwise left untouched. \return \c true on success, \c
   * false otherwise. \remark The function will fail if the mapped material is
   * not per face (FbxLayerElement::eByPolygon) or if a material is
   * multi-layered. It will create as many meshes as there are materials applied
   * to it. If one mesh have some polygons with material A, some polygons with
   * material B, and some polygons with NO material, 3 meshes distinct will be
   * created. The newly created meshes will be automatically attached to the
   * same FbxNode that holds the original FbxMesh. If the original mesh have
   * tangents, they will be regenerated on the new meshes. */
  bool SplitMeshPerMaterial(FbxMesh *pMesh, bool pReplace);
  //@}

  /** Re-parent nodes at root node level under a new node to re-center them at
   * world center. Basically, this function calculates the scene bounding box in
   * world coordinates, and test if the center of that bounding box distance
   * from the world center is larger or equal than the threshold. If true, a new
   * node with the proper negative offset position will become the new parent of
   * all nodes at root node level. \param pScene The scene to process. \param
   * pThreshold Threshold at which all nodes will be re-centered. \return \c
   * true if any nodes were re-centered, otherwise \c false. */
   ...
   * e) if there are skins for old mesh, merge these skins. The new skin
   *clusters link to old skeletons.
   *
   ...
   *polygon group, if any mesh misses these element, the merge for this kind of
   *element is skipped. For layer element crease, hole, visibility and user
   *data, if any mesh has such element, the kind of element will be merged. The
   *missing element will be filled with default values. For meshes with skin
   *binding, if the pose of frame 0 is different with bind pose, the new mesh
   *will be distorted.
   */
   ...
   * \param pAffectedNodes The list of nodes that have been affected by this
   * operation. \remarks  If the cleaned-up mesh becomes invalid, it is removed
   * entirely.
   */
  void RemoveBadPolygonsFromMeshes(FbxScene *pScene,
                                   FbxArray<FbxNode *> *pAffectedNodes = NULL);
...
private:
   ...
   * -- Triangulation algorithm is strip, Sorting vertex index for strip in
   * clockwise.
   *
   *          2           3         4
   *           0----------0---------0
   ...
   * [{0,1,2},{2,3,0},{0,3,7},{3,4,7},{7,4,6},{4,5,6}]
   */
  static void FbxTriangulation(int *Index, int pNumSide);

  bool
  ComputePatchToMeshControlPointsWeightedMapping(FbxPatch *pSrcPatch,
                                                 FbxMesh *pDstMesh,
                                                 FbxWeightedMapping *pMapping,
                                                 bool pSwapUV = false);
  bool
  ComputeNurbsToMeshControlPointsWeightedMapping(FbxNurbsSurface *pSrcNurbs,
                                                 FbxMesh *pDstMesh,
                                                 FbxWeightedMapping *pMapping,
  ...
  void ConvertShapes(const FbxGeometry *pSource,
  ...
  void ConvertClusters(const FbxGeometry *pSource,
                       FbxGeometry *pDestination,
                       FbxWeightedMapping *pSourceToDestinationMapping);
  void ConvertClusters(FbxArray<FbxCluster *> const &pSourceClusters,
                       ...
                       FbxWeightedMapping *pSourceToDestinationMapping);
  void
  BuildClusterToSourceMapping(FbxArray<FbxCluster *> const &pSourceClusters,
  ...
  void
  FlipControlPoints(FbxGeometryBase *pPoints, int pUCount, int pVCount) const;
