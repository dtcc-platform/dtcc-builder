...
/** This base class is an abstract implementation of a renaming strategy for
 * avoiding name clashes. An implementation of a reader (FbxReader) or writer
 * (FbxWriter) class must call a concrete implementation of
 * "FbxRenamingStrategyInterface::Rename()" every time a name is imported or
 * exported to avoid name clashes. Any class deriving from
 * FbxRenamingStrategyInterface must implement
 * FbxRenamingStrategyInterface::Clear(),
 * FbxRenamingStrategyInterface::Rename(), and
...
public:
  ...
  /** Create a dynamic renaming strategy instance of the same type as the child
   * class. \return New instance.
   */
  virtual FbxRenamingStrategyInterface *Clone() = 0;
};

/** Implements a renaming strategy that resolves name clashes by adding number
...
private:
  struct NameCell
  {
    NameCell(const char *pName) : mName(pName), mInstanceCount(0) {}
 ...
 * scene. It can remove name clashing, remove illegal characters, manage
 * namespaces and manage backward compatibility. It is better to choose
 * FbxSceneRenamer instead of this class to simplify the usage. \remark This
 * class implements a legacy renaming strategy where unsupported characters are
 * replaced with the underscore character. Improved renaming strategies solvers
 * can be implemented using the functions provided by the
 * FbxRenamingStrategyUtils class and the FbxRenamingStrategyBase.
 * \nosubgrouping
 * \see FbxSceneRenamer
 */
...
{
public:
  /** \enum EDirection The mode describing the convention direction, from FBX
   * format or to FBX format.
  ...
  /** Constructor.
   * \param pMod The mode describing the convention direction, from FBX format
   * or to FBX format. \param pOnCreationRun
   */
   ...
   * \param pIsCaseSensitive     Set to \c true to make the name clashing case
   * sensitive.
   */
   ...
   * \param pReplaceNonAlphaNum     Set to \c true to replace illegal characters
   ...
   * \param pFirstNotNum     Set to \c true to add an underscore to the name if
  ...
  /** Recursively renames all the unparented namespaced objects (Prefix mode)
   * starting from this node. \param pNode       Parent node. \param pIsRoot The
   * root node. \return            \c true if the "_NSclash" have been added to
   * one or more node. \remarks           This function adds "_NSclash" when it
   * encounters an unparented namespaced object.
   */
  virtual bool RenameUnparentNameSpace(FbxNode *pNode, bool pIsRoot = false);

  /** Recursively removes all the unparented namespaced "key" starting from this
   * node. \param pNode     Parent node. \return          \c true if
   * successfully removed the "_NSclash" from one or more nodes. \remarks This
   * function removes "_NSclash" when encountered. This is the opposite from
   ...
   * \param pNameSpaceList        output the namespace list from pNode's parent
   * to the root node.
   */
   ...
   * \remarks            This function only replaces the first occurence of
   * OldNS with NewNS.
   */
  virtual bool
  PropagateNameSpaceChange(FbxNode *pNode, FbxString OldNS, FbxString NewNS);
 ...
 * without using the FbxRenamingStrategy class. FbxSceneRenamer can remove name
 * clashing and illegal characters. It also manages namespaces.
 *
 * Example:
 * Maya only accepts names with letters, digits, or underscores, and we want to
 * convert all the names of a scene from FBX format to Maya format. \code
...
private:
  void ResolveNameClashing(bool pFromFbx,
                           ...
                           bool pFirstNotNum,
                           FbxString pInNameSpaceSymbol,
                           FbxString pOutNameSpaceSymbol,
                           bool pNoUnparentNS /*for MB < 7.5*/,
