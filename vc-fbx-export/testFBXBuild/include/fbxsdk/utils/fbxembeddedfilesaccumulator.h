...
/** This processor is used to accumulate the list of file dependencies (embedded
 * files) in a hierarchy of objects. It retrieves information of embedded files
 * from objects and accumulates them to its class member mEmbeddedFiles. \see
...
public:
  ...
  //! Comparer for PropertyUrlIndexSet, which outputs consistent partial orders
  //! for PropertyUrlIndex pairs
  struct FbxPropertyUrlIndexCompare
  {
    inline int operator()(const PropertyUrlIndex &pKeyA,
                          const PropertyUrlIndex &pKeyB) const
    {
  ...
  typedef FbxSet<PropertyUrlIndex, FbxPropertyUrlIndexCompare>
      PropertyUrlIndexSet;
   ...
   * To simply get the list of file dependencies, iterate through this map and
   * query all the keys.
   */
  //@{
...
public:
   ...
   * The property filter is a list of strings, property names, which are
   * automatically ignored when encountered. Property names must be the full
   * hierarchical property name (ie: parent|child|child).
   *
  ...
  FbxEmbeddedFilesAccumulator(FbxManager &pManager,
                              const char *pName,
                              FbxSet<FbxString> &pPropertyFilter);
