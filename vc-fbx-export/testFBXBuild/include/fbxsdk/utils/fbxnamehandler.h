...
/** A name is a case-sensitive string ID of a property, a node, a node
 * attribute, a texture, etc. The characters constituting a name has no specific
 * limitation. An initial name is the original name (from importing a file, for
 * example), which is saved up for reversible renaming. A current name is the
 * name used in FBX. A namespace is a simple grouping of objects under a given
 * name. Namespaces are primarily used to resolve name-clash issues in FBX,
 * where a new object has the same name as an existing object.
 *
 * For example, Maya only accepts names with letters, digits, or underscores.
 * And when a user import FBX into Maya, a node whose name contains whitespace
 * will be renamed. But the connections and references to this node in FBX scene
 * graph still use the original name, so users have to use the initial name to
...
public:
  /** Constructor.
   * \param pInitialName Name string used to initialize both members
   * (initialName and currentName) of this class.
   */
   ...
   *
   * Some renaming strategies classes need to store the parent name to
   * successfully apply the renaming algorithms. The methods in this section
   * allow them to do so. \remark Because of the very specific use of the
   * mParentName string, callers of the FbxNameHandler class should never assume
   * that mParentName is correctly initialized nor contains a meaningful value
   * outside the scope of the renaming strategy class that used it.
   */
  //@{
   ...
   * \remarks The array is filled in reverse order. For example, with a
   * namespace string defined as: a:b:c:d, the filled array is: array[0]=d,
   * array[1]=c, array[2]=b and array[3]=a
   */
