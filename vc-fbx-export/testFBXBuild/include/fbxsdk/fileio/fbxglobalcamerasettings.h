 ...
 * \nosubgrouping
 * \remarks This class exists for FBX version 6.x and earlier. The new FBX v7.x
 * file format that is now the default no longer uses it. The relevant data (a
 * subset of this class) has been moved to the FbxGlobalSettings object and
...
public:
   ...
   * \return \c True if camera name is valid, returns \c false if it is not
   * valid. \remark A valid camera name must be either one of the defined tokens
   * (FBXSDK_CAMERA_PERSPECTIVE, FBXSDK_CAMERA_TOP, FBXSDK_CAMERA_BOTTOM,
   * FBXSDK_CAMERA_FRONT, FBXSDK_CAMERA_BACK, FBXSDK_CAMERA_RIGHT,
   * FBXSDK_CAMERA_LEFT and FBXSDK_CAMERA_SWITCHER) or the name of a camera that
   * is inserted in the node tree under the scene's root node. */
  bool SetDefaultCamera(const char *pCameraName, FbxStatus *pStatus = NULL);

  /** Returns the default camera name.
   * \return The default camera name, or returns an empty string if no camera
   ...
   * Producer cameras are global cameras in MotionBuilder you use to view the
   * scene. You cannot animate Producer cameras but you can specify their
   * default positions. */
  //@{
   ...
   * \return \c True if it is a producer camera, returns \c false if it is not a
   * producer camera. */
   ...
   * \remark This node has a \c FbxNodeAttribute::eCameraSwitcher node attribute
   * type. This node is not saved when there is no camera in the scene. Nodes
   * inserted below are never saved. Camera indices start at 1. Out of range
   * indices are clamped between 1 and the number of cameras in the scene. The
   * index of a camera refers to its order of appearance when searching the node
   * tree depth first. If a camera is added or removed after camera indices have
   * been set, the camera indices must be updated. It is easier to set camera
   * indices once every camera have been set. Camera index keys must be set
   * using constant interpolation to ensure that camera switches occur exactly
   * at key time. */
  ...
  /** Assignment operator.
   * \param pGlobalCameraSettings FbxGlobalCameraSettings object assigned to
   * this one. */
  const FbxGlobalCameraSettings &
  operator=(const FbxGlobalCameraSettings &pGlobalCameraSettings);
