...
public:
   ...
   * \param pEnable                  Set to \c true to activate the fog option
   * or set to \c false to disable the fog option.
   */
  void SetFogEnable(bool pEnable);

  /** Returns the fog option's current state.
   * \return                         \c True if fog is activated, returns \c
   * false if fog is disabled.
   */
   ...
   * \param pDensity                 The fog density to be set. It can be any
   * double value, however it can happen that other sections of FBX SDK may
   * clamp values to reasonable values. \remarks                        This
   * function is only used when the fog mode is set to exponential or square
   * root exponential.
   */
   ...
   * \remarks                        This function is only used when the fog
   * mode is set to exponential or square root exponential.
   */
   ...
   * \remarks                        This function is only used when the fog
   * mode is set to linear. The new value is clamped to fit inside the interval
   * [0, FogEnd()].
   */
  void SetFogStart(double pStart);

  /** Returns the distance from the view where the fog begins.
   * \return                         The distance from the view where the fog
   * begins. \remarks                        This function is only used when the
   * fog mode is set to linear.
   */
   ...
   * \remarks                        This function is only used when the fog
   * mode is set to linear. The new value is adjusted to fit within the interval
   * [FogStart(), inf).
   */
   ...
   * \return                         The distance from the view where the fog
   * ends. \remarks                        This function is only used when the
   * fog mode is set to linear.
   */
  ...
  /**
   * \name Shadow Planes
   * The functions in this section are supported only by FiLMBOX version 2.7 and
   * earlier. FiLMBOX 3.0 supports shadow planes within a specific shader, which
   * is not supported by the FBX SDK.
   */
  //@{
   ...
   * \param pShadowEnable        Set to \c true to display shadow planes in the
   * scene.
   */
  void SetShadowEnable(bool pShadowEnable);

  /** Returns the current state of the shadow enable flag.
   * \return                     \c True if shadow planes are set to be
   ...
   * \return                     The intensity applied to all shadow planes in
   * the scene. \remarks                    Ranges from 0 to 300.
   */
   ...
   * \return                     Pointer the shadow plane, or \c NULL if the
  ...
  /** Assignment operator.
   * \param pGlobalLightSettings FbxGlobalLightSettings object assigned to this
   * one.
   */
  const FbxGlobalLightSettings &
  operator=(const FbxGlobalLightSettings &pGlobalLightSettings);
