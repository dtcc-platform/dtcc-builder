...
// Undefine the macro mkdir, since it conflict with function mkdir in
// Qt\4.2.3\src\corelib\io\qdir.h
...
/** FbxIOSettings is a collection of properties, arranged as a tree, that
  ...
  * The FBX plugins of some Autodesk products expose a UI representing the
  content of those options
  * to let users see and choose options when an import or export operation is
  about to be done.
  * The tree of options is extensible.
  *
  * Options can be saved or loaded from an XML file using the functions:
  * ReadXMLFile(), WriteXMLFile(), WriteXmlPropToFile(). This functionality can
  be useful
  * for plugins that use preset files.
  *
  * An instance of FbxIOSettings must be created to be used before an
  import/export operation.
  * When a new FbxIOSettings instance is created, all options are created with
  default values.
  ...
  * FbxIOSettings * ios = FbxIOSettings::Create((FbxManager *) mManager,
  IOSROOT);
  ...
  * of FbxIOSettings, set particular options and use it for import/export
  operation.
  *
  ...
  * \verbatim
                     ...
                     |                                      |
                     -OPTION_GROUP_EXPORT (IOSN_EXPORT)     -OPTION_GROUP_IMPORT
  (IOSN_IMPORT) |                                      | -OPTION_GROUP_A
  -OPTION_GROUP_A |     |                                |     | |     -OPTION_A
  |     -OPTION_A |     -OPTION_B                        |     -OPTION_B | |
                          -OPTION_GROUP_B                        -OPTION_GROUP_B
                          |     |                                |     |
                          |     -OPTION_GROUP_A                  |
  -OPTION_GROUP_A |     |     |                          |     |     | |     |
  -OPTION_A                  |     |     -OPTION_A |     |     -OPTION_B |     |
  -OPTION_B |     |                                |     | |     -OPTION_GROUP_B
  |     -OPTION_GROUP_B |           |                          |           | |
  -OPTION_A                  |           -OPTION_A |           -OPTION_B |
  -OPTION_B |                                      | -OPTION_GROUP_C
  -OPTION_GROUP_C |                                      | -OPTION_A -OPTION_A

  \endverbatim
  *
  * Any group of options can contain sub options, or group of sub options.
  * To access an option value, we must pass the full path to the Get/Set
  functions
  * Ex:
  * \code
  * ios->GetBoolProp("Import|IncludeGrp|Animation", true); // the root node name
  is not required
  * \endcode
  *
  * All options path are defined in the file kfbxiosettingspath.h to ease the
  access of any options.
  * Then "Import|IncludeGrp|Animation" == IMP_ANIMATION since IMP_ANIMATION is
  defined in kfbxiosettingspath.h
  * All options defined path start with "IMP_" for import branch or "EXP_" for
  export branch.
  *
  * We strongly encourage to use the defined path in kfbxiosettingspath.h, this
  way if the parent group of an option is changed
  * the change occur only in kfbxiosettingspath.h not in the code elsewhere.
  *
  * Ex: to get the boolean import "Animation" option
  * \code
  * bool anim = ios->GetBoolProp(IMP_ANIMATION, true); // will return true if
  not found, since we pass true as second param
  * \endcode
  *
  ...
  * FbxProperty import_Group = ios->GetProperty( IOSN_IMPORT ); // IOSN_IMPORT
  is defined as "Import" in kfbxiosettingspath.h
  * if(import_Group.IsValid()) // check if we have found the IOSN_IMPORT parent
  option
  * {
  *     // add a new group of options "myOptionGroup"
  *		FbxProperty myOptionGrp = ios->AddPropertyGroup(import_Group,
  "myOptionGroup", FbxStringDT, "My Option Group UI Label");
  * }
  * \endcode
  *
  * Ex: to create a new boolean option under the "myOptionGroup"
  * \code
  * FbxProperty myOptionGrp = ios->GetProperty( "Import|myOptionGroup" ); // can
  also use IOSN_IMPORT|"myOptionGroup"
  * if(myOptionGrp.IsValid()) // check if we have found the "myOptionGroup"
  * {
  *     bool defaultValue = true;
  *		FbxProperty myOption = ios->AddProperty(myOptionGrp,
  "myOptionName", FbxBoolDT, "My Option UI label" , &defaultValue, eFbxBool);
  * }
  * \endcode
  *
  ...
  * if(myOption.IsValid())
  * {
  *		myOPtion.ModifyFlag(FbxPropertyFlags::eUIHidden, true);   // to
  make that option not visible to the UI *
  myOPtion.ModifyFlag(FbxPropertyFlags::eNotSavable, true); // to avoid the
  read/save of that option in XML file
  * }
  * \endcode
  */
...
public:
  ...
  /** Add a property under another parent property with a value to set and a min
   * max values \param pParentProperty \param pName \param pDataType \param
   * pLabel     (optional, used by the UI as widget label) \param pValue \param
   * pMinValue \param pMaxValue \param pVisible   (used by the UI to show or not
   * that property) \param pSavable   (to enable a read & write to an XML file)
   * \param pEnabled   (used by the widget UI to show enabled or disabled)
   * \return a new FbxProperty created
   * \remarks Normally used with numeric properties Ex: integer, float, double,
   * etc.
   */
  ...
  /** Get a property using the full path in the tree ex:
   * "Export|IncludeGrp|Animation" \param pName \return a FbxProperty found
   * \remarks We strongly encourage to use the defined path in
   * kfbxiosettingspath.h ex: EXP_ANIMATION == "Export|IncludeGrp|Animation"
   */
  ...
  /** Get the FbxString at current index of an enum property using the full
   * path. \param pName \param pDefValue Value returned if the property is not
   * found \return a FbxString
   */
  ...
  /** Set the current index using an existing FbxString of an enum property
   * using the full path. \param pName \param pValue \remarks The current index
   * will not change if the FbxString is not found
   */
   ...
   * if the same FbxString exist in other index, if the current index was on the
   * FbxString found the current index will be set to 0
   */
   ...
   */
  bool
  SetFlag(const char *pName, FbxPropertyFlags::EFlags propFlag, bool pValue);
   ...
   * \param path The path of the XML file.
   * \return \c True on success, \c false otherwise.
   * \remarks The flag of the property must be FbxPropertyFlags::eNotSavable ==
   * false
   */
...
*****************************************************************************************************************************/
  ...
  bool ReadXmlPropFromMyDocument(const FbxString &subDir,
                                 const FbxString &filename);

  // Write property branch to an XML file in MyDocument dir
  bool WriteXmlPropToMyDocument(const FbxString &subDir,
                                const FbxString &filename,
                                const FbxString &propPath);
