...
public:
  ...
  enum EInfoRequest
  {
    ...
    eInfoDescriptions,   //!<  To get the file description for a writer
                         //!<  ex:"Autodesk FBX (*.fbx)".
    eInfoVersions,       //!<  To get the file version for a writer ex: 7100.
    eInfoCompatibleDesc, //!<  To get the file compatible description for a
                         //!<  writer.
    eInfoUILabel, //!<  To get the file UI label to show for a writer ex: file
                  //!<  labels shown in "Open file dialog".
    eReserved1 = 0xFBFB,
  };

  //! Helper typedef for passing FbxWriter creator function as argument (used
  //! internally).
  typedef FbxWriter *(*CreateFuncType)(FbxManager &pManager,
                                       FbxExporter &pExporter,
                                       int pSubID,
                                       int pPluginID);

  //! Helper typedef for passing FbxIOSettings creator function as argument
  //! (used internally).
  typedef void (*IOSettingsFillerFuncType)(FbxIOSettings &pIOS);

  //! Helper typedef for passing EInfoRequest function as argument (used
  //! internally).
  ...
  /** Writes extension plug-ins name, version and parameters, so that we can
   * remember if a plug-in was used during export. This is especially useful for
   * extension plug-ins that modify the scene and also to warn users during
   * import if an extension plug-in was used that could be missing. \param
   * pParams The parameters of the extension plug-in. The properties of the
   * objects are used as the parameters of the extension plug-in. \remark This
   * function has no implementation in this class. Only sub-class should
   * implement it as needed. For example, FBX 6 and FBX 7 does implement it.
   */
  ...
  /** Checks if there are spaces in the names of specified node (and its
   *children nodes), and writes the returned node's name in the specified string
   *list. \param pNode	Specifies the node to check. \param pNodeNameList
   *Specifies the string list where the node name that has spaces in it is
   *recorded. \return \c true If there are no spaces in the name of specified
   *node (and its children nodes), \c false  If spaces are found.
   */
   ...
   * A non genuine plug-in is a plug-in made by someone external to Autodesk FBX
   *SDK group. \return \c true If the file format is an internal plug-in ,\c
   *false Otherwise .
   */
  bool IsGenuine();
  ...
  /** Returns true if this writer supports FbxStream I/O. Default value is
   * false. */
