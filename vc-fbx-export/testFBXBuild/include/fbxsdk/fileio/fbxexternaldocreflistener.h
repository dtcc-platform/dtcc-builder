...
{
  FbxString
      mDocumentName; //!< Bare name of external document in document hierarchy.
  FbxString
      mClassName; //!< Class name of the document (FbxDocument, FbxLibrary...).
  FbxString mParentFullName; //!< Full name of the parent document in document
                             //!< hierarchy.
...
class FBXSDK_DLL FbxEventReferencedDocument
    : public FbxEvent<FbxEventReferencedDocument>,
      public FbxExternalDocumentInfo
{
...
public:
  ...
  /** External document reference event handler.
   * \param pEvent
   * \remarks    Operation: calls FindDocument() to find the specified external
   * document and if not found calls LoadDocument() either directly, if it has
   * not parent, or via ConnectToParentLibrary(). If its parent cannot be found,
   * it's added to the dangling document list (and it is not loaded until it's
   * parent is found). After, it tries to resolve dangling documents by calling
   *             TryConnectingDanglingLibraries().
   */
...
protected:
  /**
   * Turn a relative path to an absolute path using the file path of the
   * original document being loaded. If the path is already is absolute, it is
   * returned as is.
   */
