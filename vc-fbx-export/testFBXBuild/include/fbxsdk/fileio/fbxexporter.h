 ...
 * the exporter will ask for all registered writers if an FBX 7 file writer is
 * available, then if a writer is found, the exporter will create the
 * specialized FBX 7 writer and write the file. This way, an exporter can
 * "write" many different type of files like FBX 5/6/7, 3DS, Obj, Dxf, Collada,
 * etc. \see FbxWriter
 *
 ...
 * \endcode
 *
 * \remarks According to the file suffix, a specialized writer will be created
 * internally.\n Ex: for .fbx files a FBX Writer, for .3ds files, a 3ds writer,
 * etc.\n Supported files formats: FBX 5/6/7 Binary & ASCII, Collada, DXF, OBJ,
 * 3DS \nosubgrouping
 */
...
* \param pFileFormat   file format identifier User does not need to specify it
by default. if not specified, plugin will request the file format from the
stream.
  * \param pIOSettings   client IOSettings, if not specified, a default
IOSettings will be created
* \return              \c true on success, \c false otherwise.
  * \remarks             To identify the error that occurred, inspect the status
object accessed
*                      using the GetStatus() function.
  */
  ...
  /** Export the document to the currently created file.
    ...
    * \param pNonBlocking       If true, the export process will be executed in
a new thread, allowing it to be non-blocking. To determine if the export
...
*                           using the GetStatus() function.
   ...
   * \param pExportResult  This parameter, after the export finished, will
   * contain the result of the export success or failure. \return Return true if
   * the exporter is currently exporting. \remarks              This function
   * will always return false if Export() was called with pNonBlocking set to
   * false. This function should be used only in the context of pNonBlocking set
   * to true. It is very important to periodically check if the export finished
   ...
   * \remarks the strings returned match the writers registered for the current
   * format. The array items can be retrieved with the following code: \code
   ...
   * \param pW                  width.
   * \param pH                  height.
   * \remark These values are ignored when exporting to FBX 7.x and higher. With
   * FBX version 6.x and lower, the HeaderInfo is still accessible for legacy
   * reasons and any other custom writers. For FBX filles, these values are used
   * by the FBX QuickTime plug-in (obsolete now) to help it get the window size
   *         without loading the whole file. The information contained in the
   * FbxIOFileHeaderInfo is a duplicate of AspectRatioMode, AspectWidth and
   * AspectHeight properties defined in the FbxCamera class. Retrieveing the
   * FileHeaderInfo starting from FBX 7.x will always return the uninitialized
   * structure.
   */
