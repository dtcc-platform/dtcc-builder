 ...
 *
 * This class also lets you create and read formats other than FBX SDK native
 * formats. Users of FBX SDK can write their own plug-ins to read or write
 * arbitrary file formats. Once their plug-ins are registered in this class, FBX
 * SDK is able to read or write these file formats.
 *
 * Each FbxManager has a unique FbxIOPluginRegistry. To get an instance of this
 * class: \code FbxIOPluginRegistry* registry = manager->GetIOPluginRegistry();
 * \endcode
...
public:
  ...
  /** Registers a Reader.
   *	\param pCreateF             The function that creates the Reader to be
   *registered. \param pInfoF               The function that provides
   *information about the Reader file format, such as the file extension and
   *description. \param pFirstPluginID       Contains the ID of the first
  ...
  /** Registers a Writer.
   *	\param pCreateF             The function that creates the Writer to be
   *registered. \param pInfoF               The function that provides
   *information about the Writer file format, such as the file extension,
   *description and version. \param pFirstPluginID       Contains the ID of the
   *first plug-in found. \param pRegisteredCount     Contains the number of
   *registered Writers. \param pIOSettingsFillerF   The function that fills the
   *IO settings for the Writer. \param pOverride			Override
   *any existing writer that is using the same extension.
   */
  void
  RegisterWriter(FbxWriter::CreateFuncType pCreateF,
   ...
   * \param pDesc            The file format description.
   *	\return                 The Writer ID if found, if not found, returns
   *-1.
   */
   ...
   * \param pFileFormat      The file format identifier.
   *	\return                 \c True if the file format of the Reader is FBX,
   *DXF, 3DS, OBJ and DAE, return \c false otherwise.
   */
   ...
   *              different file formats. For example, eFBX_BINARY, eFBX_ASCII
   *and eFBX_ENCRYPTED are counted as three separate file formats.
   */
  int GetWriterFormatCount() const;

  /** Returns the description of an importable file format.
   *	\param pFileFormat     The file format identifier.
   *	\return                A pointer to the character representation of the
   *description.
   */
   ...
   *	\param pFileFormat     The file format identifier.
   *	\return                A pointer to a list of user-readable strings that
   ...
   * \param pFileFormat     It equals the file format identifier if this
   * function returns \c true. If this function returns \c false, it is
   * unmodified. \return                \c True if the file has been determined
   * successfully, returns \c false otherwise. \remarks               This
   * function attempts to detect the specified file's file format based on the
   * file extension and, in some cases, its content. This function may not be
   * able to determine all file formats. Use this function as a helper before
   * calling \c SetFileFormat(). \note                  The file must be
   * unlocked (already open) for this function to succeed.
   */
