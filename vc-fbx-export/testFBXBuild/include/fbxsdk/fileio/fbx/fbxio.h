...
/**
    Defines the current FBX file version number in four digits. The first digit
   is the major version number a the last three digits are the minor version
   number (e.g. 7100 = 7.1). The following is the version history of FBX:

    \li Version 2000 - New KFCurve and embedded FBX, no FCurve/FCurve node
   storing. No more .takf file like in earlier version, no history.

    \li Version 2001 - Version incremented to support FbxTime save in native
   (integer, not double) format.

    \li Version 3000 - FiLMBOX 3.0 version, nothing changed in current class
   since version 2001. FBX SDK 3.0 and 3.6

    \li Version 3001 - FiLMBOX 3.0 encrypted version, only a trial. Encrypted
   files could only be written in debug versions. Cannot switch to a higher
   version number now because any file with a version number >= 3001 is read as
   encrypted. Hence, this value now only gives file type. (3000 or less ->
   binary, 3001 or more -> encrypted) FiLMBOX 3.2, FiLMBOX 3.5 and "Online" 3.51
   have been released with version 3000.

    \li Version 4000 - MotionBuilder 4.0, new type in KFCurve tangents,
   supported in FiLMBOX 3.5 but not by earlier versions. Version number is now
   stored in section footer. Before September 3rd 2002, the version number was
   always 3000 in main section footer. Now the main section footer has version
   number 4000. The minimum version number in footer of an extension section is
   4000.

    \li Version 4001 - ASCII Header is 4.1. MotionBuilder 4.01, to fix
   FCurveNode problem with layer types in version 4000 the main section footer
   has version number 4001. Now the footer for extension sections has version
   number 4001.

    \li Version 4050 - ASCII Header is 4.5. MotionBuilder 4.1 or 4.5 before
    January 22nd 2003. This is because EvaluationProperties now have color. Now
   the main section footer has version number 4050. Now the footer for extension
   sections has version number 4050.

    \li Version 5000 - ASCII Header is not compatible anymore with
   MotionBuilder 4.0, 4.01 and 4.02 and FBX SDK 3.6 and 3.7 MotionBuilder 4.03
   and 4.1 or 4.5 from January 22nd 2003 FBX SDK 3.6.1. New extended header to
   improve FBX file version management. Now the extended header and the main
   section footer have version number 5000. Now the footer for extension
   sections has version number 5000.

    \li Version 5800 - This was a temporary version while waiting for version
   6000 renders the previous versions incompatible with MotionBuilder 6.0. For
   now, however, this format is needed to allow some tools/plugins (For example
   Maya) to correctly detect that the file has some features that are not
    completely backward compatible (For example: pivots defined with _pre/_post
   nodes which require a special processing). By incrementing only the minor
   version we do not compromise the integrity of the files.
    ...
    A creator tag (string) is now stored in the Extended header. This contain
   the originator (MB/FBXSDK) of the file and the build number of the
   originator. First release of the file format using the KProperties to
   store/retrieve information.

    \li Version 6100 - Added support for multiple attributes (mainly multiple
   geometry) at the node level. The pointer to the node attribute have been
   replaced by a connection between the node and its attribute(s).

    \li Version 7000 -
    First version of the 7.0 series; most likely very short-lived, developed for
   Protein, before ADP. Supports reference cloning, external documents, blobs,
   unique IDs (per file), property templates. So many changes that it was
   decided to break from 6.0 to keep Motion Builder intact.
    ...
    First version of FBX SDK with FBX 7 files as the default file format.
        ...
        \li Version 7200
        Added support for multiple blend shape deformers and In-Between
   blend-shapes on each geometry. Moved shape(FbxShape) to its own section other
   than as a part of geometry section. Add support to store blend shape
   deformer(FbxBlendShape), blend shape channel(FbxBlendShapeChannel),
        Substance(FbxProceduralTexture) and Lines(FbxLine).
        Add support to store 3 different smooth binding modes of FbxSkin,
   including classic linear, dual quaternion and blended mode of previous two
   modes. Added the CLAMP_PROGRESSIVE tangent mode. The
   KFCurve::KeyAttrDataFloat data array now stores as integer values (ASCII
   mode) to to eliminate float to int precision errors. FbxLayeredTexture now
   stores alphas for its sub textures.

        \li Version 7300
        Changed the way the CharacterPoses are written.
        Changed light property name HotSpot and ConeAngle to InnerAngle and
   OuterAngle

        \li Version 7400
        Normals, tangents and binormals save the 4th component into a separate
   array

        \li Version 7500
        Added support for large files (>2GB). NOTE: This breaks forward
   compatibility (i.e. older products won't be able to open these files!!)
...
#define FBX_FILE_VERSION_6100                                                  \
  6100 // FBX 6.1 (guarantee compatibility with Autodesk 2010 products)
...
#define FBX_FILE_VERSION_7100                                                  \
  7100 // FBX 7.1 (guarantee compatibility with Autodesk 2011 products)
#define FBX_FILE_VERSION_7200                                                  \
  7200 // FBX 7.2 (guarantee compatibility with Autodesk 2012 products)
#define FBX_FILE_VERSION_7300                                                  \
  7300 // FBX 7.3 (guarantee compatibility with Autodesk 2013 products)
#define FBX_FILE_VERSION_7400                                                  \
  7400 // FBX 7.4 (guarantee compatibility with Autodesk 2014/2015 products)
#define FBX_FILE_VERSION_7500                                                  \
  7500 // FBX 7.5 (guarantee compatibility with Autodesk 2016 products)
...
/** Convert the FBX file version string to an integral number for <= or >= tests
 * purposes. \param pFileVersion File version string. Some examples: \code int
 * version; version = FileVersionStrToInt(FBX2012_00_COMPATIBLE);    // version
 * = 201200 version = FileVersionStrToInt(FBX60_COMPATIBLE);         // version
 * = 6000 version = FileVersionStrToInt("FBX200900");              // version =
 * 200900 version = FileVersionStrToInt("Toto");                   // version =
 * 0 version = FileVersionStrToInt("");                       // version = -1
 * \endcode
 * \returns the file version number or 0 if an unsupported string value is
 * passed.
 */
...
public:
  ...
  /** resolution mode. ex: "Fixed Resolution","Fixed Ratio","Fixed Width","Fixed
   * Height","Window Size"*/
 ...
 * to get or set important values like the file format version number
 * (mFileVersion). The file version number will be used to select a particular
 * Reader or Writer. \nosubgrouping */
...
public:
  ...
  /** A derived class can override this function to read additional information
   * from the file header. \return false in case of failure that should stop
   * loading the file.
   */
  ...
  /** Indicates whether the mCreationTimeStamp member variable contains the
   * actual creation time of the file. */
  ...
  /** The flag indicates that the header was created by a personal learning
   * edition (PLE) of FBX. */
  bool mPLE;
  //@}
};
 ...
 * It is primarily used by FBX importers (FbxImporter) and exporter
 *(FbxExporter) when reading or writing data from or to a disk or memory. Most
 *users will not use the FbxIO class directly but will use an instance of either
 *FbxImporter or FbxExporter to read or write FBX files.
 *
 ...
 * // Open the file with ProjectOpen( ... ) a NULL pointer can be passed for
 *(FbxReader)* param
 * // ProjectOpen_xxx_Section() to open a particular section
 * int nbSec = FieldGetCount(); // to get the number of fields of the current
 ...
 *if(fieldType == 'I') FieldReadI(...) to read a int
 *		    ...
...
{
public:
  /** \internal Exception-safe way of setting/resetting the xref manager in a
   * FbxIO object.
   */
  ...
  /**
   * \name Project Global
   * The term "Project" here is an abstract name chosen to represent a group of
   * data ex: a file, a stream, a memory buffer, etc.
   */
  //@{
  ...
  /** Write FBX signature at the top of the file, prepare file for writing
   * header information \return \c true on success, \c false otherwise.
   */
   ...
   */
  bool
  ProjectWrite_WriteExtendedHeader(const FbxIOFileHeaderInfo *pExtendedHeader);
   ...
   * \remarks If NULL is used, the old behavior (using the .fbx's folder) is
   * used instead.
   */
  ...
  /** Explicitly set the embedding extraction folder. If this is never called,
   * the FBX SDK will determine the best folder to extract embedded files.
   * \param pExtractionFolder The file path name where the embedded files should
   * be extracted.
   */
  void SetEmbeddingExtractionFolder(const char *pExtractionFolder);

  /** Retrieve the current folder destination where the embedded files will be
   * extracted. This might not be initialized until file I/O is performed.
   */
   ...
   *   \return \c true if CRC code is valid or file is not open, binary and
   * encrypted.
   */
  bool CheckCRC();
  ...
  /**
    * \name FBX 7 Format specific functions.
     ...
     not to compress anyway.  Flags are written in the file to help the FBX7
    reader to know if a decompression is required, on a per-array basis. The
    following functions address specific topics of the FBX 7 file format.
    */
  //@{
   ...
   * \remarks The allowed range for pLevel is [0-9] where 0 equals no
   * compression and 9 is as-much-as-we-can.
   */
  ...
  /** Create an extension section in a project, not allowed in ASCII and
   * encrypted modes. \param pOverwriteLastExtensionSection \return \c true on
   * success, \c false otherwise.
   */
  bool
  ProjectCreateExtensionSection(bool pOverwriteLastExtensionSection = false);
   ...
   * \remarks For main section it can be either 1000, 2000, 2001, 3000, 3001,
   * 4000, 4001 or 4050. For the extension section it can be either 4000, 4001
   * or 4050. Returns 0 if section number does not exist.
   */
  ...
  /** Set password protection flag to \c false if the argument matches the
   * password stored in the section. \param pPassword \return \c true if the
   * argument matches the password stored in the section, \c false otherwise.
   ...
   * \param pPassword
   * \return \c true on success, \c false otherwise.
   * \remarks This function only works in write mode and out of any enclosing
   * block. \remarks This function must not be called more than once per
   * section.
   */
  ...
  /**
   * \name Read related functions used to get information of a field or a group
   * of fields. Can be used to get the field content data or to navigate from
   * field to field.
   */
  //@{
  ...
  /** Start to read field instance referred by field indexed pFieldIndex,
   * instance indexed pInstance. \param pFieldIndex \param pInstance \return \c
   * true on success, \c false otherwise.
   */
  ...
  /** Start to read field instance referred field pFieldName, instance indexed
   * pInstance. \param pFieldName \param pInstance \return \c true on success,
   * \c false otherwise.
   */
   ...
   * \param pIsFileCreated Status of the extraction of the embedded data. Set to
   *\c true if the embedded media is correctly extracted in the media directory.
   * \remarks Only works when file is binary. This function is not related to
   *flag mEmbedded. \return \c false if operation failed.
   */
  ...
  /**
   * \name Write related functions used to write information of a field or a
   * group of fields. Can be used to write the field content data or to navigate
   * from field to field.
   */
  //@{
   ...
   * \param pValue
   * \remarks Only compatible with 1) MotionBuilder 4.0 and later 2) FBX
   * SDK 3.6.1 and later.
   */
  ...
  // The maximum number of value entries is, in theory, 2**32.  In practice it
  // should be a lot less than that. pSize is the number of values to write from
  // each pointer location, and stride is how much we advance to get to the next
  // value; if the stride is zero, values are tighly packed together. So in
  // total we'll write n * pSize items.
   ...
   */
  void
  FieldWriteArrayD(int n, const double *pValue, int pSize = 1, int pStride = 0);
  /** Write array to file.
   * \param n Nb of items in the array.
   * \param pValue Pointer to the data.
   * \param pSize Size of each item in the array.
   ...
   */
  void
  FieldWriteArrayF(int n, const float *pValue, int pSize = 1, int pStride = 0);
  /** Write array to file.
   * \param n Nb of items in the array.
   * \param pValue Pointer to the data.
   * \param pSize Size of each item in the array.
   ...
   */
  void
  FieldWriteArrayI(int n, const int *pValue, int pSize = 1, int pStride = 0);
  /** Write array to file.
   * \param n Nb of items in the array.
   * \param pValue Pointer to the data.
   * \param pSize Size of each item in the array.
   ...
   */
  void
  FieldWriteArrayB(int n, const bool *pValue, int pSize = 1, int pStride = 0);
  /** Write array to file.
   * \param n Nb of items in the array.
   * \param pValue Pointer to the data.
   * \param pSize Size of each item in the array.
  ...
  /** ASCII files may limit how big you can write your raw data, forcing you to
   * break it down into chunks. \return int
   */
   ...
   * \remarks  An embedded file is a file like a JPEG image used for texture or
   *an AVI file for video. When files are embedded, the size of the FBX file can
   *be very large since other files are embedded in it. FBX Version 6 and lower
   *cannot embed files when saved in ASCII. FBX Version 7 and over can embed
   *files even when saved in ASCII mode.
   */
...
private:
  // to resolve warning C4512: 'class' : assignment operator could not be
  // generated
  ...
  bool ProjectReadHeader(bool pCheckASCIIHeader,
                         bool pCheckCRC,
                         bool pOpenMainSection,
                         FbxIOFileHeaderInfo *pFileHeaderInfo);
  bool ProjectReadExtendedHeader(FbxInt64 &pExtendedHeaderEnd,
                                 FbxIOFileHeaderInfo *pFileHeaderInfo);
  ...
  bool BinaryReadExtensionCode(FbxInt64 pFollowingSectionStart,
  ...
  bool BinaryWriteExtensionCode(FbxInt64 pSectionStart,
                                FbxUInt32 pSectionVersion);
  ...
  void EncryptPasswordV1(FbxString pOriginalPassword,
                         FbxString &pEncryptedPassword);
  void DecryptPasswordV1(FbxString pEncryptedPassword,
                         FbxString &pDecryptedPassword);

  //! Read

  void CheckValidityOfFieldName(const char *pFieldName);
  void GetUnusedEmbeddedName(const FbxString &pDirectory,
