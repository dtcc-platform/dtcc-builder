...
public:
   ...
   * \return if the object is successfully added return \c true, otherwise
   *return \c false.
   */
...
private:
   ...
   * \return If the property has been modified return \c true, otherwise return
   *\c false
   */
 ...
 * The reader provide you the ability to read the global settings, objects and
 * animation information from file.
 *
 */
...
public:
   ...
   * \return if the file is open successfully return true, otherwise return
   * false
   */
   ...
   *	\param pFileName     name of the File to open
   * \return if the file is open successfully return \c true, otherwise return
   *\c false
   */
   ...
   * \return if the file is closed successfully return \c true, otherwise return
   * \c false
   */
  virtual bool FileClose();

  /** Check whether the file stream is open.
   *	\return if the file stream is open return \c true, otherwise return \c
   *false.
   */
   ...
   * \return if either pAxisSystem or pSystemUnits is \c NULL return \c false,
   ...
   *before loading the whole scene from the file.
   */
  virtual bool GetFrameRate(FbxTime::EMode &pTimeMode);

  /** Get the statistics from file
   *	\param pStats statistics in file
   *	\return if fetching statistics is successfully return \c true, otherwise
   *return \c false.
   */
   ...
   *	\return if fetching statistics is successful return \c true, otherwise
   *return \c false.
   */
   ...
   *	\return if reading the file is successful return \c true, otherwise
   *return \c false.
   */
...
private:
   ...
   */
  FbxObject *
  CreateGenericObject(FbxDocument *pDocument,
                      char *pObjectType,
                      char *pObjectSubType,
                      char *pObjectName,
                      FbxObject::EObjectFlag pFlags = FbxObject::eSavable);
   ...
   *	\return if reading description section is successful return \c true,
   ...
   *	\return if reading reference section is successful return \c true,
   *otherwise return \c false.
   */
  bool ReadReferenceSection(FbxDocument *pDocument,
   ...
   *	\return if reading definition section is successful return \c true,
   *otherwise return \c false.
   */
  bool ReadDefinitionSection(FbxDocument *pDocument,
   ...
   * \param pObjectContent   Object type info array to fill
   * \param pDocReferences   External references to fill
   *	\return if reading object section is successful return \c true,
   *otherwise return \c false.
   */
  bool ReadObjectSection(FbxDocument *pDocument,
                         FbxArray<Fbx6ObjectTypeInfo *> &pObjectContent,
                         Fbx6TypeReadReferences &pDocReferences);
   ...
   *	\return if reading object is successful return \c true, otherwise return
   *\c false.
   */
  bool ReadObject(FbxDocument *pDocument,
   ...
   *	\param pDocument        Document to read
   *	\return if reading connection section is successful return \c true,
   *otherwise return \c false.
   */
   ...
   *	\param pDocument        Document to read
   *	\return if reading animation is successful return \c true, otherwise
   *return \c false.
   */
   ...
   */
  void ReadObjectAnimation(FbxIO &pFileObject,
                           FbxObject *pNode,
   ...
   * \param pAnimStack       Animation stack to read from
   */
  void ReadPropertyAnimation(FbxIO &pFileObject,
                             FbxProperty *pProp,
                             FbxAnimStack &pAnimStack);
   ...
   * \remarks This function will allocate the corresponding FbxAnimStack with
   ...
   * \param pTakeInfo        Take info to read from
   */
  bool ReadNodeAnimation(FbxIO &pFileObject,
                         FbxScene &pScene,
                         FbxAnimStack &pAnimStack,
   ...
   */
  void ReadTimeWarps(FbxIO &pFileObject,
                     FbxMultiMap &pTimeWarpSet,
                     FbxScene &pScene);
   ...
   * \param pTimeOffsetType  A value from the FbxTakeInfo::EImportOffsetType
   *enumeration that indicates the time shift offset type \param pTimeOffset The
   *time shift offset \return if time shifting is successful return \c true,
   *otherwise return \c false.
   */
  bool TimeShiftNodeAnimation(FbxScene &pScene,
                              FbxAnimStack &pAnimStack,
                              int pTimeOffsetType,
                              FbxTime pTimeOffset);
  //@}
   ...
   */
  void
  ReadCharacter(FbxCharacter &pCharacter, int &pInputType, int &pInputIndex);
   ...
   * \return if reading character pose is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading pose is successful return \c true, otherwise return \c
   *false.
   */
   ...
   * \param pEmbeddedMediaDirectory  the directory path storing the embedded
   *media \return if reading media is successful return \c true, otherwise
   *return \c false.
   */
  bool ReadMedia(FbxDocument *pDocument,
                 const char *pEmbeddedMediaDirectory = "");

  /** Read global settings from file
   *	\param pGlobalSettings          global settings object to fill
   * \return if reading global settings is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \param pDocReferences   External reference
   * \return if reading node data is successful return \c true, otherwise return
   *\c false.
   */
   ...
   * \return if reading data to container is successful return \c true,
   *otherwise return \c false.
   */
   ...
   * \return if reading data to generic node is successful return \c true,
   *otherwise return \c false.
   */
   ...
   * \return if reading shading information to node is successful return \c
   ...
   * \return if reading culling type to node is successful return \c true,
   *otherwise return \c false.
   */
  bool ReadNodeCullingType(
      FbxNode &pNode); // TBV, probablement passe tout en property

  /** Read target transform for node
   *	\param pNode                fbx node
   * \return if reading target transform to node is successful return \c true,
   *otherwise return \c false.
   */
   ...
   * \return if reading node attribute is successful return \c true, otherwise
   *return \c false.
   */
  bool ReadNodeAttribute(FbxNode &pNode,
                         FbxString &pObjectSubType,
                         bool &pCreatedAttribute,
                         Fbx6TypeReadReferences &pDocReferences);
   ...
   * \return A pointer of node attribute. Return \c NULL if the attribute does
   * not exist
   */
  FbxNodeAttribute *ReadNodeAttribute(FbxString &pObjectSubType,
                                      FbxString &pObjectName,
                                      FbxString &pObjectUniqueId,
                                      FbxObject *pReferencedObject);
   ...
   * \return if reading node properties is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading layered texture is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading geometry links is successful return \c true, otherwise
   * return \c false.
   */
   ...
   *	\param pGeometry                        FBX geometry
   * \return if reading geometry shapes is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading null node is successful return \c true, otherwise return
   *\c false.
   */
   ...
   * \return if reading marker node is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading camera node is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading stereo camera node is successful return \c true,
   *otherwise return \c false.
   */
   ...
   *	\param pCameraStereo                            Stereo camera node
   * \return if reading precomp file is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading light node is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading binding table is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading binding operator is successful return \c true, otherwise
   *return \c false.
   */
  bool ReadBindingOperator(FbxBindingOperator &pOperator);

  /** Read vertices, polygon indices, edges, layer elements, geometry links and
   *shapes to mesh object \param pMesh                            fbx mesh
   * \return if reading mesh object is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading mesh smoothness is successful return \c true, otherwise
   * return \c false.
   */
   ...
   *	\param pMesh                            fbx mesh
   * \return if reading mesh vertices is successful return \c true, otherwise
   *return \c false.
   */
   ...
   *	\param pMesh                            fbx mesh
   * \return if reading polygon indices is successful return \c true, otherwise
   *return \c false.
   */
   ...
   *	\param pMesh                            fbx mesh
   * \return if reading mesh edges is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading document information is successful return \c true,
   *otherwise return \c false.
   */
   ...
   * \return if reading fbx collection is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading fbx selection set is successful return \c true,
   *otherwise return \c false.
   */
  ...
  /** Read nurb data including surface types, nurb type, display type, steps,
   *control points and UV. \param pNurbs                            Nurb object
   * \return if reading nurb data is successful return \c true, otherwise return
   *\c false.
   */
  bool ReadNurb(FbxNurbs &pNurbs);

  /** Read nurb surface data including surface types, surface type, display
   *type, steps, control points and UV vectors. \param pNurbs Nurb surface
   *object \return if reading nurb surface data is successful return \c true,
   *otherwise return \c false.
   */
  bool ReadNurbsSurface(FbxNurbsSurface &pNurbs);

  /** Read patch data including patch type, dimension, display type, steps, UV
   *cap and control points. \param pPatch                           Patch object
   * \return if reading patch data is successful return \c true, otherwise
   *return \c false.
   */
  ...
  /** Read nurb curve data including types, dimension, rational-ness, control
   *points and knots. \param pNurbsCurve                      Nurb curve \return
   *if reading nurb curve data is successful return \c true, otherwise return \c
   *false.
   */
  bool ReadNurbsCurve(FbxNurbsCurve &pNurbsCurve);

  /** Read trim Nurb surface objects with properties
   *	\param pNurbs                            Trim nurb surface * \return if
   *reading trim nurb surface is successful return \c true, otherwise return \c
   *false.
   */
   ...
   * \return if reading fbx boundary is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading shape object properties is successful return \c true,
   *otherwise return \c false.
   */
   ...
   * \return if reading fbx implementation is successful return \c true,
   *otherwise return \c false.
   */
   ...
   * \return if reading texture object is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \param pReferencedMaterial  Reference material to clone from if it is not
   *\c NULL \return A pointer of read surface material
   */
  FbxSurfaceMaterial *
  ReadSurfaceMaterial(const char *pName,
                      const char *pMaterialType,
                      FbxSurfaceMaterial *pReferencedMaterial);

  /** Read video object from file
   *	\param pVideo               Fbx video object
   * \return if reading video object is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading thumbnail is successful return \c true, otherwise return
   *\c false.
   */
   ...
   * \return if reading all layer elements is successful return \c true,
   *otherwise return \c false.
   */
   ...
   * \return if reading material layer elements is successful return \c true,
   *otherwise return \c false.
   */
  bool
  ReadLayerElementsMaterial(FbxGeometry *pGeometry,
                            FbxArray<FbxLayerElement *> &pElementsMaterial);

  /** Read normal layer elements for geometry
   *	\param pGeometry                geometry to fill
   * \param pElementsNormal          normal layer element array
   * \return if reading normal layer elements is successful return \c true,
   *otherwise return \c false.
   */
  bool ReadLayerElementsNormal(FbxGeometry *pGeometry,
                               FbxArray<FbxLayerElement *> &pElementsNormal);

  /** Read Tangent layer elements for geometry
   *	\param pGeometry                geometry to fill
   * \param pElementsTangent          Tangent layer element array
   * \return if reading Tangent layer elements is successful return \c true,
   *otherwise return \c false.
   */
  bool ReadLayerElementsTangent(FbxGeometry *pGeometry,
                                FbxArray<FbxLayerElement *> &pElementsTangent);

  /** Read Binormal layer elements for geometry
   *	\param pGeometry                geometry to fill
   * \param pElementsBinormal          Binormal layer element array
   * \return if reading Binormal layer elements is successful return \c true,
   *otherwise return \c false.
   */
  bool
  ReadLayerElementsBinormal(FbxGeometry *pGeometry,
                            FbxArray<FbxLayerElement *> &pElementsBinormal);

  /** Read vertex color layer elements for geometry
   *	\param pGeometry                geometry to fill
   * \param pElementsVertexColor     vertex color layer element array
   * \return if reading vertex color layer elements is successful return \c
   *true, otherwise return \c false.
   */
  bool ReadLayerElementsVertexColor(
      FbxGeometry *pGeometry,
      FbxArray<FbxLayerElement *> &pElementsVertexColor);
   ...
   * \return if reading texture layer elements is successful return \c true,
   *otherwise return \c false.
   */
  bool ReadLayerElementsTexture(FbxGeometry *pGeometry,
                                FbxArray<FbxLayerElement *> &pElementsTexture,
   ...
   * \param pTextureType             the type of elements to read
   * \return if reading UV layer elements is successful return \c true,
   *otherwise return \c false.
   */
  bool ReadLayerElementsChannelUV(FbxGeometry *pGeometry,
                                  FbxArray<FbxLayerElement *> &pElementsUV,
                                  FbxLayerElement::EType pTextureType);

  /** Read polygon group layer elements for geometry
   *	\param pGeometry                geometry to fill
   * \param pElementsPolygonGroup    polygon group layer element array
   * \return if reading polygon group layer elements is successful return \c
   *true, otherwise return \c false.
   */
  bool ReadLayerElementsPolygonGroup(
      FbxGeometry *pGeometry,
      FbxArray<FbxLayerElement *> &pElementsPolygonGroup);

  /** Read smoothing layer elements for geometry
   *	\param pGeometry                geometry to fill
   * \param pElementsSmoothing       Smoothing group layer element array
   * \return if reading smoothing layer elements is successful return \c true,
   *otherwise return \c false.
   */
  bool
  ReadLayerElementsSmoothing(FbxGeometry *pGeometry,
                             FbxArray<FbxLayerElement *> &pElementsSmoothing);

  /** Read user data layer elements for geometry
   *	\param pGeometry                geometry to fill
   * \param pElementsUserData        User data layer element array
   * \return if reading user data layer elements is successful return \c true,
   *otherwise return \c false.
   */
  bool
  ReadLayerElementsUserData(FbxGeometry *pGeometry,
                            FbxArray<FbxLayerElement *> &pElementsUserData);

  /** Read visibility layer elements for geometry
   *	\param pGeometry                geometry to fill
   * \param pElementsVisibility      visibility layer element array
   * \return if reading visibility layer elements is successful return \c true,
   *otherwise return \c false.
   */
  bool
  ReadLayerElementsVisibility(FbxGeometry *pGeometry,
                              FbxArray<FbxLayerElement *> &pElementsVisibility);

  /** Read edge crease layer elements for geometry
   *	\param pGeometry                geometry to fill
   * \param pElementsEdgeCrease      edge crease layer element array
   * \return if reading edge crease layer elements is successful return \c true,
   *otherwise return \c false.
   */
  bool
  ReadLayerElementEdgeCrease(FbxGeometry *pGeometry,
                             FbxArray<FbxLayerElement *> &pElementsEdgeCrease);

  /** Read vertex crease layer elements for geometry
   * \param pGeometry                geometry to fill
   * \param pElementsVertexCrease    vertex crease layer element array
   * \return if reading vertex crease layer elements is successful return \c
   * true, otherwise return \c false.
   */
  bool ReadLayerElementVertexCrease(
      FbxGeometry *pGeometry,
      FbxArray<FbxLayerElement *> &pElementsVertexCrease);

  /** Read hole layer elements for geometry
   * \param pGeometry                geometry to fill
   * \param pElementsHole            hole layer element array
   * \return if reading hole layer elements is successful return \c true,
   * otherwise return \c false.
   */
  bool ReadLayerElementHole(FbxGeometry *pGeometry,
                            FbxArray<FbxLayerElement *> &pElementsHole);
  //@}
   ...
   * \return if reading geometry weight map is successful return \c true,
   *otherwise return \c false.
   */
   ...
   * \return if reading link object is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading skin object is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading vertex cache deformer is successful return \c true,
   *otherwise return \c false.
   */
   ...
   * \return if reading cluster object is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return if reading constraint object is successful return \c true,
   *otherwise return \c false.
   */
   ...
   * \return if reading cache data is successful return \c true, otherwise
   *return \c false.
   */
   ...
   * \return return the index of the array if the string is found, otherwise
   *return \c -1
   */
   ...
   * \param pReadNodeAttributeProperties whether to read properties for node
   *attributes
   */
  bool ReadProperties(FbxObject *pFbxObject,
                      FbxIO *pFbxFileObject,
                      bool pReadNodeAttributeProperties = true);

  /** Read properties and flags for fbx object from file object
   *	\param pFbxObject                   fbx object
   * \param pFbxFileObject               fbx file object
   * \param pReadNodeAttributeProperties whether to read properties for node
   *attributes \return if reading properties and flags is successful return \c
   *true, otherwise return \c false.
   */
  bool ReadPropertiesAndFlags(FbxObject *pFbxObject,
                              FbxIO *pFbxFileObject,
                              bool pReadNodeAttributeProperties = true);
   ...
   * \return if reading flags is successful return \c true, otherwise return \c
   *false.
   */
  ...
  /** Convert shape deform property to DeformPercent property of
   *FbxBlendShapeChannel \param pScene           Fbx scene
   */
  ...
  bool
  WriteOptionsInExtensionSection(bool pOverwriteLastExtensionSection = false);
  //--------------- end in progress ----------------------------
