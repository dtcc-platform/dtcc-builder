...
public:
   ...
   * \param pAxisSystem              The coordinate system whose Up Axis is
   * copied.
   */
  void SetOriginalUpAxis(const FbxAxisSystem &pAxisSystem);

  /** Returns the coordinate system's original Up Axis.
   * \return                         The coordinate system's original Up Axis
   * when the scene is created. 0 is X, 1 is Y, 2 is Z axis.
   */
   ...
   * \return                         The original unit of measurement used by
   * the system.
   */
   ...
   * \remarks                        The ambient color only uses the RGB
   * channels.
   */
   ...
   * \return                         \c true if camera name is valid, returns \c
   * false if the camera does not have a valid name. \remarks A valid camera
   * name can be either one of the defined tokens (FBXSDK_CAMERA_PERSPECTIVE,
   *                                 FBXSDK_CAMERA_TOP, FBXSDK_CAMERA_FRONT,
   * FBXSDK_CAMERA_BACK, FBXSDK_CAMERA_RIGHT, FBXSDK_CAMERA_LEFT and
   * FBXSDK_CAMERA_BOTTOM) or the name of a camera inserted in the node tree
   * under the scene's root node.
   */
  bool SetDefaultCamera(const char *pCameraName);

  /** Returns the default camera name.
   * \return                         The default camera name, or an empty string
   * if no camera name has been set.
   */
  ...
  //@{
  /** Sets the time mode.
   * \param pTimeMode                  One of the defined modes in class
   * FbxTime.
   */
   ...
   * \return                     The currently set time protocol (default
   * FbxTime::eFrameCount).
   */
   ...
   * \param pSnapOnFrameMode     One of the following values: eNoSnap,
   * eSnapOnFrame, ePlayOnFrame, or eSnapAndPlayOnFrame.
   */
  void SetSnapOnFrameMode(ESnapOnFrameMode pSnapOnFrameMode);

  /** Returns the snap on frame mode.
   * \return                     The currently set snap on frame mode (default
   * eNoSnap).
   */
   ...
   * \return                     A copy of the time marker at the given index,
   * or an empty one if an error occurred.
   */
   ...
   * \param pStatus              The FbxStatus object to hold error codes.
   * \return                     \c true if successful, or returns \c false if
   * the index is not valid.
   */
  bool SetCurrentTimeMarker(int pIndex, FbxStatus *pStatus = NULL);

  /** Returns the current time marker index.
   * \return                     The current time marker index, or -1 if no
   * current time marker has been set.
   */
...
private:
  ...
  void AddSetTimeMarker(int pIndex,
                        const TimeMarker &pTimeMarker,
                        FbxStatus *pStatus,
                        bool pAdd);
