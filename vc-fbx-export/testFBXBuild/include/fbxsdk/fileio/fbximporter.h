 ...
 * the importer will ask for all registered readers if an FBX 7 file reader is
 * available, then if a reader is found, the importer will create the
 * specialized FBX 7 reader and read the file. This way, an importer can "read"
 * many different type of files like FBX 5/6/7, 3DS, Obj, Dxf, Collada, etc.
 * \see FbxReader
 *
 ...
 * -# set numerous states, take information, defining how the importer will
 * behave
 ...
 * \endcode
 *
 * \remarks According to the file suffix, a specialized reader will be created
 * internally. Ex: for .fbx files a FBX Reader, for .3ds files, a 3ds reader,
 * etc. Supported files formats: FBX 5/6/7 Binary & ASCII, Collada, DXF, OBJ,
 * 3DS \nosubgrouping
 */
...
* \param pFileFormat        file format identifier User does not need to specify
it by default. if not specified, plugin will detect the file format according to
file suffix automatically.
    * \param pIOSettings        client IOSettings, if not specified, a default
IOSettings will be created
    * \return                   \c true on success, \c false otherwise.
* \remarks                  To identify the error that occurred, inspect the
status object accessed
*                           using the GetStatus() function.
* \remarks                  You do not need to give the pFileFormat if the
suffix of pFileName is recognized
    */
...
* \param pFileFormat        file format identifier User does not need to specify
it by default. if not specified, plugin will request the file format from the
stream automatically.
    * \param pIOSettings        client IOSettings, if not specified, a default
IOSettings will be created *	\return                   \c true on success, \c
false otherwise.
* \remarks                  To identify the error that occurred, inspect the
status object accessed
*                           using the GetStatus() function.
...
*/
   ...
   * \return                    \c true if the default rendering settings are
   * defined in the file, otherwise returns \c false with empty parameters.
   */
  ...
  /** Read the currently opened file header to retrieve information related to
   *takes. \param pStreamOptionsGeneration     Stream options identifier.
   *	\return                \c true on success, \c false otherwise.
   * \remarks                            Caller gets ownership of the returned
   *structure.
   */
  bool GetImportOptions(
      EStreamOptionsGeneration pStreamOptionsGeneration = eParseFile);

  /** Read the currently opened file header to retrieve information related to
   *takes. \param pFbxObject     Target FBX file. \return                \c true
   *on success, \c false otherwise. \remarks              Caller gets ownership
  ...
  /** Import the currently opened file into a scene.
    * \param pDocument          Document to fill with file content.
        * \param pNonBlocking       If true, the import process will be executed
    in a new thread, allowing it to be non-blocking. To determine if the import
    finished, refer to the function IsImporting(). *	\return \c true on
    success, \c false otherwise.
    * \remarks                  To identify the error that occurred, inspect the
    status object accessed
    *                           using the GetStatus() function.
        *                           If the imported file is password protected
    and the password is not
        *                           set or wrong, the FbxStatus object access
    with GetStatus() will be set with
   ...
   * \param pImportResult  This parameter, after the import finished, will
   * contain the result of the import success or failure. \return Return true if
   * the importer is currently importing. \remarks              This function
   * will always return false if Import() was called with pNonBlocking set to
   * false. This function should be used only in the context of pNonBlocking set
   * to true. It is very important to periodically check if the import finished
   * using this function, since it will also free up the thread's allocations
   * when its done.
   */
   ...
   * \param pExtractFolder The file path name where the embedded files should be
   * extracted.
   */
   ...
   * \return The pointer to IOSettings or \c NULL \c if the object has not been
   * allocated.
   */
   ...
   * Use the returned reference to a FbxTakeInfo object to set whether the
   *indexed take is imported. \param pIndex     Index of the requested take.
   ...
   * \return     Active animation stack name if there is one, otherwise returns
   * an empty string. \remarks    This function must be called after
   * FbxImporter::Initialize().
   */
  ...
  /** Returns the index of the reader (FbxReader) associated with the file
     format. This index is considered the identifier of the file format. The
     array of registered readers can't be retrieved. \return Index of the
     registered FbxReader associated with the file format. If no reader found
     return -1. \remarks According to the number of readers registered this
     value can change for the same reader between SDK Manager instantiations.
  */
  int GetFileFormat();
...
protected:
  ...
  virtual void SetOrCreateIOSettings(FbxIOSettings *pIOSettings,
                                     bool pAllowNULL);
