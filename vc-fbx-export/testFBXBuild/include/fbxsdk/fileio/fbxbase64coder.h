...
public:
   ...
   * \param pInSize the size of the input data in bytes (must be a multiple of
   * 4) \param pOutBuffer the destination buffer. \param pOutSize the capacity
   * of the output buffer in bytes. \return the number of bytes put in the
   * output buffer, or -1 if the output buffer is too small, or contains invalid
   * characters */
  ...
  /** Decodes the input buffer.
   * \param pInBuffer the input buffer containing Base64 data; its length is
   * computed using strlen(). \param pOutBuffer the destination buffer. \param
...
public:
   ...
   * \param pOutSize the capacity of the output buffer in bytes, which should be
   * at least 33% larger than the input buffer size, or 4 bytes whichever is
   * more.
   * \return the number of bytes put in the output buffer, or -1 if we ran out
   * of room. */
