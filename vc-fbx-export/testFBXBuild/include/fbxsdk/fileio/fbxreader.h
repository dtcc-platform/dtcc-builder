...
public:
  ...
  enum EInfoRequest
  {
    ...
    eInfoDescriptions, //!< To get the file description for a reader ex:
                       //!< "Autodesk FBX (*.fbx)".
    eReserved1 = 0xFBFB,
  };

  /** Flags for reading parts of file.
   * \remarks Used internally when an importer is initialized to get some
   * information very fast.
   */
  enum EFileOpenSpecialFlags
  {
    eParseForGlobalSettings = 1, //!< Used for reading the Global settings
                                 //!< section when an importer is initialized.
    eParseForStatistics = 2 //!< Used for reading a group of statistics when an
                            //!< importer is initialized.
  };

  /** \internal Helper typedef for passing FbxReader creator function as
   * argument (used internally) */
  typedef FbxReader *(*CreateFuncType)(FbxManager &pManager,
                                       FbxImporter &pImporter,
                                       int pSubID,
                                       int pPluginID);

  /** \internal Helper typedef for passing FbxIOSettings creator function as
   * argument (used internally) */
  typedef void (*IOSettingsFillerFuncType)(FbxIOSettings &pIOS);

  /** \internal Helper typedef for passing EInfoRequest function as argument
   * (used internally) */
   ...
   *	\param pParseFileAsNeeded       Sets whether to parse file as read
   *options \return                         true on success, otherwise return
   *false.
   */
  ...
  /** Reads extension plug-ins name, version and parameters, so that we can
  ...
  /** If default camera resolution is OK, returns information about the
   *resolution of the render. \param pCamName			Default camera
   *name. \param pResolutionMode	Default resolution mode. \param pW
   ...
   * An internal (genuine) plug-in is one created by the Autodesk FBX product
   *team. \return \c true If the file format is internal plug-in , \c false
   *Otherwise.
   */
  bool IsGenuine();
  ...
  /** Returns true if this reader supports FbxStream I/O. Default value is
   * false. */
