 ...
 * The source string is made up with many groups and each group contains
 * pSourceGroupSize units separated by spaces; The destination array is also
 * made up with many groups and each unit contains pDestGroupSize units. The
 * valid unit range in each source group is [pSourceUnitOffset,
 * pSourceUnitOffset + pSourceValidUnitCount). The valid unit range in each
 * destination unit is [pDestUnitOffset, pDestUnitOffset + pDestValidUnitCount).
                      ...
                      TYPE pDefaultValue = TYPE())
{
  ...
  FBX_ASSERT(pSourceValidUnitCount >= 0 &&
             pSourceUnitOffset + pSourceValidUnitCount <= pSourceGroupSize);
  FBX_ASSERT(pDestUnitOffset >= 0 && pDestUnitOffset < pDestGroupSize);
  FBX_ASSERT(pDestValidUnitCount >= 0 &&
             pDestUnitOffset + pDestValidUnitCount <= pDestGroupSize);
  ...
  const int lDestUnitGap =
      pDestGroupSize - pDestValidUnitCount - pDestUnitOffset;
  while (lSource && *lSource)
  {
    TYPE lData;
    const char *lSourceStart = lSource;
    if (FromString(&lData, lSource, &lSource) &&
        lSourceCounter >= pSourceUnitOffset &&
        lSourceCounter < lSourceUnitValidEnd)
    {
    ...
    {
      // we met a stop condition of FromString. In the normal case, lSource
      // should now be "" or ' '. If not, the converted string is corrupted and
      // we have to break the loop. We can detect this by checking if lSource
      // pointer has moved.
...
template <typename T> const FbxString ToString(const T &pValue)
{
  return FbxString(pValue);
}
template <> const FbxString ToString(const FbxVector4 &pValue);
template <> const FbxString ToString(const FbxAMatrix &pValue);

//----------------------------------------------------------------------------//

/** Decode percent encoded characters, returns an empty string if there's an
 * error. For example, a string like "abc%20abc" is converted into "abc abc".
