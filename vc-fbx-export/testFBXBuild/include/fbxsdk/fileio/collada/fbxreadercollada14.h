 ...
 *
 * Element: An XML document consists primarily of elements. An element is a
 * block of information that is bounded by tags at the beginning and end of the
 * block. Elements can be nested, producing a hierarchical data set.
 *
 * Tag: Each XML element begins with a start tag and ends with an end tag.
 *
 * Attribute: An XML element can have zero or more attributes. Attributes are
 * given within the start tag and follow the tag name. Each attribute is a
 * name-value pair. The value portion of an attribute is always surrounded by
 * quotation marks (" "). Attributes provide semantic information about the
 * element on which they are bound. For example: \code <tagName
 * attribute="value"> \endcode
 *
 * URI Addressing: Refers to the id attribute of an element. Used in url, source
 * or target attributes. In a url, source or target attribute, the URI fragment
 * identifier is preceded with the pound sign ("#").
 *
 * Markup and Content: The characters which make up an XML document are divided
 * into markup and content. Markup and content may be distinguished by the
 * application of simple syntactic rules. All strings which constitute markup
 * either begin with the character "<" and end with a ">", or begin with the
 * character "&" and end with a ";". Strings of characters which are not markup
 * are content. \nosubgrouping
 */
...
public:
   ...
   * \param pParseFileAsNeeded whether parse file as needed, the default value
   * is true. \return true
   */
  ...
  /** Read from Collada file and import it to the FBX document, according to the
   * given options settings. \param pDocument FBX Document to import. \return
...
private:
   ...
   * \param pXmlNode The XML Node to import, it should be the root of Collada
   * nodes tree. \return true on success, false otherwise.
   */
   ...
   * \return Return \c true on success and \c false otherwise.
   */
  bool ImportVisualSceneMax3DExtension(xmlNode *pTechniqueElement,
                                       FbxScene *pScene);
   ...
   * \return Return \c true on success and \c false otherwise.
   */
  bool ImportVisualSceneFCOLLADAExtension(xmlNode *pTechniqueElement,
                                          FbxScene *pScene);
   ...
   * \return Return \c true on success and \c false otherwise.
   */
  bool ImportVisualSceneMayaExtension(xmlNode *pTechniqueElement,
                                      FbxScene *pScene);
   ...
   * Axis System, system Unit, author, comments, title, subject, keywords,
   * revision, etc.
   */
  bool ImportAsset(xmlNode *pXmlNode,
                   FbxGlobalSettings &pGlobalSettings,
                   FbxDocumentInfo &pSceneInfo);

  /** Import a COLLADA node element.
   * If the COLLADA node element contains only one node attribute (node
   * attribute means instance_geometry, instance_camera, instance_light and
   * instance_controller), node attribute will be attached directly under the
   * node; If not, one child node will be created for each node attribute except
   * the first one. \param pXmlNode The COLLADA node element. \return The
   * created node and return \c NULL if failed.
   */
   ...
   * \param pMaterialSequence A ordered sequence of material symbols connecting
   * to the geometry. \return The created geometry object and return \c NULL if
   * failed. \remarks Except mesh, other types of geometry are not supported
   * now.
   */
  FbxGeometry *
  ImportGeometry(const FbxString &pGeometryID,
                 const FbxDynamicArray<FbxString> &pMaterialSequence);
   ...
   * \param pMaterialSequence A ordered sequence of material symbols connecting
   * to the target geometry. \return Return the pointer to the target geometry.
   */
  FbxGeometry *ImportMorph(xmlNode *pMorphElement,
                           const FbxDynamicArray<FbxString> &pMaterialSequence);
   ...
   */
  FbxGeometry *
  ImportController(const FbxString &pControllerID,
                   const FbxDynamicArray<FbxString> &pMaterialSequence);
   ...
   * \remarks Camera parameters will also be imported, such as FOV, aspect
   * ratio, etc.
   */
   ...
   * Following types of light are supported now: ambient light, directional
   * light, point light, and spot light.
   */
   ...
   * \remarks Following types of texture are supported now: ambient, diffuse,
   * emission, reflective, specular, transparent.
   */
   ...
   * If this mesh element contains polygons, polygon list or triangles elements,
   * a FBX mesh will be created. If it contains lines or line strips elements, a
   * FBX line will be created. \param pXmlNode The COLLADA mesh element. \param
   * pMaterialSequence A ordered sequence of material symbols connecting to the
   * geometry. \param pObjects List of all the created objects. If this mesh
   * contains lines or line strips as well as the polymesh, the array will be
   * filled with: [mesh, line, line strip] \return The created geometry object
   * and return \c NULL if failed.
   */
  FbxGeometry *ImportMesh(xmlNode *pXmlNode,
                          const FbxDynamicArray<FbxString> &pMaterialSequence,
                          FbxArray<FbxObject *> &pObjects);
   ...
   * \param pMaterialSequence A ordered sequence of material symbols connecting
   * to the mesh. \return true on success, false otherwise. \remarks Vertex
   * colors, normals, UVs, textures and materials which related to polygons, are
   * also imported.
   */
  bool ImportPolygons(xmlNode *pXmlNode,
                      FbxMesh &pMesh,
                      const FbxDynamicArray<FbxString> &pMaterialSequence);
   ...
   * \return true on success, false otherwise.
   * \remarks Transformation will cover matrix, translation, rotation, scale,
  ...
  /** Import Collada look at node, and computed camera position, interest, up
   * vector, etc. \param pXmlNode Pointer to XML look at Node. \param
   * lCameraPosition Vector4 value to return camera position. \param
   * lInterestPosition Vector4 value to return camera interest position. \param
   * lUpVector Vector4 value to return camera up vector. \param
   * lCameraTransformMatrix XMatrix value to return camera transform matrix.
   * \return true on success, false otherwise.
   * \remarks Computed camera parameters are saved in lCameraPosition,
   * lInterestPosition, lUpVector, lCameraTransformMatrix.
   */
  bool ImportLookAt(xmlNode *pXmlNode,
   ...
   * \param pVersionString The string representing the COLLADA version, like
   * "1.4.1". \return \c true if the Collada version is 1.4.* or \c false
   * otherwise.
   */
  ...
  /** Build up the cache map from COLLADA element id to their data, which
   * contain COLLADA elements and FBX objects.
   */
  void BuildUpLibraryMap();

  /** Recursively build up the cache map from COLLADA element id to their data,
   * which contain COLLADA elements and FBX objects. /param pElement The parent
   * element which may contain target elements. /param pElementTag The tag of
   * target elements.
   */
  void BuildUpLibraryMap(xmlNode *pElement, const FbxString &pElementTag);

  /** Retrieve the material map from the bind_material element under an
   * instance_geometry or instance_controller. And connected the created
   * materials to the node. \param pNode The node to which the materials
   * connect. \param pElement The COLLADA instance_goemetry or
   * instance_controller element. \param pMaterialSequence The ordered material
   * symbols which is used to create geometries. \return Return \c true on
   * success and \c false otherwise.
   */
  bool ConnectMaterialsToNode(FbxNode *pNode,
                              xmlNode *pElement,
                              FbxDynamicArray<FbxString> &pMaterialSequence);

  /** Import COLLADA transparent and transparency attributes in a COLLADA effect
   * element. \param pElement A COLLADA Lambert, Phong, constant or Blinn
   * element. \param pSurfaceMaterial A FBX surface material. \return Return \c
   * true on success and \c false otherwise.
   */
  bool ImportTransparent(xmlNode *pElement,
   ...
   */
  FbxObject *GetLibrary(const LibraryTypeTraits &pTypeTraits,
                        const FbxString &pID);
  FbxObject *GetLibrary(const LibraryTypeTraits &pTypeTraits,
                        xmlNode *pElement);
   ...
   * \return Return \c true on success and \c false otherwise.
   */
  bool ImportMatrixAnimation(FbxNode *pNode,
   ...
   * \return Return \c true on success and \c false otherwise.
   */
  bool ImportPropertyAnimation(FbxProperty &pProperty,
                               const FbxString &pAnimationChannelID,
   ...
   */
  void ImportPropertyValue(FbxObject *pObject,
  ...
  struct AnimationClipData
  {
    ...
    FbxSet<FbxString>
        mAnimationElementIDs; // IDs of animation belong to this animation clip
