...
// Some information connecting COLLADA layer string, such as "NORMAL" or "UV",
// to FBX layer element type.
...
xmlNode *DAE_AddParameter(xmlNode *parentXmlNode,
                          ...
                          const FbxVector4 &vector);
xmlNode *
DAE_AddParameter(xmlNode *parentXmlNode, const char *name, double value);
 ...
 * \param pDefaultAttributeValue The default value of the attribute used when
 * the attribute is not found explicitly. \return Return NULL if no child
 * element has the given attribute value.
 */
xmlNode *
DAE_FindChildElementByAttribute(xmlNode *pParentElement,
 ...
 * \param pFindFrom Find from the next child after pFindFrom if pFindFrom is not
 * NULL. \return Return NULL if no child element has the given tag.
 */
 ...
 * \return The value of the attribute in the form of a string. If the attribute
 ...
 * \param pAttributeName The name of the specific attribute.
 * \param pData The returned data.
 * \return Return \c true on success and \c false if no attribute has the given
 * name.
 */
template <typename TYPE>
                                  ...
                                  TYPE &pData)
{
  ...
  FbxAutoFreePtr<xmlChar> lPropertyValue(
      xmlGetProp(pElement, (const xmlChar *)pAttributeName));
  if (lPropertyValue)
  {
...
/** Compare the value of specific attribute of specific element with given
 * value. \param pElement The specific element. \param pAttributeName The name
 * of the specific attribute. \param pValue The value to compare with. \return
 ...
 * \return The ID of another element if success, or an empty string if no url
 * attributes are found.
 */
const FbxString DAE_GetIDFromUrlAttribute(xmlNode *pElement,
                                          FbxString &pExternalRef);

/** Get the ID of another element from the source attribute of the given
 * element. \param pElement The specific XML element in which the ID is looked
 * for. \return The ID of another element if success, or an empty string if no
 * source attributes are found.
 */
const FbxString DAE_GetIDFromSourceAttribute(xmlNode *pElement);

/** Get the ID of another element from the target attribute of the given
 * element. Note that in target attribute, the URI identifier may or may not
 * preceded with the pound sign. \param pElement The specific XML element in
 * which the ID is looked for. \return The ID of another element if success, or
 * an empty string if no target attributes are found.
 */
const FbxString DAE_GetIDFromTargetAttribute(xmlNode *pElement);

/** Set the name of the object with a given name. If the name is empty, use the
 * ID. \param pObject The object whose name is to be set. \param pName The name
 * string. \param pID The ID string.
 */
void DAE_SetName(FbxObject *pObject,
                 const FbxString &pName,
                 const FbxString &pID);

/** Get the COLLADA source element with a semantic meaning and a consumer
 * element; The COLLADA input element declares the input connections to a data
 * source that a consumer requires. A data source is a container of raw data
 * that lacks semantic meaning so that the data can be reused within the
 * document. To use the data, a consumer declares a connection to it with the
 * desired semantic information. \param pConsumerElement A consumer element,
 * like sampler element in animation system or joints element in controller
 * system. \param pSemantic A semantic meaning, like "INPUT", "OUTPUT" or
 * "INTERPOLATION" in animation system. \param pSourceElements The container of
 * raw data. \return Return \c NULL is failed.
 */
...
{
  return xmlNewNode(NULL,
                    reinterpret_cast<xmlChar *>(const_cast<char *>(pTag)));
}
...
/** If the specific node has animation on its local translation, increase every
 * key by the offset. \param pNode The specific node. \param pOffset The
 * specific offset value.
 */
void IncreaseLclTranslationAnimation(FbxNode *pNode, FbxDouble3 &pOffset);

/** Search the elements with given tag, push the found results to the end of the
 * given array. \param pBaseElement Search from this element. \param pTag The
 * given tag. \param pResult The array to return the found results.
 */
