...
template <typename T> inline const FbxString TypeToArrayTag()
{
  return COLLADA_FLOAT_ARRAY_STRUCTURE;
}

template <> inline const FbxString TypeToArrayTag<bool>()
{
  return COLLADA_BOOL_ARRAY_STRUCTURE;
}

template <> inline const FbxString TypeToArrayTag<int>()
{
  return COLLADA_INT_ARRAY_STRUCTURE;
}

template <> inline const FbxString TypeToArrayTag<FbxString>()
{
  return COLLADA_NAME_ARRAY_STRUCTURE;
}

// Utility functions to convert type to parameter tag used in COLLADA source
// element
template <typename T> inline const FbxString TypeToParameterTag()
{
  return COLLADA_FLOAT_TYPE;
}

template <> inline const FbxString TypeToParameterTag<bool>()
{
  return COLLADA_BOOL_TYPE;
}

template <> inline const FbxString TypeToParameterTag<int>()
{
  return COLLADA_INT_TYPE;
}

template <> inline const FbxString TypeToParameterTag<FbxString>()
{
...
struct ElementContentAccessor
{
  ...
  template <typename TYPE> bool GetNext(TYPE *pData)
  {
               ...
               TYPE pDefaultValue = TYPE())
  {
    if (pArray)
    {
      return FromStringToArray(mPointer, pArray, pArraySize, pSourceUnitOffset,
                               pSourceUnitValidCount, pSourceUnitSize,
                               pDestUnitOffset, pDestUnitValidCount,
                               pDestUnitSize, pDefaultValue);
    }
    return 0;
  }
...
struct SourceElementContentAccessor : public ElementContentAccessor
{
      ...
      : mCount(0), mStride(1), mOffset(0)
  {
    ...
    xmlNode *lTechniqueElement = DAE_FindChildElementByTag(
        pSourceElement, COLLADA_TECHNIQUE_COMMON_ELEMENT);
    if (lTechniqueElement)
    {
      xmlNode *lAccessorElement = DAE_FindChildElementByTag(
          lTechniqueElement, COLLADA_ACCESSOR_STRUCTURE);
      if (lAccessorElement)
      {
        DAE_GetElementAttributeValue(lAccessorElement, COLLADA_COUNT_PROPERTY,
                                     mCount);
        DAE_GetElementAttributeValue(lAccessorElement, COLLADA_STRIDE_PROPERTY,
                                     mStride);
        DAE_GetElementAttributeValue(lAccessorElement, COLLADA_OFFSET_PROPERTY,
                                     mOffset);
      }
      lReadCount = false;
    }

    xmlNode *lDataArrayElement =
        DAE_FindChildElementByTag(pSourceElement, TypeToArrayTag<TYPE>());
    // Some COLLADA exporters use IDREF_array instead of Name_array
    if (!lDataArrayElement &&
        TypeToArrayTag<TYPE>() == COLLADA_NAME_ARRAY_STRUCTURE)
      lDataArrayElement = DAE_FindChildElementByTag(
          pSourceElement, COLLADA_IDREF_ARRAY_STRUCTURE);
    FBX_ASSERT(lDataArrayElement);

    if (lDataArrayElement && lReadCount)
      DAE_GetElementAttributeValue(lDataArrayElement, COLLADA_COUNT_PROPERTY,
                                   mCount);
...
public:
  ...
  // The name of user property in FBX which is used to preserve the ID of
  // COLLADA element
                          ...
                          int pStride = 1)
{
  ...
  xmlNode *lSourceElement =
      DAE_AddChildElement(pParentElement, COLLADA_SOURCE_STRUCTURE);
  ...
  xmlNode *lTechniqueCommonElement =
      DAE_AddChildElement(lSourceElement, COLLADA_TECHNIQUE_COMMON_ELEMENT);
  xmlNode *lAccessElement =
      DAE_AddChildElement(lTechniqueCommonElement, COLLADA_ACCESSOR_STRUCTURE);
  ...
  {
    xmlNode *lParamElement =
        DAE_AddChildElement(lAccessElement, COLLADA_PARAMETER_STRUCTURE);
    DAE_AddAttribute(lParamElement, COLLADA_TYPE_PROPERTY,
                     TypeToParameterTag<T>());
  }

  return lSourceElement;
}

/** Populate the layer element with direct array and return index array for
 * later use. \param pLayerElement The layer element to be populated. \param
 * pSourceElement The source element containing the direct array data. \param
 * pSize The count of double data of direct array element. \return Return the
 * index array of the layer element.
 */
template <typename TYPE>
FbxLayerElementArray *PopulateLayerElementDirectArray(
    FbxLayerElement *pLayerElement, xmlNode *pSourceElement, int pSize)
{
  SourceElementContentAccessor<TYPE> lSourceElementAccessor(pSourceElement);

  FbxLayerElementTemplate<TYPE> *lLayerElement =
      (FbxLayerElementTemplate<TYPE> *)pLayerElement;
  ...
  lSourceElementAccessor.GetArray((double *)lArray,
                                  lSourceElementAccessor.mCount * pSize, 0,
                                  pSize, lSourceElementAccessor.mStride, 0,
                                  pSize, sizeof(TYPE) / sizeof(double), 1.0);
