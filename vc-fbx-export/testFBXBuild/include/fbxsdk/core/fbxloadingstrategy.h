...
public:
   ...
   */
  //@{
  /** Execute the operation of loading the plug-in(s). The way it is executed is
   * determined by the specific implementations. \param pData  Plug in data that
   * can be access inside the plug-ins. \return The state of the loading action.
   */
  EState Load(FbxPluginData &pData);

  /** Execute the operation of unloading the plug-in(s). The way it is executed
   * is determined by the specific implementations.
   */
  void Unload();
  //@}
...
protected:
   ...
   */
  //@{
  /** Called by the Load method, it contains the specific user implementation
   * strategy to load the desired plug-in(s). \param pData  Plug in data that
   * can be access inside the plug-ins. \return If the plugin loading is
   * successful return \c true, otherwise return \c false
   */
  virtual bool SpecificLoad(FbxPluginData &pData) = 0;

  /** Called by the Unload method, it contains the specific user implementation
   * strategy to unload the desired plug-in(s).
   */
