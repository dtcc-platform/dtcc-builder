 ...
 * For instance, it can be used to read data from a memory source, thus making
 * it possible to import files from memory. However, for the time being, the
 * FbxStream class is only supported with FBX files.
 */
...
public:
   ...
   * \remark Each time the stream is open or closed, the stream position must be
   * reset to zero. */
   ...
   * The default implementation is written in terms of Read() but does not cope
   * with DOS line endings. Subclasses may need to override this if DOS line
   * endings are to be supported. \param pBuffer Pointer to the memory block
   * where the read bytes are stored. \param pMaxSize Maximum number of bytes to
   * be read from the stream. \param pStopAtFirstWhiteSpace Stop reading when
   * any whitespace is encountered. Otherwise read to end of line (like
   * fgets()). \return pBuffer, if successful, else NULL. \remark The default
   * implementation terminates the \e pBuffer with a null character and assumes
   * there is enough room for it.
   * For example, a call with \e pMaxSize = 1 will fill \e pBuffer with the null
   * character only. */
  virtual char *
  ReadString(char *pBuffer, int pMaxSize, bool pStopAtFirstWhiteSpace = false);
   ...
   * \param pSeekPos Pre-defined position where offset is added
   * (FbxFile::eBegin, FbxFile::eCurrent:, FbxFile::eEnd)
   * \param pOffset Number of bytes to offset from pSeekPos. */
  virtual void Seek(const FbxInt64 &pOffset,
                    const FbxFile::ESeekPos &pSeekPos) = 0;
   ...
   * \param pPosition Number of bytes from the beginning of the stream to seek
   * to. */
