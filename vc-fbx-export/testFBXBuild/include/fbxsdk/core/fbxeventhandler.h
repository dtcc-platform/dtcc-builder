 ...
 * Event handler is used to bind emitter and listener together. Its callback
 * function can process event data. To generate a valid event handler, you can
 * create an event emitter and event listener first and then call
 * FbxListener::Bind(). It will create an event handler automatically and bind
 * the handler to the listener and the created emitter. After that, the emitter
 * and listener are bound together via event handler. \remarks An
 * object(emitter) can emit a certain type of event, the object(listener) who
...
public:
   ...
   * \param pEvent specify the event type. pEvent could be a specific class
   * which derived from FbxEventBase. \see FbxEventBase */
...
public:
  FbxFuncEventHandler(FbxListener *pListener, CallbackFnc pFunction)
      : mListener(pListener), mFunction(pFunction)
  {
  }
  virtual int GetHandlerEventType() { return EventType::GetStaticTypeId(); }
  virtual void FunctionCall(const FbxEventBase &pEvent)
