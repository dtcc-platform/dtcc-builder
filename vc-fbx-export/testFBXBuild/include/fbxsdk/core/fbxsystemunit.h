...
/** \brief This class describes the units of measurement used within a
 * particular scene. \nosubgrouping
 */
class FBXSDK_DLL FbxSystemUnit
{
public:
  /** Struct to define various options that you can use to convert the system
   * unit of a scene. The default values are: mConvertRrsNodes = true
   *         mConvertLimits = true
   ...
   *
   * The default configuration have been tested to give the best conversion
   * results in the majority of the case. \remark Changing any of these values
   * will have a direct impact on the whole scene behavior.
   */
  struct ConversionOptions
  {
    //! This flag indicates whether or not to convert the nodes that do not
    //! inherit their parent's scale.
    ...
    //! This flag indicates whether or not to convert the light intensity
    //! property.
    bool mConvertLightIntensity;

    //! This flag indicates whether or not to convert photometric lights
    //! properties.
  ...
  /** Constructor.
   * \param pScaleFactor The equivalent number of centimeters in the new system
   * unit. For example, an inch unit uses a scale factor of 2.54. \param
...
#define FBXSDK_SYSTEM_UNIT_PREDEF_COUNT 9
  ...
  //! Points to a FbxSystemUnit array to store the predefined system units. The
  ...
  void ConvertScene(
      FbxScene *pScene,
      const ConversionOptions &pOptions = DefaultConversionOptions) const;

  /** Converts the child (or children) of the given node from the system unit to
   * this system unit. Unlike the ConvertScene() method, this method does not
   * set the axis system of the scene to which the pRoot node belongs. It also
   * does not adjust FbxPose as they are not stored under the scene, and not
   * under a particular node. \param pRoot The given node. \param pSrcUnit The
   * source system unit. \param pOptions Conversion options,
   * see:FbxSystemUnit::ConversionOptions.
   */
  ...
  /** Converts a scene from its system unit to this system unit, using the
   * specified Fbx_Root node. This method is provided for backwards
   * compatibility only and instead you should use ConvertScene( FbxScene* ,
   * const ConversionOptions&  ) whenever possible. \param pScene The scene to
   * convert. \param pFbxRoot The Fbx_Root node to use for conversion. \param
   ...
   * This factor scales system unit values to centimeters. If you want to scale
   * values to centimeters, use this value. Ignore the "multiplier" (returned by
   * GetMultiplier()) value. \return The the system unit's scale factor,
  ...
  /** Returns a unit label for the current scale factor.
   * The first letter of the label is in upper case and the label should be
   * pluralized. \return The unit label for the current scale factor.
   */
  ...
  /** Returns the conversion factor from this system unit to the target system
   * unit, excluding the multiplier factor. \param pTarget The target system
   * unit.
   */
  double GetConversionFactorTo(const FbxSystemUnit &pTarget) const;

  /** Returns the conversion factor from the source system unit to this system
   * unit, excluding the multiplier factor. \param pSource The source system
   * unit.
   */
...
private:
  ...
  void ConvertSTProperties(FbxArray<FbxNode *> &pNodes,
                           double pConversionFactor) const;
  void ConvertSProperty(FbxArray<FbxNode *> &pNodes,
                        double pConversionFactor) const;
  void ConvertAnimCurveNode(FbxArray<FbxAnimCurveNode *> &pFCurves,
                            double pConversionFactor) const;
  double GetConversionFactor(double pTargetScaleFactor,
                             double pSourceScaleFactor) const;
  void AdjustPivots(FbxNode *pNode,
                    double pConversionFactor,
                    FbxAMatrix &pOriginalGlobalM) const;
  ...
  void AdjustLightIntensity(FbxNode *pNode,
                            const double pConversionFactor) const;
  ...
  void ConvertChildren(FbxNode *pRoot,
                       ...
                       const ConversionOptions &pOptions) const;
