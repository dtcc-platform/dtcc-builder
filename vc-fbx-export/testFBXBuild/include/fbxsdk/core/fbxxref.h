...
public:
  ...
  //! This project represents an URL for storing localization files (that is not
   ...
   *
   * If we cannot write in that folder, we look at the sTemporaryFileProject
   * location. If no folder is set in the sTemporaryFileProject location, or it
   * is not writable, the operating system's Temp folder becomes the location.
   */
  ...
  /** Checks whether the URL at the given index stored in the property is
   * relative or not. \param pProperty                The property. \param
   * pIndex                   The URL index. \return                         \c
   * True if the URL is relative, \c false if the URL is not relative.
   */
   ...
   * \return                         \c True if the URL is resolved, return \c
   * false if the URL is not resolved.
   */
  bool GetResolvedUrl(FbxProperty const &pProperty,
                      int pIndex,
                      FbxString &pResolvedPath) const;

  /** Tries to resolve the specified URL.
   * \param pUrl                     The specified URL.
   * \param pDoc                     The document whose ".fbm" folder is used to
   * resolve the URL. \param pResolvedPath            Filled with the resolved
   * path. \return                         \c True if the URL is resolved,
   * return \c false if the URL is not resolved.
   */
   ...
   *
   * If a document is given, we start by looking at the document's ".fbm"
   * folder. \param pPrefix                  The prefix of the pattern. \param
   * pOptExt                  The extension of the pattern. \param pDoc The
   * given document. \param pResolvedPath            Filled with the first
   * matching URL. \return                         \c True if one matching file
   * is found, returns \c false if no matching file is found.
   */
   ...
   * \return                         \c True if the project is added
   * successfully, \c false if no project is added.
   */
   ...
   * \param pUrl                     The URL to be associated with the project.
   * \return                         \c True if the project is added
   * successfully, returns \c false if no project is added.
   */
  bool
  AddXRefProject(const char *pName, const char *pExtension, const char *pUrl);

  /** Adds an XRef project based on the document's EmbeddedUrl
   * property if set, if EmbeddedUrl is not set, based on its current URL
   * property. \param pDoc                     The document used to name the
   * project and to specify the URL. \return                         \c True if
   * the project is added successfully, returns \c false if no project is added.
   * \remarks                        The project name is set as the document
   * name and the URL is set as EmbeddedUrl or URL of the document.
   */
  bool AddXRefProject(FbxDocument *pDoc);

  /** Removes an XRef Projects.
   * \param pName                    The name of the project to be removed.
   * \return                         \c True if the project is removed
   * successfully, returns \c false if the project with the name does not exist.
   */
   ...
   * \return                         The base URL of the project or returns NULL
   * if the project with the name is not found.
   */
  const char *GetXRefProjectUrl(
      const char *pName); // FIXME: Should be const, will break AV.
   ...
   * \return                         The base URL of the project or NULL if the
   ...
   * \param pName                    The name of the project.
   * \return                         \c True if the project is defined in this
   * manager, returns \c false if it isn't defined in this manager.
   */
   ...
   * \return                         \c True if the URL is resolved, returns \c
   * false if the URL is not resolved.
   */
