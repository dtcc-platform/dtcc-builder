...
/** Base class to manage query. A query contains a filter and reference ID,
 * which will be used to search and retrieve objects. The derived query classes
 * are used to create FbxCriteria. \nosubgrouping */
 ...
 * properties, so that only those satisfying the criteria are affected by the
 * query. Some examples of kinds of criteria are object type, connection type,
 * or property. Criteria can be combined using logical operators such as "and"
 * and "or". \note Objects are basic elements in FBX. Each of them has a
 * hierarchy type and some properties. Objects and properties can be connected
 * through a connection to represent a relationship between them. (e.g.
 * child-parent, container membership, reference, etc.,). In a query, you could
 * select object or properties based on these criteria. Here are some examples:
 * \code
 * FbxObject* lObject = FbxObject::Create(lManager, "Object");
 * int lSrcLightCount =
 * lObject->RootProperty.GetSrcObjectCount(FbxCriteria::ObjectType(FbxLight::ClassId));
 * int lSrcDeformerCount =
 * lObject->RootProperty.GetSrcObjectCount(FbxCriteria::ObjectTypeStrict(FbxDeformer::ClassId));
 * int lSrcPropertyCount =
 * lObject->RootProperty.GetSrcCount(FbxCriteria::IsProperty()); \endcode \see
 * FbxQuery \see FbxProperty::GetSrcObjectCount(const FbxCriteria&) const \see
 * FbxCollection::GetMemberCount(const FbxCriteria&) const \nosubgrouping */
class FBXSDK_DLL FbxCriteria
{
public:
  /** Creates a new query criteria that only selects objects which have a
   * specific class ID or derive from a class with a specific class ID. \param
   * pClassId The base type class ID */
  static FbxCriteria ObjectType(const FbxClassId &pClassId);

  /** Creates a new query criteria that only selects objects which have a
   * specific class ID. \param pClassId The type class ID */
  ...
  /** Gets a logical conjunction (and) criteria from this and the specified
   * criteria. \param pCriteria The specified criteria */
  FbxCriteria operator&&(const FbxCriteria &pCriteria) const;

  /** Gets a logical disjunction (or) criteria from this and the specified
   * criteria. \param pCriteria The specified criteria */
  FbxCriteria operator||(const FbxCriteria &pCriteria) const;
...
{
  inline int operator()(const FbxCriteria &pKeyA,
                        const FbxCriteria &pKeyB) const
  {
