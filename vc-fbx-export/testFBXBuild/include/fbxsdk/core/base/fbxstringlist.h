...
{
  return FBXSDK_stricmp((*(FbxStringListItem **)E1)->mString.Buffer(),
                        (*(FbxStringListItem **)E2)->mString.Buffer());
}
...
{
  return strcmp((*(FbxStringListItem *)E1).mString.Buffer(),
                (*(FbxStringListItem **)E2)->mString.Buffer());
}
...
public:
   ...
   * \remarks If the given index is out of range, the pointer is appended at the
   * end of the array.
   */
   ...
   * \return Index of first matching item found or -1 if there is no matching
   * element.
   */
...
public:
  ...
  /** Find first matching item in array whose string address is the same as
   * given pointer. \return Index of first matching item found or -1 if  there
   ...
   * \return The value of reference of the first matching item found or NULL if
  ...
  //! Remove first matching item in array whose string address is the same as
  //! given pointer.
  ...
  void Sort()
  {
    qsort(&(mList.GetArray()[0]), mList.GetCount(), sizeof(FbxStringListItem *),
          FbxCompareStringListSort);
  }

  /** Find first matching item which has the same string as given parameter,not
   * case sensitive. \return the pointer of matching item found or NULL if there
   * is no matching element. \remark To cast the returned pointer to the
   * FbxStringListItem you need a double indirection: (FbxStringListItem**)
   */
  void *FindEqual(const char *pString) const
  {
    ...
    if (mList.GetCount() != 0)
    {
      return bsearch(&Key, &(mList.GetArray()[0]), mList.GetCount(),
                     sizeof(FbxStringListItem *),
  ...
  /** Find first matching item which has the same string as given parameter,
   * case sensitive. \return the pointer of matching item found or NULL if there
   * is no matching element. \remark To cast the returned pointer to the
   * FbxStringListItem you need a double indirection: (FbxStringListItem**)
   */
  ...
  /** Remove the item at the given position in the array and delete the
   * associated object. \param pIndex Position of the item to remove. \remarks
   * If the index is not valid, nothing is performed. Otherwise, the item is
   * removed from the array and the items are shifted to fill the empty slot.
   */
  virtual void RemoveAt(int pIndex) { FbxDelete(mList.RemoveAt(pIndex)); }

  //! Delete the array.
  virtual void Clear() { FbxArrayDelete(mList); }
  ...
  /** Clear the array and set the array's new items with the substring separated
   * by '~' from the given string. \param pList The string which used to
   * generate the new items. \return The last index of the item in the new
   * array. \remarks The number of items in the new array is the same as the
   * number of substrings, and the string of each item is decided by the content
   * of each substring.
   */
