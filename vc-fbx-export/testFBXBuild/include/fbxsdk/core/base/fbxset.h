...
/** This class implements an efficient set based on value comparison, which
 * stores values. It executes insertion, deletion and query operations in
 * O(log(n)) time. */
...
public:
   ...
   * \return If the value is already present in the map, returns the existing
   * value and false; else returns the pointer to the new value and true. */
  ...
  inline int Remove(const ValueType &pValue) { return mTree.Remove(pValue); }
