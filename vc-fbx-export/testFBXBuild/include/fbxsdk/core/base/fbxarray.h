...
/** Class for array of basic elements such as pointers and basic types. This
 * class will not call constructor and destructor for elements, thus it is not
 * suitable for object references. Memory allocations are always done in a
...
public:
  ...
  /** Insert an element at the given position, growing the array if capacity is
   * not sufficient. \param pIndex Position where to insert the element. Must be
   * a positive value. \param pElement Element to insert in the array. \param
   * pCompact If \c true and capacity is exceeded, grow capacity by one,
   * otherwise double capacity (default). \return -1 if insert failed, otherwise
   * the position of the inserted element in the array. \remark If the given
   * index is greater than Size(), the element is appended at the end. Use
   * compact mode only if you need to save memory. */
  inline int
  InsertAt(const int pIndex, const T &pElement, bool pCompact = false)
  {
    ...
    if (mSize >= mCapacity)
    {
      ...
      int lNewCapacity =
          FbxMax(pCompact ? mCapacity + 1 : mCapacity * 2,
                 1); // We always double capacity when not compacting
      ...
      return InsertAt(
          pIndex,
          lElement); // Insert copied element because reference might be moved
    }

    if (lIndex <
        mSize) // Move elements to leave a space open to insert the new element
    {
      // If pElement is inside memmove range, copy element and insert copy
      // instead
      ...
      memmove(&mArray[lIndex + 1], &mArray[lIndex],
              (mSize - lIndex) * sizeof(T));
    }
  ...
  /** Append an element at the end of the array, doubling the array if capacity
   * is not sufficient. \param pElement Element to append to the array.
   * \return -1 if add failed, otherwise the position of the added element in
   * the array. */
  inline int Add(const T &pElement) { return InsertAt(mSize, pElement); }

  /** Append an element at the end of array, if not already present, doubling
   * the array if capacity is not sufficient. \param pElement Element to append
   * to the array.
   * \return -1 if add failed, otherwise the position of the added element in
   * the array. */
  ...
  /** Append an element at the end of the array, growing the array by one
   * element if capacity is not sufficient. \param pElement Element to append to
   * the array. \return -1 if add failed, otherwise the position of the added
  ...
  /** Retrieve the number of element contained in the array. To increase the
   * capacity without increasing the size, please use Reserve(). \return The
  ...
  inline T &operator[](const int pIndex) const
  {
    ...
    if (pIndex >= mSize)
    {
      if (pIndex < mCapacity)
      {
        FBX_ASSERT_NOW("Index is out of range, but not outside of capacity! "
                       "Call SetAt() to use reserved memory.");
      }
      else
   ...
   * \remark No error will be thrown if the index is out of bounds. */
  inline T GetAt(const int pIndex) const { return operator[](pIndex); }

  /** Retrieve a copy of the first element.
   * \return Copy of the first element.
   * \remark The array should have at least one element and no error will be
   * thrown if the array is empty. */
  inline T GetFirst() const { return GetAt(0); }

  /** Retrieve a copy of the last element.
   * \return Copy of the last element.
   * \remark The array should have at least one element and no error will be
   * thrown if the array is empty. */
  inline T GetLast() const { return GetAt(mSize - 1); }
   ...
   * \return Position of first matching element or -1 if there is no matching
   * element. */
   ...
   * \return Position of first matching element or -1 if there is no matching
   * element. */
  inline int FindReverse(const T &pElement,
                         const int pStartIndex = FBXSDK_INT_MAX) const
  {
  ...
  /** Request for allocation of additional memory without inserting new
   * elements. After the memory has been reserved, please use SetAt() to
   * initialize elements. \param pCapacity The number of additional element
   * memory allocation requested. \return \c true if the memory allocation
   * succeeded or if the capacity is unchanged, \c false otherwise. \remark If
   * the requested capacity is less than or equal to the current capacity, this
   * call has no effect. In either case, Size() is unchanged. */
   ...
   * \remark If the index is outside range, and outside capacity, this call has
   * no effect. However, if index is within capacity range, element count is
   * increased such that Size() will become pIndex + 1. */
   ...
   * \remark The array should have at least one element and no error will be
   * thrown if the array is empty. */
  inline void SetFirst(const T &pElement) { SetAt(0, pElement); }

  /** Set the value of the last element.
   * \param pElement The new value of the last element.
   * \remark The array should have at least one element and no error will be
   * thrown if the array is empty. */
  inline void SetLast(const T &pElement) { SetAt(mSize - 1, pElement); }
  ...
  inline T RemoveAt(const int pIndex)
  {
    ...
    if (pIndex + 1 < mSize)
    {
      memmove(&mArray[pIndex], &mArray[pIndex + 1],
              (mSize - pIndex - 1) * sizeof(T));
    }
    mSize--;
    return lElement;
  }
   ...
   * \remark The array should have at least one element and no error will be
   * thrown if the array is empty. */
  inline T RemoveFirst() { return RemoveAt(0); }

  /** Remove the last element in the array.
   * \return Removed element.
   * \remark The array should have at least one element and no error will be
   * thrown if the array is empty. */
  inline T RemoveLast() { return RemoveAt(mSize - 1); }

  /** Remove first matching element in the array.
   * \param pElement Element to be removed.
   * \return \c true if a matching element is found and removed, \c false
   * otherwise. */
  ...
  inline void RemoveRange(const int pIndex, const int pCount)
  {
    ...
    {
      memmove(&mArray[pIndex], &mArray[pIndex + pCount],
              (mSize - pIndex - pCount) * sizeof(T));
    }
    mSize -= pCount;
  }

  /** Inserts or erases elements at the end such that Size() becomes pSize,
   * increasing capacity if needed. Please use SetAt() to initialize any new
   * elements. \param pSize The new count of elements to set the array to. Must
   * be greater or equal to zero. \return \c true if the memory (re)allocation
   * succeeded, \c false otherwise. \remark If the requested element count is
   * less than or equal to the current count, elements are freed from memory.
   * Otherwise, the array grows and elements are unchanged. */
  ...
  inline bool Grow(const int pSize) { return Resize(mSize + pSize); }

  /** Reduce size of array by the specified size.
   * \param pSize The size to remove from the array size.
   * \return \c true if operation succeeded, \c false otherwise. */
  inline bool Shrink(const int pSize) { return Resize(mSize - pSize); }

  /** Compact the array so that its capacity is the same as its size.
   * \return \c true if operation succeeded, \c false otherwise. */
  inline bool Compact() { return Resize(mSize); }

  /** Reset the number of element to zero and free the memory allocated.
   * \remark This only free the memory allocated by the array, and doesn't call
   * the destructor of each element. */
  inline void Clear()
  {
    if (mArray != NULL)
    {
  ...
  inline void AddArray(const FbxArray<T> &pOther)
  {
    ...
    {
      memcpy(&mArray[mSize - pOther.mSize], pOther.mArray,
             pOther.mSize * sizeof(T));
    }
  }

  /** Append the elements of another array at the end of this array if they are
   * not present. \param pOther Another array. */
...
private:
  ...
  // Previously class FbxArray is for pointers. Somehow, it's used to store
  // other types. Here's a compile-time checking for known incompatible classes.
  // If it happens you find new incompatible ones, declare them with macro
  // FBXSDK_INCOMPATIBLE_WITH_ARRAY. Also see file fbxstring.h.
  FBX_ASSERT_STATIC(FBXSDK_IS_SIMPLE_TYPE(T) || __is_enum(T) ||
                    (__has_trivial_constructor(T) &&
                     __has_trivial_destructor(T)) ||
                    !FBXSDK_IS_INCOMPATIBLE_WITH_ARRAY(T));
#endif
...
//! Make sure to break build if someone try to make FbxArray<FbxArray<T>>, which
//! is not supported.
