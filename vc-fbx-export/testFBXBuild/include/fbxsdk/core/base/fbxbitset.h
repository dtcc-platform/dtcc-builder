 ...
 *
 * The bit array will automatically grow when specifying bit indexes that are
 * greater than the array size when calling SetBit or UnsetBit. Indexes can vary
 * from 0 to FBXSDK_UINT_MAX-1. When an invalid index is returned from any
 * functions, FBXSDK_UINT_MAX is returned. The bit array is not thread safe.
 */
class FBXSDK_DLL FbxBitSet
{
public:
  /** Constructor.
   * \param pInitialSize Initial bit array size in bit count (not in byte
   * count!).
   */
  ...
  /** Set the bit at the specified bit index to true regardless of its current
   * value. \param pBitIndex The bit index in the array in the range of [0,
   * FBXSDK_UINT_MAX-1].
   */
  ...
  /** Set the bit at the specified bit index to false regardless of its current
   * value. \param pBitIndex The bit index in the array in the range of [0,
   * FBXSDK_UINT_MAX-1].
   */
   ...
   * \param pBitIndex The bit index in the array in the range of [0,
   * FBXSDK_UINT_MAX-1]. \return True if the bit is set, false otherwise.
   */
   ...
   * \param pBitIndex The start bit index in the array in the range of [0,
   * FBXSDK_UINT_MAX-1]. \return The bit index of the next set bit,
   * FBXSDK_UINT_MAX if none found.
   */
   ...
   * \param pBitIndex The start bit index in the array in the range of [0,
   * FBXSDK_UINT_MAX-1]. \return The bit index of the previous set bit,
   * FBXSDK_UINT_MAX if none found.
   */
