...
public:                                                                        \
  inline FbxListNode<Class> &GetListNode(int index = 0)                        \
  {                                                                            \
    return this->mNode[index];                                                 \
  }                                                                            \
                                                                               \
private:                                                                       \
  FbxListNode<Class> mNode[NodeCount];
...
public:
  ...
  FbxIntrusiveList() : mHead(0) { mHead.mNext = mHead.mPrev = &mHead; }
  ~FbxIntrusiveList()
  {
    while (!Empty())
      Begin().Get()->Disconnect(); // LINUXNote:  should be Erase(Begin()); but
                                   // there's an issue with gcc 4.2
  };
...
public:
  ...
  public:
    ...
    operator=(const IntrusiveListConstIterator &other)
    {
      mPtr = other.mPtr;
      return *this;
    }
