...
/** Default compare functor for FbxMap and FbxSet, which assumes operator < is
defined. Here is examples of different compare class implementations: With Key =
int \code class IntCompare
{
...
/** This class implements an efficient map based on key comparison, which stores
key-value pairs. It executes insertion, deletion and query operations in
O(log(n)) time. */
...
public:
  ...
  inline int GetSize() const { return mTree.GetSize(); }
  ...
  inline FbxPair<RecordType *, bool> Insert(const KeyType &pKey,
  ...
  inline bool Remove(const KeyType &pKey) { return mTree.Remove(pKey); }

  //! Clear the map.
  inline void Clear() { mTree.Clear(); }

  //! Query whether the map is empty.
  inline bool Empty() const { return mTree.Empty(); }
  ...
  //! Retrieve the begin iterator of the map.
  ConstIterator Begin() const { return ConstIterator(Minimum()); }
  ...
  inline RecordType *Find(const KeyType &pKey) { return mTree.Find(pKey); }

  /** Find the key-value pair with the smallest key greater than a specified
   * key. \param pKey The key. \return The found key-value pair. */
  ...
  inline const RecordType *Minimum() const { return mTree.Minimum(); }

  //! Retrieve the key-value pair which is the minimum key in map.
  inline RecordType *Minimum() { return mTree.Minimum(); }

  //! Retrieve the key-value pair which is the maximum key in map.
  inline const RecordType *Maximum() const { return mTree.Maximum(); }

  //! Retrieve the key-value pair which is the maximum key in map.
  inline RecordType *Maximum() { return mTree.Maximum(); }
...
public:
  ...
  inline Iterator GetFirst() const { return (Iterator)mMap.Minimum(); }
  ...
  inline void Reserve(int pSize) { mMap.Reserve(pSize); }

  /** Query the count of elements in the map.
   * \return The count of elements. */
  inline int GetCount() const { return mMap.GetSize(); }
...
public:
   ...
   */
  inline FbxObject *
  Get(typename FbxSimpleMap<Type, FbxObject *, Compare>::Iterator pIterator)
  {
 ...
 * \nosubgrouping */
class FBXSDK_DLL FbxObjectStringMap
    : public FbxObjectMap<FbxString, FbxStringCompare>
{
public:
  //! Constructor
...
{
  for (typename FbxMap<K, V, C, A>::Iterator i = pMap.Begin(); i != pMap.End();
       ++i)
  {
    FbxFree(i->GetValue());
  }
