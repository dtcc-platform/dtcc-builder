 ...
 * \return True if the system temporary folder path has been set and False
 * otherwise. \remark The system temporary folder is limited to _MAX_PATH
 * characters. Trying to set a longer value will fail and the current system
 * temporary folder path is left unchanged.
 */
...
public:
   ...
   * \param pCleanPath If true, the resulting path will be cleaned via
   * FbxPathUtils::Clean(). \return Both paths binded together forming a new
   * file path. \remark If the file path is already a full valid path, pFilePath
   * is returned.
   */
  static FbxString
  Bind(const char *pRootPath, const char *pFilePath, bool pCleanPath = true);

  /** Extract the folder name from the given file path.
   * \param pFilePath The given file path.
   * \return The folder name. If there isn't any '\\' or '/' in  the given file
   * path, it will return pFilePath.
   */
  static FbxString GetFolderName(const char *pFilePath);

  /** Extract file name from the given file path.
   * \param pFilePath The given file path.
   * \param pWithExtension Decide the file name with extension or without
   * extension. If it is true, return the file name with extension; if it is
   * false, return the file name without extension.
   */
   ...
   * \remark Return empty string if the file path doesn't contain a valid
   * extension.
   */
   ...
   * \remark If the file path doesn't end with a valid file name, pFilePath is
   * returned.
   */
   ...
   * \param pNewPath The given new path. If it is only file name, the default
   * directory is work directory. \return The relative path. \remarks If the
   * given two paths have the same drive, the function will turn  '\\' in the
   * relative path to  '/'.
   */
   ...
   * \param pFolder The folder where the file name should be attempted to be
   * created. \param pPrefix The prefix of generated file name. \return A valid
   * file path that can safely be used to create a new file.
   */
   ...
   * \remark This function work for relative paths. It will search from the
   * current work path. */
   ...
   * \return True if folder path already exist, or if it was successfully
   * created, false otherwise. \remark This function will create multiple
   * folders if needed, and it also work for relative paths. */
 ...
 * This object is not used internally by the FBX SDK. It is provided for
 * convenience and its usage is shown in the custom reader/writers samples. */
