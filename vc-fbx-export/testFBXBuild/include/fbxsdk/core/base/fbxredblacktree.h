  ...
  */
  ...
  private:
    ...
    inline RecordType(const DataType &pData)
        : mData(pData), mParent(0), mLeftChild(0), mRightChild(0), mColor(eRed),
          mBlackDepth(0)
    {
    }

    inline RecordType(const RecordType &pRecordType)
        : mData(pRecordType.mData), mParent(0), mLeftChild(0), mRightChild(0),
          mColor(pRecordType.mColor), mBlackDepth(pRecordType.mBlackDepth)
    {
    }
...
public:
  ...
  inline FbxRedBlackTree &operator=(const FbxRedBlackTree &pTree)
  {
    if (this != &pTree)
    {
      ...
      if (pTree.mRoot)
      {
        ...
        mRoot = new (lBuffer) RecordType(*(
            pTree.mRoot)); // in-place new won't allocate memory, so it is safe
  ...
  inline bool operator==(const FbxRedBlackTree &pTree) const
  {
    ...
    // Iterator through all nodes; if we reach the end of both iterators at the
    // same time then we have two iterators that match.
  ...
  inline FbxPair<RecordType *, bool> Insert(const DataType &pData)
  {
    ...
    {
      void *lBuffer = mAllocator.AllocateRecords();
      lNode = new (lBuffer) RecordType(
          pData); // in-place new won't allocate memory, so it is safe
      mSize++;
...
protected:
  ...
  inline RecordType *DuplicateSubTree(const RecordType *pNode)
  {
    ...
    {
      void *lBuffer = mAllocator.AllocateRecords();
      lNewSubTree = new (lBuffer) RecordType(
          *pNode); // in-place new won't allocate memory, so it is safe
  ...
  inline RecordType *Sibling(const RecordType *pParent,
                             const RecordType *pNode) const
  {
    if (pParent)
    {
  ...
  inline void FixNodesAfterRemoval(RecordType *pParent, RecordType *pNode)
  {
    ...
    while (!lDone)
    {
      ...
      else if (lParent != NULL)
      {
        ...
        if (lSibling && IsBlack(lParent) && IsBlack(lSibling) &&
            IsBlack(lSibling->mLeftChild) && IsBlack(lSibling->mRightChild))
        {
        ...
        {
          if (!IsBlack(lParent) && IsBlack(lSibling) &&
          ...
          {
            if ((lNode == lParent->mLeftChild) && IsBlack(lSibling) &&
            ...
            else if ((lNode == lParent->mRightChild) && IsBlack(lSibling) &&
                     IsBlack(lSibling->mLeftChild) &&
                     !IsBlack(lSibling->mRightChild))
            {
              lSibling->mColor = RecordType::eRed;
  ...
  inline int GetSubTreeSize(RecordType *pNode) const
  {
    if (pNode)
    {
      return GetSubTreeSize(pNode->mLeftChild) +
             GetSubTreeSize(pNode->mRightChild) + 1;
    }
    else
    {
      return 0;
