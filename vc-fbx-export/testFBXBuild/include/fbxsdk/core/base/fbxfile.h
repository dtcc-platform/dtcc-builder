...
/**
    Class for interfacing with files, providing a similar interface for files
   independant of the OS or filesystem.
*/
...
public:
   ...
   * \param pMode Mode in which to open the file, e.g. eReadOnly,
   * eCreateReadWrite, etc. \param pBinary Whether the file is to be opened in
   * binary or text mode. \return True if opening is successful.
   */
   ...
   * \param pStreamData User-defined data to pass as a parameter to the stream's
   * Open() method. \param pMode Deprecated/Unused. \return True if opening is
   * successful.
   */
  ...
  /** Seek to a specific position in the file, starting from either beginning,
   * current position or end \param pOffset Offset to seek to (advance the file
   * position cursor) starting from pSeekPos \param pSeekPos Starting position
   * from which to seek to.  Beginning, current position or end.
   */
  virtual void Seek(const FbxInt64 pOffset, const ESeekPos pSeekPos = eBegin);

  /** Returns the position at which the file cursor currently is.  For example,
   * will be ==0 for beginning and ==FileSize for end. \return The position at
   * which the file cursor currently is.
   */
   ...
   * \param pStopAtFirstWhiteSpace If true, will stop reading at first white
   * space, otherwise it will stop at the first line feed (\n) \return Pointer
   * on the data read.  Equivalent to parameter pDstBuf
   */
   ...
   * \param ... Variable number of arguments describing the values in the
   * previous parameter. \return True if data was successfully written
   */
  virtual bool WriteFormat(const char *pFormat, ...);

  /** Modify the size of a file. Null characters ('\0') are appended if the file
   * is extended. If the file is truncated, all data from the end of the
   * shortened file to the original length of the file is lost. Please note that
   * this function considers the current file cursor as the beginning of the
   * file. It is therefore required to use Seek(0) prior to calling it if we
   * want the size specified by the pSize parameter to be absolute. \param pSize
   * New desired file size \return True if file was successfully truncated
   */
  ...
  /** Unused function in this default implementation.  Must be implemented by
   * memory files. \param pMemPtr Unused \param pSize Unused
   */
  ...
  /** Checks whether the file is currently opened with a user-provided streaming
   * interface instead of just the file name \return True if file has been
   * opened with a stream interface, false otherwise
   */
  bool IsStream() const;
  ...
  /** Returns the mode with which the file was opened, when calling the Open()
   * method. \return Mode with which the file was opened
   */
  ...
  /** Resets the current error code and the end of file indicator of the opened
   * file
   */
  void ClearError();
...
public:
  ...
  /** Copy one file's content to another file (if the destination file not
   * exist, it will be created). \param pDestination_UTF8 The destination file
   * path \param pSource_UTF8 The source file path \return Return true if copy
   * is successfully.
   */
