 ...
 * \nosubgrouping
 * FbxTime can measure time in hour, minute, second, frame, field, residual and
 * also combination of these units. It is recommended to use FbxTime for all
 * time related operations. For example, currently it is used in
 * FbxGlobalSettings, FbxGlobalTimeSettings, FbxCache, all curve filters and all
 * animation-related classes, etc. FbxTime is just used to represent a moment,
 * to represent a period of time, FbxTimeSpan should be used. \see FbxTimeSpan
 */
...
public:
   ...
   * \param pFrameRate Custom framerate, only have effect in case of pTimeMode =
   * FbxTime::eCustom \remarks It is meaningless to set default time mode to \c
   * eDefaultMode.
   */
   ...
   * \remarks It is meaningless to set default time protocol to \c
   * eDefaultProtocol.
   */
   ...
   * \return The corresponding time mode identifier or \c eDefaultMode if no
   * time mode associated to the given frame rate is found.
   */
   ...
   * \param pTimeMode  Time mode identifier.
   * \remarks Parameters pHour, pMinute, pSecond, pFrame and pField are summed
   * together. For example, it is possible to set the time to 83 seconds in the
   * following ways: SetTime(0,1,23) or SetTime(0,0,83).
   */
   ...
   * \param pTimeMode   The time mode identifier which will dictate the
   * extraction algorithm. \return \c true if the pTimeMode parameter is a valid
   * identifier and thus the extraction succeeded. If the function returns \c
   * false, all the values are set to 0.
   */
  bool GetTime(int &pHour,
               ...
               int &pField,
               int &pResidual,
               EMode pTimeMode = eDefaultMode) const;

  /** Snaps a time value to the time value associated with the nearest frame.
   * \param pRound  If \c true the return value is rounded to the nearest
   * integer. \return        The snapped time value.
   */
  FbxTime GetFramedTime(bool pRound = true) const;

  /** Set time in frame format.
   * \param pFrames The number of frames.
   * \param pTimeMode The time mode identifier which will dictate the extraction
   * algorithm.
   */
   ...
   * \param pTimeString An array large enough to contain a minimum of 19
   * characters. \param pTimeStringSize Size of the pTimeString buffer used with
   * secure functions. \param pInfo The amount of information if time protocol
   * is \c eSMPTE: <ul><li>1 means hours only <li>2 means hours and minutes
   ...
   *     <li>6 means hours, minutes, seconds, frames, field and residual
   * value</ul> \param pTimeMode Requested time mode. \param pTimeFormat
   * Requested time protocol. \return pTimeString parameter filled with a time
   * value or set to a empty string if parameter pInfo is not valid.
   */
  char *GetTimeString(char *pTimeString,
                      const FbxUShort &pTimeStringSize,
                      int pInfo = 5,
                      EMode pTimeMode = eDefaultMode,
                      EProtocol pTimeFormat = eDefaultProtocol) const;
   ...
   * \param pTime The FbxTime to be compared.
   * \return \c true if this FbxTime is superior or equal to the passed FbxTime,
   * \c false otherwise.
   */
  inline bool operator>=(const FbxTime &pTime) const
  {
   ...
   * \param pTime The FbxTime to be compared.
   * \return \c true if this FbxTime is inferior or equal to the passed FbxTime,
   * \c false otherwise.
   */
  inline bool operator<=(const FbxTime &pTime) const
  {
   ...
   * \param pTime The FbxTime to be compared.
   * \return \c true if this FbxTime is superior to the passed FbxTime, \c false
   * otherwise.
   */
  inline bool operator>(const FbxTime &pTime) const
  {
   ...
   * \param pTime The FbxTime to be compared.
   * \return \c true if this FbxTime is inferior to the passed FbxTime, \c false
   * otherwise.
   */
  inline bool operator<(const FbxTime &pTime) const
  {
  ...
  FbxTime operator*(const FbxTime &pTime) const;
  /*
                  //! Increment time of one unit of the internal format (prefix
     form). inline FbxTime& operator++() { mTime += 1; return (*this); }

                  //! Increment time of one unit of the internal format (postfix
     form). inline const FbxTime operator++(int) { FbxTime lOld = *this;
     ++(*this); return lOld; }

                  //! Decrement time of one unit of the internal format (prefix
     form). inline FbxTime& operator--() { mTime -= 1; return (*this); }

                  //! Decrement time of one unit of the internal format (postfix
     form). inline const FbxTime operator--(int) { FbxTime lOld = *this;
     --(*this); return lOld; }*/
  //@}
...
private:
  ...
  void InternalSetTime(int pHour,
                       int pMinute,
                       int pSecond,
                       FbxLongLong pFrame,
                       int pField,
  ...
  friend FBXSDK_DLL void FbxSetGlobalTimeMode(FbxTime::EMode pTimeMode,
                                              double pFrameRate);
...
public:
   ...
   * \return \c FBXSDK_TIME_FORWARD if time interval is forward, \c
   ...
   * \remarks This function assumes both of the FbxTimeSpan objects are in the
   * same direction. Use FBXSDK_TIME_FORWARD when start < stop in both timespan
...
// Use those functions to keep the compatibility with old time mode since we
// added new time mode.
FBXSDK_DLL FbxTime::EOldMode
FbxGetOldTimeModeCorrespondance(FbxTime::EMode pMode);
FBXSDK_DLL FbxTime::EMode
FbxGetTimeModeFromOldValue(FbxTime::EOldMode pOldMode);
...
FBXSDK_DLL int
FbxTimeToSelection(FbxTime::EMode pTimeMode = FbxTime::eDefaultMode,
