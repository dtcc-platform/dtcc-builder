...
public:
  //! Default constructor.
  FbxDynamicArray()
      : mArray(NULL), mCapacity(0), mSize(0), mAllocator(sizeof(Type))
  {
  }
  ...
  /** Assures that sufficient memory is allocated to hold n objects in the
   * array, and increases the capacity if necessary. \param pCount Number of
   * objects to reserve */
  void Reserve(const size_t pCount)
  {
    if (pCount > mCapacity)
    {
      // We don't use mAllocator.PreAllocate, because we want our array to be
  ...
  void Insert(const size_t pIndex, const Type &pItem, const size_t pNCopies = 1)
  {
    ...
    {
      size_t lNewSize =
          mCapacity + mCapacity / 2; // not enough room, grow by 50%
      if (mSize + pNCopies > lNewSize)
      {
        lNewSize = mSize + pNCopies;
      }
      ...
      MoveArray(lNewArray + pIndex + pNCopies, mArray + pIndex,
                mSize - pIndex); // copy suffix
      mAllocator.FreeMemory(mArray);
      mArray = lNewArray;
      mSize += pNCopies;
      mCapacity = lNewSize;
    ...
    else
    {
      ...
      MoveArrayBackwards(mArray + pIndex + pNCopies, mArray + pIndex,
                         mSize - pIndex);
  ...
  void Remove(const size_t pIndex, size_t pNElements = 1)
  {
    ...
    else
    {
      ...
      MoveOverlappingArray(&mArray[pIndex], &mArray[pIndex + pNElements],
                           mSize - pIndex - pNElements);
      mSize -= pNElements;
    }
  }
   ...
   * \param pIndex position index */
  Type &operator[](const size_t pIndex) { return mArray[pIndex]; }

  /** Gets nth object in the array.
   * \param pIndex position index */
  const Type &operator[](const size_t pIndex) const { return mArray[pIndex]; }

  /** Retrieve the first item in the array.
   * \return The first item in the array. */
  Type &First() { return operator[](0); }

  /** Retrieve the first item in the array.
   * \return The first item in the array. */
  const Type &First() const { return operator[](0); }

  /** Retrieve the last item in the array.
   * \return The last item in the array. */
  Type &Last() { return operator[](mSize - 1); }

  /** Retrieve the last item in the array.
   * \return The last item in the array. */
  const Type &Last() const { return operator[](mSize - 1); }
   ...
   * \return Index of the first matching item, otherwise returns -1 (equivalent
   * of SIZE_MAX for size_t). */
  ...
  /** Assignment operator.
   * \remarks The copy constructor of \c Type will be invoked in order to copy
   * the value of elements to the new array. */
...
private:
  static void CopyArray(Type *pDest, const Type *pSrc, size_t pCount)
  {
    ...
    {
      new (&(pDest[i]))
          Type(pSrc[i]); // in-place new won't allocate memory, so it is safe
    }
  }
  ...
  static void MoveArrayBackwards(Type *pDest, const Type *pSrc, size_t pCount)
  {
    for (int i = 0; i < int(pCount); ++i)
    {
      new (&(pDest[pCount - 1 - i]))
          Type(pSrc[pCount - 1 -
                    i]); // in-place new won't allocate memory, so it is safe
  ...
  static void Fill(Type *pDest, const Type &pItem, size_t pCount)
  {
    ...
    {
      new (&(pDest[i]))
          Type(pItem); // in-place new won't allocate memory, so it is safe
    }
  }
