 ...
 * status code and the internal FBXSDK error code as returned by the API
 * functions. \nosubgrouping
 */
...
public:
   ...
   * \return \c True if all the members of \e rhs are equal to this instance
   * members and \c False otherwise.
   */
  bool operator==(const FbxStatus &rhs) const { return (mCode == rhs.mCode); }
  /** Equivalence operator.
   * \param pCode Status code to compare.
   * \return \c True if the code member of this instance equals \e pCode and \c
   * False otherwise.
   */
  bool operator==(const EStatusCode pCode) const { return (mCode == pCode); }
  /** Non-Equivalence operator.
   * \param rhs Status object to compare.
   * \return \c True if at least one member of \e rhs is not equal to this
   * instance member and \c True otherwise.
   */
  bool operator!=(const FbxStatus &rhs) const { return (mCode != rhs.mCode); }
  /** Non-Equivalence operator.
   * \param rhs Status code to compare.
   * \return \c True if the code member of this instance equals \e rhs and \c
   * False otherwise.
   */
   ...
   * \return \c True if an error occured and \c False if the operation was
   * sucessful.
   */
  bool Error() const { return !this->operator bool(); }

  //! Clear error code and message from the instance. After this call, it will
  //! behave as if it contained eSuccess.
  void Clear();
  ...
  /** Change the current code of the instance.
   * \param rhs New code value.
   * \param pErrorMsg Optional error description string. This string can have
   * formatting characters The function will use the vsnprintf function to
   * assemble the final string using an internal buffer of 4096 characters.
   */
