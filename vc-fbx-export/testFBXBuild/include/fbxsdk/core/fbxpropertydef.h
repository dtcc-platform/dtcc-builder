...
public:
  ...
  {
    eNone = 0,        //!< No flag.
    eStatic = 1 << 0, //!< Property is defined in the class declaration, so it
                      //!< wasn't created dynamically.
    eAnimatable = 1 << 1, //!< Property can be animated, thus is can have am
                          //!< animation curve node connected.
    eAnimated = 1 << 2,   //!< Property is animated, so it also has an animation
                          //!< curve node connected.
    eImported = 1 << 3,   //!< Property has been created during import process
                          //!< when reading FBX file.
    eUserDefined =
        1 << 4, //!< Property has been defined by user, not by the FBX SDK.
    eHidden = 1 << 5, //!< Property should not be displayed on user interface.
    eNotSavable =
        1 << 6, //!< Property value must not be exported when writing FBX files.
    ...
    eLockedAll =
        eLockedMember0 | eLockedMember1 | eLockedMember2 | eLockedMember3,
    ...
    eCtrlFlags = eStatic | eAnimatable | eAnimated | eImported | eUserDefined |
                 eHidden | eNotSavable | eLockedAll | eMutedAll,
  ...
  GetMergedFlags(FbxPropertyFlags::EFlags pFlags) const;
  bool ModifyFlags(FbxPropertyFlags::EFlags pFlags, bool pValue);
  FbxPropertyFlags::EInheritType
  GetFlagsInheritType(FbxPropertyFlags::EFlags pFlags) const;
  ...
  bool Equal(const FbxPropertyFlags &pOther,
             FbxPropertyFlags::EFlags pFlags) const;
...
*****************************************************************************************************************************/
  ...
  static const int sLockedMembersMax =
      4; // Maximum number of property sub-member that can be locked.
  static const int sLockedMembersBitOffset =
      7; // Number of bits to shift to get to the first locked member flag.
  static const int sMutedMembersMax =
      4; // Maximum number of property sub-member that can be muted.
  static const int sMutedMembersBitOffset =
      11; // Number of bits to shift to get to the first muted member flag.
