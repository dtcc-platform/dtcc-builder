 ...
 *
 * It is possible to override memory allocation functions throughout the FBX SDK
 * by providing system memory allocation functions using the handler set
 * functions below. The Microsoft Windows implementation in debug mode allows to
 * specify where the allocations happen by providing the standard block type,
...
#define FBXSDK_MEMORY_COPY(dst, src, size)                                     \
  {                                                                            \
    memcpy(dst, src, size);                                                    \
  }

typedef void *(*FbxMallocProc)(
    size_t); //! Function pointer signature used to replace "malloc"
typedef void *(*FbxCallocProc)(
    size_t, size_t); //! Function pointer signature used to replace "calloc"
typedef void *(*FbxReallocProc)(
    void *, size_t); //! Function pointer signature used to replace "realloc"
typedef void (*FbxFreeProc)(
    void *); //! Function pointer signature used to replace "free"

/** Set the global memory allocation function used internally by the FBX SDK.
 * \param pHandler Function pointer that implements the necessary procedure to
 * allocate memory in the system. */
FBXSDK_DLL void FbxSetMallocHandler(FbxMallocProc pHandler);

/** Set the global zero'd memory allocation function used internally by the FBX
 * SDK.
 * \param pHandler Function pointer that implements the necessary procedure to
 * allocate zero'd memory in the system. */
 ...
 * \param pHandler Function pointer that implements the necessary procedure to
 * re-allocate memory in the system. */
...
/** Get the global zero'd memory allocation function used internally by the FBX
 * SDK. \return pHandler Function pointer on FBX's internal calloc */
...
/** Get the default global memory allocation function used internally by the FBX
 * SDK. \return pHandler Function pointer on FBX's internal malloc */
FBXSDK_DLL FbxMallocProc FbxGetDefaultMallocHandler();

/** Get the default global zero'd memory allocation function used internally by
 * the FBX SDK. \return pHandler Function pointer on FBX's internal calloc */
FBXSDK_DLL FbxCallocProc FbxGetDefaultCallocHandler();

/** Get the default global memory re-allocation function used internally by the
 * FBX SDK. \return pHandler Function pointer on FBX's internal realloc */
FBXSDK_DLL FbxReallocProc FbxGetDefaultReallocHandler();

/** Get the default global memory freeing function used internally by the FBX
 * SDK. \return pHandler Function pointer on FBX's internal free */
...
// These versions of allocators use the default system mallocs, and on Windows
// we also pass the debugging parameters. If you define FBXSDK_ALLOC_DEBUG in
// your project, the FBX SDK will use these debug versions everywhere.
FBXSDK_DLL void *
FbxMallocDebug(size_t pSize, int pBlock, const char *pFile, int pLine);
FBXSDK_DLL void *FbxCallocDebug(
    size_t pCount, size_t pSize, int pBlock, const char *pFile, int pLine);
FBXSDK_DLL void *FbxReallocDebug(
    void *pData, size_t pSize, int pBlock, const char *pFile, int pLine);
FBXSDK_DLL void FbxFreeDebug(void *pData, int pBlock);

// When FBXSDK_ALLOC_DEBUG is defined, redirect allocation calls to the debug
// version.
...
#define FbxRealloc(p, s)                                                       \
  FbxReallocDebug(p, s, _NORMAL_BLOCK, __FILE__, __LINE__)
...
//! Deletion policy for pointer template classes that uses the FbxDelete()
//! function.
...
//! Deletion policy for pointer template classes that uses the FbxFree()
//! function.
...
//! Deletion policy for pointer template classes that uses the Destroy()
//! function.
...
/** FbxAutoPtr mimics the \c auto_ptr class template implementation available in
 * the C++ Standard Library. The \c auto_ptr template class describes an object
 * that stores a pointer to a single allocated object of type Type* that ensures
...
//! Scoped pointer for FbxObject derived classes, which call Destroy() to
//! deallocate.
template <class Type>
...
/** FbxSharedPtr class describes an object that stores a pointer to a single
 * allocated object of type Type* that ensures that the object to which it
 * points gets destroyed automatically when the control leaves a scope and the
...
public:
  // Default constructor.
  FbxSharedPtr() : mPtr(0), mRef(0) {}

  //! Construct from a pointer.
  explicit FbxSharedPtr(Type *pPtr) : mPtr(pPtr), mRef(0)
  {
    if (pPtr != 0)
    {
  ...
  //! Copy constructor
  FbxSharedPtr(const FbxSharedPtr &pSPtr) : mPtr(pSPtr.mPtr), mRef(pSPtr.mRef)
  {
...
//! Scoped pointer for FbxNew allocations, which call FbxDelete() to deallocate.
template <class Type>
class FbxSharedDeletePtr
    : public FbxSharedPtr<Type, FbxDeletionPolicyDelete<Type>>
{
public:
  //! Construct from a pointer.
...
//! deallocate.
template <class Type>
class FbxSharedDestroyPtr
    : public FbxSharedPtr<Type, FbxDeletionPolicyObject<Type>>
{
public:
  //! Construct from a pointer.
