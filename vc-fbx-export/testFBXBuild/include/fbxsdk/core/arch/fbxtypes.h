 ...
 *
 * Standard basic types used across the FBX SDK. There is also platform
 * independent definitions that guarantee size across operating systems. The
 * FBXSDK_SYSTEM_IS_LP64 define is set to 1 when the operating system defines
 * the "long" C++ type as 64-bit.
 */
...
// it is still a 32-bits for backward compatibility. We stick with Windows
// standard.
...
template <class T> inline T FbxMin(const T x, const T y)
{
  return (x < y) ? x : y;
}
template <class T> inline T FbxMax(const T x, const T y)
{
  return (x > y) ? x : y;
}
...
public:
  ...
  {
    return ((mData[0] == pVector.mData[0]) && (mData[1] == pVector.mData[1]) &&
            (mData[2] == pVector.mData[2]) && (mData[3] == pVector.mData[3]));
  }
  inline bool operator!=(const FbxVectorTemplate4<T> &pVector) const
  {
    return !operator==(pVector);
