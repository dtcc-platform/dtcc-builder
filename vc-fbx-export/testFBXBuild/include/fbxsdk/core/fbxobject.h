...
#define FBXSDK_FBXOBJECT_DECLARE(Class, Parent)                                \
  ...
  static Class *Create(FbxObject *pContainer, const char *pName);              \
                                                                               \
protected:                                                                     \
  static Class *Allocate(FbxManager *pManager, const char *pName,              \
                         const Class *pFrom);
...
#define FBXSDK_OBJECT_IMPLEMENT(Class)                                         \
  ...
  {                                                                            \
    return (Class *)pManager->CreateNewObjectFromClassId(Class::ClassId,       \
                                                         pName);               \
  }                                                                            \
  Class *Class::Create(FbxObject *pContainer, const char *pName)               \
  {                                                                            \
    FBX_ASSERT_RETURN_VALUE(pContainer && pContainer->GetFbxManager(), NULL);  \
    return (Class *)pContainer->GetFbxManager()->CreateNewObjectFromClassId(   \
        Class::ClassId, pName, pContainer);                                    \
  }                                                                            \
  Class *Class::Allocate(FbxManager *pManager, const char *pName,              \
                         const Class *pFrom)                                   \
  {                                                                            \
...
/** The base class of most FBX objects. Provides the benefits of connectivity,
 * identity, run-time typing, properties, naming, copying, cloning, selection,
 * and automated file IO. Most of the FBX SDK API deals with FbxObject pointers
 * when it comes to manipulate objects in its simplest form.
 *
 * The ClassID mechanism replaces the dynamic_cast mechanism for efficient
 * run-time type information.
 *
 * The FbxObject provides methods for managing the connections between objects.
 * Using connections, objects can be related to each other to form hierarchies
 * or structures. All of the FBX scene's object relations are expressed as
 * connections between objects. Those connections can be altered as needed to
 * reflect most kind of setups encountered in this world. For example,
 * connections can be used to express parenting between transform nodes.
 * Connections are not strict in the sense that we allow any type of objects to
 * connect to any other type of objects. The meaning of the connection is purely
 * semantic. As of yet, we do not provide the functionality to validate if the
 * connections made by the users are allowed or not.
 *
 * FbxObject provide a property (FbxProperty) mechanism to describe
 * characteristics of objects in a scene. Properties may be either static or
 * dynamic. Static properties are defined in the class direction and can be
 * accessed directly by their name on the object exposing them without the need
 * for a search in the property list of the object. Dynamic properties can be
 * added during run-time, while the program is running. Objects can have an
 * unlimited amount of properties. Properties can be listed at run-time,
 * allowing for a flexible support of custom data on objects, since they might
 * be considered by the FBX file readers/writers depending on the flags set.
 *
 ...
 * {
 *     FBXSDK_OBJECT_DECLARE(MyClass, FbxObject);	//Be careful! The second
...
public:
  ...
  //@{
  /** Templated test if this class is a hierarchical children of the specified
   * class type. \return \c true if the object is a hierarchical children of the
   * type specified.
   * \remark This function will perform a complete search until it reaches the
   * top level class, but it will stop as soon as one ClassId matches the test.
   */
   ...
   * \return A const pointer to the document that contains this object or \c
   * NULL if the object does not belong to any document. */
  FbxDocument *GetDocument() const;

  /** Returns a const pointer to the root document that contains this object.
   * \return A const pointer to the root document that contains this object or
   * \c NULL if the object does not belong to any document.
   * \remarks It returns this pointer if this object is a document object and
   * does not belong to any document. That means this object is the root
   * document. */
  ...
  /** Unregister and delete this object from memory. This will also breaks all
   * connections as well as removing all the instance of all the properties of
   * this object with the object's class. \param pRecursive If true, all
   * children (source) objects will also be unregistered and deleted. */
  ...
  enum EObjectFlag
  {
    eNone = 0, //!< No flags.
    eInitialized =
        1 << 0, //!< Automatically set when FbxObject::Construct() is completed.
    eSystem = 1 << 1,  //!< When set, object is deleted upon FbxManager destroy
                       //!< only. Use carefully!
    eSavable = 1 << 2, //!< If set, object is stored in FBX file upon export.
                       //!< All objects are savable by default.
    eSelected =
        1
        << 3, //!< Used by the selection mechanic to specify a selected object.
    eHidden = 1 << 4, //!< Used for interface representation; if set, the object
                      //!< should not be visible.
    eContentLoaded = 1 << 5, //!< Used by load-on-demand mechanic to specify if
                             //!< an object has its content loaded.
    eDontLocalize =
        1 << 6, //!< Used by asset builder; Do not localize this object
    eCopyCalledByClone = 1 << 16 //!< Used internally. If set, modify the Copy
                                 //!< behavior of the object
  };

  /** Set the state of object flags.
   * \param pFlags Bit flags which value is going to be changed.
   * \param pValue If \c true, bit flags will be set, otherwise bits will be
   * un-set. */
   ...
   * \remark This function will override all flags; unspecified bit flags will
   * be un-set. */
  ...
  enum ECloneType
  {
    eDeepClone, //!< A deep copy of the object. Changes to either the original
                //!< or clone properties do not propagate to each other.
    eReferenceClone //!< Changes to original object properties propagate to
                    //!< clone. Changes to clone properties do not propagate to
                    //!< original.
  };

  /** Creates a clone of this object.
   * By default, the connections are NOT cloned. If the desired effect is to
   *clone the connections as well, you must clone using the FbxCloneManager
   * (refer to this class documentation for further details).
   *
   * \param pCloneType	    The type of clone to be created. By default, the
   *clone type is eDeepClone. \param pContainer	    An optional parameter to
  ...
  virtual FbxObject *Clone(FbxObject::ECloneType pCloneType = eDeepClone,
                           ...
                           void *pSet = NULL) const;

  /** Checks if this object is a reference clone of another object.
   * \return \c True if this object is a clone of another object, \c false
   * otherwise */
  bool IsAReferenceTo() const;

  /** If this object is a reference clone, returns the original object (from
   * which the clone originates).
   * \return The original object, or NULL (if this object is not a reference
   * clone). */
  FbxObject *GetReferenceTo() const;

  /** Checks if any objects are reference cloned from this object.
   * \return \c True if there are objects reference cloned from this object, \c
   * false otherwise. */
   ...
   * If the user data record identified by pUserID does not exist, a new user
   * data record is created and its data pointer is set as pUserData.
   */
   ...
   * \param pUserID The ID of the user data record.
   * \return The data pointer of the user data record, \c NULL if the user data
   * record is not found.
   */
  ...
  /** Returns the data pointer of the user data record whose ID is the object
   * ID. \return The data pointer of the user data record, \c NULL if the user
   ...
   */
  inline bool
  ConnectSrcObject(FbxObject *pObject,
   ...
   * \return \c True if this object connects with the source object, \c false
   * otherwise.
   */
   ...
   * \param pObject The source object from which this object will be
   * disconnected. \return \c True on success, \c false otherwise.
   */
   ...
   * \return \c True if it disconnects all source objects successfully, \c false
   * otherwise.
   */
  ...
  /** Disconnects this object from all source objects that satisfy a given
   * criteria. \param pCriteria The given criteria. \return \c True if it
   * disconnects all the source objects successfully, \c false otherwise.
   */
  ...
  /** Returns the number of source objects that satisfy the given criteria with
   * which this object connects. \param pCriteria The given criteria. \return
   * The number of source objects that satisfy the given criteria with which
  ...
  /** Returns the source object with which this object connects at the specified
   * index. \param pIndex The specified index whose default value is 0. \return
  ...
  /** Returns the source object that satisfies the criteria at the specified
   * index with which this object connects. \param pCriteria The given criteria.
   * \param pIndex The specified index whose default value is 0.
   * \return The source object that satisfies the given criteria at the
   * specified index, NULL if not found.
   */
  inline FbxObject *GetSrcObject(const FbxCriteria &pCriteria,
                                 int pIndex = 0) const
  {
    return RootProperty.GetSrcObject(pCriteria, pIndex);
  }

  /** Searches the source object with the specified name, starting at the
   * specified index. \param pName The object name. \param pStartIndex The start
   * index. \return The source object with the name, NULL if not found.
   */
  ...
  /** Searches the source object with the specified name which satisfies the
   * given criteria, starting at the specified index. \param pCriteria The given
   * criteria. \param pName The object name. \param pStartIndex The start index.
   * \return The source object with the name, NULL if not found.
   */
  inline FbxObject *FindSrcObject(const FbxCriteria &pCriteria,
                                  const char *pName,
                                  int pStartIndex = 0) const
  {
    return RootProperty.FindSrcObject(pCriteria, pName, pStartIndex);
  }

  /** Disconnects this object from all source objects of the specified class
   * type.
   * \return \c true if it disconnects all source objects successfully, \c false
   * otherwise. */
  template <class T> inline bool DisconnectAllSrcObject()
  {
    return RootProperty.DisconnectAllSrcObject(
        FbxCriteria::ObjectType(T::ClassId));
  }

  /** Disconnects this object from all source objects that are of the specified
   * class type and that satisfy the given criteria. \param pCriteria The given
   * criteria. \return \c true if it disconnects all source objects
  ...
  /** Returns the number of source objects of a specific class type with which
   * this object connects.
   * \return The number of source objects of the specified class type with which
   * this object connects. */
  template <class T> inline int GetSrcObjectCount() const
  {
    return RootProperty.GetSrcObjectCount(FbxCriteria::ObjectType(T::ClassId));
  }

  /** Returns the number of source objects with which this object connects that
   * are the specified class type and that satisfy the given criteria. \param
   * pCriteria The given criteria.
   * \return The number of source objects that are the specified class type and
  ...
  /** Returns the source object of the specified class type at the specified
   * index. \param pIndex The specified index whose default value is 0.
   * \return The source object of a specified class type at the specified index,
   * NULL if not found. */
  template <class T> inline T *GetSrcObject(int pIndex = 0) const
  {
    return (T *)RootProperty.GetSrcObject(FbxCriteria::ObjectType(T::ClassId),
                                          pIndex);
  }

  /** Returns the source object that is the specified class type and that
   * satisfies the given criteria at the specified index. \param pCriteria The
   * given criteria. \param pIndex The specified index whose default value is 0.
   * \return The source object that is of the specified class type and that
   * satisfies the given criteria at the specified index, NULL if not found. */
  template <class T>
  inline T *GetSrcObject(const FbxCriteria &pCriteria, int pIndex = 0) const
  {
    return (T *)RootProperty.GetSrcObject(
        FbxCriteria::ObjectType(T::ClassId) && pCriteria, pIndex);
  }

  /** Searches the source object with the specified name that is the specified
   * class type, starting at the specified index. \param pName The object name.
   * \param pStartIndex The start index.
   * \return The source object with the name, NULL if not found. */
  template <class T>
  inline T *FindSrcObject(const char *pName, int pStartIndex = 0) const
  {
    return (T *)RootProperty.FindSrcObject(FbxCriteria::ObjectType(T::ClassId),
                                           pName, pStartIndex);
  }

  /** Searches the source object with the specified name that is the specified
   * class type and that satisfies the given criteria, starting at the specified
   * index. \param pCriteria The given criteria. \param pName The object name.
   * \param pStartIndex The start index.
   * \return The source object with the name, NULL if not found. */
  template <class T>
  inline T *FindSrcObject(const FbxCriteria &pCriteria,
                          const char *pName,
                          int pStartIndex = 0) const
  {
    return (T *)RootProperty.FindSrcObject(
   ...
   * \param pType The connection type between this object and the destination
   * object. \return \c True on success, \c false otherwise.
   */
  inline bool
   ...
   * \return \c True if this object connects with the destination object, \c
   * false otherwise.
   */
   ...
   * \return \c True if it disconnects all destination objects successfully, \c
   * false otherwise.
   */
  ...
  /** Disconnects this object from all destination objects that satisfy given
   * criteria. \param pCriteria The given criteria. \return \c True if it
   * disconnects all destination objects successfully, \c false otherwise.
   */
  ...
  /** Returns the number of destination objects with which this object connects
   * that satisfy the given criteria. \param pCriteria The given criteria.
   * \return The number of destination objects with which this object connects
   * that satisfy the given criteria.
   */
  ...
  /** Returns the destination object at the specified index with which this
   * object connects. \param pIndex The specified index whose default value is
   * 0. \return The destination object at the specified index, NULL if not
   * found.
   */
  ...
  /** Returns the destination object with which this object connects that
   * satisfies the given criteria at the specified index. \param pCriteria The
   * given criteria. \param pIndex The specified index whose default value is 0.
   * \return The destination object that satisfies the given criteria at the
   * specified index, NULL if not found.
   */
  inline FbxObject *GetDstObject(const FbxCriteria &pCriteria,
                                 int pIndex = 0) const
  {
    return RootProperty.GetDstObject(pCriteria, pIndex);
  }

  /** Searches the destination object with the specified name, starting at the
   * specified index. \param pName The object name. \param pStartIndex The start
   * index. \return The destination object with the name, NULL if not found.
   */
  ...
  /** Searches the destination object with the specified name which satisfies
   * the given criteria, starting at the specified index. \param pCriteria The
   * given criteria. \param pName The object name. \param pStartIndex The start
   * index. \return The destination object with the name, NULL if not found.
   */
  inline FbxObject *FindDstObject(const FbxCriteria &pCriteria,
                                  const char *pName,
                                  int pStartIndex = 0) const
  {
  ...
  /** Disconnects this object from all destination objects of the specified
   * class type. \return \c true if it disconnects all destination objects of
   * the specified class type successfully, \c false otherwise. */
  template <class T> inline bool DisconnectAllDstObject()
  {
    return RootProperty.DisconnectAllDstObject(
        FbxCriteria::ObjectType(T::ClassId));
  }

  /** Disconnects this object from all destination objects that are the
   * specified class type and that satisfy the given criteria. \param pCriteria
   * The given criteria. \return \c true if it disconnects all destination
   * objects successfully, \c false otherwise. */
  template <class T>
  inline bool DisconnectAllDstObject(const FbxCriteria &pCriteria)
  {
    return RootProperty.DisconnectAllDstObject(
  ...
  /** Returns the number of destination objects of the specified class type with
   * which this object connects. \return The number of destination objects of
   * the specified class type with which this object connects. */
  template <class T> inline int GetDstObjectCount() const
  {
    return RootProperty.GetDstObjectCount(FbxCriteria::ObjectType(T::ClassId));
  }

  /** Returns the number of destination objects with which this object connects
   * that are the specified class type and that satisfy the given criteria.
   * \param pCriteria The given criteria.
   * \return The number of destination objects that are the specified class type
  ...
  /** Returns the destination object with which this object connects that is the
   * specified class type at the specified index. \param pIndex The specified
   * index whose default value is 0. \return The destination object of the
   * specified class type at the specified index, NULL if not found. */
  template <class T> inline T *GetDstObject(int pIndex = 0) const
  {
    return (T *)RootProperty.GetDstObject(FbxCriteria::ObjectType(T::ClassId),
                                          pIndex);
  }

  /** Returns the destination object with which this object connects that is the
   * specified class type and that satisfies the given criteria at the specified
   * index. \param pCriteria The given criteria. \param pIndex The specified
   * index whose default value is 0.
   * \return The destination object that is the specified class type and that
   * satisfies the given criteria at the specified index, NULL if not found. */
  template <class T>
  inline T *GetDstObject(const FbxCriteria &pCriteria, int pIndex = 0) const
  {
    return (T *)RootProperty.GetDstObject(
        FbxCriteria::ObjectType(T::ClassId) && pCriteria, pIndex);
  }

  /** Searches the destination object with the specified name which is of the
   * specified class type, starting at the specified index. \param pName The
   * object name. \param pStartIndex The start index.
   * \return The source object with the name, NULL if not found. */
  template <class T>
  inline T *FindDstObject(const char *pName, int pStartIndex = 0) const
  {
    return (T *)RootProperty.FindDstObject(FbxCriteria::ObjectType(T::ClassId),
                                           pName, pStartIndex);
  }

  /** Searches the destination object with the specified name that is the
   * specified class type and that satisfies the given criteria, starting at the
   * specified index. \param pCriteria The given criteria. \param pName The
   * object name. \param pStartIndex The start index.
   * \return The source object with the name, NULL if not found. */
  template <class T>
  inline T *FindDstObject(const FbxCriteria &pCriteria,
                          const char *pName,
                          int pStartIndex = 0) const
  {
    return (T *)RootProperty.FindDstObject(
        FbxCriteria::ObjectType(T::ClassId) && pCriteria, pName, pStartIndex);
  }
  //@}
  ...
  /** Returns the next property of this object that follows the specified
   * property. \param pProperty The specified property. \return The next
   * property of this object that follows pProperty.
   */
   ...
   * \return A valid FbxProperty if found, else an invalid FbxProperty. See
   * FbxProperty::IsValid()
   */
  inline FbxProperty FindProperty(const char *pName,
                                  bool pCaseSensitive = true) const
  {
   ...
   * \param pCaseSensitive Whether the name is case-sensitive.
   * \return A valid FbxProperty if the property is found, else an invalid
   * FbxProperty. See FbxProperty::IsValid()
   */
  inline FbxProperty FindProperty(const char *pName,
   ...
   */
  inline FbxProperty FindPropertyHierarchical(const char *pName,
                                              bool pCaseSensitive = true) const
  {
   ...
   * \return The class root property if it exists, else an invalid FbxProperty.
   * See FbxProperty::IsValid(). \remarks Class FbxObject and its sub-classes
   * all have a class root property. This class root property contains basic
   * information about the class type, such as the class name.
   */
  ...
  /** Determines whether this object connects with the specified source
   * property. \param pProperty The specified source property. \return \c True
   * if this object connects with the specified source property, \c false
   * otherwise.
   */
  ...
  /** Returns the source property at the specified index with which this object
   * connects. \param pIndex The specified index. \return The source property at
   * the specified index.
   */
  ...
  /** Searches a source property with which this object connects that has a
   * specific name, starting at the specified index. \param pName The specified
   * property name. \param pStartIndex The start index. \return The source
   * property with the specified name.
   */
  inline FbxProperty FindSrcProperty(const char *pName,
                                     int pStartIndex = 0) const
  {
  ...
  /** Determines if this object connects with the specified destination
   * property. \param pProperty The specified destination property. \return \c
   * True if this object connects with the specified destination property, \c
   * false otherwise.
   */
  ...
  /** Returns the number of destination properties with which this object
   * connects. \return The number of destination properties with which this
   * object connects.
   */
  ...
  /** Returns the destination property at the specified index with which this
   * object connects. \param pIndex The specified index. \return The destination
  ...
  /** Searches a destination property with which this object connects that has a
   * specific name, starting at the specified index. \param pName The specified
   * property name. \param pStartIndex The start index. \return The destination
   * property with the specified name.
   */
  inline FbxProperty FindDstProperty(const char *pName,
                                     int pStartIndex = 0) const
  {
   ...
   *         method. The ContentIsUnloaded() method below (implemented in this
   * class) is simply a synonym of GetObjectFlags(eCONTENT_UNLOADED_FLAG)
   */
  //@{
  /** Unloads this object's content using the offload peripheral that is
   * currently set in the document then flushes it from memory. \return 2 if the
   * object's content is already unloaded or 1 if this object's content has been
   * successfully unloaded to the current peripheral.
   *
   * \remarks If the content is locked more than once, or the peripheral cannot
   * handle this object's unloading, or if an error occurs, this method returns
   * 0 and does not flush the content.
   */
  int ContentUnload();

  /** Loads this object's content using the offload peripheral that is currently
   * set in the document. \return 1 if this object's content has been
   * successfully loaded from the current peripheral, 2 if the content is
   * already loaded, and 0 if an error occurs or the object's content is locked.
  ...
  /** Decreases the content lock count of an object. If the content lock count
   * of an object is greater than 0, the content of the object is considered
   * locked.
   */
  void ContentDecrementLockCount();

  /** Increases the content lock count of an object. If the content lock count
   * of an object is greater than 0, the content of the object is considered
   * locked.
   */
  void ContentIncrementLockCount();

  /** Judges if this object's content is locked. The content is considered
   * locked if the content lock count is greater than 0 \return \c True if this
   * object's content is locked, \c false otherwise. \remarks A locked state
   * prevents the object content from being unloaded from memory but does not
   * block the loading.
   */
   ...
   */
  //@{
  /** Emits a message in all available message emitters in the document or SDK
   * manager. \param pMessage The message to emit. \remarks The ownership of the
   * message is transferred, don't delete it.
   */
   ...
   */
  //@{
  /** Localization helper function, it calls the implementation of FBX SDK
   * manager. Sub-classes that manage their own localization could over-ride
   * this function. \param pID The identifier of the text to be localized.
   * \param pDefault The default text. Uses pID as the default text if pDefault
   * is NULL. \return The localized text or the default text if the text can't
   * be localized, .
   */
   ...
   * \return The parent library of this object, or \c NULL if the parent library
   * doesn't exist.
   */
   ...
   * \return \c True on success, \c false otherwise.
   * \remarks To succeed this function must be called with an implementation
   * that has not already been added to this node.
   */
   ...
   * \return \c True on success, \c false otherwise.
   * \remarks To succeed this function must be called with an implementation
   *that has already been added to this node. Only the implementation which has
   *already been added can be set as the default implementation.
   */
  ...
  int GetImplementationCount(
      const FbxImplementationFilter *pCriteria = NULL) const;

  /** Returns the implementation at the specified index that satisfies the given
   * criteria. \param pIndex The specified index. \param pCriteria The given
   * criteria. \return The implementation at the specified index, NULL if not
   * found.
   */
  FbxImplementation *
  GetImplementation(int pIndex,
                    const FbxImplementationFilter *pCriteria = NULL) const;
  //@}
  ...
  //@{
  /** Set the run-time ClassId for this class. In most contexts, users do not
   * have to change the run-time ClassId, they are automatically generated when
   * registered a new class during run-time. \param pClassId	The ClassId to
   * set as the run-time ClassId for this object. */
  ...
  /** Find out if the ClassId was registered during run-time rather than at
   * compile time. \return True if the run-time ClassId is inequal to the
   * ClassId. */
  bool IsRuntimePlug() const;
  //@}

  /** Compact the memory used by this object.
   * \remark Note that this function might not result in saved memory because it
   * depends if the sub-class implements it, or if any memory can actually be
   * saved. */
...
protected:
   ...
   * \remark This method is called by ContentUnload() if the object content's
   * unloading is successful. */
  virtual void ContentClear();

  /** Retrieves the peripheral of that object.
   * \return	The current peripheral for that object
   * \remark	A peripheral manipulates the content of an object. For instance,
   * a peripheral can load the connections of an object on demand. */
...
public:
  ...
  // This function will go as deep as possible to clear the Connection list
  // without sending notifications to the connections to give them the chance to
  // Disconnect themselves. This is a bypass of the intended workflow and should
  // be used with care.
  void WipeAllConnections();

  // Used as global flag to modify the behavior of FbxObject::Destruct() during
  // a ForceKill() on the scene. This is for internal use.
...
/** A utility class for iterating over source objects that connect to property
 * (FbxProperty) or object (FbxObject). \nosubgrouping
 */
...
public:
  /**
   * Constructor.
   * \param pProperty Property object. The iterator will iterate source objects
   * that connect to it. \param pClassId	The class ID specifies the type
   * of the source objects.
   */
  inline FbxIteratorSrcBase(FbxProperty &pProperty, FbxClassId pClassId)
      : mProperty(pProperty), mClassId(pClassId), mSize(0), mIndex(-1)
  {
    ResetToBegin();
  }
  ...
  inline FbxIteratorSrcBase(FbxObject *pObject, FbxClassId pClassId)
      : mProperty(pObject->RootProperty), mClassId(pClassId), mSize(0),
        mIndex(-1)
  {
    ResetToBegin();
  }
  ...
  inline FbxObject *GetNext()
  {
    ...
    return ((mIndex >= 0) && (mIndex < mSize))
               ? mProperty.GetSrcObject(FbxCriteria::ObjectType(mClassId),
                                        mIndex)
               : NULL;
  }
...
protected:
  ...
  FbxProperty mProperty; //! The property to iterate. If iterate an object, this
                         //! is the root property of the object.
  FbxClassId mClassId; //! The class ID specifies the type of the source objects
                       //! to be retrieved.
  int mSize;  //! The number of source objects whose type is specified by
              //! mClassId.
...
/**
 * A utility class for iterating over source objects that connect to property
 * (FbxProperty) or object (FbxObject). The class is a wrapper of
 * FbxIteratorSrcBase with template. \nosubgrouping
 */
...
/** A utility class for iterating over destination objects that connect to
 * property (FbxProperty) or object (FbxObject). \nosubgrouping
 */
class FbxIteratorDstBase
{
protected:
  /** The property to iterate. If iterate an object, this is the root property
   * of the object. */
  FbxProperty mProperty;
  /** The class ID specifies the type of the destination objects to be
   * retrieved. */
  FbxClassId mClassId;
...
public:
  /**
   * Constructor.
   * \param pProperty Property object. The iterator will iterate destination
   * objects that connect to it. \param pClassId	The class ID specifies
   ...
   * \return The next destination object. If there are no more objects, return
   * NULL.
   */
  inline FbxObject *GetNext()
  {
    mIndex++;
    return ((mIndex >= 0) && (mIndex < mSize))
               ? mProperty.GetDstObject(FbxCriteria::ObjectType(mClassId),
                                        mIndex)
               : NULL;
  }
   ...
   * Get next destination object that connects to the property or object.
   * \return The next destination object. If there are no more objects, return
   * NULL. \remark This method makes sure the iterate index is not out of
   * bounds.
   */
  inline FbxObject *GetSafeNext()
  {
   ...
   * \return The previous destination object. If there are no more objects,
   * return NULL.
   */
  inline FbxObject *GetPrevious()
  {
   ...
   * Get previous destination object that connects to the property or object.
   * \return The previous destination object. If there are no more objects,
   *return NULL. \remark This method makes sure the iterate index is not out of
   *bounds. If the iterate index is out of bounds, the last destination object
   *is returned.
   */
  inline FbxObject *GetSafePrevious()
  {
...
/**
 * A utility class for iterating over destination objects that connect to
 * property (FbxProperty) or object (FbxObject). The class is a wrapper of
 * FbxIteratorDstBase with template. \nosubgrouping
 */
...
/** Convert the class type parameter into a C class parameter for other function
 * inputs. Usage example: \code
...
/** Safe casting of FBX SDK objects into other FBX SDK class types. This cast
 * will perform the complete test to make sure the object inherits from the
 * requested class type. This is the equivalent of a dynamic_cast but much
 * faster. \param pObject	The object to try to cast into T type. \return
...
/** Safe const casting of FBX SDK objects into other FBX SDK class types. This
 * cast will perform the complete test to make sure the object inherits from the
 * requested class type. This is the equivalent of a dynamic_cast but much
 * faster. \param pObject	The object to try to cast into T type. \return
 * A non-null pointer if the cast was successful.
 */
...
//! Macro used to iterate over source or destination objects that connect to
//! property (FbxProperty) or object (FbxObject).
#define FbxForEach(Iterator, Object)                                           \
  for ((Object) = (Iterator).GetFirst(); (Object) != 0;                        \
       (Object) = (Iterator).GetNext())

//! Macro used to reversely iterate over source or destination objects that
//! connect to property (FbxProperty) or object (FbxObject)
#define FbxForEachReverse(Iterator, Object)                                    \
  for (Object = (Iterator).GetLast(); (Object) != 0;                           \
       Object = (Iterator).GetPrevious())
...
public:
  ...
  inline FbxConnectEvent(EType pType,
                         EDirection pDir,
                         FbxProperty *pSrc,
                         FbxProperty *pDst)
  ...
  template <class T> inline T *GetSrcIfObject() const
  {
    return mSrc->IsRoot() ? FbxCast<T>(mSrc->GetFbxObject()) : (T *)0;
  }
  template <class T> inline T *GetDstIfObject() const
  {
