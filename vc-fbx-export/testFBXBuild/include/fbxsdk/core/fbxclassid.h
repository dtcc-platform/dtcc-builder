...
typedef FbxObject *(*FbxObjectCreateProc)(FbxManager &pManager,
                                          const char *pName,
                                          const FbxObject *pFrom);

/** Internal class used to differentiate objects during run-time. Essentially,
 * each class has an unique ClassId, that the system can request in order to
 * test if the class match the description. This class implement the necessary
 * tools to be able to perform hierarchic class testing. This means that a class
 * B that inherits from the class A will answer yes to a "Is A" query of type A
 * or B, but will answer no to a class C that can still inherit from A. All
 * class must inherit from FbxObject before they can have their own ClassId.
 * When using the standard macros to create new types of objects in the FBX SDK,
 * a static ClassId will automatically be generated for that new class.
 *
 * When objects are exported to an FBX file, their class type is maintained
 * using 3 sort of strings. They are the Object Type string, the Object Sub Type
 * string and the Object Type Prefix. There is no good or bad way to choose the
 * value of these identifiers, but it is preferable to use meaningful values to
 * keep the ASCII version of FBX readable and easy to understand.
 * \see FbxObject */
...
public:
  ...
  /** Advanced constructor were we can specify the general parameters for this
   * ClassId. \param pClassName The name of the class represented. \param
   * pParentClassId The parent ClassId of this class. \param pConstructor A
   * function pointer to a construction method for this ClassId. \param pFBXType
   * The FBX file Object Type string associated to this class. \param
   ...
   * \param pManager The FBX SDK Manager to be used to instantiate this object.
   * This allow the object to use the same memory manager as the provided
   * manager. \param pName The name to assign to this new object instance.
   * \param pFrom An object to clone if it matches the same ClassId. This is an
   * optional parameter. \return The newly created instance of this class. */
  FbxObject *
  Create(FbxManager &pManager, const char *pName, const FbxObject *pFrom);

  /** Override the function pointer method to construct this object.
   * \param pConstructor A newly defined function pointer to a construction
   * method to replace the existing one. \return True if the operation was
   * successful. */
  bool Override(FbxObjectCreateProc pConstructor);

  /** Test if this class is a hierarchical children of the specified class type.
   * This is the standard method to differentiate object classes. \param pId The
   * class type to test against self. \return True if the object is a
   * hierarchical children of the type specified. \remark This function will
   * perform a complete search until it reaches the top level class, but it will
   * stop as soon as one ClassId matches the test. */
   ...
   * \remark This function only perform direct equality test, and doesn't test
   * hierarchic children. */
   ...
   * \remark This function only perform direct inequality test, and doesn't test
   * hierarchic children. */
  bool operator!=(const FbxClassId &pClassId) const;

  /** Retrieve the FBX file Object Type string associated to this class.
   * \param pAskParent If \c true, retrieve the parent ClassId, but only if self
  ...
  /** Set the Object Type Prefix string associated to this class. This will
   * change the "ObjectTypePrefix::" found in front of object name in the FBX
   * file. This is useful to differentiate objects by their name without using
   * the Object Type or Sub Type strings in the file. \param pObjectTypePrefix
  ...
  /** Retrieve the root property handle of this class. This is useful to access
   * the default property hierarchy for this class. This allow users to retrieve
   * information such as the default value for all properties of this class.
