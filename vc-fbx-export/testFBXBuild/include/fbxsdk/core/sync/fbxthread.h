...
public:
   ...
   * \return Return true if the thread was successfully suspended, otherwise
   * false. \remarks It should be used only if you can control where the thread
   * will be suspended in its procedure, otherwise the state of the thread and
   * its memory is unknown, since the code will stop anywhere.
   */
  bool Suspend();

  /** Resume the execution of the thread.
   * \return Return true if the thread was successfully resumed, otherwise
   * false.
   */
  bool Resume();
