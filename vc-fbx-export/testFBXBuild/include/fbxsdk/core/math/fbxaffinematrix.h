 ...
 * \nosubgrouping
 * Matrices are defined using the Column Major scheme. When a FbxAMatrix
 * represents a transformation (translation, rotation and scale), the last row
 * of the matrix represents the translation part of the transformation.
 *
 * \remarks It is important to realize that an affine matrix must respect a
 * certain structure.  To be sure the structure is respected, use SetT, SetR,
 * SetS, SetQ, SetTRS or SetTQS.  If by mistake bad data is entered in this
 * affine matrix, some functions such as Inverse() will yield wrong results.  If
 * a matrix is needed to hold values that aren't associate with an affine
 * matrix, please use FbxMatrix instead.
 */
...
public:
   ...
   * \remarks    The returned rotation vector is in Euler angle and the rotation
   * order is XYZ.
   */
   ...
   */
  void
  SetTQS(const FbxVector4 &pT, const FbxQuaternion &pQ, const FbxVector4 &pS);
   ...
   *                   This operator operates on the first three rows and
   * columns of the matrix. So only the rotation and scaling are scaled, not the
   * translation part. After operation, the translation vector will be set as
   * (0,0,0,1);
   */
   ...
   *                   This operator operates on the first three rows and
   * columns of the matrix. So only the rotation and scaling are scaled, not the
   * translation part. After operation, the translation vector will keep
   * original value.
   */
   ...
   *  That means to scale, then rotate, and then translate a vector V, the
   * transform should be T * R * S * V. \n Below is an example of code that
   * shows how to construct rotation transform in XYZ rotation order. \code
   *  FbxAMatrix lRotateXM, lRotateYM, lRotateZM, lRotateXYZM, lRotateM;
   *  // Construct rotation matrix around X, Y and Z axises separately and then
   * combine them. FbxVector4 lRotateX(10, 0, 0); FbxVector4 lRotateY(0, 10, 0);
   ...
   *  \endcode
   * \note                Please refer to the FBX SDK programmers guide for more
   * details.
   */
   ...
   * amount.
   * \remark This matrix and other matrix should contain only rotations,
   * otherwise result may be undefined. */
   ...
   * \return             \c true if the two matrices are equal (each element is
   * within a FBXSDK_TOLERANCE tolerance) and \c false otherwise.
   */
  bool operator==(const FbxAMatrix &pOther) const;

  /**	Non-equivalence operator.
   * \param pOther     The matrix to be compared to \e this.
   * \return            \c false if the two matrices are equal (each element is
   * within a FBXSDK_TOLERANCE tolerance) and \c true otherwise.
   */
  bool operator!=(const FbxAMatrix &pOther) const;
  //@}
   ...
   * \return \c true if the matrix is equal to identity matrix, \c false
   * otherwise. */
...
*****************************************************************************************************************************/
  ...
  void
  SetTRS(const FbxVector4 &pT, const FbxAMatrix &pRM, const FbxVector4 &pS);
  ...
  // pOrd is assumed to be an FbxEuler::EOrder (or its synonym
  // EFbxRotationOrder)
