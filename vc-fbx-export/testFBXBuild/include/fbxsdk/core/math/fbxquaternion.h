 ...
 * \nosubgrouping
 * Quaternions form a four-dimensional normed division algebra over the real
 * numbers. It is for calculations involving three-dimensional rotations.
 */
...
public:
   ...
   * \remarks          The index parameter is not checked for values out of
   * bounds. The valid range is [0,3].
   */
   ...
   */
  //@{
  /** The addition operator between the scalar part of this quaternion and a
   * scalar value, no influence on the vector part of the quaternion. \param
   * pValue     The scalar value to be added. \return           The sum of
   * addition.
   */
  FbxQuaternion operator+(double pValue) const;

  /** The subtraction operator between the scalar part of this quaternion and a
   * scalar value, no influence on the vector part of the quaternion. \param
   * pValue     The scalar subtrahend. \return           The difference of
   * subtraction.
   */
  FbxQuaternion operator-(double pValue) const;
  ...
  /** The in place addition operator between the real part of this quaternion
   * and a scalar value. \param pValue     The value to be added. \return The
   * sum of addition.
   */
  FbxQuaternion &operator+=(double pValue);

  /** The subtraction operator between the real part of this quaternion and a
   * scalar value. \param pValue     The scalar subtrahend. \return The
   ...
   * \param pValue     The value multiplying each component of the vector.
   * \return           The result of multiplying each component of the vector by
   * pValue, replacing this quaternion. \remarks          The passed value is
   * not checked.
   */
  FbxQuaternion &operator*=(double pValue);
   ...
   * \param pValue     The value dividing each component of the vector.
   * \return           The result of dividing each component of the vector by
   * pValue, replacing this quaternion. \remarks          The passed value is
   * not checked.
   */
  FbxQuaternion &operator/=(double pValue);
   ...
   * \param pOther          The quaternion to be multiplied with this
   * quaternion. \return                The product of two quaternions. \remarks
   ...
   * \remarks               If the divisor has a zero length, return zero
   * quaternion.
   */
  FbxQuaternion operator/(const FbxQuaternion &pOther) const;
   ...
   * \param pQuaternion     Quaternion to add.
   * \return                The quaternion v' = this + pQuaternion, replacing
   * this quaternion. \remarks               The values in pQuaternion are not
   * checked.
   */
  FbxQuaternion &operator+=(const FbxQuaternion &pQuaternion);
   ...
   * \param pOther          The quaternion to be multiplied with this
   * quaternion. \return                The product of two quaternions.
   */
   ...
   * \remarks     If this quaternion has a zero length, retain the original
   * value. \remarks     If the quaternion is normalized, then its inverse is
   ...
   * \param pWeight A value between 0.0 and 1.0 to specify the interpolation
   * amount. */
  FbxQuaternion Slerp(const FbxQuaternion &pOther, double pWeight) const;

  /** Create a Quaternion equivalent to the supplied Euler XYZ in spherical
   * coordinate. \param pEuler   The Euler XYZ angle (in degrees).
   */
  void ComposeSphericalXYZ(const FbxVector4 pEuler);

  /** Create an Euler XYZ equivalent to the current quaternion.
   * \return     The Euler XYZ angle (in degrees) equivalent to the current
   * quaternion in spherical coordinate.
   */
   ...
   * \return       \c true  if the two quaternions are equal (each element is
   * within a FBXSDK_TOLERANCE tolerance), \c false  otherwise.
   */
  bool operator==(const FbxQuaternion &pV) const;

  /**    Non equivalence operator.
   * \param pV     The quaternion to be compared to \e this.
   * \return       \c  false if the two quaternions are equal (each element is
   * within a FBXSDK_TOLERANCE tolerance), \c true  otherwise.
   */
  bool operator!=(const FbxQuaternion &pV) const;
  //@}
