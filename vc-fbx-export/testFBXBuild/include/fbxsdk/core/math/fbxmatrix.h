...
public:
   ...
   * shearing, scaling and sign of determinant. \param pTranslation Translation
   * element. \param pRotation Rotation element. \param pShearing Shearing
   * element. \param pScaling Scaling element.
   ...
   * \return \c true if the two matrices are equal (each element is within a
   * FBXSDK_TOLERANCE tolerance), \c false otherwise. */
   ...
   * \param pM The matrix to be compared against this matrix.
   * \return \c false if the two matrices are equal (each element is within a
   * FBXSDK_TOLERANCE tolerance), \c true otherwise. */
  bool operator!=(const FbxMatrix &pM) const;
