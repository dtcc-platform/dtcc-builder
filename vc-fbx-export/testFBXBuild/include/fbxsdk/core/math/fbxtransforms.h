...
/** FbxLimits defines a limit range for one transform component, either
 * translation, rotation or scaling. One transform component limit contains two
 * part: a min value and a max value limit, which means that each value of the
 * corresponding transform component cannot go beyond the range set by the min
 * and max values. Although the members are identified as X, Y and Z (the W
 * component is ignored) at this level, they are unitless values and will only
 * have meaning within the context they are queried.
 *
 * For each limit, there is one flag to indicate if the limit is active or not.
 * Before accessing the limit info, the caller need to query the flag first to
 * make sure that the retrieved values will be meaningful. \nosubgrouping
 */
...
public:
   ...
   * \return If any component of the minimum or maximum active state are set,
   * true is returned. \remarks The global active state will not count when
   * resolving this.
   */
...
public:
  ...
  bool
  V2VRef(FbxVector4 &pVOut, const FbxVector4 &pVIn, const FbxVector4 &pVRef);
...
public:
  ...
  void LT2DoF(FbxVector4 &pDoF,
              ...
              const FbxAMatrix &pLSM);
  void
  DoF2LRM(FbxAMatrix &pLRM, const FbxVector4 &pRDoF, bool pForLimit = false);
  void
  LRM2DoF(FbxVector4 &pRDoF, const FbxAMatrix &pLRM, bool pForLimit = false);
...
private:
  void
  SumPivots(FbxVector4 &pSum, const FbxAMatrix &pLRM, const FbxAMatrix &pLSM);
