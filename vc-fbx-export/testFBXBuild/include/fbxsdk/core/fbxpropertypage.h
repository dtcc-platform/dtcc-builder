...
{
  inline int operator()(const FbxNameMapKey &pKeyA,
                        const FbxNameMapKey &pKeyB) const
  {
...
public:
  ...
  int AddEnumValue(const char *pStringValue)
  {
    ...
    if (lType == eFbxEnum || lType == eFbxEnumM)
    {
      ...
      bool lCanAdd =
          (lType == eFbxEnumM || mEnumList->FindIndex(pStringValue) == -1);
      if (lCanAdd)
  ...
  int GetEnumCount() { return mEnumList ? mEnumList->GetCount() : 0; }
  ...
  bool HasMinMax(EValueIndex pId) const { return mMinMaxValue[pId] != NULL; }
...
private:
  FbxPropertyInfo(const char *pName, FbxPropertyPage *pTypeInfo)
      : mRef(0), mName(pName), mType(eFbxUndefined), mTypeInfo(pTypeInfo),
        mUserTag(0), mUserData(0), mFilter(0)
  {
  ...
  FbxPropertyInfo(const char *pName, EFbxType pType)
      : mRef(0), mName(pName), mType(pType), mTypeInfo(0), mUserTag(0),
        mUserData(0), mFilter(0)
  {
    for (int i = 0; i < eValueCount; i++)
    {
      mMinMaxValue[i] = 0;
...
public:
  ...
  // ClearConnectCache()
  // ------------------------------------------------------
  inline void ClearConnectCache() { mConnectionPoint.SubConnectRemoveAll(); }

  //! Clear all connect without sending any notification (Internal use ONLY)
  inline void WipeAllConnections() { mConnectionPoint.WipeConnectionList(); }
  ...
  inline FbxPropertyConnect *GetSrc(FbxConnectionPointFilter *pFilter,
                                    int pIndex)
  {
  ...
  inline FbxPropertyConnect *GetDst(FbxConnectionPointFilter *pFilter,
                                    int pIndex)
  {
...
private:
  FbxPropertyConnect(FbxPropertyPage *pPage, FbxInt pId)
      : mRef(0), mConnectionPoint(this), mPage(pPage), mId(pId)
  {
  }
...
public:
  ...
  inline bool IsEmpty()
  {
    return (mInfo || mValue || mConnect || mFlags.GetMask() != 0) ? false
                                                                  : true;
  }
...
private:
  FbxPropertyEntry(FbxInt pParentId,
                   FbxPropertyInfo *pInfo,
                   FbxPropertyValue *pValue,
                   FbxPropertyConnect *pConnect)
      : mInfo(pInfo), mValue(pValue), mConnect(pConnect), mParentId(pParentId),
...
public:
  ...
  template <class T>
  inline T *GetPropertyItem(const T *pItemType,
                            FbxInt pIndex,
                            FbxPropertyPage **pFoundIn = 0) const
  {
    FbxPropertyPage *lReferencePage = 0;
    FbxPropertyEntry *lReferenceEntry =
        GetPropertyEntry(pIndex, &lReferencePage);
    if (pFoundIn)
      *pFoundIn = 0;
    if (lReferenceEntry)
    {
      ...
      {
        return lReferencePage->mInstanceOf
                   ? lReferencePage->mInstanceOf->GetPropertyItem(
                         pItemType, pIndex, pFoundIn)
                   : 0;
      }
    }
    return 0;
  ...
  template <class T>
  inline T *ChangePropertyItemState(const T *pItemType,
                                    FbxInt pIndex,
                                    FbxPropertyFlags::EInheritType pInheritType)
  {
    ...
    if (pInheritType == FbxPropertyFlags::eOverride)
    {
      ...
      else if (lItem)
      {
        FbxPropertyEntry *lEntry =
            ChangePropertyEntryState(pIndex, FbxPropertyFlags::eOverride);
    ...
    else
    {
      ...
      bool lOwnEntry =
          !mInstanceOf ||
          (mInstanceOf->GetPropertyItem(pItemType, pIndex) == NULL);
  ...
  FbxPropertyPage *GetFirstPropertyItem(FbxInt pId, const T *pItem) const
  {
    ...
    {
      FbxPropertyPage *lReferencePage2 =
          lReferencePage->mInstanceOf->GetFirstPropertyItem(pId, pItem);
  ...
  {
    FbxPropertyInfo *lPropertyInfo =
        GetPropertyItem(FBX_TYPE(FbxPropertyInfo), pId);
  ...
  FbxInt GetParent(FbxInt pId = FBXSDK_PROPERTY_ID_ROOT) const
  {
    ...
    return lPropertyEntry ? lPropertyEntry->GetParentId()
  ...
  {
    return Add(pParentId, FbxPropertyInfo::Create(pName, pType),
               FbxPropertyValue::Create(0, pType), 0);
  }
  FbxInt Add(FbxInt pParentId, const char *pName, FbxPropertyPage *pTypeInfo)
  {
    return Add(pParentId, FbxPropertyInfo::Create(pName, pTypeInfo),
               FbxPropertyValue::Create(0, pTypeInfo->GetType()), 0);
  }
  ...
  FbxInt GetChild(FbxInt pParentId = FBXSDK_PROPERTY_ID_ROOT) const
  {
    ...
    if (pParentId != FBXSDK_PROPERTY_ID_NULL)
    {
      ...
      while (lId < lLastId && lParentId != pParentId)
  ...
  FbxInt GetSibling(FbxInt pId) const
  {
    ...
    if (pId != FBXSDK_PROPERTY_ID_NULL)
    {
      ...
      while (pId < lLastId && lReferenceParentId != FBXSDK_PROPERTY_ID_NULL &&
             lParentId != lReferenceParentId)
    ...
    if (pId != FBXSDK_PROPERTY_ID_NULL)
    {
      ...
      if (lReferenceParentId != FBXSDK_PROPERTY_ID_NULL)
      {
        ...
        } while (pId != FBXSDK_PROPERTY_ID_NULL &&
                 lEntry->GetParentId() != lReferenceParentId);
  ...
  FbxInt GetFirstDescendent(FbxInt pAnscestorId = FBXSDK_PROPERTY_ID_ROOT) const
  {
    ...
    if (pAnscestorId != FBXSDK_PROPERTY_ID_NULL)
    {
      ...
      while (lId < lLastId)
      {
        if (lParentId != FBXSDK_PROPERTY_ID_NULL &&
    ...
    if (pAnscestorId != FBXSDK_PROPERTY_ID_NULL)
    {
      for (;;)
      {
        ...
        if (lId == FBXSDK_PROPERTY_ID_NULL)
          break;
        if (lEntry->GetParentId() != FBXSDK_PROPERTY_ID_NULL &&
            IsDescendentOf(lId, pAnscestorId))
        {
          ret1 = lId;
          break;
  ...
  /** Retrieves the next descendent property id of a specified property id, with
   * given a descendent property id. \param pAnscestorId The specified property
   * id \param pId The descendent property id \return the next descendent
   * property id
   */
  FbxInt GetNextDescendent(FbxInt pAnscestorId, FbxInt pId) const
  {
    ...
    if (pId != FBXSDK_PROPERTY_ID_NULL)
    {
      ...
      while (pId < lLastId)
      {
        ...
        if (lParentId != FBXSDK_PROPERTY_ID_NULL &&
            IsDescendentOf(pId, pAnscestorId))
        {
          ret0 = pId;
          break;
        }
                  ...
                  bool pCaseSensitive)
  {
    ...
    if (mNameMap.mSecond.GetSize() > 0)
    {
      ...
      NameMap::RecordType *lIterator =
          mNameMap.mSecond.Find(FbxNameMapKey(pId, pName));
      if (!lIterator)
      {
      ...
      else
      {
        ...
        if (lId != FBXSDK_PROPERTY_ID_NULL && pTypeInfo)
        {
          ...
          // since we might have multiple property with the same name but
          // different types
          FbxPropertyInfo *lPropertyInfo =
              GetPropertyItem(FBX_TYPE(FbxPropertyInfo), lId);
          if (lPropertyInfo)
          {
    ...
    while (lId != FBXSDK_PROPERTY_ID_NULL)
    {
      ...
      if ((!pTypeInfo || lPropertyInfo->GetTypeInfo()->Is(pTypeInfo)) &&
          ((!pCaseSensitive &&
            FBXSDK_stricmp(lPropertyInfo->GetName(), pName) == 0) ||
              ...
              const char *pChildrenSeparators)
  {
    if (pChildrenSeparators)
    {
      ...
      if (lFoundIndex < strlen(pName))
      {
        ...
        return lId != FBXSDK_PROPERTY_ID_NULL
                   ? Find(lId, pName + lFoundIndex + 1, pTypeInfo,
                          pCaseSensitive, pChildrenSeparators)
                   : lId;
      }
      else
      {
  ...
  {
    FbxPropertyPage *lReferencePage = 0;
    FbxPropertyConnect *lPropertyConnect =
        GetPropertyItem(FBX_TYPE(FbxPropertyConnect), pId, &lReferencePage);
  ...
  {
    FbxPropertyEntry *lDstEntry =
        ChangePropertyEntryState(pDstId, FbxPropertyFlags::eOverride);
    FbxPropertyEntry *lSrcEntry =
        pSrcPage->ChangePropertyEntryState(pSrcId, FbxPropertyFlags::eOverride);
    FbxPropertyConnect *lDstConnect =
        lDstEntry->Get(FBX_TYPE(FbxPropertyConnect));
    FbxPropertyConnect *lSrcConnect =
        lSrcEntry->Get(FBX_TYPE(FbxPropertyConnect));
  ...
  bool DisconnectSrc(FbxInt pDstId, FbxPropertyPage *pSrcPage, FbxInt pSrcId)
  {
    ...
    FbxPropertyConnect *lDstConnect = GetPropertyItem(
        FBX_TYPE(FbxPropertyConnect), pDstId, &lDstReferencePage);
    FbxPropertyPage *lSrcReferencePage = 0;
    FbxPropertyConnect *lSrcConnect = pSrcPage->GetPropertyItem(
        FBX_TYPE(FbxPropertyConnect), pSrcId, &lSrcReferencePage);

    // Make sure we have a connection point on both sides of the connection
    if (lDstConnect && lSrcConnect && lDstReferencePage == this &&
        lSrcReferencePage == pSrcPage)
    {
  ...
  int GetSrcCount(FbxInt pId, FbxConnectionPointFilter *pFilter)
  {
    ...
    // Connections are not considered propagated so
    // make sure that we own the FbxPropertyConnect objects
    return (lPropertyConnect && lReferencePage == this)
               ? lPropertyConnect->GetSrcCount(pFilter)
               : 0;
  }
  ...
  int GetDstCount(FbxInt pId, FbxConnectionPointFilter *pFilter)
  {
    ...
    // make sure that we own the FbxPropertyConnect objects
    return (lPropertyConnect && lReferencePage == this)
               ? lPropertyConnect->GetDstCount(pFilter)
               : 0;
  }
                 ...
                 EFbxType pValueType)
  {
    ...
    return lPropertyInfo
               ? lPropertyInfo->GetMinMax(pValueId, pValue, pValueType)
               : false;
  }
                 ...
                 EFbxType pValueType)
  {
    ...
    // Don't make it writeable (Keep it shared)
    return lPropertyInfo
               ? lPropertyInfo->SetMinMax(pValueId, pValue, pValueType)
               : false;
  }
  ...
  bool Get(FbxInt pId, void *pValue, EFbxType pValueType)
  {
    FbxPropertyValue *lPropertyValue =
        GetPropertyItem(FBX_TYPE(FbxPropertyValue), pId);
           ...
           bool pCheckValueEquality)
  {
    if (pCheckValueEquality)
    {
      ...
      else
      {
        ...
        FbxPropertyValue *lPropertyValue2 =
            mInstanceOf ? mInstanceOf->GetPropertyItem(
                              FBX_TYPE(FbxPropertyValue), pId, &lReferencePage2)
                        : NULL;
        if (lReferencePage2 && lPropertyValue2)
        {
          // this page is an override, but there is another page before us that
          // overrides the value
          ...
          if (lValuesEqual)
          {
            ChangePropertyItemState(FBX_TYPE(FbxPropertyValue), pId,
                                    FbxPropertyFlags::eInherit);
            lValueChanged = true;
          }
        }
        // else this page is the originator of the property, so no need to
        // check,
      }
    ...
    FbxPropertyValue *lPropertyValue = ChangePropertyItemState(
        FBX_TYPE(FbxPropertyValue), pId, FbxPropertyFlags::eOverride);
    return lPropertyValue ? lPropertyValue->Set(pValue, pValueType) : false;
  }

  inline FbxPropertyFlags::EInheritType
  GetValueInherit(FbxInt pId, bool pCheckInstanceOf) const
  {
    FbxPropertyPage *lReferencePage = NULL;
    GetPropertyItem(FBX_TYPE(FbxPropertyValue), pId, &lReferencePage);
    ...
    if (!pCheckInstanceOf)
    {
      return lReferencePage == this ? FbxPropertyFlags::eOverride
                                    : FbxPropertyFlags::eInherit;
    }
    else
    {
      if (lReferencePage == this)
        return FbxPropertyFlags::eOverride; // this page is either an override,
                                            // or the originator
      else if (!lReferencePage->mInstanceOf)
        return FbxPropertyFlags::eInherit; // the reference is the class root,
                                           // so we must be inheriting

      // The reference page is not the class root, might be another override, or
      // the originator.
      FbxPropertyValue *lPropertyValue =
          lReferencePage->mInstanceOf->GetPropertyItem(
              FBX_TYPE(FbxPropertyValue), pId);
      ...
      return lPropertyValue ? FbxPropertyFlags::eOverride
                            : FbxPropertyFlags::eInherit;
    }
  }
  ...
  inline bool
  GetDefaultValue(FbxInt pId, void *pValue, EFbxType pValueType) const
  {
    FbxPropertyPage *lReferencePage =
        GetFirstPropertyItem(pId, FBX_TYPE(FbxPropertyValue));
    FbxPropertyValue *lPropertyValue =
        lReferencePage
            ? lReferencePage->GetPropertyItem(FBX_TYPE(FbxPropertyValue), pId)
            : NULL;
  ...
  template <class T> inline bool Set(FbxInt pId, const T &pValue)
  {
    return Set(pId, &pValue, FbxTypeOf(pValue), true);
  }
  template <class T> inline T Get(FbxInt pId, const T *pFBX_TYPE)
  {
    T lValue;
  ...
  void PushPropertiesToParentInstance()
  {
    if (mInstanceOf)
    {
      ...
      {
        FbxPropertyEntry *lParentEntry = mInstanceOf->ChangePropertyEntryState(
            (FbxInt)i, FbxPropertyFlags::eOverride);
        FbxPropertyEntry *lEntry = GetPropertyEntry((FbxInt)i);

        if (!lParentEntry)
        {
          lParentEntry =
              FbxPropertyEntry::Create(lEntry->GetParentId(), 0, 0, 0);
        ...
        /*
                ...
                mInstanceOf->Add(
                        lEntry->GetParentId(),
                        lEntry->Get(FBX_TYPE(FbxPropertyInfo)),        // The
        info lEntry->Get(FBX_TYPE(FbxPropertyValue)),       // The Value 0, //
        The connections false, false
                );
        }
        */
        ...
        // Don't touch the connections
        // -----------------------------------------
        ChangePropertyItemState(FBX_TYPE(FbxPropertyInfo), i,
                                FbxPropertyFlags::eInherit);
        ChangePropertyItemState(FBX_TYPE(FbxPropertyValue), i,
                                FbxPropertyFlags::eInherit);
        ChangePropertyItemState(FBX_TYPE(FbxPropertyFlags), i,
                                FbxPropertyFlags::eInherit);
      }
    }
  }
  ...
  FbxPropertyFlags::EFlags GetFlags(FbxInt pId = FBXSDK_PROPERTY_ID_ROOT) const
  {
    ...
    FbxPropertyFlags *lPropertyFlags =
        GetPropertyItem(FBX_TYPE(FbxPropertyFlags), pId, &lFoundIn);
                   ...
                   bool pCheckFlagEquality = true)
  {
    if (pCheckFlagEquality)
    {
      ...
      if (lFlag)
      {
        if (lFoundIn == this)
        {
          ...
          FbxPropertyFlags *lInheritedFlags =
              mInstanceOf ? mInstanceOf->GetPropertyItem(
                                FBX_TYPE(FbxPropertyFlags), pId)
                          : NULL;
          if (lInheritedFlags && lInheritedFlags->Equal(*lFlag, pFlags))
          {
            ...
            if (lFlag->GetMask() == 0)
              ChangePropertyItemState(FBX_TYPE(FbxPropertyFlags), pId,
                                      FbxPropertyFlags::eInherit);
    ...
    FbxPropertyFlags *lPropertyFlags = ChangePropertyItemState(
        FBX_TYPE(FbxPropertyFlags), pId, FbxPropertyFlags::eOverride);
    return lPropertyFlags ? lPropertyFlags->ModifyFlags(pFlags, pValue) : false;
  }

  FbxPropertyFlags::EInheritType
  GetFlagsInheritType(FbxPropertyFlags::EFlags pFlags,
                      bool pCheckInstanceOf,
                      FbxInt pId = FBXSDK_PROPERTY_ID_ROOT) const
  {
    FbxPropertyPage *lFoundIn = NULL;
    FbxPropertyFlags *lPropertyFlags =
        GetPropertyItem(FBX_TYPE(FbxPropertyFlags), pId, &lFoundIn);
    ...
    if (!pCheckInstanceOf)
      return lFoundIn != this
                 ? FbxPropertyFlags::eInherit
                 : (lPropertyFlags ? lPropertyFlags->GetFlagsInheritType(pFlags)
                                   : FbxPropertyFlags::eInherit);
    else
    {
      ...
      {
        lPropertyFlags =
            lRefPage->GetPropertyItem(FBX_TYPE(FbxPropertyFlags), pId);

        if (!lPropertyFlags)
          break; // gone too far, break.

        if (lPropertyFlags->GetFlagsInheritType(pFlags) ==
            FbxPropertyFlags::eOverride)
        {
          if (this == lRefPage || lFoundOverride)
            return FbxPropertyFlags::eOverride; // found two overrides or this
                                                // page is the override.
          else
                           ...
                           FbxInt pId = FBXSDK_PROPERTY_ID_ROOT)
  {
    ...
    {
      lPropertyFlags = ChangePropertyItemState(FBX_TYPE(FbxPropertyFlags), pId,
                                               FbxPropertyFlags::eOverride);

      // we should initialize our flag to the inherited value, if any.
      FbxPropertyFlags *lParentFlags =
          mInstanceOf
              ? mInstanceOf->GetPropertyItem(FBX_TYPE(FbxPropertyFlags), pId)
              : NULL;
  ...
  inline void BeginCreateOrFindProperty()
  {
    if (0 == mNameMap.mFirst)
    {
      ...
      // push the existing properties into the map. Note: this includes the root
      // property!
      ...
      while (lFoundId != FBXSDK_PROPERTY_ID_NULL)
      {
        ...
        if (lInfo)
        {
          mNameMap.mSecond.Insert(
              FbxNameMapKey(lEntry->GetParentId(), lInfo->GetName()), lFoundId);
        }
...
protected:
  ...
  ~FbxPropertyPage()
  {
    ...
    for (i = 0; i < mInstances.GetCount(); ++i)
    {
      ...
      {
        if (mInstances[i]->ChangePropertyEntryState(
                (FbxInt)j, FbxPropertyFlags::eOverride))
        {
          // Clone the info and values. Don't clone the connections,
          // since they aren't propagated.
          mInstances[i]->ChangePropertyItemState(FBX_TYPE(FbxPropertyInfo),
                                                 (FbxInt)j,
                                                 FbxPropertyFlags::eOverride);
          mInstances[i]->ChangePropertyItemState(FBX_TYPE(FbxPropertyValue),
                                                 (FbxInt)j,
                                                 FbxPropertyFlags::eOverride);

          // Since all entries have their own flags, just override the ones in
          // the instance.
          mInstances[i]->SetFlagsInheritType(FbxPropertyFlags::eOverride,
                                             FbxPropertyFlags::eAllFlags,
                                             (FbxInt)j);
        }
      }
    ...
    if (mInstanceOf)
    {
      ...
      mInstanceOf->mInstances.SetAt(
          lIndex,
          mInstanceOf->mInstances[mInstanceOf->mInstances.GetCount() - 1]);
...
private:
  /** Retrieves the smallest property id of which are larger than a specified
   * one. \param pId The specified property id \param pIncrementIfNone Whether
   * it returns FBXSDK_PROPERTY_ID_NULL or pId+1, if not found. \return The
   * property id described above.
   */
  FbxInt GetMinimumPropertyId(FbxInt pId, bool pIncrementIfNone = true) const
  {
    ...
    FbxInt lParentMin = mInstanceOf
                            ? mInstanceOf->GetMinimumPropertyId(pId, false)
                            : FBXSDK_PROPERTY_ID_NULL;
  ...
  /** Retrieves the smallest property id of which are larger than a specified
   * one, and retrieve its entry. \param pId The specified property id \param
   * pEntry The returned property entry \return The property id described above.
   */
  FbxInt GetMinimumPropertyIdAndEntry(FbxInt pId,
                                      FbxPropertyEntry **pEntry) const
  {
    ...
    FbxInt lParentMin =
        mInstanceOf
            ? mInstanceOf->GetMinimumPropertyIdAndEntry(pId, &lParentEntry)
            : FBXSDK_PROPERTY_ID_NULL;
  ...
  FbxPropertyEntry *GetPropertyEntry(FbxInt pIndex,
                                     FbxPropertyPage **pFoundIn = 0) const
  {
  ...
  FbxPropertyEntry *
  ChangePropertyEntryState(FbxInt pIndex,
                           FbxPropertyFlags::EInheritType pInheritType)
  {
    FbxPropertyPage *lReferencePage = 0;
    FbxPropertyEntry *lReferenceEntry =
    ...
    if (pInheritType == FbxPropertyFlags::eOverride)
    {
      ...
      else if (lReferenceEntry)
      {
        ...
        FbxPropertyEntry *lEntry =
            FbxPropertyEntry::Create(lReferenceEntry->GetParentId(), 0, 0, 0);
             ...
             bool pRecursive = true)
  {
    ...
    FbxPropertyEntry *lEntry =
        FbxPropertyEntry::Create(pParentId, pInfo, pValue, pConnect);
    ...
    // We only add to the map if this Add is called after
    // BeginCreateOrFindProperty() in which case the size is always > 0 because
    // it includes the root property
    if (mNameMap.mSecond.GetSize() > 0)
    ...
    if (pRecursive)
    {
      ...
      if (lTypeInfo)
      {
        ...
        {
          FbxPropertyInfo *lPropertyInfo =
              lTypeInfo->GetPropertyItem(FBX_TYPE(FbxPropertyInfo), lChildId);
          FbxPropertyValue *lPropertyValue =
              lTypeInfo->GetPropertyItem(FBX_TYPE(FbxPropertyValue), lChildId);
          FbxPropertyConnect *lPropertyConnect = lTypeInfo->GetPropertyItem(
              FBX_TYPE(FbxPropertyConnect), lChildId);

          Add(lId, lPropertyInfo ? lPropertyInfo->Clone(this) : 0,
              lPropertyValue ? lPropertyValue->Clone(this) : 0,
  ...
  // Property management
  typedef FbxMap<FbxInt,
                 FbxPropertyEntry *,
                 FbxLessCompare<FbxInt>,
                 FbxHungryAllocator>
      EntryMap;
  EntryMap mEntryMap;
