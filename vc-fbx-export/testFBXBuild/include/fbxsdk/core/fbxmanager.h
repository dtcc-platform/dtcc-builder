 ...
 *""). \li scene element deallocation, call FbxManager::Destroy() to deallocates
 *all object created by the SDK manager. \li scene element search and access,
 *please see \ref GlobalObjectManagement section.
 *
 ...
 *functions below. It must be done before the first FbxManager creation.
 *
 *	FbxSetMallocHandler();
 * FbxSetCallocHandler();
 ...
 * FbxSetFreeHandler();
 *
 * Upon destruction, all objects allocated by the SDK manager and not explicitly
 *destroyed are destroyed as well. A derived class can be defined to allocate
 *and deallocate specialized scene elements. \remarks You could create more than
 *one SDK manager. However, it's better to NOT share the same object among
 *different managers. \nosubgrouping
 */
...
public:
   ...
   * \param pFull	If true, the complete version string including revision
   *number and release date will be returned, otherwise only the version
   *numbering is returned.
   */
  static const char *GetVersion(bool pFull = true);

  /** Get the current default FBX file format version number for this version of
   * the FBX SDK. \param pMajor        Version major number. \param pMinor
   * Version minor number. \param pRevision     Version revision number.
   */
   ...
   * Such as:
   * \code RegisterFbxClass("FbxCamera", FBX_TYPE(FbxCamera),
   * FBX_TYPE(FbxNodeAttribute)); \endcode
   */
  template <typename T1, typename T2>
  inline FbxClassId RegisterFbxClass(const char *pName,
                                     const T1 * /*T1*/,
                                     const T2 * /*T2*/,
                                     const char *pFbxFileTypeName = 0,
                                     const char *pFbxFileSubTypeName = 0)
  {
    T1::ClassId = Internal_RegisterFbxClass(
        pName, T2::ClassId, (FbxObjectCreateProc)T1::Allocate, pFbxFileTypeName,
        pFbxFileSubTypeName);
    return T1::ClassId;
  }
   ...
   * "FbxUIWidgetBoolean".
   * \param T						FBX type of parent
   * class. \param pFbxFileTypeName         The type name of the class in FBX
   * file. \param pFbxFileSubTypeName      The sub type name of the class in FBX
   * file. \return The class Id of the newly register class. Such as: \code
   * RegisterRuntimeFbxClass( "FbxUIWidgetBoolean",
   * FBX_TYPE(FbxUIWidgetDefinition), NULL, "FbxUIWidgetBoolean"); \endcode
   */
  template <typename T>
  inline FbxClassId RegisterRuntimeFbxClass(const char *pName,
                                            const T * /*T*/,
                                            const char *pFbxFileTypeName = 0,
                                            const char *pFbxFileSubTypeName = 0)
  {
    return Internal_RegisterFbxClass(pName, T::ClassId,
                                     (FbxObjectCreateProc)T::Allocate,
                                     pFbxFileTypeName, pFbxFileSubTypeName);
  }
   ...
   */
  template <typename T1, typename T2>
  inline FbxClassId OverrideFbxClass(const T1 *pFBX_TYPE_Class,
                                     const T2 *pFBX_TYPE_OverridenClass)
  {
    T1::ClassId = Internal_OverrideFbxClass(T2::ClassId,
                                            (FbxObjectCreateProc)T1::Allocate);
    return T1::ClassId;
  }
   ...
   * \param pContainer	An optional parameter to specify which object will
   ...
   * \param pFbxFileSubTypeName  Specify by The sub type name in FBX file to
   * find.
   */
  FbxClassId FindFbxFileClass(const char *pFbxFileTypeName,
                              const char *pFbxFileSubTypeName) const;
   ...
   * \param pMessageEmitter the emitter to use, passing NULL will reset to the
   * default emitter. The object will be deleted when the SDK manager is
   * destroyed, thus ownership is transfered.
   */
...
//@{
   ...
   * \param pIOS The properties hierarchies to fill.
   * \param pImport If \c true, import properties are set, otherwise export
   * properties are set.
   */
   ...
   * Use IncreasDestroyingSceneFlag() and DecreaseDestroyingSceneFlag() in
   * pairs.
   */
   ...
   * \return If the reference is correctly added to the scene, return \c true
   * otherwise, if the reference is already there, returns \c false.
   */
   ...
   * \return If the reference was successfully removed, return \c true
   * otherwise, if the reference could not be found returns \c false.
   */
  bool RemoveReference(FbxSceneReference *pReference);

  /** Clear the specified reference from the SDK manager.
   * \param pReference The reference to be removed.
   * \return If the reference was successfully cleared from the SDK manager,
   * return \c true otherwise, if the reference could not be found returns \c
   * false.
   */
   ...
   * \return The document at the specified index. Will return NULL if index is
   * invalid.
   */
...
*****************************************************************************************************************************/
  ...
  FbxClassId
  GetNextFbxClass(FbxClassId pClassId /* invalid id: first one */) const;
...
private:
  bool CanAutoDestroySrcObject(FbxObject *pObject,
  ...
  FbxClassId Internal_RegisterFbxClass(const char *pClassName,
                                       FbxClassId pParentClassId,
                                       FbxObjectCreateProc = 0,
                                       const char *pFbxFileTypeName = 0,
                                       const char *pFbxFileSubTypeName = 0);
  bool Internal_RegisterFbxClass(FbxClassId pClassId);
  FbxClassId Internal_OverrideFbxClass(FbxClassId pClassId,
                                       FbxObjectCreateProc = 0);
