...
def check_volume_mesh(volume_mesh: VolumeMesh):
    ...
    title("Cell-based quality metrics for volume mesh")

    v = TetrahedronMeshQuality.volume(volume_mesh)
    title("Cell volume (allowed to vary)")
    ...
    title("Orthogonal quality (0-1, 1 is optimal)")
    ...
    title("Face edge ratio (1-inf, 1 optimal)")
    ...
    title("Face aspect ratio (1-inf, 1 is optimal)")
    ...
    title("Face skewness (0-1, 0 is optimal)")
    print("MIN\t",       np.nanmin(sk))
    print("MAX\t",       np.nanmax(sk))
    # print("MEAN\t",     np.nanmean(sk))
    # print("MEDIAN\t", np.nanmedian(sk))
