...
*  - Partition index "errors" are counting how many times index is getting out of vertex column bounds.
*/
...
{   
    ...
    const double adjusted_domain_height = std::ceil(domain_height/max_layer_height) * max_layer_height; 
    info("Domain height adjusted to fit  chosen layer heights: " + str(adjusted_domain_height) + "m"); 

    // Each Vertex inherits the minimum color (layer height) from the faces it belongs to.
    std::vector<int> vertex_colors(num_vertices, num_layers); 
    for (size_t i = 0; i < num_faces; i++)
    {
      const std::vector<size_t> face = {mesh.faces[i].v0, mesh.faces[i].v1, mesh.faces[i].v2}; 
      for( const auto &j : face)
      {
        if (vertex_colors[j] > face_colors[i])  vertex_colors[j] = face_colors[i];     
      }
    }
    ...
    for (size_t i = 0; i < num_vertices; i++)
    {
      ...
      { 
        Vector3D v(mesh.vertices[i].x,mesh.vertices[i].y, mesh.vertices[i].z + j * layer_h); 
    ...
    // {
    //   std::cout <<i <<") Col index offset "<< col_index_offset[i] <<" " <<col_vertices[i].size() <<std::endl;
    // }
    ...
    { 
      ...
      std::array<int,3> v_colors = {vertex_colors[face_simplex.v0], vertex_colors[face_simplex.v1], vertex_colors[face_simplex.v2]};
      ...
      // std::cout << "Face: "<<i << " coff0: "<< column_offsets[0] <<" coff1: "<< column_offsets[1] << " coff2: "<< column_offsets[2] <<  std::endl; 

      const std::array<size_t,3> column_len = {col_index_offset[face[0]+1] - col_index_offset[face[0]],
                                            col_index_offset[face[1]+1] - col_index_offset[face[1]],
                                            col_index_offset[face[2]+1] - col_index_offset[face[2]]};
      // std::cout << "Face: "<<i << " cl0: "<< column_len[0] <<" cl1: "<< column_len[1] << " cl2: "<< column_len[2] <<  std::endl; 
        ...
        for (const auto& ar: prism_iterator)
          {
            ...
            if (k + 1 >= col_index_offset[face[0] + 1]) partition_errors[0]++;
            if (l + 1 >= col_index_offset[face[1] + 1]) partition_errors[0]++;
            if (m + 1 >= col_index_offset[face[2] + 1]) partition_errors[0]++;
            ...
            Simplex3D K0(bot_triangle[0], bot_triangle[1], bot_triangle[2], top_triangle[2]);
            Simplex3D K1(bot_triangle[0], top_triangle[1], bot_triangle[1], top_triangle[2]);
            Simplex3D K2(bot_triangle[0], top_triangle[0], top_triangle[1], top_triangle[2]);
      ...
      case 1:
      {
        for (const auto& ar: prism_iterator)
        {
          ...
          if (k + 2 >= col_index_offset[face[0] + 1]) partition_errors[1]++;
          if (l + 1 >= col_index_offset[face[1] + 1]) partition_errors[1]++;
          if (m + 1 >= col_index_offset[face[2] + 1]) partition_errors[1]++;
          ...
          Simplex3D K0(bot_triangle[0], bot_triangle[1], bot_triangle[2], mid_triangle[0]);
          Simplex3D K1(bot_triangle[1], top_triangle[2], bot_triangle[2], mid_triangle[0]);
          Simplex3D K2(bot_triangle[1], top_triangle[2], mid_triangle[0], top_triangle[1]);
          Simplex3D K3(mid_triangle[0], top_triangle[0], top_triangle[1], top_triangle[2]);
      ...
      case 2:
      {
        for (const auto& ar: prism_iterator)
        {
          ...
          Simplex3D K0(bot_triangle[0], bot_triangle[1], bot_triangle[2], mid_triangle[1]);
          Simplex3D K1(bot_triangle[0], bot_triangle[2], mid_triangle[0], mid_triangle[1]);
          Simplex3D K2(bot_triangle[2], top_triangle[2], mid_triangle[0], mid_triangle[1]);
          Simplex3D K3(top_triangle[0], top_triangle[2], top_triangle[1], mid_triangle[0]);
          Simplex3D K4(top_triangle[1], top_triangle[2], mid_triangle[1], mid_triangle[0]); 
      ...
      case 3:
      {
        for (const auto& ar: prism_iterator)
        {
          ...
          if (k + 2 >= col_index_offset[face[0] + 1])
          { 
            // std::cout <<i << " Partition 3 k+2 = " << k+2 << " Next Column index" <<  col_index_offset[face[0] + 1] << " | " << col_index_offset[face[0]] << std::endl; 
            partition_errors[3]++;
            continue;
          }
          if (l + 2 >= col_index_offset[face[1] + 1]) 
          { 
            // std::cout << "Partition 3 l+2 = " << l+2 << " Next Column index" <<  col_index_offset[face[1] + 1] << " | " << col_index_offset[face[1]] << std::endl; 
            partition_errors[3]++;
            continue;
          }
          if (m + 2 >= col_index_offset[face[2] + 1])
          { 
            // std::cout << "Partition 3 m+2 = " << m+2 << " Next Column index" <<  col_index_offset[face[2] + 1] << " | " << col_index_offset[face[2]] << std::endl; 
            partition_errors[3]++;
            continue;
          }
          ...
          Simplex3D K0(bot_triangle[0], bot_triangle[1], bot_triangle[2], mid_triangle[2]);
          Simplex3D K1(bot_triangle[0], mid_triangle[1], bot_triangle[1], mid_triangle[2]);
          Simplex3D K2(bot_triangle[0], mid_triangle[0], mid_triangle[1], mid_triangle[2]);
          Simplex3D K3(mid_triangle[0], mid_triangle[1], mid_triangle[2], top_triangle[2]);
          Simplex3D K4(mid_triangle[0], top_triangle[1], mid_triangle[1], top_triangle[2]);
          Simplex3D K5(mid_triangle[0], top_triangle[0], top_triangle[1], top_triangle[2]);
    ...
    std::cout << "Partition 0 Indexing errors: " << partition_errors[0] << std::endl;  
    std::cout << "Partition 1 Indexing errors: " << partition_errors[1] << std::endl; 
    std::cout << "Partition 2 Indexing errors: " << partition_errors[2] << std::endl; 
    std::cout << "Partition 3 Indexing errors: " << partition_errors[3] << std::endl; 
