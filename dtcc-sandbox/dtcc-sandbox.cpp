...
// std::set<size_t>
// getNeighborsSet(size_t id, const Grid2D& inGrid, const std::vector<Point2D>&
// inCenter, const std::vector<std::set<size_t>> bins, double distance)
// {
...
std::set<size_t> getNeighborsSet(size_t footprintId,
                                 ...
                                 double distance)
{
  ...
  for (size_t i = 0; i < footprints.size(); i++)
  {
    ...
    if (dist2 < distance)
    {
      // std::cout<<"building: "<< footprintId<<" is neighbors with:" << i << "
      // with a distance of:"<<dist2<<std::endl;
      neighbors.insert(i);
    }
  }

  std::cout << "Building: " << footprintId
            << " is neighbors with:" << std::endl;
  printSet(neighbors);

  return neighbors;
}

bool canMergeFootprints(const Polygon &footprintToCheck,
                        const std::vector<Polygon> &footprints,
                        ...
                        double tol)
{
  ...
  for (auto it = neighbors.cbegin(); it != neighbors.cend(); it++)
  {
    ...
    if (d2 < tol)
    {
      // std::cout<<"Can merge footprints: " << footprintToCheck<< " with: "<<
      // *it << std::endl;
      std::cout << "Can merge footprint:" << *it << " with a distance of:" << d2
                << std::endl;
      mergeIndex = *it;
      return true;
    }
...
void mergeFootprints(const size_t &startFootprintIndex,
                     const std::vector<Polygon> &footprints,
                     std::set<size_t> neighbors,
                     double tol)
{
  ...
  while (canMergeFootprints(pivotFootprint, footprints, neighbors, mergeIndex,
                            tol))
  {
    ...
    if (mergeIndex == startFootprintIndex)
    {
      ...
      std::cout << "Tried to merge footprint with itself, skipping..."
                << std::endl;
      continue;
    }
...
int main(int argc, char *argv[])
{
  ...
  // neighbor print....
  std::cout << "Printing neighbors of buildings with getNeighborsSet...."
            << std::endl;
  for (size_t i = 0; i < footprints.size(); i++)
  {
    ...
    {
      for (auto it = neighborsVecSet[j].cbegin();
           it != neighborsVecSet[j].cend(); it++)
      {
        std::cout << *it << ' ';
      }
      std::cout << std::endl;
  ...
  std::cout << "Printing neighbors of buildings with brute force...."
            << std::endl;
  ...
  for (size_t i = 0; i < footprints.size(); i++)
  {
    ...
    // Brute force...
    // std::set<size_t> neighbors =
    // getNeighborsSet(i,footprints,p["MinBuildingDistance"]);
  ...
  //   indices.pop();
  //   std::vector<std::set<size_t>> neighbors = getNeighborsSet(i, grid,
  //   centers, setbins); for(size_t j=0;j<neighbors.size();j++)
  //   {
