...
public:
    ...
    // Add domain markers
    {
      ...
      for (size_t layer = 0; layer < numLayers; layer++)
      {
        for (const auto &marker : mesh2D.Markers)
        {
          ...
          // Later adjusted to -4 above buildings in TrimMesh3D.
          else
          {
            m = marker;
          }
                         ...
                         size_t numLayers)
  {
    ...
    // Get sizes
    const size_t numBuildings = cityModel.Buildings.size();
    const size_t numCells2D = mesh2D.Cells.size();
    const size_t numCells3D = mesh3D.Cells.size();
    ...
    {
      const int buildingIndex = mesh2D.Markers[cellIndex2D];
      if (buildingIndex >= 0)
        buildingCells2D[buildingIndex].push_back(cellIndex2D);
    }

    // Debug: Print building cells
         ...
         buildingIndex++)
    {
      ...
      for (size_t layer = 0; layer < numLayers; layer++)
      {
        ...
        {
          const double z = mesh3D.MidPoint(cellIndex3D).z;
          const double h = cityModel.Buildings[buildingIndex].MaxHeight();
          if (z < h)
          {
            trimLayer = true;
            break;
    ...
    for (size_t layer = 1; layer < numLayers - 1; layer++)
    {
      for (size_t cellIndex2D = 0; cellIndex2D < numCells2D; cellIndex2D++)
        for (size_t j = 0; j < 3; j++)
          mesh3D.Markers[Index3D(layer, layerSize, cellIndex2D, j)] = -4;
    }
    ...
    {
      for (size_t j = 0; j < 3; j++)
        mesh3D.Markers[Index3D(numLayers - 1, layerSize, cellIndex2D, j)] = -3;
    }
         ...
         buildingIndex++)
    {
      ...
      if (layer == 0)
      {
        warning("MeshGenerator: Building " + str(buildingIndex) +
      ...
      {
        for (size_t j = 0; j < 3; j++)
          mesh3D.Markers[Index3D(layer, layerSize, cellIndex2D, j)] = marker;
      }
    }
    ...
    size_t k = 0;
    size_t l = 0;
    for (size_t cellIndex3D = 0; cellIndex3D < mesh3D.Cells.size();
         cellIndex3D++)
    {
      if (!trimCell[cellIndex3D])
      {
        // Get cell
        const Simplex3D &T = mesh3D.Cells[cellIndex3D];
    ...
    // Debug: Print cell markers
    // for (size_t cellIndex3D = 0; cellIndex3D < mesh3D.Cells.size();
    //      cellIndex3D++)
    //   std::cout << "Cell " << cellIndex3D << ": " <<
    //   mesh3D.Markers[cellIndex3D]
    //             << std::endl;
  }
                                 ...
                                 double resolution)
  {
    ...
    for (auto const &building : cityModel.Buildings)
    {
      ...
      Mesh2D _mesh2D;
      CallTriangle(_mesh2D, building.Footprint.Vertices, subDomains, resolution,
                   false);
...
private:
  ...
  static void CallTriangle(Mesh2D &mesh2D,
