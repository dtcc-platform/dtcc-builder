...
namespace DTCC
{
  ...
  public:
    ...
    //
    // The domain markers for the 3D mesgh are propagated as is from
    // the domain markers of the 2D mesh.
    //
                               ...
                               double meshResolution)
    {
      ...
      mesh3D.DomainMarkers.resize(mesh3D.Cells.size());
      ...
      // Add domain markers
      {
        ...
        for (size_t layer = 0; layer < numLayers; layer++)
        {
          // Iterate over markers in layer
          for (const auto &marker : mesh2D.DomainMarkers)
            mesh3D.DomainMarkers[k++] = marker;
        }
      }
    }
