...
#include "JSONUtils.h"
  ...
  static void Deserialize(Parameters &parameters, const nlohmann::json &json)
  {
    ...
    for (auto &it : parameters.Map)
    {
      if (!HasKey(it.first, json))
        Warning("Missing parameter \"" + it.first + "\", using default value");
      else
      {
                        ...
                        const Point2D &origin)
  {
    ...
    for (auto const &building : cityModel.Buildings)
    {
      ...
      jsonBuilding["RoofSegments"] = nlohmann::json::array();
      for (auto const &segment : building.RoofSegments)
      {
  ...
  static void Serialize(const CityJSON &cityJson, nlohmann::json &json)
  {
    ...
    for (auto const &cityObject : cityJson.CityObjects)
    {
      ...
      attributesObj["measuredHeight"] =
          cityObject.ObjectAttributes.MeasuredHeight;
      ...
      geometryObj["type"] = CityObject::Geometry::GeometryTypeToString(
          cityObject.ObjectGeometry.Type);
      ...
      if (cityObject.ObjectGeometry.Type ==
          CityObject::Geometry::GeometryType::Solid)
      {
  ...
  static void Deserialize(CityJSON &cityJson, const nlohmann::json &json)
  {
    ...
    {
      // debug
      // std::cout << cityObjectIterator->first << std::endl; // getting City
      // Object ID here
  ...
  static void Deserialize(ColorMap &colorMap, const nlohmann::json &json)
  {
    ...
    {
      colorMap.InsertColor((double)c[0],
                           Color((double)c[1], (double)c[2], (double)c[3]));
    }
  }
  ...
  static void Deserialize(RoadNetwork &road, const nlohmann::json &json)
  {
    ...
    for (auto it = jsonVertexValues.begin(); it != jsonVertexValues.end(); it++)
    {
      ...
      road.VertexValues.emplace(
          it.key(), std::vector<double>(jsonVertexValuesArray.size()));
    ...
    for (auto it = jsonEdgeValues.begin(); it != jsonEdgeValues.end(); it++)
    {
      ...
      road.EdgeValues.emplace(it.key(),
                              std::vector<double>(jsonEdgeValuesArray.size()));
  ...
  Serialize(const RoadNetwork &road, nlohmann::json &json, Point2D origin)
  {
    ...
    for (auto it = road.VertexValues.begin(); it != road.VertexValues.end();
         it++)
    {
  ...
  static void
  Deserialize(Property &property, const nlohmann::json &json, std::string uuid)
  {
                        ...
                        bool isTopObject = true)
  {
    ...
    SerializeArea(primaryArea, json, isTopObject, jsonPrimArea, "PrimaryArea");
  }
...
private:
                            ...
                            std::string typeName)
  {
    ...
    if (!isTopObject)
      json[typeName == "Property" ? "Properties" : (typeName + "s")].push_back(
          jsonArea);
    else
      for (nlohmann::json::iterator it = jsonArea.begin(); it != jsonArea.end();
           ++it)
        json[it.key()] = it.value();
  }

  /// Serialize a footprint.
  static void SerializeFootprint(const Polygon &footprint, nlohmann::json &json)
  {
