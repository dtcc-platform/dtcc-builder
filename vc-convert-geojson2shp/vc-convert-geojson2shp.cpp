...
//./ogr2ogr -f "ESRI ShapeFile"  -s_srs EPSG:3006 -t_srs EPSG:3006 /tmp/PropertyMap.shp /tmp/PropertyMap.geojson
...
int main(int argc, char* argv[])
{
	...
	if (ogr.fail()) {
		std::cout << "ogr2ogr binary not found. Please install gdal-bin." << std::endl;
		return 1;
	}
	ogr.close();	
	system(("ogr2ogr -f \"ESRI Shapefile\"  -s_srs EPSG:3006 -t_srs EPSG:3006 "+ fileout+" "+filein).c_str());
	return 0;
}
